    ...
    def test_left_padding_compatibility(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            for _ in range(10):  # there may be false positives with 10 runs, we rely on the CI to catch the flakiness
                ...
                if "position_ids" in signature:
                    position_ids = ops.cumsum(padded_attention_mask, axis=-1) - 1
                    position_ids = position_ids.masked_fill(padded_attention_mask == 0, 1)
                    model_kwargs["position_ids"] = position_ids
                next_logits_with_padding = model(**model_kwargs).logits[:, -1, :]
                if not np.allclose(next_logits_wo_padding.asnumpy(), next_logits_with_padding.asnumpy(), atol=1e-7):
                    no_failures = False
                    break
