...
def prepare_image_inputs(
    ...
    if mindsporeify:
        image_inputs = [Tensor.from_numpy(image) for image in image_inputs]
...
def prepare_video(num_frames, num_channels, width=10, height=10, numpify=False, mindsporeify=False):
    ...
    if mindsporeify:
        video = [Tensor.from_numpy(frame) for frame in video]
...
class ImageProcessingTestMixin:
    ...
    def test_cast_dtype_device(self):
        if self.test_cast_dtype is not None:
            ...
            encoding.update({"input_ids": mindspore.LongTensor([[1, 2, 3], [4, 5, 6]])})
