...
from mindnlp.transformers.models.kosmos2.configuration_kosmos2 import (
    Kosmos2TextConfig,
    Kosmos2VisionConfig,
)
...
class Kosmos2TextModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        if input_mask is not None:
            ...
            rnd_start_indices = ops.randint(1, seq_length - 1, size=(batch_size,))
...
class Kosmos2ModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        (Kosmos2ForConditionalGeneration,) if is_mindspore_available() else ()
    )
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=Kosmos2Config, hidden_size=37
        )
...
class Kosmos2ModelIntegrationTest(unittest.TestCase):
    ...
    def test_snowman_image_captioning(self):
        ...
        np.testing.assert_allclose(
            ops.concat(scores[1:4])[:3, :3].asnumpy(),
            np.array(
                [
        ...
        np.testing.assert_allclose(
            ops.concat(scores[-3:])[-3:, -3:].asnumpy(),
            np.array(
                [
        ...
        # fmt: on
        self.assertListEqual(generated_ids.asnumpy().tolist(), EXPECTED_IDS)
        ...
        # fmt: on
        self.assertListEqual(generated_ids.asnumpy().tolist(), EXPECTED_IDS_LONG)
