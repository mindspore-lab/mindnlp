...
if is_mindspore_available():
    ...
    from mindnlp.transformers.models.patchtsmixer import (
        PatchTSMixerConfig,
    ...
    from mindnlp.transformers.models.patchtsmixer.modeling_patchtsmixer import (
        PatchTSMixerEncoder,
...
@require_mindspore
class PatchTSMixerModelTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            PatchTSMixerModel,
            PatchTSMixerForPrediction,
    ...
    all_generative_model_classes = (
        (PatchTSMixerForPrediction, PatchTSMixerForPretraining) if is_mindspore_available() else ()
    )
    pipeline_model_mapping = {"feature-extraction": PatchTSMixerModel} if is_mindspore_available() else {}
    ...
    def test_forward_signature(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if model_class == PatchTSMixerForPretraining:
            ...
            elif model_class == PatchTSMixerForPrediction:
                expected_arg_names = [
                    "past_values",
                    "observed_mask",
                    "future_values",
                ...
                expected_arg_names = []  # 如果有其他模型类，添加对应的expected_arg_names
...
class PatchTSMixerModelIntegrationTests(unittest.TestCase):
    def test_pretrain_head(self):
        model = PatchTSMixerForPretraining.from_pretrained("ibm/patchtsmixer-etth1-pretrain")
        batch = prepare_batch()

        mindspore.set_seed(0)
        output = model(past_values=batch["past_values"]).prediction_outputs
        num_patch = (
        ...
        expected_slice = mindspore.tensor([[[[-0.9106]],[[1.5326]],[[-0.8245]],[[0.7439]],[[-0.7830]],[[2.6256]],[[-0.6485]],]])  # fmt: skip
        self.assertTrue(ops.allclose(output[0, :7, :1, :1], expected_slice, atol=TOLERANCE))

    def test_forecasting_head(self):
        model = PatchTSMixerForPrediction.from_pretrained("ibm/patchtsmixer-etth1-forecasting")
    ...
    def test_prediction_generation(self):
        model = PatchTSMixerForPrediction.from_pretrained("ibm/patchtsmixer-etth1-generate")
        batch = prepare_batch(file="forecast_batch.pt")
...
class PatchTSMixerFunctionalTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.flat_enc_output = ops.rand(
            batch_size,
            cls.num_patches,
            cls.params["d_model"],
        )

        cls.correct_pred_output = ops.rand(
            batch_size,
            cls.params["prediction_length"],
            cls.params["num_input_channels"],
        )
        cls.correct_regression_output = ops.rand(batch_size, cls.params["num_targets"])

        cls.correct_pretrain_output = ops.rand(
            batch_size,
            cls.params["num_input_channels"],
            cls.num_patches,
            cls.params["patch_length"],
        ...
        cls.correct_forecast_output = ops.rand(
            batch_size,
            cls.params["prediction_length"],
            cls.params["num_input_channels"],
        )

        cls.correct_sel_forecast_output = ops.rand(batch_size, cls.params["prediction_length"], 2)

        cls.correct_classification_output = ops.rand(
            batch_size,
            cls.params["num_targets"],
        )

        cls.correct_classification_classes = ops.randint(0, cls.params["num_targets"], (batch_size,))
    ...
    def check_module(
        ...
        if task == "forecast":
            ...
            ref_samples = target_output.unsqueeze(1).broadcast_to((-1, config.num_parallel_samples, -1, -1))
            ...
            ref_samples = target_output.unsqueeze(1).broadcast_to((-1, config.num_parallel_samples, -1))
            ground_truth_arg = "target_values"
    ...
    def test_forecast(self,  mode = "mix_channel",
                      self_attn = True,
                      scaling = "std",
                      gated_attn = True,
    ...
    def test_classification(self, mode="common_channel", self_attn=True, scaling="std", gated_attn=True, head_aggregation=None):
        params = self.__class__.params.copy()
        params.update(
            mode=mode,
            self_attn=self_attn,
    ...
    def test_regression(self,mode="common_channel",self_attn=True,scaling="std",gated_attn=True,head_aggregation=None,loss="mse"):
        params = self.__class__.params.copy()
        params.update(
            mode=mode,
            self_attn=self_attn,
    ...
    def forecast_full_module(self, params=None, output_hidden_states=False, return_dict=None):
        ...
        if config.loss == "nll":
            samples = mdl.generate(self.__class__.data)
            ref_samples = target_val.unsqueeze(1).broadcast_to((-1, params["num_parallel_samples"], -1, -1))
            self.assertEqual(samples.sequences.shape, ref_samples.shape)
    ...
    def test_regression_full_distribute(self):
        ...
        if config.loss == "nll":
            samples = mdl.generate(self.__class__.data)
            ref_samples = self.__class__.correct_regression_output.unsqueeze(1).broadcast_to((
