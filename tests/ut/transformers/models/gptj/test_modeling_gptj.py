...
class GPTJModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_batch_generation(self):
        ...
        model = GPTJForCausalLM.from_pretrained("EleutherAI/gpt-j-6B", revision="float16", torch_dtype=mindspore.float16)
    ...
    def test_model_from_pretrained(self):
        ...
        model = GPTJModel.from_pretrained(model_name, revision="float16", torch_dtype=mindspore.float16)
...
class GPTJModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_lm_generate_gptj(self):
        ...
        for checkpointing in [True, False]:
            model = GPTJForCausalLM.from_pretrained(
                "EleutherAI/gpt-j-6B", revision="float16", torch_dtype=mindspore.float16
            )
            # if checkpointing:
            #     model.gradient_checkpointing_enable()
            # else:
            #     model.gradient_checkpointing_disable()
            input_ids = ms.Tensor([[464, 3290]], dtype=ms.int64)  # The dog
