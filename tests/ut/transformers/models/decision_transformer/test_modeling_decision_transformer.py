...
class DecisionTransformerModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (DecisionTransformerModel,) 
    all_generative_model_classes = ()
    pipeline_model_mapping = {"feature-extraction": DecisionTransformerModel} 
...
class DecisionTransformerModelIntegrationTest(unittest.TestCase):
    ...
    def test_autoregressive_prediction(self):
        ...
        state = mindspore.ops.randn(1, 1, config.state_dim)  # env.reset()
        ...
        states = state
        actions = mindspore.ops.zeros(1, 0, config.act_dim)
        rewards = mindspore.ops.zeros(1, 0)
        timesteps = mindspore.ops.zeros(0).reshape(1, 1)

        for step in range(NUM_STEPS):
            actions = mindspore.ops.cat([actions, ops.zeros(1, 1, config.act_dim)], axis=1)
            rewards = mindspore.ops.cat([rewards, ops.zeros(1, 1)], axis=1)

            attention_mask = mindspore.ops.ones(1, states.shape[1])
            ...
            self.assertTrue(np.allclose(action_pred.asnumpy()[0, -1], expected_outputs.asnumpy()[step], atol=1e-4))
            state, reward, _, _ = (  # env.step(action)
                mindspore.ops.randn(1, 1, config.state_dim),
                1.0,
                False,
                {},
            )
            ...
            states = mindspore.ops.cat([states, state], dim=1)
            pred_return = returns_to_go[0, -1] - reward
            returns_to_go = mindspore.ops.cat([returns_to_go, pred_return.reshape(1, 1, 1)], axis=1)
            timesteps = mindspore.ops.cat(
                [timesteps, mindspore.ops.ones((1, 1)) * (step + 1)], axis=1
            )