...
class ProphetNetModelTester:
    ...
    def create_and_check_model_fp16_forward(
        ...
        model = ProphetNetModel(config=config).set_train(False)
    ...
    def check_fast_integration(
        ...
        *args,
        ...
        input_ids = ms.tensor([[7, 4, 78, 0, 24, 52, 43]], dtype=ms.int64)
        decoder_input_ids = ms.tensor([[12, 62, 25, 11, 47, 15, 14]], dtype=ms.int64)
        attention_mask = ms.tensor([[1, 1, 1, 0, 1, 0, 0]], dtype=ms.int64)
        decoder_attention_mask = ms.tensor([[1, 1, 1, 0, 0, 1, 0]], dtype=ms.int64)
        lm_labels = ms.tensor([[62, 25, 11, 47, 15, 14, 24]], dtype=ms.int64)
        # torch.manual_seed(0)
        config.ngram = 4
        ...
        self.parent.assertTrue(np.allclose(result.loss.asnumpy(), ms.tensor(4.5892).asnumpy(), atol=1e-3))

        expected_logit_slice = ms.tensor(
...
class ProphetNetModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    @unittest.skip("CPU Cant not do half precision")
...
class ProphetNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_pretrained_checkpoint_hidden_states(self):
        ...
        decoder_prev_ids = ms.tensor([[102, 2129, 2116, 2372, 2024, 2006, 2169, 1997, 2122, 2048, 2780, 1029]])
        output = model(
        ...
        expected_encoder_outputs_slice = ms.tensor(
