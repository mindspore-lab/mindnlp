...
class DetrModelIntegrationTestsTimmBackbone(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        ...
        expected_slice_logits = ops.tensor(
        ...
        expected_shape_boxes = ops.Size((1, model.config.num_queries, 4))
        self.assertEqual(outputs.pred_boxes.shape, expected_shape_boxes)
        expected_slice_boxes = ops.tensor(
        ...
        expected_scores = ops.tensor([0.9982, 0.9960, 0.9955, 0.9988, 0.9987])
        expected_labels = [75, 75, 63, 17, 17]
        expected_slice_boxes = ops.tensor([40.1633, 70.8115, 175.5471, 117.9841])

        self.assertEqual(len(results["scores"]), 5)
        self.assertTrue(ops.allclose(results["scores"], expected_scores, atol=1e-4))
        self.assertSequenceEqual(results["labels"].tolist(), expected_labels)
        self.assertTrue(ops.allclose(results["boxes"][0, :], expected_slice_boxes))
    ...
    def test_inference_panoptic_segmentation_head(self):
        ...
        expected_shape_masks = ops.Size((1, model.config.num_queries, 200, 267))
        self.assertEqual(outputs.pred_masks.shape, expected_shape_masks)
        expected_slice_masks = ops.tensor(
            [[-7.7558, -10.8788, -11.9797], [-11.8881, -16.4329, -17.7451], [-14.7316, -19.7383, -20.3004]]
        )
        self.assertTrue(ops.allclose(outputs.pred_masks[0, 0, :3, :3], expected_slice_masks, atol=1e-3))
        ...
        expected_shape = ops.Size([480, 640])
        expected_slice_segmentation = ops.tensor([[4, 4, 4], [4, 4, 4], [4, 4, 4]], dtype=mindspore.int32)
        ...
        self.assertTrue(ops.allclose(results["segmentation"][:3, :3], expected_slice_segmentation, atol=1e-4))
