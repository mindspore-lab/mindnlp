...
from mindnlp.utils.testing_utils import require_vision, slow, is_mindspore_available, require_mindspore, is_vision_available
...
class DetrImageProcessingTest(AnnotationFormatTestMixin, ImageProcessingTestMixin, unittest.TestCase):
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        # verify pixel values
        expected_shape = ops.Size([1, 3, 800, 1066])
        self.assertEqual(encoding["pixel_values"].shape, expected_shape)

        expected_slice = ops.tensor([0.2796, 0.3138, 0.3481])
        self.assertTrue(ops.allclose(encoding["pixel_values"][0, 0, 0, :3], expected_slice, atol=1e-4))

        # verify area
        expected_area = ops.tensor([5887.9600, 11250.2061, 489353.8438, 837122.7500, 147967.5156, 165732.3438])
        self.assertTrue(ops.allclose(encoding["labels"][0]["area"], expected_area))
        # verify boxes
        expected_boxes_shape = ops.Size([6, 4])
        self.assertEqual(encoding["labels"][0]["boxes"].shape, expected_boxes_shape)
        expected_boxes_slice = ops.tensor([0.5503, 0.2765, 0.0604, 0.2215])
        self.assertTrue(ops.allclose(encoding["labels"][0]["boxes"][0], expected_boxes_slice, atol=1e-3))
        # verify image_id
        expected_image_id = ops.tensor([39769])
        self.assertTrue(ops.allclose(encoding["labels"][0]["image_id"], expected_image_id))
        # verify is_crowd
        expected_is_crowd = ops.tensor([0, 0, 0, 0, 0, 0])
        self.assertTrue(ops.allclose(encoding["labels"][0]["iscrowd"], expected_is_crowd))
        # verify class_labels
        expected_class_labels = ops.tensor([75, 75, 63, 65, 17, 17])
        self.assertTrue(ops.allclose(encoding["labels"][0]["class_labels"], expected_class_labels))
        # verify orig_size
        expected_orig_size = ops.tensor([480, 640])
        self.assertTrue(ops.allclose(encoding["labels"][0]["orig_size"], expected_orig_size))
        # verify size
        expected_size = ops.tensor([800, 1066])
        self.assertTrue(ops.allclose(encoding["labels"][0]["size"], expected_size))
    ...
    def test_call_pytorch_with_coco_panoptic_annotations(self):
        ...
        # verify area
        expected_area = ops.tensor([147979.6875, 165527.0469, 484638.5938, 11292.9375, 5879.6562, 7634.1147])
        self.assertTrue(ops.allclose(encoding["labels"][0]["area"], expected_area))
        # verify boxes
        expected_boxes_shape = ops.Size([6, 4])
        self.assertEqual(encoding["labels"][0]["boxes"].shape, expected_boxes_shape)
        expected_boxes_slice = ops.tensor([0.2625, 0.5437, 0.4688, 0.8625])
        self.assertTrue(ops.allclose(encoding["labels"][0]["boxes"][0], expected_boxes_slice, atol=1e-3))
        # verify image_id
        expected_image_id = ops.tensor([39769])
        self.assertTrue(ops.allclose(encoding["labels"][0]["image_id"], expected_image_id))
        ...
        self.assertTrue(ops.allclose(encoding["labels"][0]["iscrowd"], expected_is_crowd))
        # verify class_labels
        expected_class_labels = ops.tensor([17, 17, 63, 75, 75, 93])
        self.assertTrue(ops.allclose(encoding["labels"][0]["class_labels"], expected_class_labels))
        # verify masks
    ...
    def test_batched_coco_detection_annotations(self):
        ...
        expected_shape = ops.Size([2, 3, postprocessed_height, postprocessed_width])
        self.assertEqual(encoding["pixel_values"].shape, expected_shape)

        # Check the bounding boxes have been adjusted for padded images
        self.assertEqual(encoding["labels"][0]["boxes"].shape, ops.Size([6, 4]))
        self.assertEqual(encoding["labels"][1]["boxes"].shape, ops.Size([6, 4]))
        expected_boxes_0 = ops.tensor(
            [
        ...
        self.assertTrue(ops.allclose(encoding["labels"][0]["boxes"], expected_boxes_0, rtol=1e-3))
        self.assertTrue(ops.allclose(encoding["labels"][1]["boxes"], expected_boxes_1, rtol=1e-3))

        # Check the masks have also been padded
        self.assertEqual(encoding["labels"][0]["masks"].shape, ops.Size([6, 800, 1066]))
        self.assertEqual(encoding["labels"][1]["masks"].shape, ops.Size([6, 800, 1066]))
        ...
        self.assertTrue(ops.allclose(encoding["labels"][0]["boxes"], expected_boxes_0, rtol=1))
        self.assertTrue(ops.allclose(encoding["labels"][1]["boxes"], expected_boxes_1, rtol=1))
    ...
    def test_max_width_max_height_resizing_and_pad_strategy(self):
        ...
        # do_pad=False, max_height=100, max_width=100, image=200x100 -> 10x50
        image_processor = DetrImageProcessor(
            size={"max_height": 100, "max_width": 100},
            do_pad=False,
        )
        inputs = image_processor(images=[image_1], return_tensors="pt")
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 100, 50]))
        ...
        inputs = image_processor(images=[image_1], return_tensors="pt")
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 100, 100]))
        ...
        inputs = image_processor(images=[image_1], return_tensors="pt")
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 301, 101]))
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([2, 3, 150, 100]))
    ...
    def test_longest_edge_shortest_edge_resizing_strategy(self):
        ...
        inputs = image_processor(images=[image_1], return_tensors="pt")
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 640, 436]))
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 436, 640]))
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 98, 118]))
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 128, 50]))
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.Size([1, 3, 50, 50]))
