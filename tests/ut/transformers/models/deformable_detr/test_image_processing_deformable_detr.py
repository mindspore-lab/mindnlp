...
from mindnlp.utils.testing_utils import require_vision, slow, is_mindspore_available, is_vision_available, get_tests_dir
...
class DeformableDetrImageProcessingTest(AnnotationFormatTestMixin, ImageProcessingTestMixin, unittest.TestCase):
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        # verify pixel values
        expected_shape = (1, 3, 800, 1066)  # Change from list to tuple
        self.assertEqual(encoding["pixel_values"].shape, expected_shape)

        expected_slice = np.array([0.2796, 0.3138, 0.3481])
        self.assertTrue(np.allclose(encoding["pixel_values"].asnumpy()[0, 0, 0, :3], expected_slice, atol=1e-4))

        # verify area
        expected_area = np.array([5887.9600, 11250.2061, 489353.8438, 837122.7500, 147967.5156, 165732.3438])
        self.assertTrue(np.allclose(encoding["labels"][0]["area"].asnumpy(), expected_area))

        # verify boxes
        expected_boxes_shape = (6, 4)  # Change from list to tuple
        self.assertEqual(encoding["labels"][0]["boxes"].shape, expected_boxes_shape)
        expected_boxes_slice = np.array([0.5503, 0.2765, 0.0604, 0.2215])
        self.assertTrue(np.allclose(encoding["labels"][0]["boxes"].asnumpy()[0], expected_boxes_slice, atol=1e-3))

        # verify image_id
        expected_image_id = np.array([39769])
        self.assertTrue(np.allclose(encoding["labels"][0]["image_id"].asnumpy(), expected_image_id))

        # verify is_crowd
        expected_is_crowd = np.array([0, 0, 0, 0, 0, 0])
        self.assertTrue(np.allclose(encoding["labels"][0]["iscrowd"].asnumpy(), expected_is_crowd))

        # verify class_labels
        expected_class_labels = np.array([75, 75, 63, 65, 17, 17])
        self.assertTrue(np.allclose(encoding["labels"][0]["class_labels"].asnumpy(), expected_class_labels))

        # verify orig_size
        expected_orig_size = np.array([480, 640])
        self.assertTrue(np.allclose(encoding["labels"][0]["orig_size"].asnumpy(), expected_orig_size))

        # verify size
        expected_size = np.array([800, 1066])
        self.assertTrue(np.allclose(encoding["labels"][0]["size"].asnumpy(), expected_size))
    ...
    def test_call_pytorch_with_coco_panoptic_annotations(self):
        ...
        self.assertTrue(np.allclose(encoding["pixel_values"][0, 0, 0, :3], expected_slice, atol=1e-4))
        ...
        self.assertTrue(np.allclose(encoding["labels"][0]["boxes"][0], expected_boxes_slice, atol=1e-3))
        # verify image_id
        expected_image_id = mindspore.Tensor([39769])
        self.assertTrue(np.allclose(encoding["labels"][0]["image_id"], expected_image_id))
        # verify is_crowd
        expected_is_crowd = mindspore.Tensor([0, 0, 0, 0, 0, 0])
        self.assertTrue(np.allclose(encoding["labels"][0]["iscrowd"], expected_is_crowd))
        # verify class_labels
        expected_class_labels = mindspore.Tensor([17, 17, 63, 75, 75, 93])
        self.assertTrue(np.allclose(encoding["labels"][0]["class_labels"], expected_class_labels))
        # verify masks
        expected_masks_sum = 822873
        self.assertEqual(encoding["labels"][0]["masks"].sum().item(), expected_masks_sum)
        # verify orig_size
        expected_orig_size = mindspore.Tensor([480, 640])
        self.assertTrue(np.allclose(encoding["labels"][0]["orig_size"], expected_orig_size))
        # verify size
    ...
    def test_batched_coco_detection_annotations(self):
        ...
        self.assertTrue(np.allclose(encoding["labels"][0]["boxes"], expected_boxes_0, rtol=1e-3))
        self.assertTrue(np.allclose(encoding["labels"][1]["boxes"], expected_boxes_1, rtol=1e-3))
        ...
        unnormalized_boxes_0 = mindspore.ops.vstack(
            [
        ...
        unnormalized_boxes_1 = mindspore.ops.vstack(
            [
        ...
        expected_boxes_0 = mindspore.ops.vstack(
            [
        ...
        expected_boxes_1 = mindspore.ops.vstack(
            [
        ...
        self.assertTrue(np.allclose(encoding["labels"][0]["boxes"], expected_boxes_0, rtol=1))
        self.assertTrue(np.allclose(encoding["labels"][1]["boxes"], expected_boxes_1, rtol=1))
    ...
    def test_max_width_max_height_resizing_and_pad_strategy(self):
        image_1 = mindspore.ops.ones([200, 100, 3], dtype=mindspore.uint8)
        ...
        self.assertEqual(tuple(inputs["pixel_values"].shape), (1, 3, 100, 50))  # 转换为元组进行比较
        ...
        inputs = image_processor(images=[image_1], return_tensors="ms")
        self.assertEqual(tuple(inputs["pixel_values"].shape), (1, 3, 100, 100))  # 转换为元组进行比较
        ...
        inputs = image_processor(images=[image_1], return_tensors="ms")
        self.assertEqual(tuple(inputs["pixel_values"].shape), (1, 3, 301, 101))  # 转换为元组进行比较

        ### Check for batch
        image_2 = mindspore.ops.ones([100, 150, 3], dtype=mindspore.uint8)
        ...
        self.assertEqual(tuple(inputs["pixel_values"].shape), (2, 3, 150, 100))  # 转换为元组进行比较

    def test_longest_edge_shortest_edge_resizing_strategy(self):
        image_1 = mindspore.ops.ones([958, 653, 3], dtype=mindspore.uint8)
        ...
        image_2 = mindspore.ops.ones([653, 958, 3], dtype=mindspore.uint8)
        ...
        image_3 = mindspore.ops.ones([100, 120, 3], dtype=mindspore.uint8)
        ...
        image_4 = mindspore.ops.ones([128, 50, 3], dtype=mindspore.uint8)
        ...
        image_5 = mindspore.ops.ones([50, 50, 3], dtype=mindspore.uint8)
