...
class no_grad:
    def __enter__(self):
        context.set_context(enable_grad=False)

    def __exit__(self, exc_type, exc_value, traceback):
        context.set_context(enable_grad=True)

if is_mindspore_available():
    import mindspore

    from mindnlp.transformers import DeformableDetrForObjectDetection, DeformableDetrModel
...
class DeformableDetrModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        pixel_mask = mindspore.ops.ones([self.batch_size, self.image_size, self.image_size])
...
class DeformableDetrModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
        ...
        if return_labels:
            if model_class.__name__ == "DeformableDetrForObjectDetection":
                ...
                for i in range(self.model_tester.batch_size):
                    ...
                    target["masks"] = mindspore.ops.ones(
                    ...
                    if isinstance(tuple_object, (List, Tuple)):
                        ...
                        self.assertTrue(
                            ...
                            msg=(
                                ...
                                f" {mindspore.ops.max(mindspore.ops.abs(tuple_object - dict_object))}. Tuple has `nan`:"
                                f" {mindspore.ops.isnan(tuple_object).any()} and `inf`: {mindspore.ops.isinf(tuple_object)}. Dict has"
                                f" `nan`: {mindspore.ops.isnan(dict_object).any()} and `inf`: {mindspore.ops.isinf(dict_object)}."
                            ),
                        )
    ...
    def create_and_check_model_bf16_forward(self):
        ...
        model = model_class(config, torch_dtype=mindspore.float16)
