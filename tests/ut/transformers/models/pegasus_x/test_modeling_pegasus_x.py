...
class PegasusXModelTester:
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = PegasusXModel(config=config).get_decoder()
        ...
        self.parent.assertTrue(numpy.allclose(output_from_past_slice.asnumpy(), output_from_no_past_slice.asnumpy(), atol=1e-2))

    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        model = PegasusXModel(config=config)
        model.set_train(False)
...
class PegasusXModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_generate_fp16(self):
        ...
        model = PegasusXForConditionalGeneration(config)
        model.set_train(False)
        model.half()
...
def assert_tensors_close(a, b, atol=1e-12, prefix=""):
    ...
    if a is None and b is None:
        return True
    try:
        if numpy.allclose(a.asnumpy(), b.asnumpy(), atol=atol):
            return True
        raise
    except Exception:
        pct_different = (ops.gt((a - b).abs(), atol)).float().mean()
        if a.numel() > 100:
...
class PegasusXModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        self.assertTrue(numpy.allclose(output[:, :3, :3].asnumpy(), expected_slice.asnumpy(), atol=TOLERANCE))
...
class PegasusXStandaloneDecoderModelTester:
    ...
    def create_and_check_decoder_model_past(
        ...
        model = PegasusXDecoder(config=config)
        model.set_train(False)
        # first forward pass
        ...
        # test that outputs are equal for slice
        assert numpy.allclose(output_from_past_slice.asnumpy(), output_from_no_past_slice.asnumpy(), atol=1e-3)
    ...
    def create_and_check_decoder_model_attention_mask_past(
        ...
        # test that outputs are equal for slice
        assert numpy.allclose(output_from_past_slice.asnumpy(), output_from_no_past_slice.asnumpy(), atol=1e-2)
