...
from mindnlp.transformers import XGLMConfig
...
if is_mindspore_available():
    ...
    from mindnlp.transformers import XGLMForCausalLM, XGLMModel, XGLMTokenizer
...
class XGLMModelTester:
    ...
    def create_and_check_xglm_model_attention_mask_past(self, config, input_ids, input_mask, head_mask, *args):
        ...
        next_input_ids = ops.cat([input_ids, next_tokens], axis=-1)
        attn_mask = ops.cat(
            [attn_mask, ops.zeros((attn_mask.shape[0], 1), dtype=mindspore.int64)],
            axis=1,
        )
    ...
    @unittest.skip("state_dict() not exist")
...
class XGLMModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (XGLMForCausalLM, ) if is_mindspore_available() else ()
    all_generative_model_classes = (XGLMForCausalLM,) if is_mindspore_available() else ()
    pipeline_model_mapping = (
        {"feature-extraction": XGLMModel, "text-generation": XGLMForCausalLM} if is_mindspore_available() else {}
    )
    fx_compatible = True
...
class XGLMModelLanguageGenerationTest(unittest.TestCase):
    ...
    def _test_lm_generate_xglm_helper(
        ...
        input_ids = mindspore.tensor([[2, 268, 9865]], dtype=mindspore.int64)  # The dog
    ...
    def test_batch_generation(self):
        ...
        outputs = model.generate(
            input_ids=input_ids, attention_mask=inputs["attention_mask"], max_new_tokens=12
        )
    ...
    def test_xglm_sample(self):
        ...
        tokenized = tokenizer("Today is a nice day and", return_tensors="ms")
        # tokenized = tokenizer("The dog is a very", return_tensors="ms")
        ...
        EXPECTED_OUTPUT_STRS = [
            ...
            # torch 2.0 + other (When do_Sample=True, there is randomness, and each machine is different)
            "Today is a nice day and the temperature has fallen on the high part... which of course means",
        ]
    ...
    def test_batched_nan_fp16(self):
        ...
        model = XGLMForCausalLM.from_pretrained(model_name, ms_dtype=mindspore.float16, use_cache=True)
