...
class VideoMAEModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    all_model_classes = (
        (VideoMAEModel, VideoMAEForPreTraining, VideoMAEForVideoClassification) if is_mindspore_available() else ()
    )
...
class VideoMAEModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_for_pretraining(self):
        model = VideoMAEForPreTraining.from_pretrained("MCG-NJU/videomae-base-short", ms_dtype=mindspore.float16)
        ...
        inputs_ = {'pixel_values':ops.cast(inputs["pixel_values"], mindspore.float16),
        ...
        # verify the logits
        # expected_shape = ops.shape(mindspore.Tensor([1, 1408, 1536]))
        expected_shape = (1, 1408, 1536)
        expected_slice = mindspore.Tensor(
            [[0.7994, 0.9612, 0.8508], [0.7401, 0.8958, 0.8302], [0.5862, 0.7468, 0.7325]]
        )
        self.assertEqual(outputs.logits.shape, expected_shape)
        self.assertTrue(np.allclose(outputs.logits[0, :3, :3].asnumpy(), expected_slice.asnumpy(), atol=1e-2))

        # verify the loss (`config.norm_pix_loss` = `True`)
        expected_loss = mindspore.Tensor([[0.5142]])
        # print(outputs.loss.asnumpy())
        self.assertTrue(np.allclose(outputs.loss.asnumpy(), expected_loss.asnumpy(), atol=1e-3))
        ...
        # expected_loss = mindspore.Tensor(mindspore.Tensor([0.6469]))
