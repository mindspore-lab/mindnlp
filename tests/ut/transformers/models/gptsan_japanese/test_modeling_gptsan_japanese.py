...
class GPTSanJapaneseForConditionalGenerationTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def test_logits(self):
        ...
        print(f"MindSpore logits: {output_logits}")
        ...
        mismatches = [(i, target[i], predict[i]) for i in range(len(target)) if not check(target[i], predict[i])]
        if mismatches:
            print("Mismatches found at indices (index, target, predict):")
    ...
    def test_batch_generation(self):
        ...
        print(f"MindSpore input_ids: {input_ids}")
        # 打印 attention_mask 以检查是否有差异
        ...
        print(f"MindSpore Generated outputs: {outputs}")
        ...
        print(f"MindSpore generated sentences: {batch_out_sentence}")
        non_padded_sentence = tokenizer.decode(output_non_padded[0], skip_special_tokens=True)
        padded_sentence = tokenizer.decode(output_padded[0], skip_special_tokens=True)

        print(f"Non-padded sentence: {non_padded_sentence}")
    ...
    def test_spout_generation(self):
        ...
        # 调整 spout 张量形状，确保它与输入张量的形状兼容
        for i, spout in enumerate(spouts):
            if spout.ndim == 1:  # 如果 spout 是一维的，将其扩展为二维
                spouts[i] = spout.expand_dims(0)
        
        # Debug: 打印 spout 的形状
        print(f"Adjusted spout shapes: {[spout.shape for spout in spouts]}")
    ...
    def test_prefix_lm_generation(self):
        ...
        # Debug: 打印输入文本和生成配置
        print(f"Input 1: {prefix_text_1 + input_text_1}")
        print(f"Input 2: {prefix_text_2 + input_text_2}")
        ...
        # Debug: 打印编码后的输入ID
        print(f"Input Token IDs 1: {input_tok_1.input_ids}")
        print(f"Input Token IDs 2: {input_tok_2.input_ids}")
        ...
        # Debug: 打印生成的输出ID
        print(f"Generated Output IDs 1: {output1}")
        print(f"Generated Output IDs 2: {output2}")
        ...
        # Debug: 打印生成的输出句子
        print(f"Generated Sentence 1: {out1_sentence}")
        print(f"Generated Sentence 2: {out2_sentence}")
        print(f"Batch Generated Sentences: {batch_out_sentence}")
