...
class CohereModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (CohereForCausalLM) if is_mindspore_available() else ()
...
class CohereIntegrationTest(unittest.TestCase):
    ...
    def test_batched_small_model_logits(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained(model_id)
        model = CohereForCausalLM.from_pretrained(model_id, low_cpu_mem_usage=True, torch_dtype=mindspore.float16)
        ...
        self.assertTrue(np.allclose(EXPECTED_LOGITS, logits[:, :3, :3], rtol=1e-3, atol=1e-3))
