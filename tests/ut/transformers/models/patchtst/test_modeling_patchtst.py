...
from mindnlp.utils.testing_utils import is_flaky, require_mindspore, slow, is_mindspore_available
...
class PatchTSTModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_model_main_input_name(self):
        model_signature = inspect.signature(getattr(PatchTSTModel, "construct"))
...
def prepare_batch(repo_id="hf-internal-testing/etth1-hourly-batch", file="train-batch.pt"):
    ...
    # batch = mindspore.load(file, map_location=torch_device)
...
class PatchTSTModelIntegrationTests(unittest.TestCase):
    ...
    def test_pretrain_head(self):
        ...
        self.assertTrue(np.allclose(output[0, :7, :1, :1].asnumpy(), expected_slice.asnumpy(), atol=TOLERANCE))
    ...
    def test_prediction_head(self):
        ...
        self.assertTrue(np.allclose(output[0, :1, :7].asnumpy(), expected_slice.asnumpy(), atol=TOLERANCE))

    @unittest.skip('Mindspore cannot load torch .pt file.')
    def test_prediction_generation(self):
        model = PatchTSTForPrediction.from_pretrained("namctin/patchtst_etth1_forecast", from_pt = True)
        batch = prepare_batch(file="test-batch.pt")
        ...
        expected_slice = expected_shape.tensor(
            [[0.4075, 0.3716, 0.4786, 0.2842, -0.3107, -0.0569, -0.7489]],
            # device=torch_device,
        )
        mean_prediction = outputs.sequences.mean(axis=1)
        self.assertTrue(np.allclose(mean_prediction[0, -1:].asnumpy(), expected_slice.asnumpy(), atol=TOLERANCE))
    ...
    def test_regression_generation(self):
        ...
        mean_prediction = outputs.sequences.mean(axis=1)
        self.assertTrue(np.allclose(mean_prediction[-5:].asnumpy(), expected_slice.asnumpy(), rtol=TOLERANCE))
