...
from mindnlp.dataset import hf_duconv, duconv_process
...
class TestDuConv(unittest.TestCase):
    ...
    def test_duconv_process(self):
        ...
        duconv_dev = duconv_process(dataset=dev_dataset, char_vocab=char_vocab)
        ...
        assert (next(duconv_dev)[6]).dtype == ms.int32
        assert (next(duconv_dev)[7]).dtype == ms.int32
        assert (next(duconv_dev)[8]).dtype == ms.int32
    @pytest.mark.download
