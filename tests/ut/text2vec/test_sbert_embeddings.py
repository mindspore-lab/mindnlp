...
from mindnlp.text2vec import SentenceModel
...
def use_transformers(sentences=('如何更换花呗绑定银行卡', '花呗更改绑定银行卡')):
    ...
    def mean_pooling(model_output, attention_mask):
        ...
        return ops.sum(token_embeddings * input_mask_expanded, 1) / ops.clamp(input_mask_expanded.sum(1), min=1e-9)
...
class SBERTEmbeddingsTestCase(unittest.TestCase):
    ...
    def test_sbert_dim(self):
        m = SentenceModel('shibing624/text2vec-base-chinese')
        print('dim:', m.bert.pooler.dense.out_channels)
        def get_sentence_embedding_dimension(model):
            if model:
                sent_embedding_dim_method = getattr(model.bert.pooler.dense, "out_channels", None)
