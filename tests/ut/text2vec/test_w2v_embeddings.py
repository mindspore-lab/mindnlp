...
import numpy as np
from mindnlp.text2vec import Word2Vec
...
class EmbeddingsTestCase(unittest.TestCase):
    ...
    def test_encode_char(self):
        """测试文本 char encode结果"""
        char = '卡'
        emb = self.w2v_model.encode(char)
        t = type(emb)
        print(type(emb))
    ...
    def test_encode_word(self):
        """测试文本 word encode结果"""
        word = '银行卡'
        emb = self.w2v_model.encode(word)
        print(word, emb[:10])
        self.assertEqual(emb.shape, (200,))
    ...
    def test_oov_emb(self):
        ...
        w = '，'
        comma_res = self.w2v_model.encode(w)
        print(w, comma_res)
        self.assertEqual(comma_res[0], 0.0)

        w = '特价机票'
        r = self.w2v_model.encode(w)
        print(w, r[:10])

        w = '特价'
        r1 = self.w2v_model.encode(w)
        print(w, r1[:10])

        w = '机票'
        r2 = self.w2v_model.encode(w)
        print(w, r2[:10])
