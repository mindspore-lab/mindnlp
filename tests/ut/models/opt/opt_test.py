...
class OPT_test():
    ...
    def test_OPTLearnedPositionalEmbedding(self):
        ...
        #print(mindspore.ops.shape(output_ms))
    ...
    def test_OPTDecoderLayer(self):
        ...
        #type NONE has no shape
        #assert output_ms[1].shape == output_pt[1].shape
        #assert output_ms[2].shape == output_pt[2].shape
        assert np.allclose(output_ms[0].asnumpy(), output_pt[0].detach().numpy(), 1e-5, 1e-5)
        #assert np.allclose(output_ms[0].asnumpy(), output_pt[0].detach().numpy(), 1e-4, 1e-4)
        '''
        cannot match percision 1e-5
        pass 1e-5 adding epsilon in layernorm
        '''
    ...
    def test_OPTDecoder(self):
        ...
        #print("---")
        #print(output_ms[0].shape)
        #print(output_ms[0])
        #print("---")
        #print(output_pt[0].shape)
        #print(output_pt[0])
