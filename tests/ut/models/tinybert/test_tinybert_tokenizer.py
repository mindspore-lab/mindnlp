...
# ============================================================================
"""Test the TinyBert Tokenizer"""

import mindspore as ms
from mindspore.dataset import GeneratorDataset
from mindspore.dataset.text import Vocab as msVocab
from mindnlp import Vocab
from mindnlp.transformers import TinyBertTokenizer


def test_tinybert_tokenizer_from_pretrained():
    """test GPTTokenizer from pretrained."""
    texts = ['i make a small mistake when i\'m working! 床前明月光']
    test_dataset = GeneratorDataset(texts, 'text')

    tinybert_tokenizer = TinyBertTokenizer.from_pretrained('tinybert_4L_zh')
    tinybert_tokenizer.return_token = True
    test_dataset = test_dataset.map(operations=tinybert_tokenizer)
    dataset_after = next(test_dataset.create_tuple_iterator())[0]

    assert len(dataset_after) == 21
    assert dataset_after.dtype == ms.string

def test_tinybert_tokenizer_mindnlp_vocab():
    """test TinyBert Tokenizer by dataset.map"""
    texts = ['i make a small mistake when i\'m working! 床前明月光']
    test_dataset = GeneratorDataset(texts, 'text')
    ...
    tinybert_tokenizer = TinyBertTokenizer(vocab=vocab, lower_case=True, return_token=True)
    test_dataset = test_dataset.map(operations=tinybert_tokenizer)
    dataset_after = next(test_dataset.create_tuple_iterator())[0]
...
def test_tinybert_tokenizer_mindspore_vocab():
    ...
    bert_tokenizer = TinyBertTokenizer(vocab=vocab, lower_case=True, return_token=True)
...
def test_tinybert_tokenizer_add_special_tokens():
    """test add special tokens."""
    bert_tokenizer = TinyBertTokenizer.from_pretrained('tinybert_4L_zh')
