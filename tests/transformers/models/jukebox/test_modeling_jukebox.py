...
class Jukebox1bModelTester(unittest.TestCase):
    all_model_classes = () # To be set in MindNLP version if JukeboxModel is implemented
    ...
    def prepare_inputs(self):
        try:
            ...
            self.skipTest("JukeboxTokenizer not available in MindNLP")

    @slow
    def test_sampling(self):
        try:
            import mindspore
            from mindnlp.transformers import JukeboxModel
            
            model = JukeboxModel.from_pretrained(self.model_id, min_duration=0).eval()
            labels = self.prepare_inputs()

            set_seed(0)
            zs = [mindspore.zeros((1, 0), dtype=mindspore.int64) for _ in range(3)]
            zs = model._sample(zs, labels, [0], sample_length=40 * model.priors[0].raw_to_tokens, save_results=False)
            self.assertIn(zs[0][0].asnumpy().tolist(), [self.EXPECTED_OUTPUT_2, self.EXPECTED_OUTPUT_2_PT_2])

            set_seed(0)
            zs = model._sample(zs, labels, [1], sample_length=40 * model.priors[1].raw_to_tokens, save_results=False)
            self.assertIn(zs[1][0].asnumpy().tolist(), [self.EXPECTED_OUTPUT_1, self.EXPECTED_OUTPUT_1_PT_2])

            set_seed(0)
            zs = model._sample(zs, labels, [2], sample_length=40 * model.priors[2].raw_to_tokens, save_results=False)
            self.assertIn(zs[2][0].asnumpy().tolist(), [self.EXPECTED_OUTPUT_0, self.EXPECTED_OUTPUT_0_PT_2])
        except ImportError:
            self.skipTest("JukeboxModel not available in MindNLP")
    ...
    def test_conditioning(self):
        try:
            ...
            mindspore.backends.cuda.matmul.allow_tf32 = False
            mindspore.backends.cudnn.allow_tf32 = False
            model = JukeboxModel.from_pretrained(self.model_id, min_duration=0).eval()
            ...
            self.assertListEqual(metadata.asnumpy()[0][:10].tolist(), self.EXPECTED_Y_COND)

            audio_conditioning, metadata_conditioning, lyric_tokens = top_prior.get_cond(music_token_conds, metadata)
            mindspore.testing.assert_close(
                audio_conditioning[0][0][:30].asnumpy(), np.array(self.EXPECTED_AUDIO_COND), atol=1e-4, rtol=1e-4
            )
            mindspore.testing.assert_close(
                metadata_conditioning[0][0][:30].asnumpy(), np.array(self.EXPECTED_META_COND), atol=1e-4, rtol=1e-4
            )
            mindspore.testing.assert_close(
                lyric_tokens[0, :30].asnumpy(), np.array(self.EXPECTED_LYRIC_COND), atol=1e-4, rtol=1e-4
            )
        except ImportError:
            self.skipTest("JukeboxModel not available in MindNLP")
    ...
    def test_primed_sampling(self):
        try:
            ...
            model = JukeboxModel.from_pretrained(self.model_id, min_duration=0).eval()
            set_seed(0)
            waveform = mindspore.rand((1, 5120, 1))
            ...
            mindspore.testing.assert_close(zs[0][0][:40], mindspore.tensor(self.EXPECTED_PRIMED_0))

            upper_2 = mindspore.cat((zs[0], mindspore.zeros(1, 2048 - zs[0].shape[-1])), dim=-1).long()
            ...
            mindspore.testing.assert_close(zs[1][0][:40], mindspore.tensor(self.EXPECTED_PRIMED_1))

            upper_1 = mindspore.cat((zs[1], mindspore.zeros(1, 2048 - zs[1].shape[-1])), dim=-1).long()
            ...
            mindspore.testing.assert_close(zs[2][0][:40], mindspore.tensor(self.EXPECTED_PRIMED_2))
        except ImportError:
            self.skipTest("JukeboxModel not available in MindNLP")
    ...
    def test_vqvae(self):
        try:
            ...
            model = JukeboxModel.from_pretrained(self.model_id, min_duration=0).eval()
            set_seed(0)
            x = mindspore.rand((1, 5120, 1))
            with mindspore.no_grad():
                zs = model.vqvae.encode(x, start_level=2, bs_chunks=x.shape[0])
            mindspore.testing.assert_close(zs[0][0], mindspore.tensor(self.EXPECTED_VQVAE_ENCODE))

            with mindspore.no_grad():
                x = model.vqvae.decode(zs, start_level=2, bs_chunks=x.shape[0])
            mindspore.testing.assert_close(x[0, :40, 0], mindspore.tensor(self.EXPECTED_VQVAE_DECODE), atol=1e-4, rtol=1e-4)
        except ImportError:
            self.skipTest("JukeboxModel not available in MindNLP")
...
class Jukebox5bModelTester(unittest.TestCase):
    ...
    def test_slow_sampling(self):
        try:
            ...
            model = JukeboxModel.from_pretrained(self.model_id, min_duration=0).eval()
            labels = [i for i in self.prepare_inputs(self.model_id)]
            set_seed(0)
            model.priors[0]
            zs = [mindspore.zeros((1, 0), dtype=mindspore.int64) for _ in range(3)]
            zs = model._sample(zs, labels, [0], sample_length=60 * model.priors[0].raw_to_tokens, save_results=False)
            mindspore.testing.assert_close(zs[0][0].cpu(), mindspore.tensor(self.EXPECTED_GPU_OUTPUTS_2))
            model.priors[0].cpu()

            set_seed(0)
            model.priors[1]
            zs = model._sample(zs, labels, [1], sample_length=60 * model.priors[1].raw_to_tokens, save_results=False)
            mindspore.testing.assert_close(zs[1][0].cpu(), mindspore.tensor(self.EXPECTED_GPU_OUTPUTS_1))
            model.priors[1].cpu()

            set_seed(0)
            model.priors[2]
            zs = model._sample(zs, labels, [2], sample_length=60 * model.priors[2].raw_to_tokens, save_results=False)
            mindspore.testing.assert_close(zs[2][0].cpu(), mindspore.tensor(self.EXPECTED_GPU_OUTPUTS_0))
        except ImportError:
            self.skipTest("JukeboxModel not available in MindNLP")
    ...
    def test_fp16_slow_sampling(self):
        try:
            import mindspore
            from mindnlp.transformers import JukeboxPrior
            
            prior_id = "ArthurZ/jukebox_prior_0"
            model = JukeboxPrior.from_pretrained(prior_id, min_duration=0).eval().half()

            labels = self.prepare_inputs(prior_id)[0]
            ...
            self.skipTest("JukeboxPrior not available in MindNLP") 