...
from mindtorch.distributed._shard._utils import narrow_tensor_by_index
from mindtorch.distributed.checkpoint import FileSystemReader, FileSystemWriter
from mindtorch.distributed.checkpoint._nested_dict import flatten_state_dict
from mindtorch.distributed.checkpoint.default_planner import (
...
from mindtorch.distributed.checkpoint.planner import LoadItemType, LoadPlan, LoadPlanner
from mindtorch.distributed.checkpoint.planner_helpers import _create_chunk_list
from mindtorch.distributed.checkpoint.state_dict_loader import _load_state_dict
from mindtorch.distributed.checkpoint.state_dict_saver import _save_state_dict
...
class BroadcastingTorchSaveReader(StorageReader):
    ...
    def read_data(self, plan: LoadPlan, planner: LoadPlanner) -> Future[None]:
        ...
        if self.is_coordinator:
            ...
            torch_state_dict = mindtorch.load(
        ...
        for req in plan.items:
            ...
            if self.is_coordinator:
                ...
                tensor = mindtorch.empty_like(planner.state_dict[req.storage_index.fqn])
...
class DynamicMetaLoadPlanner(DefaultLoadPlanner):
    ...
    def set_up_planner(
        ...
        for key, tensor in self.state_dict.items():
            if not mindtorch.is_tensor(tensor):
                raise RuntimeError(
...
def dcp_to_torch_save(
    ...
    mindtorch.save(sd, torch_save_path)
...
def torch_save_to_dcp(
    ...
    state_dict = mindtorch.load(torch_save_path, weights_only=False)
    # we don't need stateful behavior here because the expectation is anything loaded by
    # mindtorch.load would not contain stateful objects.
    _save_state_dict(
