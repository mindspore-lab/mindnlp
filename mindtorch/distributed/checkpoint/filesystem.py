...
# from mindtorch._utils import _get_available_device_type, _get_device_module
...
from mindtorch.distributed.checkpoint.staging import BlockingAsyncStager
from mindtorch.distributed.checkpoint.storage import (
    StorageReader,
    StorageWriter,
    WriteResult,
)
from mindtorch.distributed.checkpoint.utils import _create_file_view
...
class _TensorLoader(ABC):
    ...
    @abstractmethod
    def values(self) -> Iterator[Tuple[mindtorch.Tensor, object]]:
        pass
...
class _OverlappingCpuLoader(_TensorLoader):
    def __init__(
        ...
        stream: Optional[mindtorch.Stream] = None,
        ...
        self.stream = cast(
            mindtorch.cuda.Stream, stream or self.device_module.current_stream()
        )
    ...
    def _drain(self) -> List[Tuple[mindtorch.Tensor, object]]:
        drained = []
    ...
    def _refill(self) -> None:
        with self.device_module.stream(self.stream):
            while not self._done and self.in_flight_data < self.inflight_threshhold:
                ...
                elif tensor.device == mindtorch.device("cpu"):
                    if (
    ...
    def _finish(self) -> Iterable[Tuple[mindtorch.Tensor, object]]:
        assert self._done
...
def _item_size(item: WriteItem) -> int:
    ...
    return size * mindtorch._utils._element_size(dtype)
...
def _write_item(
    stream: io.IOBase,
    data: Union[io.BytesIO, mindtorch.Tensor],
    ...
    if write_item.type == WriteItemType.BYTE_IO:
        ...
        assert isinstance(data, mindtorch.Tensor)
        assert data.device == mindtorch.device("cpu")
        mindtorch.save(data, cast(IO[bytes], stream))
...
def _write_files_from_queue(
    ...
    try:
        while True:
            ...
            custom_backend_name = mindtorch._C._get_privateuse1_backend_name()
