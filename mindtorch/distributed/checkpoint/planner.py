...
class WriteItem:
    ...
    def tensor_storage_size(self) -> Optional[int]:
        ...
        dtype_size = mindtorch._utils._element_size(self.tensor_data.properties.dtype)
...
class SavePlanner(abc.ABC):
    ...
    >>>                 p.tensor_data.properties.dtype = mindtorch.float16
    >>>         return plan
    >>>
    >>>     def resolve_data(self, write_item):
    >>>         item = super().resolve_data(write_item)
    >>>         return item if write_item.type == WriteItemType.BYTE_IO else item.to(mindtorch.float16)
...
class LoadPlanner:
    ...
    >>>         self.original_state_dict[read_item.dest_index.fqn[4:]] = mindtorch.load(value, weights_only=False)
    ...
    >>>         return mindtorch.empty_like(tensor, device="cpu")
    >>>
    ...
    @abc.abstractmethod
    def resolve_tensor(self, read_item: ReadItem) -> mindtorch.Tensor:
        """
