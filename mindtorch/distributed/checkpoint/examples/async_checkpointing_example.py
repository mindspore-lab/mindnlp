...
class Model(mindtorch.nn.Module):
...
def _init_model(rank, world_size):
    ...
    optim = mindtorch.optim.Adam(model.parameters(), lr=0.0001)
...
def _input():
    x = mindtorch.rand(128, 8, device="cuda")
    y = mindtorch.zeros(128, 1, device="cuda")

    y[mindtorch.sum(x, dim=1) >= 4] = 1.0
...
def run(rank, world_size):
    ...
    loss_calc = mindtorch.nn.BCELoss()

    f = None
    for epoch in range(NUM_EPOCHS):
        try:
            mindtorch.manual_seed(epoch)
            x, y = _input()
