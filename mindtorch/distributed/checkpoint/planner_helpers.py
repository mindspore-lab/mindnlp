...
def _create_chunk_from_tensor(tensor: mindtorch.Tensor) -> ChunkStorageMetadata:
    return ChunkStorageMetadata(
        offsets=mindtorch.Size([0] * len(tensor.size())), sizes=tensor.size()
    )


def _chunk_for_shard(shard_md: ShardMetadata) -> ChunkStorageMetadata:
    return ChunkStorageMetadata(
        offsets=mindtorch.Size(shard_md.shard_offsets),
        sizes=mindtorch.Size(shard_md.shard_sizes),
    )
...
#     )
#     sizes, offsets = mindtorch.Size(sizes), mindtorch.Size(offsets)
...
def _create_write_item_for_tensor(fqn: str, tensor: mindtorch.Tensor) -> WriteItem:
    offsets = mindtorch.Size([0] * len(tensor.size()))
    return WriteItem(
        index=MetadataIndex(fqn, offsets),
...
def _create_read_item_for_byteio(
    ...
    return ReadItem(
        ...
        dest_offsets=mindtorch.Size((dest_offset,)),
        storage_index=storage_index,
        storage_offsets=mindtorch.Size((storage_offset,)),
        lengths=mindtorch.Size((length,)),
    )
...
def _create_read_item_for_tensor(
    ...
    return ReadItem(
        ...
        dest_index=dest_index,
        dest_offsets=mindtorch.Size(dest_offsets),
        storage_index=storage_index,
        storage_offsets=mindtorch.Size(storage_offsets),
        lengths=mindtorch.Size(lengths),
    )
...
def _create_default_metadata_only_plan(state_dict: STATE_DICT_TYPE) -> SavePlan:
    ...
    for fqn, obj in state_dict.items():
        ...
        elif isinstance(obj, mindtorch.Tensor):
...
def _create_write_items(fqn: str, object: Any) -> List[WriteItem]:
    ...
    elif isinstance(object, mindtorch.Tensor):
...
def _create_chunk_list(tensor: mindtorch.Tensor) -> List[ChunkStorageMetadata]:
...
def _init_state_dict(state_dict: Dict[str, Any]) -> Any:
    """
    Initializes meta tensor if the meta tensor is DTensor or mindtorch.Tensor.
    """
    ...
    #         device = cast(
    #             mindtorch.device, _get_device_module(device_type).current_device()
    #         )
    #         new_local_tensor = mindtorch.empty_like(value.to_local(), device=device)
    ...
    def tensor_func(value: mindtorch.Tensor):
        device = getattr(value, "device", None)
        if device == mindtorch.device("meta"):
            device_type = dist.distributed_c10d._get_pg_default_device().type
            device = cast(
            ...
            tensor = mindtorch.empty_like(value, device=device)
            return tensor
        else:
            return value
...
def _iterate_state_dict(
    ...
    elif isinstance(iter_object, mindtorch.Tensor):
