...
from mindtorch.distributed.checkpoint.default_planner import _EmptyStateDictLoadPlanner
...
def load(
    ...
    *,
    ...
    .. warning::
        ...
        All non-tensor data is loaded using `mindtorch.load()` and modified in place
        on state_dict.
    ...
    Examples
        ...
        >>> fs_storage_reader = mindtorch.distributed.checkpoint.FileSystemReader("/checkpoint/1")

        >>> mindtorch.distributed.checkpoint.load_state_dict(
    ...
    .. note::
        ...
        rank has an individual GPU, via ``mindtorch.cuda.set_device()``.
    """

    no_dist = not (dist.is_available() and dist.is_initialized())
    if no_dist:
        warnings.warn(
            "mindtorch.distributed is unavailable or uninitialized, assuming the intent is to load in a single process."
        )
...
def _load_state_dict(
    ...
    # mindtorch._C._log_api_usage_once("mindtorch.distributed.checkpoint.load_state_dict")
...
def _load_state_dict_from_keys(
    ...
    *,
    ...
    .. warning::
        ...
        All non-tensor data is loaded using `mindtorch.load()`
    ...
    mindtorch._C._log_api_usage_once(
        "mindtorch.distributed.checkpoint._load_state_dict_from_keys"
    )
