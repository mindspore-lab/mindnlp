...
from mindtorch.distributed._state_dict_utils import _offload_state_dict_to_cpu
from mindtorch.distributed.checkpoint._storage_utils import _storage_setup
from mindtorch.distributed.checkpoint.default_planner import DefaultSavePlanner
from mindtorch.distributed.checkpoint.logger import _dcp_method_logger
from mindtorch.distributed.checkpoint.metadata import Metadata, STATE_DICT_TYPE
from mindtorch.distributed.checkpoint.planner import SavePlan, SavePlanner
from mindtorch.distributed.checkpoint.staging import AsyncStager
from mindtorch.distributed.checkpoint.stateful import Stateful
from mindtorch.distributed.checkpoint.storage import StorageWriter
...
def save(
    ...
    *,
    ...
    This function is different from ``mindtorch.save()`` as it handles
    ...
    Example:
        ...
        >>> fs_storage_writer = mindtorch.distributed.checkpoint.FileSystemWriter("/checkpoint/1")
        >>> mindtorch.distributed.checkpoint.save(
    ...
    .. note::
        ...
        In this case, the device used is given by ``mindtorch.cuda.current_device()``
        and it is the user's responsibility to ensure that this is set so that
        each rank has an individual GPU, via ``mindtorch.cuda.set_device()``.
    """
    mindtorch._C._log_api_usage_once("mindtorch.distributed.checkpoint.save")

    no_dist = not (dist.is_available() and dist.is_initialized())
    if no_dist:
        warnings.warn(
            "mindtorch.distributed is unavailable or uninitialized, assuming the intent is to save in a single process."
        )
...
def async_save(
    ...
    *,
    ...
    Example:
        ...
        >>> fs_storage_writer = mindtorch.distributed.checkpoint.FileSystemWriter("/checkpoint/1")
        >>> checkpoint_future = mindtorch.distributed.checkpoint.async_save(
        >>>     state_dict=state_dict,
        >>>     storage_writer=fs_storage_writer,
        >>> )
        >>>
    ...
    """
    mindtorch._C._log_api_usage_once("mindtorch.distributed.checkpoint.async_save")

    if dist.is_available() and dist.is_initialized():
        pg = process_group or _get_default_group()
        assert (
            mindtorch.device("cpu") in pg._device_types  # type: ignore[attr-defined]
...
def _save_state_dict(
    ...
    mindtorch._C._log_api_usage_once("mindtorch.distributed.checkpoint.save_state_dict")
