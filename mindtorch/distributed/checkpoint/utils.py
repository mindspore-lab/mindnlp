...
def _find_shard(tensor: ShardedTensor, index: MetadataIndex) -> Shard:
    ...
    if index.index is not None:
        if (
            ...
            and mindtorch.Size(shards[index.index].metadata.shard_offsets) == index.offset
        ):
            return shards[index.index]

    for shard in shards:
        if mindtorch.Size(shard.metadata.shard_offsets) == index.offset:
            return shard
    raise ValueError(f"Could not find shard at '{index.offset}' for FQN: '{index.fqn}'")


def find_tensor_shard(tensor: mindtorch.Tensor, index: MetadataIndex) -> mindtorch.Tensor:
    ...
    if index.offset is not None:
        ...
        if index.offset == mindtorch.Size([0] * len(tensor.size())):
            return tensor
        raise ValueError(
...
def _create_file_view(file: io.IOBase, offset: int, length: int) -> io.IOBase:
    # FIXME (kumpera) mindtorch.load fails if we wrap with io.BufferedReader
