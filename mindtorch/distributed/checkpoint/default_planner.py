...
from mindtorch.distributed.checkpoint._dedup_save_plans import dedup_save_plans
from mindtorch.distributed.checkpoint._nested_dict import (
    FLATTEN_MAPPING,
    flatten_state_dict,
)
from mindtorch.distributed.checkpoint._sharded_tensor_utils import _flatten_sharded_tensors
from mindtorch.distributed.checkpoint._traverse import set_element
...
from mindtorch.distributed.checkpoint.utils import find_state_dict_object
...
class DefaultSavePlanner(SavePlanner):
    ...
    def resolve_data(self, write_item: WriteItem) -> Union[mindtorch.Tensor, io.BytesIO]:
...
class DefaultLoadPlanner(LoadPlanner):
    ...
    def load_bytes(self, read_item: ReadItem, value: io.BytesIO) -> None:
        if self.flatten_state_dict:
            set_element(
                ...
                mindtorch.load(value, weights_only=False),
            )
        else:
            self.state_dict[read_item.dest_index.fqn] = mindtorch.load(
    ...
    def commit_tensor(self, read_item: ReadItem, tensor: mindtorch.Tensor) -> None:
        pass
    ...
    def transform_tensor(self, read_item: ReadItem, tensor: mindtorch.Tensor):
        """Extension from the planner interface to make it easy to extend the default planner."""
    ...
    def set_up_planner(
        ...
        for k, v in metadata.state_dict_metadata.items():
            ...
            if isinstance(v, TensorStorageMetadata):
                v = mindtorch.empty(v.size, dtype=v.properties.dtype)  # type: ignore[assignment]
...
def _check_box_bounds(
    outer_box_size: mindtorch.Size, inner_box: ChunkStorageMetadata
) -> bool:
