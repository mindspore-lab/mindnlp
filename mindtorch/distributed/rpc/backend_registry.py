...
_backend_type_doc = """
    ...
    :func:`~mindtorch.distributed.rpc.backend_registry.register_backend` function.
"""
...
def _tensorpipe_exchange_and_check_all_device_maps(
    ...
    gathered: List[
        Tuple[str, int, Dict[str, Dict[mindtorch.device, mindtorch.device]], List[mindtorch.device]]
...
def _create_reverse_mapping(my_name, all_names, all_device_maps):
    reverse_device_maps: Dict[str, Dict[mindtorch.device, mindtorch.device]] = {}
    for node in all_names:
...
def _get_device_infos():
    ...
    device_count = mindtorch.cuda.device_count()
    if mindtorch.cuda.is_available() and opts.devices:
        mindtorch.cuda.init()
...
def _tensorpipe_init_backend_handler(
    ...
    if is_static_group:
        ...
        if mindtorch.cuda.is_available() and devices:
