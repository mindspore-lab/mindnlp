...
import mindtorch
from mindtorch._C._distributed_rpc import (
...
from mindtorch.futures import Future
    ...
    def wrapper(*args, **kwargs):
        if not _is_current_rpc_agent_set():
            raise RuntimeError(
                ...
                "mindtorch.distributed.rpc.init_rpc first."
            )
...
def _wait_all():
    ...
    Example::
        ...
        >>> with rpc._wait_all():
        >>>    fut_1 = rpc.rpc_async(dst, mindtorch.add, (mindtorch.ones(2, 2), 1))
        >>>    fut_2 = rpc.rpc_async(dst, mindtorch.add, (mindtorch.ones(2, 2), 1))
    ...
    try:
        ...
        try:
            mindtorch.futures.wait_all(_thread_local_var.future_list)
        finally:
...
def _all_gather(obj, worker_names=None, timeout: float = UNSET_RPC_TIMEOUT):
    r"""
    This is similar to mindtorch.distributed.all_gather(), but is using RPC. It
...
def shutdown(graceful=True, timeout=DEFAULT_SHUTDOWN_TIMEOUT):
    ...
    .. warning::
        For :class:`~mindtorch.futures.Future` objects returned by
        :meth:`~mindtorch.distributed.rpc.rpc_async`, ``future.wait()`` should not
    ...
    Example::
        ...
        on both workers. Refer to :meth:`~mindtorch.distributed.init_process_group`
        ...
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> # do some work
        >>> result = rpc.rpc_sync("worker1", mindtorch.add, args=(mindtorch.ones(1), 1))
        >>> # ready to shutdown
        >>> rpc.shutdown()
...
def get_worker_info(worker_name=None):
    r"""
    Get :class:`~mindtorch.distributed.rpc.WorkerInfo` of a given worker name.
    Use this :class:`~mindtorch.distributed.rpc.WorkerInfo` to avoid passing an
    ...
    Returns:
        :class:`~mindtorch.distributed.rpc.WorkerInfo` instance for the given
        ``worker_name`` or :class:`~mindtorch.distributed.rpc.WorkerInfo` of the
...
for method_name, method in inspect.getmembers(PyRRef):
    ...
    # It's like,
    """
    to_here(self: mindtorch.distributed.rpc.PyRRef, timeout: float=-1.0) -> object
    ...
    docstring = docstring.replace(
        "mindtorch.distributed.rpc.PyRRef", "mindtorch.distributed.rpc.RRef"
    )
...
def remote(to, func, args=None, kwargs=None, timeout=UNSET_RPC_TIMEOUT):
    ...
    :class:`~mindtorch.distributed.rpc.RRef` to the result value immediately.
    Worker ``to`` will be the owner of the returned
    :class:`~mindtorch.distributed.rpc.RRef`, and the worker calling ``remote`` is
    a user. The owner manages the global reference count of its
    :class:`~mindtorch.distributed.rpc.RRef`, and the owner
    :class:`~mindtorch.distributed.rpc.RRef` is only destructed when globally there
    are no living references to it.

    Args:
        to (str or WorkerInfo or int): name/rank/``WorkerInfo`` of the destination worker.
        func (Callable): a callable function, such as Python callables, builtin
                         operators (e.g. :meth:`~mindtorch.add`) and annotated
        ...
        timeout (float, optional): timeout in seconds for this remote call. If the
                                   creation of this
                                   :class:`~mindtorch.distributed.rpc.RRef` on worker
    ...
    Returns:
        A user :class:`~mindtorch.distributed.rpc.RRef` instance to the result
        value. Use the blocking API :meth:`mindtorch.distributed.rpc.RRef.to_here`
    ...
    .. warning ::
        ...
        :meth:`mindtorch.distributed.rpc.RRef.confirmed_by_owner` API.
    ...
    Example::
        ...
        >>> import mindtorch.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> rref1 = rpc.remote("worker1", mindtorch.add, args=(mindtorch.ones(2), 3))
        >>> rref2 = rpc.remote("worker1", mindtorch.add, args=(mindtorch.ones(2), 1))
        ...
        >>> # On both workers:
        >>> @mindtorch.jit.script
        >>> def my_script_add(tensor: mindtorch.Tensor, scalar: int):
        >>>    return mindtorch.add(tensor, scalar)

        >>> # On worker 0:
        >>> import mindtorch.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> rref = rpc.remote("worker1", my_script_add, args=(mindtorch.ones(2), 3))
        >>> rref.to_here()
        >>> rpc.shutdown()
        ...
        >>> rpc.shutdown()
    """
    mindtorch._C._log_api_usage_once("mindtorch.distributed.rpc_remote")
    qualified_name = mindtorch.jit._builtins._find_builtin(func)
    ...
    with ctx_manager as rf:
        ...
        if is_async_exec:
            ...
            if isinstance(wrapped, mindtorch.jit.ScriptFunction):
                func = wrapped
        ...
        elif isinstance(func, mindtorch.jit.ScriptFunction):
            rref = _invoke_remote_torchscript(
                dst_worker_info.name,
                mindtorch._jit_internal._qualified_name(func),
                timeout,
                is_async_exec,
                *args,
                **kwargs,
        ...
        if should_profile:
            assert mindtorch.autograd._profiler_enabled()
...
def rpc_sync(to, func, args=None, kwargs=None, timeout: float = UNSET_RPC_TIMEOUT):
    ...
    Example::
        ...
        >>> import mindtorch.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> ret = rpc.rpc_sync("worker1", mindtorch.add, args=(mindtorch.ones(2), 3))
        >>> rpc.shutdown()
        ...
        >>> import mindtorch.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> ret = rpc.rpc_sync("worker1", my_script_add, args=(mindtorch.ones(2), 3))
        >>> rpc.shutdown()
    ...
    """
    mindtorch._C._log_api_usage_once("mindtorch.distributed.rpc_sync")
...
def rpc_async(to, func, args=None, kwargs=None, timeout=UNSET_RPC_TIMEOUT):
    ...
    :class:`~mindtorch.futures.Future` that can be awaited on.
    ...
    Returns:
        Returns a :class:`~mindtorch.futures.Future` object that can be waited
        on. When completed, the return value of ``func`` on ``args`` and
        ``kwargs`` can be retrieved from the :class:`~mindtorch.futures.Future`
        object.
    ...
    .. warning ::
        ...
        :class:`~mindtorch.futures.Future` completes.
    ...
    Example::
        ...
        >>> import mindtorch.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> fut1 = rpc.rpc_async("worker1", mindtorch.add, args=(mindtorch.ones(2), 3))
        ...
        >>> import mindtorch.distributed.rpc as rpc
        >>> rpc.init_rpc("worker0", rank=0, world_size=2)
        >>> fut = rpc.rpc_async("worker1", my_script_add, args=(mindtorch.ones(2), 3))
        >>> ret = fut.wait()
        >>> rpc.shutdown()
        ...
        >>> rpc.shutdown()
    """
    mindtorch._C._log_api_usage_once("mindtorch.distributed.rpc_async")
...
def _get_should_profile():
    ...
    # Kineto profiler.
    ActiveProfilerType = mindtorch._C._profiler.ActiveProfilerType
    return (
        mindtorch.autograd._profiler_enabled()
        and mindtorch._C._autograd._profiler_type()
...
def _enable_rpc_profiler(
    ...
    if should_profile:
        ...
        if qualified_name is None:
            func_name = (
                mindtorch._jit_internal._qualified_name(func)
                if isinstance(func, mindtorch.jit.ScriptFunction)
        ...
        ctx_manager = mindtorch.autograd.profiler.record_function(rpc_profiling_key)  # type: ignore[assignment]
