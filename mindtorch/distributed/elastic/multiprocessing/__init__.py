...
For functions, it uses ``mindtorch.multiprocessing`` (and therefore python
...
::
 ...
 from mindtorch.distributed.elastic.multiprocessing import Std, start_processes
...
Just like ``mindtorch.multiprocessing``, the return value of the function
...
from mindtorch.distributed.elastic.multiprocessing.api import (  # noqa: F401
    _validate_full_rank,
    DefaultLogsSpecs,
    LogsDest,
    LogsSpecs,
...
def start_processes(
    ...
    .. note:: When the ``entrypoint`` is a binary (``str``), ``args`` can only be strings.
              ...
              ``mindtorch.distributed.elastic.multiprocessing.errors.record``. For function launches,
