...
@custom_sharding_spec_op(ChunkShardingSpec, mindtorch.nn.functional.embedding)
def sharded_embedding(types, args, kwargs, pg):
    """
    Handles ``__torch_function__`` dispatch for ``mindtorch.nn.functional.embedding``.
...
def _handle_col_wise_sharding(
    ...
    output = _handle_col_wise_sharding_base(
        mindtorch.nn.functional.embedding,
        len(input.size()),
        input,
        world_size,
        weight,
...
def _handle_row_wise_sharding(
    ...
    local_input_embeddings = mindtorch.nn.functional.embedding(
        lookup_input,
