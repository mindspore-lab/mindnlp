...
import mindtorch
from mindtorch.distributed import distributed_c10d as c10d
...
def _validate_output_tensor_for_gather(
    ...
    dst_rank: int,
    size: mindtorch.Size,
    dst_tensor: Optional[mindtorch.Tensor],
) -> None:
    if dst_rank == my_rank:
...
def _flatten_tensor_size(size) -> mindtorch.Size:
    """
    Checks if tensor size is valid, then flatten/return a mindtorch.Size object.
    """
...
def build_metadata_from_local_shards(
    ...
    for local_shard in local_shards:
        ...
        if (
            local_shard_tensor.layout != mindtorch.strided
            or local_shard_tensor.layout != first_shard_layout
        ):
            raise ValueError(
                f"Only mindtorch.strided layout is currently supported, but found "
