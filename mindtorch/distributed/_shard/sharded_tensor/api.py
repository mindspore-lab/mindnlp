...
from mindtorch.distributed import distributed_c10d, rpc
from mindtorch.distributed._shard._utils import DEPRECATE_MSG
from mindtorch.distributed._shard.sharding_spec._internals import (
    check_tensor,
    validate_non_overlapping_shards_metadata,
)
from mindtorch.distributed._shard.sharding_spec.api import (
    _dispatch_custom_op,
    _has_custom_op,
)
...
class ShardedTensorBase(mindtorch.Tensor):
    ...
    def _init_from_local_shards_and_global_metadata(
        ...
        # if tensor_properties.layout != mindtorch.strided:
        #     raise ValueError("Only mindtorch.strided layout is currently supported")
...
class ShardedTensor(ShardedTensorBase):
    """
    ShardedTensor is an mindtorch.Tensor subclass to represent Tensors that are sharded
    ...
    create_op specified by tensor_init_params.create_op, e.g., mindtorch.ones, or
    mindtorch.empty

    Args:
        sharding_spec (:class:`mindtorch.distributed._shard.sharding_spec.ShardingSpec`): The specification
    ...
    Keyword args:
        ...
        layout (:class:`mindtorch.layout`, optional): the desired layout of returned Tensor.
            Default: ``mindtorch.strided``.
        requires_grad (bool, optional): If autograd should record operations on the
            returned tensor. Default: ``False``.
        pin_memory (bool, optional): If set, returned tensor would be allocated in
            the pinned memory. Works only for CPU tensors. Default: ``False``.
        memory_format (:class:`mindtorch.memory_format`, optional): the desired memory format of
            returned Tensor. Default: ``mindtorch.contiguous_format``.
        init_rrefs (bool, optional): Whether or not to initialize
            :class:`mindtorch.distributed.rpc.RRef`s pointing to remote shards.
    ...
    .. note:: ShardedTensor uses collectives to do various operations, i.e. it
        ...
        used is given by ``mindtorch.cuda.current_device()`` and it is the user's
        responsibility to ensure that this is set so that each rank has an
        individual GPU, via ``mindtorch.cuda.set_device()``
    ...
    def __init__(
        ...
        *size,
        ...
        if memory_format != mindtorch.contiguous_format:
            raise ValueError(
                "Only mindtorch.contiguous_format memory_format is currently supported"
            )
        ...
        if self._init_rrefs:
            ...
            if not rpc._is_current_rpc_agent_set():
                raise RuntimeError(
                    ...
                    " mindtorch.distributed.rpc.init_rpc if init_rrefs is set to True"
                )
            self._init_rpc()
    ...
    def _init_rpc(self):
        ...
        mindtorch.futures.wait_all(futs)

        # Barrier for all RPCs to finish on all ranks.
        rpc.api._all_gather(None)

    def _get_preferred_device(self) -> mindtorch.device:
        """
        ...
        if dist.get_backend(self._process_group) == dist.Backend.NCCL:
            return mindtorch.device(mindtorch.cuda.current_device())
        return mindtorch.device("cpu")

    def gather(  # type: ignore[override]
        self,
        dst: int = 0,
        out: Optional[mindtorch.Tensor] = None,
        ...
        Args:
            dst(int): The rank where full tensor is constructed.
                Default: 0
            out (:class `mindtorch.Tensor`, optional): The output full tensor.
            ...
            dtype (mindtorch.dtype): Force the gathered tensors to be this dtype.
                Default: ``None``
        """
        ...
        gather_list: Optional[List[mindtorch.Tensor]]
        if rank == dst:
            ...
            gather_list = [
                mindtorch.empty((max_rank_size,), device=out.device, dtype=dtype)
    ...
    def cpu(
        self, memory_format=mindtorch.preserve_format, process_group=None
    ) -> ShardedTensor:
        """
        ...
        # mindtorch.Tensor subclass, see https://github.com/pytorch/pytorch/issues/75402
        if (
            memory_format != mindtorch.preserve_format
            and memory_format != mindtorch.contiguous_format
        ):
            raise RuntimeError(
                "Only `mindtorch.contiguous_format` or "
                "`mindtorch.preserve_format` is supported!"
            )
        all_on_cpu = True
        ...
        for shard in self._local_shards:
            ...
            metadata.placement._device = mindtorch.device("cpu")  # type: ignore[union-attr]
        ...
        for meta in st_meta.shards_metadata:
            if meta.placement.device().type != "cpu":  # type: ignore[union-attr]
                meta.placement._device = mindtorch.device("cpu")  # type: ignore[union-attr]
    ...
    def cuda(
        ...
        if device is not None:
            device = mindtorch.device(device) if isinstance(device, str) else device
            assert (
                isinstance(device, mindtorch.device)
                and device.index == mindtorch.cuda.current_device()
            ), """Only device without device id (e.g. "cpu" or "cuda") is expected for ShardedTensor!"""

        current_device = mindtorch.device(mindtorch.cuda.current_device())
    ...
    def to(self, *args, **kwargs) -> ShardedTensor:
        ...
        if len(args) == 1:
            if isinstance(args[0], mindtorch.dtype):
                dtype_to = args[0]
            elif isinstance(args[0], mindtorch.device):
                device_to = args[0]
            elif isinstance(args[0], (str, int)):
                device_to = mindtorch.device(args[0])
            elif isinstance(args[0], mindtorch.Tensor):
        ...
        device_to = (
            mindtorch.device(device_to) if isinstance(device_to, (str, int)) else device_to
        )

        if device_to.type == "cuda":
            # if device_to set to cuda, set to current device even
            # if user specify the device index.
            current_idx = mindtorch.cuda.current_device()
            ...
            device_to = mindtorch.device(current_idx)

        copy_tensor = kwargs.get("copy", False)
        non_blocking = kwargs.get("non_blocking", False)
        memory_format = kwargs.get("memory_format", mindtorch.preserve_format)
    ...
    def _init_from_local_tensor(
        ...
        *global_size: Sequence[int],
        ...
        Args:
            ...
            sharding_spec (:class:`mindtorch.distributed._shard.sharding_spec.ShardingSpec`):
        ...
        Examples:
            ...
            >>> tensor = mindtorch.arange(2, dtype=mindtorch.int64) + 1 + 2 * rank
            >>> local_tensor = mindtorch.unsqueeze(mindtorch.cat([tensor, tensor + 2]))
            >>> local_tensor
    ...
    def reshard(self, resharding_spec: shard_spec.ShardingSpec) -> ShardedTensor:
        ...
        Args:
            resharding_spec (:class:`mindtorch.distributed._shard.sharding_spec.ShardingSpec`): The
        ...
        Examples:
            >>> # xdoctest: +SKIP
            >>> # We have 2 process groups, 2 ranks.
            >>> tensor = mindtorch.arange(4, dtype=mindtorch.int64) + 1 + 2 * rank
            >>> tensor = mindtorch.stack([tensor, tensor])
            >>> tensor
    ...
    def __setstate__(self, state):
        ...
        # Setup process group
        from mindtorch.distributed._shard.api import _get_current_process_group
