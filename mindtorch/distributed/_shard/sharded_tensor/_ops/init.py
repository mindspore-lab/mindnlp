...
import mindtorch
import mindtorch.distributed._shard.sharded_tensor as sharded_tensor
...
@_sharded_op_impl(mindtorch.nn.init.uniform_)
def uniform_(types, args=(), kwargs=None, pg=None):
    ...
    for shard in sharded_tensor.local_shards():
        mindtorch.nn.init.uniform_(shard.tensor, a=a, b=b)
    return sharded_tensor


@_sharded_op_impl(mindtorch.nn.init.normal_)
def normal_(types, args=(), kwargs=None, pg=None):
    ...
    for shard in sharded_tensor.local_shards():
        mindtorch.nn.init.normal_(shard.tensor, mean=mean, std=std)
    return sharded_tensor


@_sharded_op_impl(mindtorch.nn.init.kaiming_uniform_)
def kaiming_uniform_(types, args=(), kwargs=None, pg=None):
    ...
    for shard in sharded_tensor.local_shards():
        mindtorch.nn.init.kaiming_uniform_(
            shard.tensor, a=a, mode=mode, nonlinearity=nonlinearity
        )
    return sharded_tensor


@_sharded_op_impl(mindtorch.nn.init.constant_)
def constant_(types, args=(), kwargs=None, pg=None):
    ...
    for shard in sharded_tensor.local_shards():
        mindtorch.nn.init.constant_(shard.tensor, val=val)
    return sharded_tensor


tensor_like_creation_op_map = {
    mindtorch.full_like: sharded_tensor.full,
    mindtorch.empty_like: sharded_tensor.empty,
    mindtorch.zeros_like: sharded_tensor.zeros,
    mindtorch.ones_like: sharded_tensor.ones,
    mindtorch.rand_like: sharded_tensor.rand,
    mindtorch.randn_like: sharded_tensor.randn,
}
...
def register_tensor_creation_op(op):
    ...
    def tensor_creation_op(types, args=(), kwargs=None, pg=None):
        ...
        takes a ShardedTensor as argument, such as ``mindtorch.zeros_like`` or
        ``mindtorch.full_like``.
        """
...
register_tensor_creation_op(mindtorch.full_like)
register_tensor_creation_op(mindtorch.empty_like)
register_tensor_creation_op(mindtorch.zeros_like)
register_tensor_creation_op(mindtorch.ones_like)
register_tensor_creation_op(mindtorch.rand_like)
register_tensor_creation_op(mindtorch.randn_like)
