...
import mindtorch
from mindtorch.autograd.graph import GradientEdge, Node
...
def _get_grad_fn_or_grad_acc(t: mindtorch.Tensor) -> Union[Node, None]:
    """
...
def stage_backward_input(
    stage_outputs_or_loss: List[mindtorch.Tensor],
    output_grads: Optional[List[mindtorch.Tensor]],
    input_values: List[mindtorch.Tensor],
    weights: Iterator[Parameter],
) -> Tuple[Tuple[Optional[mindtorch.Tensor], ...], List[Dict[str, Any]]]:
    """
    ...
    if output_grads is None:
        ...
        output_grads = [
            mindtorch.ones_like(stage_output) for stage_output in stage_outputs_or_loss
        ]

    dinputs = mindtorch.autograd.grad(
...
def stage_backward_weight(
...
) -> Tuple[Optional[mindtorch.Tensor], ...]:
    # map weights to param_group_weights
    grad_acc_to_weight = {}
    weight_grads: List[Optional[mindtorch.Tensor]] = []
    ...
    for param_group in param_groups:
        ...
        dweights = mindtorch.autograd.grad(
            intermediate_edges,
            weights_edges,
...
def stage_backward(
    ...
    try:
        ...
        # tensor values here
        stage_output_tensors: List[mindtorch.Tensor] = []
        output_grad_tensors: List[Optional[mindtorch.Tensor]] = []
        ...
        def extract_tensors_with_grads(
            ...
            if isinstance(output_val, mindtorch.Tensor):
                if not output_val.requires_grad and output_val.grad_fn is None:
                    return
                assert isinstance(
                    grad_val, (mindtorch.Tensor, type(None))
        ...
        # Alternative impl: `mindtorch.autograd.grad`.
        # Note that `mindtorch.autograd.grad` will not accumulate gradients into the
        # model's parameters.
        """
        inputs_with_grad = []
        for val in input_values:
            if isinstance(val, mindtorch.Tensor) and val.requires_grad:
                inputs_with_grad.append(val)

        grad_inputs = mindtorch.autograd.grad(
