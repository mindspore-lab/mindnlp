...
from mindtorch.distributed import ProcessGroup
from mindtorch.export import ExportedProgram
from mindtorch.export.unflatten import (
    _assign_attr,
    _AttrKind,
    _sink_params,
    InterpreterModule,
...
from mindtorch.fx.passes.split_module import split_module
...
def _find_loss_output(mod: mindtorch.nn.Module, g: fx.Graph, output_loss_value_spec):
...
class PipeSequential(mindtorch.nn.Sequential):
    @staticmethod
    def from_sequential(sequential_instance: mindtorch.nn.Sequential):
...
class LossWrapper(mindtorch.nn.Module):
    """
    ...
    be backpropagated by PiPPy. The above class would then be instantiated like::
        ...
        loss_fn = mindtorch.nn.MSELoss() # for the sake of demonstration
...
mindtorch.library.define("pippy::_pipe_split", "() -> ()")


@mindtorch.library.impl("pippy::_pipe_split", "BackendSelect")
def _pipe_split():
    return None


@mindtorch.library.register_fake("pippy::_pipe_split")  # type: ignore[no-redef]
def _pipe_split():  # noqa: F811
    return None


# Add an alias for convenience
aten_pipe_split_alias = mindtorch.ops.pippy._pipe_split.default
...
def pipe_split():
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> def forward(self, x):
        >>>     x = mindtorch.mm(x, self.mm_param)
        >>>     x = mindtorch.relu(x)
        >>>     pipe_split()
        >>>     x = self.lin(x)
        >>>     return x
    ...
    """
    return mindtorch.ops.pippy._pipe_split()
...
class DetachExecutor(fx.Interpreter):
    ...
    def call_module(self, target, args, kwargs):
        def detach_tensors(a):
            if isinstance(a, mindtorch.Tensor) and a.requires_grad:
...
def _direct_serialization_deserialize(body, nodes):
    ...
    class DummyModule(mindtorch.nn.Module):
...
def _modify_graph_op_device(
    ...
    generator ops, like mindtorch.ones.
    """
    modified = False
    for node in gm.graph.nodes:
        if node.op == "call_function":
            ...
            if isinstance(submod, mindtorch.fx.GraphModule):
...
class Pipe(mindtorch.nn.Module):
    def __init__(
        self,
        ...
        mindtorch.nn.Module.__init__(self)
        ...
        params_to_users: Dict[mindtorch.nn.Parameter, Dict[str, str]] = {}
    ...
    def get_stage_module(self, stage_idx: int) -> mindtorch.nn.Module:
        """
    ...
    def _from_traced(
        ...
        def move_param_to_callee(
            ...
            assert isinstance(param_val, mindtorch.Tensor), (
                f"Expected '{param_fqn}' to be {mindtorch.Tensor} but got {type(param_val)}."
                + (
                    f" It might happen if module '{param_fqn}' was passed to some 'leaf function'"
                    f"(see https://pymindtorch.org/docs/stable/fx.html#fx.wrap). Please inspect "
                    f"usages of '{param_fqn}' in the traced graph."
                    if isinstance(param_val, mindtorch.nn.Module)
                    else ""
                )
            )
    ...
    def _trace_with_export(
        ...
        try:
            ep = mindtorch.export.export_for_training(
                mod,
                example_args,
                example_kwargs,
            )
            ...
            raise RuntimeError(
                ...
                "graph breaks, see https://pymindtorch.org/docs/stable/export.html"
            ) from e
    ...
    def build_stage(
        ...
        # `mindtorch.ones`, `mindtorch.zeros`, `mindtorch.rand`, etc.
        if isinstance(stage_module, mindtorch.fx.GraphModule):
            _modify_graph_op_device(stage_module, device)
        else:
            logger.warning(
                f"Expected a `mindtorch.fx.GraphModule` but got {type(stage_module)}"  # noqa: G004
            )
...
def annotate_split_points(mod: mindtorch.nn.Module, spec: Dict[str, SplitPoint]):
