...
from mindtorch.distributed import group, ReduceOp
...
def _all_gather_base(output_tensor, input_tensor, group=group.WORLD):
    ...
    Examples:
        ...
        >>> output_tensor = mindtorch.zeros(2, dtype=mindtorch.int64)
        >>> output_tensor
        [tensor([0, 0])] # Rank 0 and 1
        >>> tensor = mindtorch.arange(1, dtype=mindtorch.int64) + 1 + rank
        >>> tensor
        tensor([1]) # Rank 0
        tensor([2]) # Rank 1
...
class _Broadcast(Function):
    ...
    def forward(ctx, src, group, tensor):
        ...
        # mindtorch.distributed makes all the calls in place
...
class _Gather(Function):
    ...
    def forward(ctx, dst, group, tensor):
        ...
        # gathering
        tensor_list = [
            mindtorch.zeros_like(tensor) for i in range(dist.get_world_size(group=group))
        ]
...
class _Scatter(Function):
    ...
    def forward(ctx, src, group, *tensors):
        ...
        output = mindtorch.zeros_like(tensors[0])
...
class _AllGather(Function):
    ...
    def forward(ctx, group, tensor):
        ...
        ctx.group = group
        out_tensor_list = [
            mindtorch.empty_like(tensor) for _ in range(dist.get_world_size(group=group))
        ]
    ...
    def backward(ctx, *grad_outputs):
        if dist.get_backend(group=ctx.group) is dist.Backend.NCCL:
            ...
            gx = mindtorch.empty_like(grad_outputs[rank])
            ...
            tensor_list = [mindtorch.empty_like(tensor) for tensor in grad_outputs]
            gxs = _AlltoAll.apply(ctx.group, tensor_list, *grad_outputs)
            gx = mindtorch.sum(mindtorch.stack(gxs), dim=0)
        return (None, gx)
