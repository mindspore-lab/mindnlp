...
import mindtorch
from mindtorch.distributed.nn.jit.templates.remote_module_template import (
...
def get_arg_return_types_from_interface(module_interface):
    ...
    ), "Expect a TorchScript class interface decorated by @mindtorch.jit.interface."
    qualified_name = mindtorch._jit_internal._qualified_name(module_interface)
    cu = mindtorch.jit._state._python_cu
...
def instantiate_scriptable_remote_module_template(
    ...
    if not getattr(module_interface_cls, "__torch_script_interface__", False):
        raise ValueError(
            ...
            "@mindtorch.jit.interface"
        )

    # Generate the template instance name.
    module_interface_cls_name = mindtorch._jit_internal._qualified_name(
    ...
    str_dict = dict(
        ...
        args=args_str,
        kwargs=kwargs_str,
        jit_script_decorator="@mindtorch.jit.script",
    )
