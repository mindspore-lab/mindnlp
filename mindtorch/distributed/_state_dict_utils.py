...
if dist.is_available() or TYPE_CHECKING:
    ...
    # from mindtorch.distributed.tensor import distribute_tensor, DTensor, Replicate
    # from mindtorch.distributed.tensor._utils import compute_local_shape_and_global_offset


def _identity_func(
    obj: mindtorch.Tensor,
    pg: Optional[dist.ProcessGroup],
    device: Optional[mindtorch.device],
    companion_obj: Any,
) -> mindtorch.Tensor:
    return obj
...
def _iterate_state_dict(
    ...
    *,
    ...
    Args:
        ...
        device (Optional[mindtorch.device]): device passed to tensor functions
        ...
        type_check (bool): check if the instance data type is a supported type
            ...
            mindtorch.Tensor, DTensor, int, float, str, list, dict, None.
    ...
    if not ranks_only or dist.get_rank(pg) in ranks_only:
        if isinstance(ret, mindtorch.Tensor):
    ...
    *,
    ...
    Args:
        ...
        pg (Optional[dist.ProcessGroup]): the process group that is used to
            ...
            DTensor.
        device: (Optional[mindtorch.device]): the device that is used to
...
def _create_cpu_state_dict(
    ...
    def tensor_func(
        ...
        if len(obj.size()) == 0:
            return mindtorch.tensor(0, dtype=obj.dtype)

        if share_memory:
            t = mindtorch.empty(*tuple(obj.size()), dtype=obj.dtype)
            t = t.share_memory_()
            if pin_memory:

                def unpin_memory(t):
                    succ = int(mindtorch.cuda.cudart().cudaHostUnregister(t.data_ptr()))
                    assert (
                        succ == 0
                ...
                succ = int(
                    mindtorch.cuda.cudart().cudaHostRegister(
                        t.data_ptr(),
            ...
            return mindtorch.empty(*tuple(obj.size()), dtype=obj.dtype).pin_memory()
        else:
            return mindtorch.empty(*tuple(obj.size()), dtype=obj.dtype)
...
def _broadcast_tensors(
    ...
    for key in keys:
        if dist.get_rank() == 0:
            ...
            assert isinstance(full_state, mindtorch.Tensor)
            full_tensor = full_state.detach().to(device)
        else:
            tensor_info = full_state_dict[key]
            full_tensor = mindtorch.empty(
...
def _distribute_tensors(
    ...
    for key in keys:
        ...
        if _local_state is None or mindtorch.is_tensor(_local_state):
            continue
...
def _distribute_state_dict(
    ...
    for key, value in full_state_dict.items():
        ...
        if not mindtorch.is_tensor(value):
            ...
            assert isinstance(value, mindtorch.Tensor)
            local_state = local_state_dict.get(key, None)
            if local_state is None:
                continue
            elif isinstance(local_state, DTensor):
...
# These APIs are from mindtorch.distributed.checkpoint.
