...
from mindtorch.distributed.rpc import RRef, TensorPipeRpcBackendOptions
from mindtorch.distributed.rpc.backend_registry import BackendType
...
class HybridModel(mindtorch.nn.Module):
    r"""
    ...
    def __init__(self, emb_rref_list, device):
        ...
        fc1 = mindtorch.nn.Linear(512, 256)
        fc2 = mindtorch.nn.Linear(256, 128)
        relu = mindtorch.nn.ReLU()
        fc3 = mindtorch.nn.Linear(128, 64)
        fc4 = mindtorch.nn.Linear(64, 32)
        fc5 = mindtorch.nn.Linear(32, 8)
    ...
    def forward(self, indices, offsets):
        ...
        for emb_rref in self.emb_rref_list:
            ...
            emb_lookups_cat = mindtorch.cat(emb_lookups, dim=1)
...
def _run_printable(cmd):
    ...
    buffer = io.BytesIO()
    mindtorch.save(proc.stdout.decode("utf-8"), buffer)
    input_tensor = mindtorch.ByteTensor(list(buffer.getvalue()))

    output = []
    buffer = io.BytesIO(np.asarray(input_tensor).tobytes())
    output.append(mindtorch.load(buffer))
    return output
...
def _run_trainer(emb_rref_list, rank):
    ...
    def get_next_batch(rank):
        for _ in range(10):
            ...
            indices = mindtorch.LongTensor(num_indices).random_(0, NUM_EMBEDDINGS)
            ...
            offsets_tensor = mindtorch.LongTensor(offsets)
            target = mindtorch.LongTensor(batch_size).random_(8).cuda(rank)
...
if __name__ == "__main__":
    ...
    print("-------------------------------------------")
    print()
    print(f"* PyTorch version: {mindtorch.__version__}")
    print(f"* CUDA version: {mindtorch.version.cuda}")
    print()
