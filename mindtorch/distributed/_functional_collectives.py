...
# from mindtorch.fx.experimental.proxy_tensor import get_proxy_mode

from . import _functional_collectives_impl as fun_col_impl


# try:
#     from mindtorch.utils._cxx_pytree import tree_map_only
# except ImportError:
#     from mindtorch.utils._pytree import tree_map_only  # type: ignore[no-redef]


# if mindtorch._running_with_deploy():

#     def is_torchdynamo_compiling():
#         """Can't import mindtorchdynamo in torchdeploy builds currently."""
#         return False

# else:
#     try:
#         from mindtorch.compiler import is_dynamo_compiling as is_torchdynamo_compiling
#     except Exception:
#         warnings.warn(
#             "Unable to import mindtorchdynamo util `is_torchdynamo_compiling`, so won't support torchdynamo correctly"
#         )
...
Issues:
* Where should these ops live? Couldn't `import mindtorch` if putting these ops in existing mindtorch.distributed files
...
tensor - even return a tensor with a different shape (see ``mindtorch.vmap``).
...
Here's how it works under mindtorch.compile/dynamo:
all_reduce(...)
...
def wait_tensor(tensor):
    ...
    """
    return mindtorch.ops._c10d_functional.wait_tensor(tensor)  # type: ignore[attr-defined]


def broadcast(self: mindtorch.Tensor, src: int, group: RANK_TYPES, tag: str = ""):
    """
    ...
    tensor = mindtorch.ops._c10d_functional.broadcast(self, src, group_name)
    return _maybe_wrap_tensor(tensor)


def all_reduce(self: mindtorch.Tensor, reduceOp: str, group: RANK_TYPES, tag: str = ""):
    """
    ...
    """
    group_name = _resolve_group_name(group, tag)
    tensor = mindtorch.ops._c10d_functional.all_reduce(self, reduceOp.lower(), group_name)
    return _maybe_wrap_tensor(tensor)


def all_gather_tensor(
    self: mindtorch.Tensor,
    gather_dim: int,
    group: RANK_TYPES,
    tag: str = "",
):
    ...
    tensor = mindtorch.ops._c10d_functional.all_gather_into_tensor(
        self, group_size, group_name
    )
    res = _maybe_wrap_tensor(tensor)
    # TODO this should be done inside AsyncCollectiveTensor to delay the wait() call
    if gather_dim != 0:
        # mindtorch.cat access the data so we already need to wait here, first do wait
        ...
        res = mindtorch.cat(mindtorch.chunk(res, group_size, dim=0), dim=gather_dim)
    return res
...
def all_gather_tensor_autograd(
    ...
    tensor = mindtorch.ops._c10d_functional_autograd.all_gather_into_tensor(
        self, group_size, group_name
    )
...
def reduce_scatter_tensor(
    ...
    if scatter_dim != 0:
        tensor_list = mindtorch.chunk(self, group_size, dim=scatter_dim)
        self = mindtorch.cat(tensor_list)

    tensor = mindtorch.ops._c10d_functional.reduce_scatter_tensor(
        self,
        reduceOp.lower(),
        group_size,
...
def reduce_scatter_tensor_autograd(
    ...
    tensor = mindtorch.ops._c10d_functional_autograd.reduce_scatter_tensor(
        self,
        reduceOp.lower(),
        group_size,
        group_name,  # type: ignore[possibly-undefined]
...
def all_reduce_coalesced(
    self: List[mindtorch.Tensor], reduceOp: str, group: RANK_TYPES, tag: str = ""
    ...
    """
    group_name = _resolve_group_name(group, tag)
    tensor_list = mindtorch.ops._c10d_functional.all_reduce_coalesced(  # type: ignore[attr-defined]
        self,
        reduceOp.lower(),
        group_name,
    )
...
def all_gather_into_tensor_coalesced(
    self: List[mindtorch.Tensor], group: RANK_TYPES, tag: str = ""
) -> List[mindtorch.Tensor]:
    """
    ...
    group_name = _resolve_group_name(group, tag)
    group_size = c10d._get_group_size_by_name(group_name)
    tensor_list = mindtorch.ops._c10d_functional.all_gather_into_tensor_coalesced(  # type: ignore[attr-defined]
        self,
        group_size,
        group_name,
    )
...
def reduce_scatter_tensor_coalesced(
    ...
    for idx, (dim, tensor) in enumerate(zip(scatter_dim, inputs)):
        ...
        if dim != 0:
            tensor_list = mindtorch.chunk(tensor, group_size, dim=dim)
            inputs[idx] = mindtorch.cat(tensor_list)

    tensor_list = mindtorch.ops._c10d_functional.reduce_scatter_tensor_coalesced(  # type: ignore[attr-defined]
        inputs,
        reduceOp.lower(),
        group_size,
        group_name,  # type: ignore[possibly-undefined]
...
def _is_view_op(tgt):
    assert isinstance(tgt, mindtorch._ops.OpOverload)
    schema = tgt._schema
...
def all_to_all_single(
    ...
    if output_split_sizes is not None:
        assert all(
            isinstance(size, (int, mindtorch.SymInt)) for size in output_split_sizes
        ), output_split_sizes
    if input_split_sizes is not None:
        assert all(
            isinstance(size, (int, mindtorch.SymInt)) for size in input_split_sizes
        ), input_split_sizes
    group_name = _resolve_group_name(group, tag)
    group_size = c10d._get_group_size_by_name(group_name)
    ...
    tensor = mindtorch.ops._c10d_functional.all_to_all_single(  # type: ignore[attr-defined]
        self,
        output_split_sizes,
        input_split_sizes,
        group_name,
...
def all_to_all_single_autograd(
    ...
    if output_split_sizes is None or input_split_sizes is None:
        ...
        output_split_sizes = [self.shape[0] // group_size] * group_size
        input_split_sizes = output_split_sizes
    tensor = mindtorch.ops._c10d_functional_autograd.all_to_all_single(  # type: ignore[attr-defined]
        self,
        output_split_sizes,
        input_split_sizes,
        group_name,
...
class AsyncCollectiveTensor(mindtorch.Tensor):
    r"""
    ...
    def functional_collective(self, group, tag):
        ...
        tensor = mindtorch.ops.c10d_functional.{collective}(self, tag, rankset, group_size)
        return _maybe_wrap_tensor(tensor)
    """
    elem: mindtorch.Tensor
    completed: bool

    __slots__ = ["elem", "completed"]

    @staticmethod
    def __new__(cls, elem: mindtorch.Tensor):
    ...
    def __coerce_same_metadata_as_tangent__(
        ...
        if expected_type is not mindtorch.Tensor:
            return None
    ...
    def wait(self) -> mindtorch.Tensor:
    ...
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
        if func == mindtorch.ops.aten.view.default:
        ...
        if is_view_op:
            out = tree_map_only(mindtorch.Tensor, wrap, out)
...
def _expand_group(group: RANK_TYPES, tag: str = "") -> Tuple[str, List[int], int]:
    ...
    # graph_break [('mindtorch.* op returned non-Tensor int
    # caused by 'cast_*` functions being treated as 'mindtorch.*' ops (iiuc)
    if TYPE_CHECKING:
...
class _FromTorchTensor(mindtorch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output: mindtorch.Tensor) -> mindtorch.Tensor:  # type: ignore[override]
        return grad_output
...
def _are_we_tracing() -> bool:
    ...
    if (
        mindtorch._C._get_dispatch_mode(mindtorch._C._TorchDispatchModeKey.FUNCTIONAL)
        is not None
    ):
        return True
...
def _maybe_wrap_tensor(self) -> mindtorch.Tensor:
    if _are_we_tracing():
        return wait_tensor(self)
    res = AsyncCollectiveTensor(self)
    return cast(mindtorch.Tensor, res)


@contextlib.contextmanager
def allow_inflight_collective_as_graph_input_ctx(value: bool = True):
    """
    Context manager to temporarily set whether inflight collectives are allowed as mindtorch.compile graph inputs.
    ...
    @mindtorch.compile(fullgraph=True)
    def all_reduce_wait_compiled(y):
        mindtorch.ops.c10d_functional.wait_tensor(y)
        return y * y

    x = mindtorch.ones(1280, 1280, device="cuda") + self.rank
    ...
    """
    previous = mindtorch._C._distributed_c10d._allow_inflight_collective_as_graph_input()

    try:
        mindtorch._C._distributed_c10d._set_allow_inflight_collective_as_graph_input(value)
        yield
    finally:
        mindtorch._C._distributed_c10d._set_allow_inflight_collective_as_graph_input(
            previous
        )
...
def _broadcast_meta(self, *args):
    return mindtorch.empty_like(self)
...
def _all_reduce_coalesced_meta(self, *args):
    return [mindtorch.empty_like(t) for t in self]
...
#     lib_impl = mindtorch.library.Library("_c10d_functional", "IMPL")
...
#     mindtorch.fx.node.has_side_effect(mindtorch.ops._c10d_functional.wait_tensor.default)
#     mindtorch.fx.node.has_side_effect(mindtorch.ops._c10d_functional.wait_tensor)

#     # Register legacy ops for backward compatibility
#     # TODO(yifu): remove these in functional collective beta release
#     legacy_lib = mindtorch.library.Library("c10d_functional", "DEF")
#     legacy_lib_impl = mindtorch.library.Library("c10d_functional", "IMPL")
#     ops_defs = [
...
#         legacy_lib.define(op_def, tags=mindtorch.Tag.pt2_compliant_tag)
...
These schemas intentionally match mindtorch.distributed.distributed_c10d.* ops that we are trying to remap from
"""


def all_gather_tensor_inplace(
    output_tensor: mindtorch.Tensor,
    input_tensor: mindtorch.Tensor,
...
def all_gather_inplace(
    tensor_list: List[mindtorch.Tensor],
    tensor: mindtorch.Tensor,
    group=None,
    async_op=False,
    tag: str = "",
