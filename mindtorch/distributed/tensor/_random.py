...
class _RNGStateTracker:
    ...
    def __init__(self, device: mindtorch.device):
        self._device = device
...
class OffsetBasedRNGTracker(_RNGStateTracker):
    ...
    def _distribute_region(self, spec: DTensorSpec):
        ...
        if self.distribute_region_enabled:
            ...
            with mindtorch.random.fork_rng(
    ...
    def _set_pre_op_offset(self, spec: DTensorSpec) -> None:
        ...
        # pymindtorch: offset must be multiple of 4
...
def _resolve_device(device_mesh: DeviceMesh) -> mindtorch.device:
    ...
    return mindtorch.device(f"{device_type}:{device_idx:d}")
