...
def _quantize_per_tensor_backend(x, scale, zero_point):
    y = mindtorch.round(x / scale) + zero_point
    y = mindtorch.clamp(y, 0, 255).to(mindtorch.uint8)
    return y


def _dequantize_per_tensor_backend(y, scale, zero_point):
    x = scale * (y.to(mindtorch.float32) - zero_point)
    return x


def _quantize_per_channel_backend(x, scale, zero_point):
    y = mindtorch.zeros(x.size(), device=x.device)
    for i in range(x.size()[0]):
        y[i, :] = mindtorch.round(x[i, :] / scale[i]) + zero_point[i]
    y = mindtorch.clamp(y, 0, 255).to(mindtorch.uint8)
    return y


def _dequantize_per_channel_backend(y, scale, zero_point):
    y = y.to(mindtorch.float32).to(y.device)
    x = mindtorch.zeros_like(y, device=y.device)
    for i in range(x.size()[0]):
...
def quantization_pertensor_hook(
    ...
    """
    Apply ``mindtorch.quantize_per_tensor`` logic to DDP using ``allgather`` protocol.
    ...
    myObserver = mindtorch.ao.quantization.MinMaxObserver().to(tensor.device)
    myObserver(tensor)

    s, z = myObserver.calculate_qparams()
    s_and_z = mindtorch.FloatTensor([s, z]).to(tensor.device)
    ...
    def dequantize_and_aggregate(fut):
        ...
        aggregated_dequantized_tensor = mindtorch.zeros_like(
            all_ranks_quantized_tensor[0], device=tensor.device, dtype=mindtorch.float32
        )
...
def quantization_perchannel_hook(
    ...
    """
    Apply``mindtorch.quantize_per_channel`` logic to DDP using ``allgather`` protocol.
    ...
    myPerChannelObserver = mindtorch.ao.quantization.PerChannelMinMaxObserver().to(
        tensor.device
    )
    myPerChannelObserver(tensor_in_channels)

    s_ch, z_ch = myPerChannelObserver.calculate_qparams()
    s_and_z = mindtorch.stack((s_ch, z_ch)).to(tensor.device)
    ...
    def dequantize_and_aggregate(fut):
        ...
        return (
            mindtorch.flatten(aggregated_dequantized_tensor).to(tensor.device)[
                : tensor.size()[0]
            ]
            / world_size
        )
