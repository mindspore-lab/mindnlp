...
TORCH_HALF_MIN = mindtorch.finfo(mindtorch.float16).min
TORCH_HALF_MAX = mindtorch.finfo(mindtorch.float16).max
...
def _fp32_to_fp16_with_clamp(tensor: mindtorch.Tensor) -> mindtorch.Tensor:
    return mindtorch.clamp(tensor, TORCH_HALF_MIN, TORCH_HALF_MAX).half()


def _quantize_tensor(tensor, qtype):
    if not isinstance(tensor, mindtorch.Tensor):
        raise RuntimeError(
            f"_quantize_tensor expecting mindtorch.Tensor as input but found {type(tensor)}"
        )
    if qtype == DQuantType.FP16:
        return _fp32_to_fp16_with_clamp(tensor)
    elif qtype == DQuantType.BFP16:
        return mindtorch.ops.quantization._FloatToBfloat16Quantized(tensor)
    else:
...
def _quantize_tensor_list(tensor_list, qtype):
    if not isinstance(tensor_list, list) or not all(
        isinstance(p, mindtorch.Tensor) for p in tensor_list
    ):
        raise RuntimeError(
            f"_quantize_tensor_list expecting list of mindtorch.Tensor as input but found {type(tensor_list)}"
        )
...
def _dequantize_tensor(tensor, qtype, quant_loss=None):
    if not isinstance(tensor, mindtorch.Tensor):
        raise RuntimeError(
            f"_dequantize_tensor expecting mindtorch.Tensor as input but found {type(tensor)}"
        )
    if qtype == DQuantType.FP16:
        if tensor.dtype != mindtorch.float16:
            raise RuntimeError(
                f"tensor dtype is {tensor.dtype} while expected to be FP16."
            )
        elif tensor.dtype == mindtorch.float16 and quant_loss is None:
            return tensor.float()
        else:
        ...
        if tensor.dtype != mindtorch.float16:
            ...
            return mindtorch.ops.quantization._Bfloat16QuantizedToFloat(tensor)
    else:
        raise RuntimeError(f"Quantization type {qtype} is not supported")
...
def _dequantize_tensor_list(tensor_list, qtype, quant_loss=None):
    if not isinstance(tensor_list, list) or not all(
        ...
        raise RuntimeError(
            f"_dequantize_tensor_list expecting list of mindtorch.Tensor as input but found {type(tensor_list)}"
        )
