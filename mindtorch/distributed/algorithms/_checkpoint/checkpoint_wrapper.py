...
from mindtorch.autograd.graph import save_on_cpu
from mindtorch.distributed.utils import _pack_kwargs, _replace_by_prefix, _unpack_kwargs
from mindtorch.utils.checkpoint import checkpoint as torch_utils_checkpoint
...
class ActivationWrapper(mindtorch.nn.Module, ABC):
    """
    ...
    def __init__(
        ...
        **checkpoint_fn_kwargs,
        ...
        if checkpoint_fn is None:
            # use mindtorch.utils.checkpoint
...
def offload_wrapper(module: mindtorch.nn.Module) -> mindtorch.nn.Module:
    """
    ...
    **checkpoint_fn_kwargs,
    ...
    Args:
        ...
        checkpoint_impl (Optional[CheckpointImpl]):
            ...
            be passed into the ``mindtorch.utils.checkpoint.checkpoint``
            ...
            from ``mindtorch.utils.checkpoint``, keyword arguments will only be
            supported if ``checkpoint_impl`` is passed as ``CheckpointImpl.REENTRANT`.
        checkpoint_fn (Optional[Callable]):
            Functional checkpoint implementation to use. If this is specified,
            it will be used over the default ``mindtorch.utils.checkpoint.checkpoint``
...
def apply_activation_checkpointing(
    ...
    from mindtorch.distributed.fsdp._wrap_utils import _construct_wrap_fn, _post_order_apply
