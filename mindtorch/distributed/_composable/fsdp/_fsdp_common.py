...
def _get_dim0_padded_size(tensor_size: mindtorch.Size, dim0_factor: int) -> mindtorch.Size:
    padded_dim0 = math.ceil(tensor_size[0] / dim0_factor) * dim0_factor
    return cast(mindtorch.Size, mindtorch.Size([padded_dim0]) + tensor_size[1:])


def _chunk_with_empty(
    tensor: mindtorch.Tensor, num_chunks: int, dim: int
) -> List[mindtorch.Tensor]:
    chunks = list(mindtorch.chunk(tensor, num_chunks, dim=dim))
...
def _get_dim_chunked_size(
    chunk: mindtorch.Tensor, unchunked_size: mindtorch.Size, dim: int
) -> mindtorch.Size:
    if chunk.numel() > 0:
        return chunk.size()
    # For 0 numel, we need to preserve nonzero-sized dims for DTensor APIs
    return cast(
        mindtorch.Size, unchunked_size[:dim] + mindtorch.Size([0]) + unchunked_size[dim + 1 :]
    )
...
def _to_dtype_if_needed(
    tensor: mindtorch.Tensor, dtype: Optional[mindtorch.dtype]
) -> mindtorch.Tensor:
...
def _cast_fp_tensor(dtype: mindtorch.dtype, x: mindtorch.Tensor) -> mindtorch.Tensor:
    if (
        not isinstance(x, mindtorch.Tensor)
        or not mindtorch.is_floating_point(x)
        or x.dtype == dtype
    ):
        return x
    return x.to(dtype)
