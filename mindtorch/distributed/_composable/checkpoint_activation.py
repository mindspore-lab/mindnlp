...
from mindtorch.utils.checkpoint import (
...
def checkpoint(module: nn.Module, **kwargs) -> nn.Module:
    ...
    Example::
        ...
        >>> model(mindtorch.zeros(2, 10)).sum().backward()

    """
    mindtorch._C._log_api_usage_once("mindtorch.distributed.checkpoint")

    use_reentrant = kwargs.pop("use_reentrant", False)
    if use_reentrant:
        raise NotImplementedError(
            "use_reentrant=True is not supported in composable checkpoint. "
            "Please use mindtorch.utils.checkpoint.checkpoint instead."
        )
