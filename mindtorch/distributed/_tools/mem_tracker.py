...
from mindtorch.utils._python_dispatch import (
    is_traceable_wrapper_subclass,
    TorchDispatchMode,
)
from mindtorch.utils._pytree import tree_flatten, tree_map_only
from mindtorch.utils.weak import WeakIdKeyDictionary, weakref
...
class _ModMemStats:
    ...
    Attributes:
        ...
        snapshots (Dict[_ModState, Dict[mindtorch.device, Dict[str, int]]]): A dictionary of memory snapshots
        of the module at different states defined by ``_ModState``.
    Note:
        The memory snapshot is stored as a dictionary - Dict[mindtorch.device, Dict[str, int]], where each key is a device,
    ...
    def __init__(self, mod_fqn: str):
        ...
        self.input_mem: int
        self.output_mem: int
        self.local_peak: Dict[mindtorch.device, int] = {}
        self.snapshots: Dict[_ModState, List[Dict[mindtorch.device, Dict[str, int]]]] = {}
...
class _WeakRefInfo:
    ...
    def __init__(
        self, size: int, element_size: int, device: mindtorch.device, reftype: _RefType
    ) -> None:
        """
        ...
        Args:
            ...
            device (mindtorch.device): The device on which the tensor is allocated.
    ...
    def update_mem_consumed(self, st: mindtorch.UntypedStorage) -> int:
        """
        Updates and returns the memory consumed if the storage size has changed.

        Args:
            st (mindtorch.UntypedStorage): The tensor storage to check for size updates.
    ...
    @staticmethod
    def get_untyped_storages(t: mindtorch.Tensor) -> Set[mindtorch.UntypedStorage]:
        """
        Recursively extracts untyped storages from a tensor or its subclasses.

        Args:
            t (mindtorch.Tensor): The tensor to extract storages from.

        Returns:
            Set[mindtorch.UntypedStorage]: A set of untyped storages.
        """
    ...
    def create_winfo(
        cls,
        st: mindtorch.UntypedStorage,
        device: mindtorch.device,
        reftype: _RefType,
        callback: Optional[Callable[[Self, weakref.ref], Any]] = None,
    ) -> Tuple[Self, weakref.ref]:
        """
        Creates a new ``_WeakRefInfo`` instance and a weak reference to a ``mindtorch.UntypedStorage`` object,
        optionally attaching a callback to the weak reference.

        Args:
            st (mindtorch.UntypedStorage): The storage object for which to create the weak reference info.
            device (mindtorch.device): The device associated with the storage object.
...
def _print_snapshot(snapshot: Dict[mindtorch.device, Dict[str, int]], units: str) -> None:
    if len(snapshot) == 0:
...
def _print_snapshot_tabular(
    snapshot: Dict[mindtorch.device, Dict[str, int]], units: str
) -> None:
    if len(snapshot) == 0:
        print("No memory tracked.")
        return
...
def _print_state_snapshots(
    snapshots: Dict[_State, List[Dict[mindtorch.device, Dict[str, int]]]], units: str
) -> None:
...
class MemTracker(TorchDispatchMode):
    ...
    Known Limitations:
        ...
        - Resizing tensor storages directly by using non-Tensor methods other than using ``mindtorch.Untyped_Storage.resize_``
          is not tracked. File a Github issue if you have use-cases for this.
        - If the tensors are not traceable or wrappable subclasses of ``mindtorch.Tensor``, then the tracker does not know how to
        ...
        - During AC in the backward pass there might be misattribution between activation and temp memory, but the peak memory
          will be tracked accurately. This will be fixed in the next update by hooking intricately with ``mindtorch.uitls.checkpoint``.
    """

    def __init__(self) -> None:
        self.memory_tracking = WeakIdKeyDictionary()
        self._curr_mem_snap: Dict[mindtorch.device, Dict[str, int]] = {}
        self._peak_mem: Dict[mindtorch.device, int] = {}
        self._peak_mem_snap: Dict[mindtorch.device, Dict[str, int]] = {}
        ...
        self._orig_resize = mindtorch.UntypedStorage.resize_
    ...
    def _track_resize(self) -> None:
        # Need to monkey-patch this because ``mindtorch.UntypedStorage.resize_`` is not captured
        # by ``TorchDispatchMode``.
        @wraps(self._orig_resize)
        def resize_(st: mindtorch.UntypedStorage, size: int) -> None:
        ...
        mindtorch.UntypedStorage.resize_ = resize_  # type: ignore[method-assign, assignment]

    def _restore_resize(self) -> None:
        mindtorch.UntypedStorage.resize_ = self._orig_resize  # type: ignore[method-assign]
    ...
    def _track(self, reftype: _RefType, t: mindtorch.Tensor) -> None:
    ...
    def get_tracker_snapshot(
    ...
    ) -> Dict[mindtorch.device, Dict[str, int]]:
        """
        ...
        Returns:
            Dict[mindtorch.device, Dict[str, int]]: A dictionary where each key is a mindtorch.device, and each value is another
    ...
    def _track_module_params_and_buffers(
        ...
        def _grad_hook(grad: mindtorch.Tensor) -> None:
    ...
    def _track_inputs_or_outputs(self, args: Any) -> int:
        ...
        def add_inps_or_outs(t: mindtorch.Tensor) -> None:
        ...
        tree_map_only(mindtorch.Tensor, add_inps_or_outs, args)
    ...
    def track_external(
        self, *external: Union[nn.Module, optim.Optimizer, mindtorch.Tensor]
    ) -> None:
        """
        ...
        Args:
            *external (Union[nn.Module, optim.Optimizer, mindtorch.Tensor]): The external modules, optimizers, and
        ...
        for obj in flat_external:
            ...
            elif isinstance(obj, mindtorch.nn.Module):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):  # type: ignore[no-untyped-def]
        ...
        tree_map_only(mindtorch.Tensor, partial(self._track, reftype), res)
