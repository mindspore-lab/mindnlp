...
import mindtorch
from mindtorch import nan, nn, UntypedStorage
from mindtorch._guards import active_fake_mode
from mindtorch._subclasses.fake_tensor import FakeTensorMode
from mindtorch.distributed._tools.mod_tracker import ModTracker
from mindtorch.distributed._tools.runtime_estimator import RuntimeEstimator
from mindtorch.testing._internal.composite_compliance import (
    is_inplace,
    is_inplace_view_fn,
    is_view_fn,
)
...
from mindtorch.utils.checkpoint import SAC_IGNORED_OPS
...
_ADDITIONAL_IGNORED_OPS = {
    ...
    mindtorch.ops.profiler._record_function_exit._RecordFunction,  # type: ignore[attr-defined]
    aten.clone.default,  # type: ignore[attr-defined] # seems needed for mindtorch.compile
}
...
def _get_untyped_storages(t: mindtorch.Tensor) -> Set[mindtorch.UntypedStorage]:
    """
    Retrieves untyped storages from a `mindtorch.Tensor` or one of its traceable wrapper-subclass.

    Args:
       t (mindtorch.Tensor): Input `mindtorch.Tensor` or traceable wrapper-subclass of `mindtorch.Tensor`.

    Returns:
        Set[mindtorch.UntypedStorage]: Set of untyped storages.
...
class SACEstimator(TorchDispatchMode):
    ...
    runtime trade-offs of functions or ``mindtorch.nn.Module``s for Selective Activation Checkpointing (SAC). It provides
    ...
    def __init__(self) -> None:
        ...
        self._saved_tensor_hook_ctx = mindtorch.autograd.graph.saved_tensors_hooks(
            self._pack_hook, lambda x: x
        )
        self._saved_tensor_ids: Set[int] = set()
        self._estimate_runtime = RuntimeEstimator._roofline_estimate

    def _pack_hook(self, x: mindtorch.Tensor) -> mindtorch.Tensor:
        ...
        # Used in conjunction with mindtorch.autograd.graph.saved_tensors_hooks
    ...
    def _get_force_store_random(self, inputs: Any) -> bool:
        ...
        return all(not isinstance(x, mindtorch.Tensor) for x in flat_inputs)
    ...
    def __torch_dispatch__(  # type: ignore[no-untyped-def]
        ...
        cuda_devices: Set[mindtorch.device] = set()
        for o in flat_outs:
            if isinstance(o, mindtorch.Tensor):
        ...
        is_rand_op = mindtorch.Tag.nondeterministic_seeded in func.tags
        if is_view_like:
            nbytes = 0
