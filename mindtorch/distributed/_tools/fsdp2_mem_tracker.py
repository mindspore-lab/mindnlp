...
from mindtorch.distributed._composable.fsdp import FSDPModule
from mindtorch.distributed._composable.fsdp._fsdp_param_group import FSDPParamGroup
from mindtorch.distributed._tools.mem_tracker import _RefType, _State, MemTracker
...
class _FSDPModMemStats:
    ...
    Attributes:
        snapshots (Dict[_FSDPModState, Dict[mindtorch.device, Dict[str, int]]]): A dictionary of memory snapshots
    ...
    def __init__(self, mod_fqn: str) -> None:
        ...
        self.snapshots: Dict[
            _FSDPModState, List[Dict[mindtorch.device, Dict[str, int]]]
        ] = {}


class FSDPMemTracker(MemTracker):
    """
    A ``TorchDispatchMode`` based context manager that extends ``mindtorch.distributed._tools.mem_tracker.MemTracker`` to track
    ...
    Args:
        mod (mindtorch.nn.Module): The root FSDP module to be tracked.
        optm (mindtorch.optim.Optimizer, optional): The optimizer to be tracked.

    Note: Please refer to ``mindtorch.distributed._tools.mem_tracker.MemTracker`` to learn about the limitations.
    ...
    def __init__(
        ...
        optm: Optional[mindtorch.optim.Optimizer] = None,
    ) -> None:
        super().__init__()
    ...
    def track_inputs(self, inputs: Tuple[Any, ...]) -> None:
        ...
        def _track_inputs(t: mindtorch.Tensor) -> None:
            self._update_and_maybe_create_winfos(
                t,
                _FSDPRefType.INP,
            )

        tree_map_only(mindtorch.Tensor, _track_inputs, inputs)
