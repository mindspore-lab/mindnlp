...
class MemoryProfileDispatchMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=..., kwargs=None):
        rs = func(*args, **kwargs)
        if func == mindtorch.ops.aten.detach.default:
            return rs
        func_name: str = (
...
class MemoryTracker:
    ...
    def __init__(self) -> None:
        mindtorch._C._log_api_usage_once("mindtorch.distributed.memory_tracker")
    ...
    def stop(self) -> None:
        ...
        """
        self._num_cuda_retries = mindtorch.cuda.memory_stats().get("num_alloc_retries", 0)
    ...
    def _create_post_forward_hook(self, name: str) -> Callable:
        ...
        def _post_forward_hook(
            module: nn.Module,
            inputs: Sequence[mindtorch.Tensor],
    ...
    def _create_backward_hook(self, name: str) -> Callable:
        ...
        def _backward_hook(
            module: nn.Module, grad_input: mindtorch.Tensor, grad_output: mindtorch.Tensor
        ) -> None:
    ...
    def _record_memory_stats(self, fn_name: str) -> None:
        ...
        """
        memory_allocated: float = mindtorch.cuda.memory_allocated() / BYTES_PER_MB
        memory_reserved: float = mindtorch.cuda.memory_reserved() / BYTES_PER_MB
        memory_active: float = (
            mindtorch.cuda.memory_stats().get("active_bytes.all.current", 0) / BYTES_PER_MB
        )
