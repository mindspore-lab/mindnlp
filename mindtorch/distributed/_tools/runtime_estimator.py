...
from mindtorch._inductor.utils import get_device_tflops, get_gpu_dram_gbps
...
from mindtorch.utils.flop_counter import flop_registry
...
class RuntimeEstimator(TorchDispatchMode):
    ...
    _float_types: Set[mindtorch.dtype] = {
        mindtorch.float16,
        mindtorch.bfloat16,
        mindtorch.float32,
        mindtorch.float64,
    }
    _no_fallback_kernel: Set[mindtorch._ops._OpNamespace] = set()
    ...
    def _maybe_run_and_benchmark_fallback_kernel(  # type: ignore[no-untyped-def]
        ...
        if mindtorch.Tag.inplace_view in func.tags:  # type: ignore[attr-defined]
        ...
        with no_dispatch():
            ...
            def to_real_tensor(e):  # type: ignore[no-untyped-def]
                if cls.fake_mode.is_our_fake(e):
                    if e.dtype in cls._float_types:
                        out = mindtorch.rand_like(e, device=e.fake_device)
                    else:
                        out = mindtorch.ones_like(e, device=e.fake_device)
                    if e.is_sparse:
            ...
            for _ in range(warmup_iters):
                func(*args, **kwargs)
            start_event = mindtorch.cuda.Event(enable_timing=True)
            end_event = mindtorch.cuda.Event(enable_timing=True)
            start_event.record(mindtorch.cuda.current_stream())
            for _ in range(actual_iters):
                func(*args, **kwargs)
            end_event.record(mindtorch.cuda.current_stream())
        ...
        for e in flat_args:
            if isinstance(e, mindtorch.Tensor):
                if not e.is_sparse:
    ...
    def _roofline_estimate(cls, func, args, kwargs) -> Tuple[Any, float]:  # type: ignore[no-untyped-def]
        ...
        def get_num_bytes(t: mindtorch.Tensor) -> int:
            """
            Calculates the memory consumption of a tensor.

            Args:
                t (mindtorch.Tensor): The input tensor.
        ...
        def get_transfer_time(flat_args_kwargs, flat_outs) -> float:  # type: ignore[no-untyped-def]
            ...
            Args:
                flat_args_kwargs (List[mindtorch.Tensor]): The flat list of arguments and keyword arguments.
                flat_outs (List[mindtorch.Tensor]): The flat list of outputs.
            ...
            read_bytes = sum(
                ...
                if isinstance(t, mindtorch.Tensor)
            )
            write_bytes = sum(
                get_num_bytes(t) for t in flat_outs if isinstance(t, mindtorch.Tensor)
            )
        ...
        if func_packet not in _IGNORE_OPS:
            ...
            out_dtypes = {
                t.dtype
                for t in flat_outs
                if isinstance(t, mindtorch.Tensor) and t.dtype in cls._float_types
            }
