...
"""
Superset of ``mindtorch.distributed.launch``.

``torchrun`` provides a superset of the functionality as ``mindtorch.distributed.launch``
...
.. note:: ``torchrun`` is a python
          ...
          to the main module
          `mindtorch.distributed.run <https://github.com/pytorch/pytorch/blob/master/torch/distributed/run.py>`_
          declared in the ``entry_points`` configuration in
          `setup.py <https://github.com/pytorch/pytorch/blob/master/setup.py>`_.
          It is equivalent to invoking ``python -m mindtorch.distributed.run``.


Transitioning from mindtorch.distributed.launch to torchrun
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~


``torchrun`` supports the same arguments as ``mindtorch.distributed.launch`` **except**
for ``--use-env`` which is now deprecated. To migrate from ``mindtorch.distributed.launch``
...
1.  If your training script is already reading ``local_rank`` from the ``LOCAL_RANK`` environment variable.
    ...
    +--------------------------------------------------------------------+--------------------------------------------+
    |         ``mindtorch.distributed.launch``                               |                ``torchrun``                |
    +====================================================================+============================================+
    |                                                                    |                                            |
    | .. code-block:: shell-session                                      | .. code-block:: shell-session              |
    |                                                                    |                                            |
    |    $ python -m mindtorch.distributed.launch --use-env train_script.py  |    $ torchrun train_script.py              |
    |                                                                    |                                            |
    +--------------------------------------------------------------------+--------------------------------------------+
...
The aformentioned changes suffice to migrate from ``mindtorch.distributed.launch`` to ``torchrun``.
...
2. ``--rdzv-backend``: An implementation of
   :py:class:`mindtorch.distributed.elastic.rendezvous.RendezvousHandler`
...
3. In your training program, you can either use regular distributed functions
   or use :func:`mindtorch.nn.parallel.DistributedDataParallel` module. If your
   training program uses GPUs for training and you would like to use
   :func:`mindtorch.nn.parallel.DistributedDataParallel` module,
   here is how to configure it.

::

    local_rank = int(os.environ["LOCAL_RANK"])
    model = mindtorch.nn.parallel.DistributedDataParallel(model,
...
9. (Recommended) On worker errors, this tool will summarize the details of the error
   ...
   For details on torchelastic error handling see: https://pymindtorch.org/docs/stable/elastic/errors.html

::

  from mindtorch.distributed.elastic.multiprocessing.errors import record
...
import mindtorch
from mindtorch.distributed.argparse_util import check_env, env
from mindtorch.distributed.elastic.multiprocessing import DefaultLogsSpecs, LogsSpecs, Std
from mindtorch.distributed.elastic.multiprocessing.errors import record
from mindtorch.distributed.elastic.rendezvous.utils import _parse_rendezvous_config
from mindtorch.distributed.elastic.utils import macros
from mindtorch.distributed.elastic.utils.logging import get_logger
from mindtorch.distributed.launcher.api import elastic_launch, LaunchConfig
from mindtorch.utils.backend_registration import _get_custom_mod_func
...
def determine_local_world_size(nproc_per_node: str):
    try:
        ...
        if nproc_per_node == "cpu":
            ...
            device_type = "gpu"
            num_proc = mindtorch.cuda.device_count()
        elif nproc_per_node == mindtorch._C._get_privateuse1_backend_name():
            ...
            if mindtorch.cuda.is_available():
                ...
                hasattr(torch, mindtorch._C._get_privateuse1_backend_name())
...
def get_use_env(args) -> bool:
    ...
    ``use_env`` is only used by the ``mindtorch.distributed.launch`` and will
...
def run(args):
    mindtorch.multiprocessing._set_thread_name("pt_elastic")
