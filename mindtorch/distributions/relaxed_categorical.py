...
from .utils import broadcast_all, clamp_probs
...
class ExpRelaxedCategorical(Distribution):
    ...
    See also: :func:`distributions.OneHotCategorical`
    ...
    def rsample(self, sample_shape=()):
        ...
        uniforms = clamp_probs(
            ops.rand(shape, dtype=self.logits.dtype)
        )
        gumbels = -((-(uniforms.log())).log())
        scores = (self.logits + gumbels) / self.temperature
        return scores - ops.logsumexp(scores, dim=-1, keepdim=True)
    ...
    def log_prob(self, value):
        ...
        log_scale = ops.full_like(
            self.temperature, float(K)
        ).lgamma() - self.temperature.log().mul(-(K - 1))
        score = logits - value.mul(self.temperature)
        score = (score - ops.logsumexp(score, dim=-1, keepdim=True)).sum(-1)
...
class RelaxedOneHotCategorical(TransformedDistribution):
    ...
    Example::
        ...
        >>> m = RelaxedOneHotCategorical(mindspore.tensor([2.2]),
        ...                              mindspore.tensor([0.1, 0.2, 0.3, 0.4]))
        >>> m.sample()
