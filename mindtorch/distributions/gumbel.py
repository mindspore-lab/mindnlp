...
from mindtorch.types import _Number
...
class Gumbel(TransformedDistribution):
    ...
    def __init__(self, loc, scale, validate_args=None):
        ...
        finfo = mindtorch.finfo(self.loc.dtype)
        if isinstance(loc, _Number) and isinstance(scale, _Number):
            base_dist = Uniform(finfo.tiny, 1 - finfo.eps, validate_args=validate_args)
        else:
            base_dist = Uniform(
                mindtorch.full_like(self.loc, finfo.tiny),
                mindtorch.full_like(self.loc, 1 - finfo.eps),
                validate_args=validate_args,
            )
        transforms = [
            ExpTransform().inv,
            AffineTransform(loc=0, scale=-mindtorch.ones_like(self.scale)),
            ExpTransform().inv,
