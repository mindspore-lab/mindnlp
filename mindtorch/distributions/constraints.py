...
class _IndependentConstraint(Constraint):
    ...
    def check(self, value):
        ...
        if result.ndim < self.reinterpreted_batch_ndims:
            ...
            raise ValueError(
                f"Expected value.ndim >= {expected} but got {value.ndim}"
            )
        result = result.reshape(
            result.shape[: result.ndim - self.reinterpreted_batch_ndims] + (-1,)
        )
        result = result.all(-1)
        return result
...
class _Simplex(Constraint):
    ...
    def check(self, value):
        return mindtorch.all(value >= 0, dim=-1) & ((value.sum(-1) - 1).abs() < 1e-6)
...
class _CorrCholesky(Constraint):
    ...
    def check(self, value):
        tol = (
            float(ops.finfo(value.dtype).eps) * value.shape[-1] * 10
        )  # 10 is an adjustable fudge factor
        row_norm = ops.norm(value.detach(), p=2, dim=-1)
...
class _Symmetric(_Square):
    ...
    def check(self, value):
        ...
        if not square_check.all():
            return square_check
        return mindtorch.isclose(value, value.mT, atol=1e-6).all(-2).all(-1)
...
class _PositiveSemidefinite(_Symmetric):
    ...
    def check(self, value):
        ...
        if not sym_check.all():
            return sym_check
        return ops.eigvalsh(value).ge(0).all(-1)
...
class _PositiveDefinite(_Symmetric):
    ...
    def check(self, value):
        ...
        if not sym_check.all():
            return sym_check
        return mindtorch.linalg.cholesky_ex(value).info.eq(0)
...
class _Cat(Constraint):
    ...
    each of size `lengths[dim]`, in a way compatible with :func:`ops.cat`.
    """
    ...
    def check(self, value):
        assert -value.ndim <= self.dim < value.ndim
        checks = []
        start = 0
