...
def get_dir():
    ...
    If :func:`~mindtorch.hub.set_dir` is not called, default path is ``$TORCH_HOME/hub`` where
...
def list(
    ...
    Example:
        ...
        >>> entrypoints = mindtorch.hub.list("pytorch/vision", force_reload=True)
    """
...
def help(github, model, force_reload=False, skip_validation=False, trust_repo=None):
    ...
    Example:
        >>> # xdoctest: +REQUIRES(env:TORCH_DOCTEST_HUB)
        >>> print(mindtorch.hub.help("pytorch/vision", "resnet18", force_reload=True))
    """
    repo_dir = _get_cache_or_reload(
        github,
        force_reload,
...
def load(
    ...
    **kwargs,
    ...
    Example:
        ...
        >>> # from a github repo
        >>> repo = "pytorch/vision"
        >>> model = mindtorch.hub.load(
        ...     repo, "resnet50", weights="ResNet50_Weights.IMAGENET1K_V1"
        ... )
        >>> # from a local directory
        >>> path = "/some/local/path/pytorch/vision"
        >>> # xdoctest: +SKIP
        >>> model = mindtorch.hub.load(path, "resnet50", weights="ResNet50_Weights.DEFAULT")
    """
    source = source.lower()
...
def download_url_to_file(
    ...
    Example:
        ...
        >>> mindtorch.hub.download_url_to_file(
    ...
    """
    file_size = None
    req = Request(url, headers={"User-Agent": "mindtorch.hub"})
    u = urlopen(req)
    meta = u.info()
...
# The legacy zip format expects only one file from mindtorch.save() < 1.6 in the zip.
# We should remove this support since zipfile is now default zipfile format for mindtorch.save().
...
@deprecated(
    ...
    "Please redo mindtorch.save() to save it in the new zipfile format.",
...
def load_state_dict_from_url(
    ...
    ``hub_dir`` is the directory returned by :func:`~mindtorch.hub.get_dir`.

    Args:
        url (str): URL of the object to download
        model_dir (str, optional): directory in which to save the object
        map_location (optional): a function or a dict specifying how to remap storage locations (see mindtorch.load)
        progress (bool, optional): whether or not to display a progress bar to stderr.
            Default: True
        ...
        weights_only(bool, optional): If True, only weights will be loaded and no complex pickled objects.
            Recommended for untrusted sources. See :func:`~mindtorch.load` for more details.

    Example:
        >>> # xdoctest: +REQUIRES(env:TORCH_DOCTEST_HUB)
        >>> state_dict = mindtorch.hub.load_state_dict_from_url(
        ...     "https://s3.amazonaws.com/pytorch/models/resnet18-5c106cde.pth"
        ... )
    ...
    return mindtorch.load(cached_file, weights_only=weights_only)
