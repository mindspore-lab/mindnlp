        ...
        for k in tensor._backward_hooks:
            ...
            if not hasattr(hook, "__torch_unserializable__"):
                warnings.warn(f"backward hook {repr(hook)} on tensor will not be "
                              ...
                              "decorate the function with @mindtorch.utils.hooks.unserializable_hook "
...
class BackwardHook:
    ...
    def _apply_on_tensors(self, fn, args):
        ...
        # if not (requires_grad and mindtorch.is_grad_enabled()):
        #     return args, None

        # new_tensors = mindtorch.nn.modules._functions.BackwardHookFunction.apply(*tensors)
        new_tensors = tensors
