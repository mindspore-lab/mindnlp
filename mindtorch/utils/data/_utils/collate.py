...
def default_convert(data):
    r"""
    Convert each NumPy array element into a :class:`mindtorch.Tensor`.

    If the input is a `Sequence`, `Collection`, or `Mapping`, it tries to convert each element inside to a :class:`mindtorch.Tensor`.
    If the input is not an NumPy array, it is left unchanged.
    This is used as the default function for collation when both `batch_sampler` and `batch_size`
    are NOT defined in :class:`~mindtorch.utils.data.DataLoader`.

    The general input type to output type mapping is similar to that
    of :func:`~mindtorch.utils.data.default_collate`. See the description there for more details.
    ...
    """
    elem_type = type(data)
    if isinstance(data, mindtorch.Tensor):
        return data
    elif (
        ...
        if (
            ...
            return data
        return mindtorch.as_tensor(data)
...
def collate(
    ...
    *,
    ...
    Examples:
        ...
        ...     collate_map = {mindtorch.Tensor: collate_tensor_fn}
        ...     return collate(batch, collate_fn_map=collate_map)
        >>> # Extend `default_collate` by in-place modifying `default_collate_fn_map`
        >>> default_collate_fn_map.update({mindtorch.Tensor: collate_tensor_fn})
...
def collate_tensor_fn(
    ...
    *,
    ...
    #     )
    # if elem.layout in {
    #     mindtorch.sparse_coo,
    #     mindtorch.sparse_csr,
    #     mindtorch.sparse_bsr,
    #     mindtorch.sparse_csc,
    #     mindtorch.sparse_bsc,
    # }:
    #     raise RuntimeError(
    #         "Batches of sparse tensors are not currently supported by the default collate_fn; "
    #         "please provide a custom collate_fn to handle them appropriately."
    #     )
    # if mindtorch.utils.data.get_worker_info() is not None:
...
def collate_numpy_array_fn(
    ...
    *,
    ...
    return collate([mindtorch.as_tensor(b) for b in batch], collate_fn_map=collate_fn_map)
...
def collate_numpy_scalar_fn(
    ...
    *,
    ...
    return mindtorch.as_tensor(batch)
...
def collate_float_fn(
    ...
    *,
    ...
    return mindtorch.tensor(batch, dtype=mindtorch.float64)
...
default_collate_fn_map: Dict[Union[Type, Tuple[Type, ...]], Callable] = {
    mindtorch.Tensor: collate_tensor_fn
}
...
def default_collate(batch):
    ...
    The exact output type can be a :class:`mindtorch.Tensor`, a `Sequence` of :class:`mindtorch.Tensor`, a
    Collection of :class:`mindtorch.Tensor`, or left unchanged, depending on the input type.
    This is used as the default function for collation when
    `batch_size` or `batch_sampler` is defined in :class:`~mindtorch.utils.data.DataLoader`.

    Here is the general input type (based on the type of the element within the batch) to output type mapping:

        * :class:`mindtorch.Tensor` -> :class:`mindtorch.Tensor` (with an added outer dimension batch size)
        * NumPy Arrays -> :class:`mindtorch.Tensor`
        * `float` -> :class:`mindtorch.Tensor`
        * `int` -> :class:`mindtorch.Tensor`
        * `str` -> `str` (unchanged)
