    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(p_state) != 0 and not mindtorch.is_tensor(p_state["step"]):
    ...
    def _init_group(
        ...
        for p in group["params"]:
            if p.grad is not None:
                has_complex |= mindtorch.is_complex(p)
                ...
                if len(state) == 0:
                    ...
                    state["step"] = (
                        ...
                        if group["capturable"] or group["fused"]
                        else mindtorch.tensor(0.0, dtype=_get_scalar_dtype())
                    )
                    # Exponential moving average of gradient values
                    state["exp_avg"] = mindtorch.zeros_like(
                        p, memory_format=mindtorch.preserve_format
                    )
                    # Exponential moving average of squared gradient values
                    state["exp_avg_sq"] = mindtorch.zeros_like(
                        p, memory_format=mindtorch.preserve_format
                    )
                    if group["amsgrad"]:
                        # Maintains max of all exp. moving avg. of sq. grad. values
                        state["max_exp_avg_sq"] = mindtorch.zeros_like(
                            p, memory_format=mindtorch.preserve_format
                        )
                ...
                if (
                    group["foreach"]
                    and mindtorch.is_tensor(group["lr"])
...
def _single_tensor_adam(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        # If compiling, the compiler will handle cudagraph checks, see note [mindtorch.compile x capturable]
        if not mindtorch.compiler.is_compiling() and capturable:
        ...
        if mindtorch.is_complex(param):
            grad = mindtorch.view_as_real(grad)
            exp_avg = mindtorch.view_as_real(exp_avg)
            exp_avg_sq = mindtorch.view_as_real(exp_avg_sq)
            if amsgrad:
                max_exp_avg_sqs[i] = mindtorch.view_as_real(max_exp_avg_sqs[i])
            param = mindtorch.view_as_real(param)
        ...
        if differentiable and isinstance(beta2, Tensor):
            if beta2.requires_grad:
                ...
                exp_avg_sq.lerp_(mindtorch.square(grad), weight=1 - beta2)
            else:
        ...
        if capturable or differentiable:
            ...
            if amsgrad:
                ...
                max_exp_avg_sqs[i].copy_(mindtorch.maximum(max_exp_avg_sq, exp_avg_sq))
            ...
            if amsgrad:
                # Maintains the maximum of all 2nd moment running avg. till now
                mindtorch.maximum(max_exp_avg_sqs[i], exp_avg_sq, out=max_exp_avg_sqs[i])
        ...
        if amsgrad and mindtorch.is_complex(params[i]):
            max_exp_avg_sqs[i] = mindtorch.view_as_complex(max_exp_avg_sqs[i])
...
def _multi_tensor_adam(
    ...
    *,
    ...
    for (
        ...
        if maximize:
            device_grads = mindtorch._foreach_neg(device_grads)  # type: ignore[assignment]

        # Update steps
        # If steps are on CPU, foreach will fall back to the slow path, which is a for-loop calling t.add(1) over
        # and over. 1 will then be wrapped into a Tensor over and over again, which is slower than if we just
        # wrapped it once now. The alpha is required to assure we go to the right overload.
        if not mindtorch.compiler.is_compiling() and device_state_steps[0].is_cpu:
            mindtorch._foreach_add_(
                device_state_steps, mindtorch.tensor(1.0, device="cpu"), alpha=1.0
            )
        else:
            mindtorch._foreach_add_(device_state_steps, 1)

        if weight_decay != 0:
            if decoupled_weight_decay:
                # Perform stepweight decay
                mindtorch._foreach_mul_(device_params, 1 - lr * weight_decay)
            else:
                # Reuse the intermediate memory (device_grads) already allocated for maximize
                if maximize:
                    mindtorch._foreach_add_(device_grads, device_params, alpha=weight_decay)
                else:
                    device_grads = mindtorch._foreach_add(  # type: ignore[assignment]
        ...
        mindtorch._foreach_lerp_(
            device_exp_avgs, device_grads, cast(float, 1 - device_beta1)
        )

        mindtorch._foreach_mul_(device_exp_avg_sqs, beta2)
        ...
        if isinstance(beta2, mindtorch.Tensor):
            scaled_device_grads = mindtorch._foreach_mul(device_grads, 1 - beta2)  # type: ignore[assignment]
            value = 1.0
        else:
            scaled_device_grads = device_grads  # type: ignore[assignment]
            value = 1 - beta2

        mindtorch._foreach_addcmul_(
        ...
        if capturable:
            bias_correction1 = mindtorch._foreach_pow(beta1, device_state_steps)  # type: ignore[arg-type]
            bias_correction2 = mindtorch._foreach_pow(beta2, device_state_steps)  # type: ignore[arg-type]
            # foreach_sub doesn't allow a scalar as the first arg
            mindtorch._foreach_sub_(bias_correction1, 1)
            mindtorch._foreach_sub_(bias_correction2, 1)
            # we do not negate bias_correction1 as it'll need to be negated later anyway
            mindtorch._foreach_neg_(bias_correction2)

            # foreach_div doesn't allow a scalar as the first arg
            mindtorch._foreach_div_(bias_correction1, lr)
            mindtorch._foreach_reciprocal_(bias_correction1)

            mindtorch._foreach_sqrt_(bias_correction2)
            ...
            if amsgrad:
                device_max_exp_avg_sqs = cast(list[Tensor], device_max_exp_avg_sqs_)
                # Maintains the maximum of all 2nd moment running avg. till now
                mindtorch._foreach_maximum_(device_max_exp_avg_sqs, device_exp_avg_sqs)  # type: ignore[assignment]

                # Set intermediate to the max. for normalizing running avg. of gradient when amsgrad
                exp_avg_sq_sqrt = mindtorch._foreach_sqrt(device_max_exp_avg_sqs)
            else:
                exp_avg_sq_sqrt = mindtorch._foreach_sqrt(device_exp_avg_sqs)

            mindtorch._foreach_div_(exp_avg_sq_sqrt, bias_correction2_sqrt)
            mindtorch._foreach_add_(exp_avg_sq_sqrt, eps)
            mindtorch._foreach_div_(exp_avg_sq_sqrt, step_size)

            # at this point, exp_avg_sq_sqrt = - (1 - beta^t) * [sqrt(exp_avg_sq / (1 - beta2^t)) + eps] / lr
            mindtorch._foreach_addcdiv_(device_params, device_exp_avgs, exp_avg_sq_sqrt)
        else:
            bias_correction1 = [
            ...
            if amsgrad:
                device_max_exp_avg_sqs = cast(list[Tensor], device_max_exp_avg_sqs_)
                # Maintains the maximum of all 2nd moment running avg. till now
                mindtorch._foreach_maximum_(device_max_exp_avg_sqs, device_exp_avg_sqs)
    ...
    *,
    ...
    for (device, _), (
        ...
        if lr_dict is not None and device not in lr_dict:
            ...
            lr = lr_dict[device]
        mindtorch._foreach_add_(device_state_steps, 1)
        func = mindtorch._fused_adam_ if not decoupled_weight_decay else mindtorch._fused_adamw_
        func(
            device_params,
            device_grads,
            device_exp_avgs,
...
def adam(
    ...
    *,
    ...
    See :class:`~mindtorch.optim.Adam` for details.
    """
