...
import mindtorch
import mindtorch.utils.hooks as hooks
...
DeviceDict = dict[Optional[mindtorch.device], mindtorch.Tensor]
DeviceDtypeDict = dict[Optional[tuple[mindtorch.device, mindtorch.dtype]], mindtorch.Tensor]
...
_foreach_supported_types = [mindtorch.Tensor, mindtorch.nn.parameter.Parameter]
...
def _get_value(x):
    ...
    if not mindtorch.jit.is_scripting() and mindtorch.compiler.is_compiling():
        return x
    else:
...
# mindtorch.jit.script nor differentiable, so we fall back to the single tensor
# implementation in those cases.
def _default_to_fused_or_foreach(
    params: list[mindtorch.Tensor], differentiable: bool, use_fused: bool = False
) -> tuple[bool, bool]:
    if mindtorch.jit.is_scripting() or differentiable:
        return False, False
    ...
    fused = use_fused and all(
        ...
        or (
            ...
            and mindtorch.is_floating_point(p)
        )
        for p in params
    )
...
def _device_dtype_check_for_fused(
    p: mindtorch.Tensor, cuda_unsupported: bool = False
) -> None:
    fused_supported_devices = _get_fused_kernels_supported_devices()
    if cuda_unsupported:
        fused_supported_devices.remove("cuda")
    if not (p.device.type in fused_supported_devices and mindtorch.is_floating_point(p)):
        raise RuntimeError(
...
def _view_as_real(params, *state_and_grads):
    for i, p in enumerate(params):
        if mindtorch.is_complex(p):
            params[i] = mindtorch.view_as_real(params[i])
            for s in state_and_grads:
                s[i] = mindtorch.view_as_real(s[i])
...
def _get_scalar_dtype(is_fused=None):
    ...
    return (
        mindtorch.float64 if mindtorch.get_default_dtype() == mindtorch.float64 else mindtorch.float32
    )
...
def _get_capturable_supported_devices(supports_xla: bool = True) -> list[str]:
    ...
    if not mindtorch.jit.is_scripting():
        capturable_supported_devices.append(mindtorch._C._get_privateuse1_backend_name())
    if supports_xla:
...
def _to_scalar(x):
    ...
    """
    if isinstance(x, mindtorch.Tensor) and x.dim() != 0:
        return x.squeeze()
    else:
        return x
...
_fused_doc = r"""fused (bool, optional): whether the fused implementation is used.
            Currently, `mindtorch.float64`, `mindtorch.float32`, `mindtorch.float16`, and `mindtorch.bfloat16`
...
_capturable_doc = r"""capturable (bool, optional): whether this instance is safe to
            capture in a graph, whether for CUDA graphs or for mindtorch.compile support.
...
_differentiable_doc = r"""differentiable (bool, optional): whether autograd should
            ...
            function runs in a mindtorch.no_grad() context. Setting to True can impair
...
ParamsT: TypeAlias = Union[
    Iterable[mindtorch.Tensor], Iterable[dict[str, Any]], Iterable[tuple[str, mindtorch.Tensor]]
]
...
class Optimizer:
    ...
    def __init__(self, params: ParamsT, defaults: dict[str, Any]) -> None:  # noqa: D107
        mindtorch._C._log_api_usage_once("python.optimizer")
        ...
        if isinstance(params, mindtorch.Tensor):
            raise TypeError(
                ...
                "an iterable of Tensors or dicts, but got " + mindtorch.typename(params)
            )
    ...
    def profile_hook_step(func: Callable[_P, R]) -> Callable[_P, R]:  # noqa: D102
        ...
        def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> R:
            ...
            # with mindtorch.autograd.profiler.record_function(profile_name):
    ...
    def _process_value_according_to_param_policy(
        ...
        if key == "step":
            if capturable or fused:
                return value.to(dtype=mindtorch.float32, device=param.device)
            else:
                return value
        else:
    ...
    def load_state_dict(self, state_dict: StateDict) -> None:
        ...
        .. warning::
            Make sure this method is called after initializing :class:`mindtorch.optim.lr_scheduler.LRScheduler`,
        ...
        Example:
            >>> # xdoctest: +SKIP
            >>> model = mindtorch.nn.Linear(10, 10)
            >>> optim = mindtorch.optim.SGD(model.parameters(), lr=3e-4)
            >>> scheduler1 = mindtorch.optim.lr_scheduler.LinearLR(
            ...     optim,
            ...     start_factor=0.1,
            ...     end_factor=1,
            ...     total_iters=20,
            ... )
            >>> scheduler2 = mindtorch.optim.lr_scheduler.CosineAnnealingLR(
            ...     optim,
            ...     T_max=80,
            ...     eta_min=3e-5,
            ... )
            >>> lr = mindtorch.optim.lr_scheduler.SequentialLR(
            ...     optim,
            ...     schedulers=[scheduler1, scheduler2],
            ...     milestones=[20],
            ... )
            >>> lr.load_state_dict(mindtorch.load("./save_seq.pt"))
            >>> # now load the optimizer checkpoint after loading the LRScheduler
            >>> optim.load_state_dict(mindtorch.load("./save_optim.pt"))
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        ...
        per_device_and_dtype_grads: Optional[
            defaultdict[mindtorch.device, defaultdict[mindtorch.dtype, list[mindtorch.Tensor]]]
        ]
        if foreach:
        ...
        for param in param_group["params"]:
            if not isinstance(param, mindtorch.Tensor):
                raise TypeError(
                    ...
                    "but one of the params is " + mindtorch.typename(param)
                )
