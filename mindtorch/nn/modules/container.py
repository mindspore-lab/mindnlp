...
class Sequential(Module):
    ...
    :class:`mindtorch.nn.ModuleList`? A ``ModuleList`` is exactly what it
...
class ModuleList(Module):
    ...
    :class:`~mindtorch.nn.ModuleList` can be indexed like a regular Python list, but
    modules it contains are properly registered, and will be visible by all
    :class:`~mindtorch.nn.Module` methods.
...
class ModuleDict(Module):
    ...
    :class:`~mindtorch.nn.ModuleDict` can be indexed like a regular Python dictionary,
    but modules it contains are properly registered, and will be visible by all
    :class:`~mindtorch.nn.Module` methods.

    :class:`~mindtorch.nn.ModuleDict` is an **ordered** dictionary that respects

    * the order of insertion, and

    * in :meth:`~mindtorch.nn.ModuleDict.update`, the order of the merged
      ``OrderedDict``, ``dict`` (started from Python 3.6) or another
      :class:`~mindtorch.nn.ModuleDict` (the argument to
      :meth:`~mindtorch.nn.ModuleDict.update`).

    Note that :meth:`~mindtorch.nn.ModuleDict.update` with other unordered mapping
    ...
    def update(self, modules: Mapping[str, Module]) -> None:
        r"""Update the :class:`~mindtorch.nn.ModuleDict` with key-value pairs from a mapping, overwriting existing keys.

        .. note::
            If :attr:`modules` is an ``OrderedDict``, a :class:`~mindtorch.nn.ModuleDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            modules (iterable): a mapping (dictionary) from string to :class:`~mindtorch.nn.Module`,
...
class ParameterDict(Module):
    ...
    :class:`~mindtorch.nn.ParameterDict` is an **ordered** dictionary.
    :meth:`~mindtorch.nn.ParameterDict.update` with other unordered mapping
    types (e.g., Python's plain ``dict``) does not preserve the order of the
    merged mapping. On the other hand, ``OrderedDict`` or another :class:`~mindtorch.nn.ParameterDict`
    will preserve their ordering.

    Note that the constructor, assigning an element of the dictionary and the
    :meth:`~mindtorch.nn.ParameterDict.update` method will convert any :class:`~mindtorch.Tensor` into
    :class:`~mindtorch.nn.Parameter`.
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        ...
        self._keys[key] = None
        attr = self._key_to_attr(key)
        if isinstance(value, mindtorch.Tensor) and not isinstance(value, Parameter):
    ...
    def update(self, parameters: Union[Mapping[str, Any], 'ParameterDict']) -> None:
        ...
        .. note::
            If :attr:`parameters` is an ``OrderedDict``, a :class:`~mindtorch.nn.ParameterDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            parameters (iterable): a mapping (dictionary) from string to
                :class:`~mindtorch.nn.Parameter`, or an iterable of
