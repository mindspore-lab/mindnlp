...
class Fold(Module):
    ...
    same specification as the output shape of :class:`~mindtorch.nn.Unfold`.) This
    operation combines these local blocks into the large :attr:`output` tensor
    of shape :math:`(N, C, \text{output\_size}[0], \text{output\_size}[1], \dots)`
    by summing the overlapping values. Similar to :class:`~mindtorch.nn.Unfold`, the
    ...
    * For the case of two output spatial dimensions this operation is sometimes
    ...
    .. note::
        :class:`~mindtorch.nn.Fold` calculates each combined value in the resulting
        large tensor by summing all values from all containing blocks.
        :class:`~mindtorch.nn.Unfold` extracts the values in the local blocks by
        copying from the large tensor. So, if the blocks overlap, they are not
        inverses of each other.

        In general, folding and unfolding operations are related as
        follows. Consider :class:`~mindtorch.nn.Fold` and
        :class:`~mindtorch.nn.Unfold` instances created with the same
        parameters:
        ...
        >>> # xdoctest: +SKIP
        >>> input_ones = mindtorch.ones(input.shape, dtype=input.dtype)
    ...
    Examples::
        ...
        >>> input = mindtorch.randn(1, 3 * 2 * 2, 12)
        >>> output = fold(input)
        >>> output.size()
        mindtorch.Size([1, 3, 4, 5])
...
class Unfold(Module):
    ...
    * For the case of two input spatial dimensions this operation is sometimes
    ...
    Examples::
        ...
        >>> input = mindtorch.randn(2, 5, 3, 4)
        >>> output = unfold(input)
        >>> # each patch contains 30 values (2x3=6 vectors, each of 5 channels)
        >>> # 4 blocks (2x3 kernels) in total in the 3x4 input
        >>> output.size()
        mindtorch.Size([2, 30, 4])

        >>> # xdoctest: +IGNORE_WANT
        >>> # Convolution is equivalent with Unfold + Matrix Multiplication + Fold (or view to output shape)
        >>> inp = mindtorch.randn(1, 3, 10, 12)
        >>> w = mindtorch.randn(2, 3, 4, 5)
        >>> inp_unf = mindtorch.nn.functional.unfold(inp, (4, 5))
        >>> out_unf = inp_unf.transpose(1, 2).matmul(w.view(w.size(0), -1).t()).transpose(1, 2)
        >>> out = mindtorch.nn.functional.fold(out_unf, (7, 8), (1, 1))
        >>> # or equivalently (and avoiding a copy),
        >>> # out = out_unf.view(1, 2, 7, 8)
        >>> (mindtorch.nn.functional.conv2d(inp, w) - out).abs().max()
        tensor(1.9073e-06)
