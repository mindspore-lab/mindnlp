...
class Flatten(Module):
    ...
    For use with :class:`~nn.Sequential`, see :meth:`mindtorch.flatten` for details.
    ...
    Examples::
        >>> input = mindtorch.randn(32, 1, 5, 5)
        >>> # With default parameters
        >>> m = nn.Flatten()
        >>> output = m(input)
        >>> output.size()
        mindtorch.Size([32, 25])
...
class Unflatten(Module):
    ...
    * :attr:`unflattened_size` is the new shape of the unflattened dimension of the tensor and it can be
      a `tuple` of ints or a `list` of ints or `mindtorch.Size` for `Tensor` input;  a `NamedShape`
    ...
    Args:
        ...
        unflattened_size (Union[mindtorch.Size, Tuple, List, NamedShape]): New shape of the unflattened dimension

    Examples:
        >>> input = mindtorch.randn(2, 50)
        >>> # With tuple of ints
        >>> m = nn.Sequential(
        >>>     nn.Linear(50, 50),
        ...
        >>> m = nn.Sequential(
        >>>     nn.Linear(50, 50),
        >>>     nn.Unflatten(1, mindtorch.Size([2, 5, 5]))
        >>> )
        >>> output = m(input)
        >>> output.size()
        mindtorch.Size([2, 2, 5, 5])
