...
class NLLLoss(_WeightedLoss):
    ...
    Examples:
        ...
        >>> N, C = 5, 4
        >>> loss_fn = nn.NLLLoss()
        >>> data = mindtorch.randn(N, 16, 10, 10)
        >>> conv = nn.Conv2d(16, C, (3, 3))
        >>> log_softmax = nn.LogSoftmax(dim=1)
        >>> # output of conv forward is of shape [N, C, 8, 8]
        >>> output = log_softmax(conv(data))
        >>> # each element in target must have 0 <= value < C
        >>> target = mindtorch.empty(N, 8, 8, dtype=mindtorch.long).random_(0, C)
...
@deprecated(
    ...
    "https://pymindtorch.org/docs/main/nn.html#mindtorch.nn.NLLLoss for more details.",
...
class GaussianNLLLoss(_Loss):
    ...
    Examples:
        ...
        >>> target = mindtorch.randn(5, 2)
        >>> var = mindtorch.ones(5, 2, requires_grad=True)  # heteroscedastic
        ...
        >>> input = mindtorch.randn(5, 2, requires_grad=True)
        >>> target = mindtorch.randn(5, 2)
        >>> var = mindtorch.ones(5, 1, requires_grad=True)  # homoscedastic
        >>> output = loss(input, target, var)
        >>> output.backward()
...
class BCELoss(_WeightedLoss):
    ...
    Examples:
        ...
        >>> m = nn.Sigmoid()
        >>> loss = nn.BCELoss()
        >>> input = mindtorch.randn(3, 2, requires_grad=True)
        >>> target = mindtorch.rand(3, 2, requires_grad=False)
...
class BCEWithLogitsLoss(_Loss):
    ...
    Examples:
        ...
        >>> pos_weight = mindtorch.ones([64])  # All weights are equal to 1
        >>> criterion = mindtorch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    ...
    Examples:
        ...
        >>> target = mindtorch.empty(3).random_(2)
        >>> output = loss(input, target)
        >>> output.backward()
    """
...
class MultiLabelMarginLoss(_Loss):
    ...
    Examples:
        ...
        >>> y = mindtorch.LongTensor([[3, 0, -1, 1]])
...
class SmoothL1Loss(_Loss):
    ...
    It is less sensitive to outliers than :class:`mindtorch.nn.MSELoss` and in some cases
...
class CrossEntropyLoss(_WeightedLoss):
    ...
    - Class indices in the range :math:`[0, C)` where :math:`C` is the number of classes; if
      ...
      Note that this case is equivalent to applying :class:`~mindtorch.nn.LogSoftmax`
      on an input, followed by :class:`~mindtorch.nn.NLLLoss`.
    ...
    Examples:
        ...
        >>> input = mindtorch.randn(3, 5, requires_grad=True)
        >>> target = mindtorch.empty(3, dtype=mindtorch.long).random_(5)
        >>> output = loss(input, target)
        >>> output.backward()
        >>>
...
class CosineEmbeddingLoss(_Loss):
    ...
    Examples:
        ...
        >>> input2 = mindtorch.randn(3, 5, requires_grad=True)
        >>> target = mindtorch.ones(3)
...
class MarginRankingLoss(_Loss):
    ...
    Examples:
        ...
        >>> input2 = mindtorch.randn(3, requires_grad=True)
        >>> target = mindtorch.randn(3).sign()
        >>> output = loss(input1, input2, target)
        >>> output.backward()
    """
...
class TripletMarginLoss(_Loss):
    ...
    See also :class:`~mindtorch.nn.TripletMarginWithDistanceLoss`, which computes the
    ...
    >>> positive = mindtorch.randn(100, 128, requires_grad=True)
    >>> negative = mindtorch.randn(100, 128, requires_grad=True)
...
class TripletMarginWithDistanceLoss(_Loss):
    ...
    See also :class:`~mindtorch.nn.TripletMarginLoss`, which computes the triplet
    ...
    >>> anchor_ids = mindtorch.randint(0, 1000, (1,))
    >>> positive_ids = mindtorch.randint(0, 1000, (1,))
    >>> negative_ids = mindtorch.randint(0, 1000, (1,))
    ...
    >>>     return mindtorch.max(mindtorch.abs(x1 - x2), dim=1).values
    >>>
...
class CTCLoss(_Loss):
    ...
    Shape:
        - Log_probs: Tensor of size :math:`(T, N, C)` or :math:`(T, C)`,
          ...
          :func:`mindtorch.nn.functional.log_softmax`).
    ...
    Examples:
        ...
        >>> input = mindtorch.randn(T, C).log_softmax(1).detach().requires_grad_()
        >>> input_lengths = mindtorch.tensor(T, dtype=mindtorch.long)
        >>>
        >>> # Initialize random batch of targets (0 = blank, 1:C = classes)
    ...
    Note:
        ...
        a performance cost) by setting ``mindtorch.backends.cudnn.deterministic =
        True``.
