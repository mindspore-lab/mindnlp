...
# functions that use `with mindtorch.no_grad()`. The JIT doesn't support context
# managers, so these need to be implemented as builtins. Using these wrappers
# lets us keep those builtins small and re-usable.
def _no_grad_uniform_(
    tensor: Tensor, a: float, b: float, generator: _Optional[mindtorch.Generator] = None
) -> Tensor:
...
def eye_(tensor: Tensor) -> Tensor:
    ...
    Args:
        tensor: a 2-dimensional `mindtorch.Tensor`
    ...
    with mindtorch.no_grad():
        mindtorch.eye(*tensor.shape, out=tensor, requires_grad=tensor.requires_grad)
    return tensor
...
def _make_deprecate(meth: Callable[_P, _R]) -> Callable[_P, _R]:
    ...
    See :func:`~mindtorch.nn.init.{new_name}` for details."""
