...
class BasePruningMethod(ABC):
    ...
    def compute_mask(self, t, default_mask):
        ...
        Args:
            t (mindtorch.Tensor): tensor representing the importance scores of the
            parameter to prune.
            default_mask (mindtorch.Tensor): Base mask from previous pruning
            iterations, that need to be respected after the new mask is
            applied. Same dims as ``t``.

        Returns:
            mask (mindtorch.Tensor): mask to apply to ``t``, of same dims as ``t``
        """
    ...
    def apply_mask(self, module):
        ...
        Returns:
            pruned_tensor (mindtorch.Tensor): pruned version of the input tensor
        """
    ...
    def apply(cls, module, name, *args, importance_scores=None, **kwargs):
        ...
        Args:
            ...
            importance_scores (mindtorch.Tensor): tensor of importance scores (of
        ...
        if not isinstance(method, PruningContainer):
            ...
            default_mask = mindtorch.ones_like(orig)  # temp
        ...
        # to go
            ...
            default_mask = (
                ...
                .detach()
                .clone(memory_format=mindtorch.contiguous_format)
            )
    ...
    def prune(self, t, default_mask=None, importance_scores=None):
        ...
        Args:
            t (mindtorch.Tensor): tensor to prune (of same dimensions as
                ``default_mask``).
            importance_scores (mindtorch.Tensor): tensor of importance scores (of
            ...
            default_mask (mindtorch.Tensor, optional): mask from previous pruning
        ...
        if importance_scores is not None:
            ...
            importance_scores = t
        default_mask = default_mask if default_mask is not None else mindtorch.ones_like(t)
...
class PruningContainer(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        * for 'global', the mask will be computed across all entries.
        ...
        Args:
            t (mindtorch.Tensor): tensor representing the parameter to prune
                (of same dimensions as ``default_mask``).
            default_mask (mindtorch.Tensor): mask from previous pruning iteration.

        Returns:
            mask (mindtorch.Tensor): new mask that combines the effects
        ...
        def _combine_masks(method, t, mask):
            ...
            Args:
                ...
                mask (mindtorch.Tensor): mask from previous pruning iteration

            Returns:
                new_mask (mindtorch.Tensor): new mask that combines the effects
...
class RandomUnstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        mask = default_mask.clone(memory_format=mindtorch.contiguous_format)

        if nparams_toprune != 0:  # k=0 not supported by mindtorch.kthvalue
            prob = mindtorch.rand_like(t)
            topk = mindtorch.topk(prob.view(-1), k=nparams_toprune)
...
class L1Unstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        if nparams_toprune != 0:  # k=0 not supported by mindtorch.kthvalue
            ...
            topk = mindtorch.topk(mindtorch.abs(t).view(-1), k=nparams_toprune, largest=False)
    ...
    def apply(cls, module, name, amount, importance_scores=None):  # type: ignore[override]
        ...
        Args:
            ...
            amount (int or float): quantity of parameters to prune.
                ...
                fraction of parameters to prune. If ``int``, it represents the
                absolute number of parameters to prune.
            importance_scores (mindtorch.Tensor): tensor of importance scores (of same
...
class LnStructured(BasePruningMethod):
    ...
    Args:
        ...
        n (int, float, inf, -inf, 'fro', 'nuc'): See documentation of valid
            entries for argument ``p`` in :func:`mindtorch.norm`.
        dim (int, optional): index of the dim along which we define
            channels to prune. Default: -1.
    """
    ...
    def compute_mask(self, t, default_mask):
        ...
        topk = mindtorch.topk(norm, k=nparams_tokeep, largest=True)
        # topk will have .indices and .values
...
def random_structured(module, name, amount, dim):
    ...
    Examples:
        ...
        >>> columns_pruned = int(sum(mindtorch.sum(m.weight, dim=0) == 0))
...
def ln_structured(module, name, amount, n, dim, importance_scores=None):
    ...
    Examples:
        >>> from mindtorch.nn.utils import prune
...
def global_unstructured(parameters, pruning_method, importance_scores=None, **kwargs):
    ...
    Examples:
        ...
        ...     amount=10,
        ... )
        >>> print(sum(mindtorch.nn.utils.parameters_to_vector(net.buffers()) == 0))
        tensor(10)
    ...
    relevant_importance_scores = mindtorch.nn.utils.parameters_to_vector(
        [
    ...
    default_mask = mindtorch.nn.utils.parameters_to_vector(
        [
            getattr(module, name + "_mask", mindtorch.ones_like(getattr(module, name)))
            for (module, name) in parameters
        ]
    )
...
def custom_from_mask(module, name, mask):
    ...
    Examples:
        ...
        ...     nn.Linear(5, 3), name="bias", mask=mindtorch.tensor([0, 1, 0])
        ... )
        >>> print(m.bias_mask)
        tensor([0., 1., 0.])
...
def _compute_norm(t, n, dim):
    ...
    Args:
        t (mindtorch.Tensor): tensor representing the parameter to prune
        n (int, float, inf, -inf, 'fro', 'nuc'): See documentation of valid
            entries for argument p in mindtorch.norm
        dim (int): dim identifying the channels to prune

    Returns:
        norm (mindtorch.Tensor): L_n norm computed across all dimensions except
    ...
    norm = mindtorch.norm(t, p=n, dim=dims)
    return norm