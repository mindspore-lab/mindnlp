...
import mindtorch._logging._internal
...
def intern_string(s: Optional[str]) -> int:
    ...
    if r is None:
        r = len(INTERN_TABLE)
        INTERN_TABLE[s] = r
        mindtorch._logging._internal.trace_structured(
...
def dump_file(filename: str) -> None:
    ...
    from mindtorch.fx.graph_module import _loader
...
def get_user_stack(num_frames: int) -> list[dict[str, Any]]:
    from mindtorch._guards import TracingContext
...
def get_framework_stack(
    ...
    """
    from mindtorch.fx.experimental.symbolic_shapes import uninteresting_files
    from mindtorch.utils._traceback import CapturedTraceback
