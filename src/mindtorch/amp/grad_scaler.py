...
import mindspore
import mindtorch
from mindtorch.configs import DEVICE_TARGET, SOC
...
def non_finite_check(inputs):
    ...
    if DEVICE_TARGET == 'Ascend':
        status = mindtorch.tensor(np.array([0] * 8), dtype=mindtorch.int32, device='npu')
        status = mindtorch.depend(status, inputs)
        found_inf = mindtorch.npu_get_float_status_v2(status)
        status = mindtorch.depend(status, found_inf)
        clear_status = mindtorch.npu_clear_float_status_v2(status)
        found_inf = mindtorch.depend(found_inf, clear_status)
        found_inf = mindtorch.not_equal(found_inf, 0)
        return found_inf.sum()

    found_inf = mindtorch.all_finite(inputs)  # pylint: disable=invalid-unary-operand-type
    # else:
        ...
        if new_scale is not None:
            ...
            if found_inf_combined > 0:
                ...
                if successful == self._growth_interval:
                    ...
                    if mindtorch.isfinite(new_scale):
                        _scale.copy_(new_scale)
                    _growth_tracker.copy_(_growth_tracker * 0)
                else:
                    _growth_tracker.copy_(
                        mindtorch.tensor(successful,
