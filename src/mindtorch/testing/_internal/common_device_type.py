...
import mindtorch
# from mindtorch._inductor.utils import GPU_TYPES
from mindtorch.testing._internal.common_cuda import (
    _get_core_cuda_version,
    _get_core_rocm_version,
    TEST_CUSPARSE_GENERIC,
    TEST_HIPSPARSE_GENERIC,
)
from mindtorch.testing._internal.common_dtype import get_all_dtypes
from mindtorch.testing._internal.common_utils import (
    _TestParametrizer,
    clear_tracked_input,
...
#   (see https://github.com/pycore/xla) can further extend the set of
...
# @dtypes(mindtorch.float32, mindtorch.int64)
...
#     - test_car_cuda_int64
#
# The dtype is passed as a mindtorch.dtype object.
#
...
#   for mindtorch.add as its "op" argument, the string 'cpu' for its "device" argument,
#   and the dtype mindtorch.float32 for is "dtype" argument. The second instantiated
#   test calls the test_car() with the OpInfo for mindtorch.sub, a CUDA device string
#   like 'cuda:0' or 'cuda:1' for its "device" argument, and the dtype
#   mindtorch.int64 for its "dtype argument."
#
    ...
    def instantiate_test(cls, name, test, *, generic_cls=None):
        ...
        for (
            ...
            cls_device_type = (
                ...
                else mindtorch._C._get_privateuse1_backend_name()
            )
...
class CUDATestBase(DeviceTypeTestBase):
    ...
    def setUpClass(cls):
        ...
        t = mindtorch.ones(1).cuda()
        cls.no_magma = not mindtorch.cuda.has_magma

        # Determines if cuDNN is available and its version
        cls.no_cudnn = not mindtorch.backends.cudnn.is_acceptable(t)
        cls.cudnn_version = None if cls.no_cudnn else mindtorch.backends.cudnn.version()

        # Acquires the current device as the primary (test) device
        cls.primary_device = f"cuda:{mindtorch.cuda.current_device()}"
...
class XPUTestBase(DeviceTypeTestBase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.primary_device = f"xpu:{mindtorch.xpu.current_device()}"
...
class PrivateUse1TestBase(DeviceTypeTestBase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.device_type = mindtorch._C._get_privateuse1_backend_name()
        cls.device_mod = getattr(core, cls.device_type, None)
        assert (
            cls.device_mod is not None
        ), f"""core has no module of `{cls.device_type}`, you should register
...
def get_device_type_test_bases():
    ...
    if IS_SANDCASTLE or IS_FBCODE:
        ...
        # ramping up support.
        # elif mindtorch.backends.mps.is_available():
...
# The following logic optionally allows downstream projects like pycore/xla to
# add more test devices.
# Instructions:
#  - Add a python file (e.g. pycore/xla/test/pycore_test_base.py) in downstream project.
...
# See pycore/xla/test/pycore_test_base.py for a more detailed example.
...
class ops(_TestParametrizer):
    def __init__(
        ...
        op_list,
        *,
        dtypes: Union[OpDTypes, Sequence[mindtorch.dtype]] = OpDTypes.supported,
        allowed_dtypes: Optional[Sequence[mindtorch.dtype]] = None,
        skip_if_dynamo=True,
    ):
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        ...
        for op in self.op_list:
            ...
            dtypes: Union[set[mindtorch.dtype], set[None]]
...
class skipPRIVATEUSE1If(skipIf):
    def __init__(self, dep, reason):
        device_type = mindtorch._C._get_privateuse1_backend_name()
        super().__init__(dep, reason, device_type=device_type)


def _has_sufficient_memory(device, size):
    if mindtorch.device(device).type == "cuda":
        if not mindtorch.cuda.is_available():
            return False
        gc.collect()
        mindtorch.cuda.empty_cache()
        # mindtorch.cuda.mem_get_info, aka cudaMemGetInfo, returns a tuple of (free memory, total memory) of a GPU
        if device == "cuda":
            device = "cuda:0"
        return (
            mindtorch.cuda.memory.mem_get_info(device)[0]
            * mindtorch.cuda.memory.get_per_process_memory_fraction(device)
        ) >= size
...
def largeTensorTest(size, device=None, inductor=TEST_WITH_TORCHINDUCTOR):
    ...
    def inner(fn):
        ...
        def dep_fn(self, *args, **kwargs):
            ...
            if inductor and mindtorch._inductor.config.cpp_wrapper and _device != "cpu":
                size_bytes *= 2
...
# Ex.
#
# @precisionOverride({mindtorch.half : 1e-2, mindtorch.float : 1e-4})
# @dtypes(mindtorch.half, mindtorch.float, mindtorch.double)
...
# Ex.
#
# @toleranceOverride({mindtorch.float : tol(atol=1e-2, rtol=1e-3},
#                     mindtorch.double : tol{atol=1e-4, rtol = 0})
# @dtypes(mindtorch.half, mindtorch.float, mindtorch.double)
# def test_X(self, device, dtype):
#   ...
#
...
#
# The above example sets atol = 1e-2 and rtol = 1e-3 for mindtorch.float and
# atol = 1e-4 and rtol = 0 for mindtorch.double.
...
# @dtypes((mindtorch.long, mindtorch.float32), (mindtorch.int, mindtorch.float64))
class dtypes:
    def __init__(self, *args, device_type="all"):
        if len(args) > 0 and isinstance(args[0], (list, tuple)):
            for arg in args:
                ...
                assert all(isinstance(dtype, mindtorch.dtype) for dtype in arg), (
                    f"Unknown dtype in {str(arg)}"
                )
        else:
            assert all(isinstance(arg, mindtorch.dtype) for arg in args), (
...
class dtypesIfPRIVATEUSE1(dtypes):
    def __init__(self, *args):
        super().__init__(*args, device_type=mindtorch._C._get_privateuse1_backend_name())
...
def onlyPRIVATEUSE1(fn):
    device_type = mindtorch._C._get_privateuse1_backend_name()
    device_mod = getattr(core, device_type, None)
    if device_mod is None:
        reason = f"Skip as core has no module of {device_type}"
...
def onlyCUDAAndPRIVATEUSE1(fn):
    @wraps(fn)
    def only_fn(self, *args, **kwargs):
        if self.device_type not in ("cuda", mindtorch._C._get_privateuse1_backend_name()):
...
def disablecuDNN(fn):
    ...
    def disable_cudnn(self, *args, **kwargs):
        if self.device_type == "cuda" and self.has_cudnn():
            with mindtorch.backends.cudnn.flags(enabled=False):
                return fn(self, *args, **kwargs)
        return fn(self, *args, **kwargs)
...
def disableMkldnn(fn):
    ...
    def disable_mkldnn(self, *args, **kwargs):
        if mindtorch.backends.mkldnn.is_available():
            with mindtorch.backends.mkldnn.flags(enabled=False):
                return fn(self, *args, **kwargs)
        return fn(self, *args, **kwargs)
...
def skipCPUIfNoLapack(fn):
    return skipCPUIf(not mindtorch._C.has_lapack, "PyTorch compiled without Lapack")(fn)


# Skips a test on CPU if FFT is not available.
def skipCPUIfNoFFT(fn):
    return skipCPUIf(not mindtorch._C.has_spectral, "PyTorch is built without FFT support")(
        fn
    )
...
def skipCPUIfNoMkldnn(fn):
    return skipCPUIf(
        not mindtorch.backends.mkldnn.is_available(),
...
def has_hipsolver():
    rocm_version = _get_core_rocm_version()
        ...
        def wrap_fn(self, *args, **kwargs):
            if self.device_type == "cuda":
                if not TEST_WITH_ROCM:
                    ...
                    raise unittest.SkipTest(reason)
                rocm_version_tuple = _get_core_rocm_version()
                if (
...
def skipCUDAVersionIn(versions: Optional[list[tuple[int, int]]] = None):
    def dec_fn(fn):
        @wraps(fn)
        def wrap_fn(self, *args, **kwargs):
            version = _get_core_cuda_version()
...
def skipCUDAIfMiopen(fn):
    return skipCUDAIf(mindtorch.version.hip is not None, "Marked as skipped for MIOpen")(fn)


def skipCUDAIfNoMiopen(fn):
    return skipCUDAIf(mindtorch.version.hip is None, "MIOpen is not available")(
...
flex_attention_supported_platform = unittest.skipUnless(
    ...
    or (
        ...
        and mindtorch.utils._triton.has_triton()
        and mindtorch.cuda.get_device_capability() >= (8, 0)
    ),
    "Requires CUDA and Triton, or CPU with avx2 and later",
)
# if mindtorch.version.hip and "gfx94" in mindtorch.cuda.get_device_properties(0).gcnArchName:
#     e4m3_type = mindtorch.float8_e4m3fnuz
#     e5m2_type = mindtorch.float8_e5m2fnuz
#     E4M3_MAX_POS = mindtorch.finfo(mindtorch.float8_e4m3fnuz).max
#     E5M2_MAX_POS = mindtorch.finfo(mindtorch.float8_e5m2fnuz).max
# else:
#     e4m3_type = mindtorch.float8_e4m3fn
#     e5m2_type = mindtorch.float8_e5m2
#     E4M3_MAX_POS = mindtorch.finfo(mindtorch.float8_e4m3fn).max
#     E5M2_MAX_POS = mindtorch.finfo(mindtorch.float8_e5m2).max