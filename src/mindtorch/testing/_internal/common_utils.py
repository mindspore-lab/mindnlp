...
no CUDA calls shall be made, including mindtorch.cuda.device_count(), etc.

mindtorch.testing._internal.common_cuda.py can freely initialize CUDA context when imported.
"""
...
from mindtorch._C import ScriptDict, ScriptList  # type: ignore[attr-defined]
from mindtorch._dynamo.trace_rules import _as_posix_path
from mindtorch._utils_internal import get_writable_path
from mindtorch._logging.scribe import open_source_signpost
...
from mindtorch.testing._comparison import (
    BooleanPair,
    NonePair,
    NumberPair,
    Pair,
...
from mindtorch.testing._comparison import not_close_error_metas
from mindtorch.testing._internal.common_dtype import get_all_dtypes
from mindtorch.utils._import_utils import _check_module_exists
import mindtorch.utils._pytree as pytree
from mindtorch.utils import cpp_extension
try:
    import pytest
    has_pytest = True
except ImportError:
...
def freeze_rng_state(*args, **kwargs):
    return mindtorch.testing._utils.freeze_rng_state(*args, **kwargs)
...
mindtorch.backends.disable_global_flags()
...
_is_fbcode_default = (
    hasattr(mindtorch._utils_internal, "IS_FBSOURCE") and
    mindtorch._utils_internal.IS_FBSOURCE
)
...
NATIVE_DEVICES = ('cpu', 'cuda', 'npu', 'meta')
...
DEVICE_LIST_SUPPORT_PROFILING_TEST = ('cpu', 'cuda', 'npu')
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        ...
        Returns:
            Generator object returning 4-tuples of:
                ...
                param_kwargs (dict): Param kwargs to pass to the test (e.g. {'op': 'add', 'dtype': mindtorch.int64})
...
class parametrize(_TestParametrizer):
    ...
    def _formatted_str_repr(self, idx, name, value):
        ...
        if isinstance(value, mindtorch.dtype):
            return dtype_name(value)
        elif isinstance(value, mindtorch.device):
            return str(value)
...
class decorateIf(_TestParametrizer):
    ...
    Examples::
        ...
        @decorateIf(
            unittest.expectedFailure,
            lambda params: params["op"].name == "add" and params["dtype"] == mindtorch.float16
        )
        @ops(op_db)
        ...
        @decorateIf(
            unittest.skip,
            lambda params: params["module_info"].module_cls is mindtorch.nn.Linear and \
                params["device"] == "cpu"
        )
        @modules(module_db)
...
def cppProfilingFlagsToProfilingMode():
    # old_prof_exec_state = mindtorch._C._jit_set_profiling_executor(True)
    # # old_prof_mode_state = mindtorch._C._get_graph_executor_optimize(True)
    # mindtorch._C._jit_set_profiling_executor(old_prof_exec_state)
    # # mindtorch._C._get_graph_executor_optimize(old_prof_mode_state)
...
def num_profiled_runs(num_runs):
    old_num_runs = mindtorch._C._jit_set_num_profiled_runs(num_runs)
    try:
        yield
    finally:
        mindtorch._C._jit_set_num_profiled_runs(old_num_runs)

# func_call = mindtorch._C.ScriptFunction.__call__
# meth_call = mindtorch._C.ScriptMethod.__call__
...
# mindtorch._C.ScriptFunction.__call__ = prof_func_call  # type: ignore[method-assign]
# mindtorch._C.ScriptMethod.__call__ = prof_meth_call  # type: ignore[method-assign]
    ...
    for test_case_or_suite in suite:
        ...
        if test_case is not None:
            if not isinstance(test_case, TestCase):
                ...
                err = "This test class should extend from mindtorch.testing._internal.common_utils.TestCase but it doesn't."
...
def is_privateuse1_backend_available():
    privateuse1_backend_name = mindtorch._C._get_privateuse1_backend_name()
...
# TEST_MKL = mindtorch.backends.mkl.is_available()
# TEST_ACL = mindtorch.backends.mkldnn.is_available() and mindtorch.ops.mkldnn._is_mkldnn_acl_supported()
TEST_MPS = mindtorch.backends.mps.is_available()
MACOS_VERSION = float('.'.join(platform.mac_ver()[0].split('.')[:2]) or -1)
# TEST_XPU = mindtorch.xpu.is_available()
# TEST_HPU = True if (hasattr(torch, "hpu") and mindtorch.hpu.is_available()) else False
TEST_CUDA = mindtorch.cuda.is_available()
TEST_NPU = mindtorch.npu.is_available()
TEST_PRIVATEUSE1 = False
# custom_device_mod = getattr(torch, mindtorch._C._get_privateuse1_backend_name(), None)
# TEST_PRIVATEUSE1 = is_privateuse1_backend_available()
# TEST_PRIVATEUSE1_DEVICE_TYPE = mindtorch._C._get_privateuse1_backend_name()
...
TEST_CUDA_GRAPH = TEST_CUDA and (not TEST_SKIP_CUDAGRAPH) and (
    mindtorch.version.cuda or
    (mindtorch.version.hip and float(".".join(mindtorch.version.hip.split(".")[0:2])) >= 5.3)
)

TEST_CUDA_CUDSS = TEST_CUDA and (mindtorch.version.cuda and int(mindtorch.version.cuda.split(".")[0]) >= 12)

TEST_CUDA_PYTHON_BINDINGS = _check_module_exists("cuda.bindings") and (
    mindtorch.version.cuda and int(mindtorch.version.cuda.split(".")[0]) >= 12
)
...
if TEST_CUDA and 'NUM_PARALLEL_PROCS' in os.environ:
    ...
    gb_available = mindtorch.cuda.mem_get_info()[1] / 2 ** 30
    # other libraries take up about a little under 1 GB of space per process
    mindtorch.cuda.set_per_process_memory_fraction(round((gb_available - num_procs * .85) / gb_available / num_procs, 2))

requires_cuda = unittest.skipUnless(mindtorch.cuda.is_available(), "Requires CUDA")
...
class CrossRefMode(mindtorch.overrides.TorchFunctionMode):
...
if TEST_WITH_TORCHDYNAMO:
    import mindtorch._dynamo
    # Do not spend time on helper functions that are called with different inputs
    mindtorch._dynamo.config.accumulated_recompile_limit = 64
    # Do not log compilation metrics from unit tests
    mindtorch._dynamo.config.log_compilation_metrics = False
    # Silence 3.13.0 guard performance warnings
    mindtorch._dynamo.config.issue_3_13_0_warning = False
    if TEST_WITH_TORCHINDUCTOR:
        import mindtorch._inductor.config
        mindtorch._inductor.config.fallback_random = True
    else:
        # only dynamo for now
        mindtorch._dynamo.config.compiled_autograd = not TEST_WITHOUT_COMPILED_AUTOGRAD
...
def runWithoutCompiledAutograd(msg="test doesn't currently work with compiled autograd"):
    ...
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            with mindtorch._dynamo.compiled_autograd._disable():
                func(*args, **kwargs)
        return wrapper
...
def markDynamoStrictTest(cls_or_func=None, nopython=False):
    ...
    Args:
    - nopython: if we should run mindtorch._dynamo.optimize with nopython={True/False}.
    """
    def decorator(cls_or_func):
        ...
        @wraps(fn)
        def wrapper(*args, **kwargs):
            mindtorch._dynamo.reset()
            with unittest.mock.patch("mindtorch._dynamo.config.suppress_errors", False):
                fn(*args, **kwargs)
            mindtorch._dynamo.reset()
        return wrapper
...
def make_dynamo_test(
    ...
    """
    from mindtorch._dynamo.testing import CompileCounter, reset, optimize_assert
    if fn is None:
...
if TEST_WITH_TV:
    mindtorch.fx.experimental._config.translation_validation = True
...
numpy_to_torch_dtype_dict = {
    np.bool_      : mindtorch.bool,
    np.uint8      : mindtorch.uint8,
    np.uint16     : mindtorch.uint16,
    np.uint32     : mindtorch.uint32,
    np.uint64     : mindtorch.uint64,
    np.int8       : mindtorch.int8,
    np.int16      : mindtorch.int16,
    np.int32      : mindtorch.int32,
    np.int64      : mindtorch.int64,
    np.float16    : mindtorch.float16,
    np.float32    : mindtorch.float32,
    np.float64    : mindtorch.float64,
    np.complex64  : mindtorch.complex64,
    np.complex128 : mindtorch.complex128
}
...
if IS_WINDOWS:
    ...
    numpy_to_torch_dtype_dict[np.intc] = mindtorch.int

# Dict of torch dtype -> NumPy dtype
torch_to_numpy_dtype_dict = {value : key for (key, value) in numpy_to_torch_dtype_dict.items()}
torch_to_numpy_dtype_dict.update({
    mindtorch.bfloat16: np.float32,
    mindtorch.complex32: np.complex64
})

# def skipIfNNModuleInlined(
#     msg="test doesn't currently work with nn module inlining",
#     condition=mindtorch._dynamo.config.inline_inbuilt_nn_modules,
# ):
#     def decorator(fn):
#         if not isinstance(fn, type):
...
def skipIfRocmArch(arch: tuple[str, ...]):
    def dec_fn(fn):
        ...
        def wrap_fn(self, *args, **kwargs):
            if TEST_WITH_ROCM:
                prop = mindtorch.cuda.get_device_properties(0)
...
def skipIfRocmVersionLessThan(version=None):
    def dec_fn(fn):
        ...
        def wrap_fn(self, *args, **kwargs):
            if TEST_WITH_ROCM:
                rocm_version = str(mindtorch.version.hip)
...
def requires_cuda_p2p_access():
    cuda_p2p_access_available = (
        mindtorch.cuda.is_available()
        and mindtorch.cuda.get_device_capability() >= (8, 0)
        and mindtorch.cuda.device_count() >= 2
    )
    num_devices = mindtorch.cuda.device_count()
    for i in range(num_devices - 1):
        for j in range(i + 1, num_devices):
            if not mindtorch.cuda.can_device_access_peer(i, j):
...
def setLinalgBackendsToDefaultFinally(fn):
    @wraps(fn)
    def _fn(*args, **kwargs):
        _preferred_backend = mindtorch.backends.cuda.preferred_linalg_library()
        try:
            fn(*args, **kwargs)
        finally:
            mindtorch.backends.cuda.preferred_linalg_library(_preferred_backend)
    return _fn
...
def setBlasBackendsToDefaultFinally(fn):
    @wraps(fn)
    def _fn(*args, **kwargs):
        _preferred_backend = mindtorch.backends.cuda.preferred_blas_library()
        try:
            fn(*args, **kwargs)
        finally:
            mindtorch.backends.cuda.preferred_blas_library(_preferred_backend)
    return _fn
...
class DeterministicGuard:
    ...
    def _current_state(cls):
        return cls(
            mindtorch.are_deterministic_algorithms_enabled(),
            warn_only=mindtorch.is_deterministic_algorithms_warn_only_enabled(),
            fill_uninitialized_memory=mindtorch.utils.deterministic.fill_uninitialized_memory,  # type: ignore[attr-defined]
        )

    def _update(self):
        mindtorch.use_deterministic_algorithms(self.deterministic, warn_only=self.warn_only)
        mindtorch.utils.deterministic.fill_uninitialized_memory = self.fill_uninitialized_memory  # type: ignore[attr-defined]
...
class AlwaysWarnTypedStorageRemoval:
    ...
    def __enter__(self):
        self.always_warn_restore = mindtorch.storage._get_always_warn_typed_storage_removal()
        mindtorch.storage._set_always_warn_typed_storage_removal(self.always_warn)

    def __exit__(self, exception_type, exception_value, traceback):
        mindtorch.storage._set_always_warn_typed_storage_removal(self.always_warn_restore)
...
class CudaSyncGuard:
    ...
    def __enter__(self):
        self.debug_mode_restore = mindtorch.cuda.get_sync_debug_mode()
        mindtorch.cuda.set_sync_debug_mode(self.mode)

    def __exit__(self, exception_type, exception_value, traceback):
        mindtorch.cuda.set_sync_debug_mode(self.debug_mode_restore)

# Context manager for setting mindtorch.__future__.set_swap_module_params_on_conversion
...
class SwapTensorsGuard:
    ...
    def __enter__(self):
        self.swap_tensors_restore = mindtorch.__future__.get_swap_module_params_on_conversion()
        if self.use_swap_tensors is not None:
            mindtorch.__future__.set_swap_module_params_on_conversion(self.use_swap_tensors)

    def __exit__(self, exception_type, exception_value, traceback):
        mindtorch.__future__.set_swap_module_params_on_conversion(self.swap_tensors_restore)

# This decorator can be used for API tests that call
# mindtorch.use_deterministic_algorithms().  When the test is finished, it will
...
# mindtorch.__future__.set_swap_module_params_on_conversion.  `swap` can be set to
...
def skipIfCompiledWithoutNumpy(fn):
    ...
    if numpy_support:
        try:
            ...
            # numpy support
            mindtorch.from_numpy(np.array([2, 2]))
        except RuntimeError:
            numpy_support = False
...
def skipIfNoXNNPACK(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        if not mindtorch.backends.xnnpack.enabled:  # type: ignore[attr-defined]
...
def skipIfNoLapack(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        if not mindtorch._C.has_lapack:
...
def skipIfNotRegistered(op_name, message):
    ...
    Args:
        op_name: Check if this op is registered in `mindtorch._REGISTERED_OPERATORS`.
...
def set_default_dtype(dtype):
    saved_dtype = mindtorch.get_default_dtype()
    mindtorch.set_default_dtype(dtype)
    try:
        yield
    finally:
        mindtorch.set_default_dtype(saved_dtype)

@contextlib.contextmanager
def set_default_tensor_type(tensor_type):
    saved_tensor_type = mindtorch.tensor([]).type()
    mindtorch.set_default_tensor_type(tensor_type)
    try:
        yield
    finally:
        mindtorch.set_default_tensor_type(saved_tensor_type)
...
def is_iterable_of_tensors(iterable, include_empty=False):
    ...
    if isinstance(iterable, mindtorch.Tensor):
        return False
    ...
    try:
        ...
        for t in iter(iterable):
            if not isinstance(t, mindtorch.Tensor):
                return False
...
class CudaNonDefaultStream:
    def __enter__(self):
        ...
        beforeDevice = mindtorch.cuda.current_device()
        self.beforeStreams = []
        for d in range(mindtorch.cuda.device_count()):
            self.beforeStreams.append(mindtorch.cuda.current_stream(d))
            deviceStream = mindtorch.cuda.Stream(device=d)
            self.beforeStreams[-1].synchronize()
            mindtorch._C._cuda_setStream(stream_id=deviceStream.stream_id,
                                     device_index=deviceStream.device_index,
                                     device_type=deviceStream.device_type)
        mindtorch._C._cuda_setDevice(beforeDevice)
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        beforeDevice = mindtorch.cuda.current_device()
        for d in range(mindtorch.cuda.device_count()):
            mindtorch._C._cuda_setStream(stream_id=self.beforeStreams[d].stream_id,
...
class CudaMemoryLeakCheck:
    def __init__(self, testcase, name=None):
        ...
        from mindtorch.testing._internal.common_cuda import initialize_cuda_context_rng
        initialize_cuda_context_rng()

    # Stores CUDA memory data provided by PyTorch's caching allocator and
    #   the CUDA driver.
    #
    # NOTE: The undocumented mindtorch.cuda.mem_get_info() returns
    ...
    def __enter__(self):
        ...
        for i in range(num_devices):
            caching_allocator_mem_allocated = mindtorch.cuda.memory_allocated(i)
            ...
            if caching_allocator_mem_allocated > 0:
                gc.collect()
                mindtorch._C._cuda_clearCublasWorkspaces()
                mindtorch.cuda.empty_cache()
                break

        # Acquires caching allocator and driver statistics before the test is run
        for i in range(num_devices):
            self.caching_allocator_befores.append(mindtorch.cuda.memory_allocated(i))
            bytes_free, bytes_total = mindtorch.cuda.mem_get_info(i)
...
def remove_device_and_dtype_suffixes(test_name: str) -> str:
    ...
    from mindtorch.testing._internal.common_device_type import get_device_type_test_bases
    device_suffixes = [x.device_type for x in get_device_type_test_bases()]
    dtype_suffixes = [str(dt)[len("mindtorch."):] for dt in get_all_dtypes()]
...
# `mindtorch.testing._comparison.are_equal`, used for example by the public testing function
# `mindtorch.testing.assert_close`, is more strict. In order to use the same framework and thus reduce the divergence
...
class RelaxedBooleanPair(BooleanPair):
    ...
    def _process_inputs(self, actual, expected, *, id):
        ...
        tensor_or_array_types: tuple[type, ...] = (mindtorch.Tensor, np.ndarray)
    ...
    def _to_bool(self, bool_like, *, id):
        ...
        elif isinstance(bool_like, (mindtorch.Tensor, np.ndarray)):
            numel = bool_like.numel() if isinstance(bool_like, mindtorch.Tensor) else bool_like.size
            if numel > 1:
                self._fail(
                    ValueError,
...
class RelaxedNumberPair(NumberPair):
    ...
    _TYPE_TO_DTYPE = {
        int: mindtorch.int64,
        float: mindtorch.float32,
        complex: mindtorch.complex64,
    }
    ...
    def _to_number(self, number_like, *, id):
        if isinstance(number_like, (mindtorch.Tensor, np.ndarray)):
            numel = number_like.numel() if isinstance(number_like, mindtorch.Tensor) else number_like.size
            if numel > 1:
                self._fail(
                    ValueError,
...
class TensorOrArrayPair(TensorLikePair):
    ...
    :class:`mindtorch.Tensor` and :class:`numpy.ndarray` rather than allowing any tensor-like than can be converted into a
    tensor. On the other hand this class is looser since it converts all inputs into tensors with no regard of their
    relationship, e.g. comparing a :class:`mindtorch.Tensor` to :class:`numpy.ndarray` is fine.
    ...
    def _process_inputs(self, actual, expected, *, id, allow_subclasses):
        self._check_inputs_isinstance(actual, expected, cls=(mindtorch.Tensor, np.ndarray))
...
class TypedStoragePair(TensorLikePair):
    """Pair for :class:`mindtorch.storage.TypedStorage` inputs."""
    def __init__(self, actual, expected, *, rtol_override=0.0, atol_override=0.0, **other_parameters):
        self._check_inputs_isinstance(actual, expected, cls=mindtorch.storage.TypedStorage)
        super().__init__(actual, expected, **other_parameters)
        self.rtol = max(self.rtol, rtol_override)
        self.atol = max(self.atol, atol_override)

    def _to_tensor(self, typed_storage):
        return mindtorch.tensor(
...
def set_warn_always_context(new_val: bool):
    old_val = mindtorch.is_warn_always_enabled()
    mindtorch.set_warn_always(new_val)
    try:
        yield
    finally:
        mindtorch.set_warn_always(old_val)
...
class TestCase(expecttest.TestCase):
    ...
    # Toggles whether to assert that `mindtorch.get_default_dtype()` returns
    # `mindtorch.float` when `setUp` and `tearDown` are called.
    ...
    def _should_stop_test_suite(self):
        if mindtorch.cuda.is_initialized():
            # CUDA device side error will cause subsequence test cases to fail.
            # stop entire test suite if catches RuntimeError during mindtorch.cuda.synchronize().
            try:
                mindtorch.cuda.synchronize()
            except RuntimeError as rte:
                print("TEST SUITE EARLY TERMINATION due to mindtorch.cuda.synchronize() failure", file=sys.stderr)
    ...
    def wrap_with_cuda_policy(self, method_name, policy):
        ...
        # -- ezyang
        from mindtorch.testing._internal.common_cuda import TEST_CUDA
    ...
    def compile_fn(self, fn, backend, nopython):
        ...
        return mindtorch._dynamo.optimize(backend, nopython=nopython)(fn)
    ...
    def setUp(self):
        ...
        # self._check_invariants = mindtorch.sparse.check_sparse_tensor_invariants.is_enabled()
        ...
        # # @mindtorch.sparse.check_sparse_tensor_invariants(False)
        # # decorator to disable the invariant checks.
        # mindtorch.sparse.check_sparse_tensor_invariants.enable()

        if self._default_dtype_check_enabled:
            assert mindtorch.get_default_dtype() == mindtorch.float

        # attempt to reset some global state at the end of the test
        self._prev_grad_state = mindtorch.is_grad_enabled()
    ...
    def tearDown(self):
        ...
        if hasattr(self, '_check_invariants'):
            ...
            if self._check_invariants:
                ...
                mindtorch.sparse.check_sparse_tensor_invariants.disable()
        ...
        if hasattr(self, '_prev_grad_state'):
            mindtorch.set_grad_enabled(self._prev_grad_state)
    ...
    def _make_crow_indices(n_rows, n_cols, nnz,
        ...
        counts = mindtorch.zeros(n_rows + 1, dtype=dtype, device=mindtorch.device('cpu'))
        ...
        if N:
            ...
            counts[1:p] = mindtorch.arange(p - 1, dtype=dtype, device=counts.device) % (n_cols - m + 1)
            # incomplete sawtooth:
            counts[p:p + k + 1] += mindtorch.arange(k + 1, dtype=dtype, device=counts.device)
        else:
        ...
        if random:
            ...
            # sequence:
            perm = mindtorch.randperm(n_rows, device=counts.device)
    ...
    def genSparseCompressedTensor(self, size, nnz, *, layout, device, dtype, index_dtype, blocksize=(), dense_dims=0):
        ...
        def random_sparse_compressed(n_compressed_dims, n_plain_dims, nnz):
            ...
            plain_indices = mindtorch.zeros(nnz, dtype=index_dtype, device=device)
            for i in range(n_compressed_dims):
                count = compressed_indices[i + 1] - compressed_indices[i]
                plain_indices[compressed_indices[i]:compressed_indices[i + 1]], _ = mindtorch.sort(
                    mindtorch.randperm(n_plain_dims, dtype=index_dtype, device=device)[:count])
            low = -1 if dtype != mindtorch.uint8 else 0
            high = 1 if dtype != mindtorch.uint8 else 2
        ...
        if layout in {mindtorch.sparse_csr, mindtorch.sparse_bsr}:
        ...
        values = mindtorch.stack(next(sparse_tensors_it)).reshape(*batch_shape, blocknnz, *blocksize, *dense_size)
        compressed_indices = mindtorch.stack(next(sparse_tensors_it)).reshape(*batch_shape, -1)
        plain_indices = mindtorch.stack(next(sparse_tensors_it)).reshape(*batch_shape, -1)
        return mindtorch.sparse_compressed_tensor(compressed_indices, plain_indices,
    ...
    def genSparseCSRTensor(self, size, nnz, *, device, dtype, index_dtype, dense_dims=0):
        return self.genSparseCompressedTensor(size, nnz, layout=mindtorch.sparse_csr, device=device,
    ...
    def genSparseCSCTensor(self, size, nnz, *, device, dtype, index_dtype, dense_dims=0):
        return self.genSparseCompressedTensor(size, nnz, layout=mindtorch.sparse_csc, device=device,
    ...
    def genSparseBSRTensor(self, size, blocksize, nnz, *, device, dtype, index_dtype, dense_dims=0):
        ...
        return self.genSparseCompressedTensor(size, nnz, layout=mindtorch.sparse_bsr, device=device,
    ...
    def genSparseBSCTensor(self, size, blocksize, nnz, *, device, dtype, index_dtype, dense_dims=0):
        assert len(blocksize) == 2
        return self.genSparseCompressedTensor(size, nnz, layout=mindtorch.sparse_bsc, device=device,
                                              dtype=dtype, index_dtype=index_dtype, blocksize=blocksize, dense_dims=dense_dims)
    ...
    def genSparseTensor(self, size, sparse_dim, nnz, is_uncoalesced, device, dtype):
        ...
        i = mindtorch.rand(sparse_dim, nnz, device=device)
        i.mul_(mindtorch.tensor(size[:sparse_dim]).unsqueeze(1).to(i))
        i = i.to(mindtorch.long)
        if is_uncoalesced:
            i1 = i[:, :(nnz // 2), ...]
            i2 = i[:, :((nnz + 1) // 2), ...]
            i = mindtorch.cat([i1, i2], 1)
        x = mindtorch.sparse_coo_tensor(i, v, mindtorch.Size(size), dtype=dtype, device=device)
    ...
    def generate_simple_inputs(self, layout,
        ...
        if index_dtype is None:
            index_dtype = mindtorch.int64

        is_compressed_sparse_layout = layout in {mindtorch.sparse_csr, mindtorch.sparse_csc, mindtorch.sparse_bsr, mindtorch.sparse_bsc}
        ...
        if output_tensor:
            for args, kwargs in self.generate_simple_inputs(layout, device=device, dtype=dtype, index_dtype=index_dtype,
                ...
                if layout is mindtorch.strided:
                    assert len(args) == 1
                ...
                elif layout is mindtorch.sparse_coo:
                    yield mindtorch.sparse_coo_tensor(*args, **kwargs)
                elif is_compressed_sparse_layout:
                    kwargs.update(layout=layout)
                    yield mindtorch.sparse_compressed_tensor(*args, **kwargs)
                else:
        ...
        def get_blockpattern(pattern, blocksize):
            ...
            block_ids = mindtorch.arange(1, blockpattern.numel() + 1).reshape(blockpattern.shape)
        ...
        def get_sparse_data(pattern):
            ...
            # We cannot use `mindtorch.sparse_xyz_tensor(pattern)` to
            # compute the sparse layout indices and values because
            # generate_simple_inputs is used to generate the inputs to
            # test `mindtorch.sparse_xyz_tensor` factory functions, so
            # we'll compute the indices and values independently of
            # the factory functions.

            indices = mindtorch.where(pattern != 0)
            coo_indices = mindtorch.stack(indices)
            crow_indices = mindtorch.zeros(basesize[0] + 1, dtype=mindtorch.int64)
            crow_indices[1:] = mindtorch.cumsum(coo_indices[0].bincount(minlength=basesize[0]), 0)
            col_indices = coo_indices[1]
            strided_values = mindtorch.zeros(basesize, dtype=mindtorch.int64)
            ...
            values = mindtorch.arange(1, 1 + len(indices[0]), dtype=mindtorch.int64)
            strided_values[indices] = values

            indices_T = mindtorch.where(pattern.transpose(0, 1) != 0)
            coo_indices_T = mindtorch.stack(indices_T)
            ccol_indices = mindtorch.zeros(basesize[1] + 1, dtype=mindtorch.int64)
            ccol_indices[1:] = mindtorch.cumsum(coo_indices_T[0].bincount(minlength=basesize[1]), 0)
            row_indices = coo_indices_T[1]
            csc_values = strided_values.transpose(0, 1)[indices_T]

            return {mindtorch.sparse_coo: (coo_indices, values),
                    mindtorch.sparse_csr: (crow_indices, col_indices, values),
                    mindtorch.sparse_csc: (ccol_indices, row_indices, csc_values),
                    mindtorch.strided: (strided_values,)}
        ...
        def get_sparse_data_with_block(pattern, blocksize):
            ...
            strided_values = nonblock_data[mindtorch.strided][0]
            block_indices = block_data[mindtorch.sparse_coo][0]
            bsr_values = mindtorch.stack([strided_values[bi * blocksize[0]:(bi + 1) * blocksize[0],
            ...
            bsc_values = bsr_values[block_data[mindtorch.sparse_csc][2] - 1]

            return {mindtorch.sparse_bsr: (*block_data[mindtorch.sparse_csr][:2], bsr_values),
                    mindtorch.sparse_bsc: (*block_data[mindtorch.sparse_csc][:2], bsc_values),
                    **nonblock_data}
        ...
        def get_batch_sparse_data(pattern, blocksize):
            ...
            for i, item in enumerate(pattern):
                for layout, d in get_batch_sparse_data(item, blocksize).items():
                    ...
                    if layout is mindtorch.sparse_coo:
                        # a "batch COO" means a COO with the leading
                        # sparse dimensions interpreted as batch
                        # dimensions
                        ext_coo_indices1 = mindtorch.cat((mindtorch.full((1, len(d[1])), i, dtype=mindtorch.int64), d[0]))
                        if target is None:
                            target = batch_data[layout] = (ext_coo_indices1, d[1])
                        else:
                            target[0].set_(mindtorch.cat((target[0], ext_coo_indices1), 1))  # type: ignore[call-overload]
                            target[1].set_(mindtorch.cat((target[1], d[1])))
                    else:
                        if target is None:
                            target = batch_data[layout] = tuple(d[j].unsqueeze(0) for j in range(len(d)))
                        else:
                            for j in range(len(d)):
                                target[j].set_(mindtorch.cat((target[j], d[j].unsqueeze(0))))  # type: ignore[call-overload]
            return batch_data
        ...
        def generate_values(base, densesize):
            ...
            if not isinstance(base, int) and base.ndim > 0:
                return mindtorch.stack([generate_values(b, densesize) for b in base])
            if base == 0:
                return mindtorch.zeros(densesize, dtype=mindtorch.int64)
            r = mindtorch.arange(densesize[0], dtype=mindtorch.int64)
            for i, d in enumerate(densesize[1:]):
                y = mindtorch.arange(d, dtype=mindtorch.int64) * (10 ** (i + 1))
                r = r[..., None] + y[None, ...]
            r.add_(base)
            return r
        ...
        def non_contiguous_copy(t, dim=-1, offset=0):
            ...
            step = max(2, offset + 1)
            tmp = mindtorch.zeros((*t.shape[:dim], t.shape[dim] * step, *t.shape[dim + 1:]), dtype=t.dtype, device=t.device)
        ...
        for pattern, blocksizes, densesizes in patterns:
            ...
            if not (densesizes and blocksizes):
                continue
            pattern = mindtorch.tensor(pattern, dtype=mindtorch.int64)
            ...
            for basesize, blocksizes, densesizes in [
                ...
                for blocksize in blocksizes:
                    for densesize in densesizes:  # type: ignore[attr-defined]
                        if layout == mindtorch.strided:
                            indices = ()  # type: ignore[assignment]
                            values = mindtorch.empty((basesize + densesize), device=device, dtype=dtype)
                        elif layout == mindtorch.sparse_coo:
                            indices = (mindtorch.empty(len(basesize), 0, device=device, dtype=index_dtype),)  # type: ignore[assignment]
                            values = mindtorch.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == mindtorch.sparse_csr:
                            crow_indices = mindtorch.tensor([0] * (basesize[0] + 1), device=device, dtype=index_dtype)
                            col_indices = mindtorch.empty(0, device=device, dtype=index_dtype)
                            indices = (crow_indices, col_indices)  # type: ignore[assignment]
                            values = mindtorch.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == mindtorch.sparse_csc:
                            ccol_indices = mindtorch.tensor([0] * (basesize[1] + 1), device=device, dtype=index_dtype)
                            row_indices = mindtorch.empty(0, device=device, dtype=index_dtype)
                            indices = (ccol_indices, row_indices)  # type: ignore[assignment]
                            values = mindtorch.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == mindtorch.sparse_bsr:
                            crow_indices = mindtorch.tensor([0] * (basesize[0] // blocksize[0] + 1), device=device, dtype=index_dtype)
                            col_indices = mindtorch.empty(0, device=device, dtype=index_dtype)
                            indices = (crow_indices, col_indices)  # type: ignore[assignment]
                            values = mindtorch.empty((0, *blocksize, *densesize), device=device, dtype=dtype)
                        elif layout == mindtorch.sparse_bsc:
                            ccol_indices = mindtorch.tensor([0] * (basesize[1] // blocksize[1] + 1), device=device, dtype=index_dtype)
                            row_indices = mindtorch.empty(0, device=device, dtype=index_dtype)
                            indices = (ccol_indices, row_indices)  # type: ignore[assignment]
                            values = mindtorch.empty((0, *blocksize, *densesize), device=device, dtype=dtype)
                        else:
    ...
    def safeToDense(self, t):
        ...
        if t.layout == mindtorch.sparse_coo:
            t = t.coalesce()
        return t.to_dense()
    ...
    def compare_with_numpy(self, torch_fn, np_fn, tensor_like,
        ...
        if isinstance(tensor_like, mindtorch.Tensor):
            assert device is None
            assert dtype is None
            t_cpu = tensor_like.detach().cpu()
            if t_cpu.dtype is mindtorch.bfloat16:
                t_cpu = t_cpu.float()
            a = t_cpu.numpy()
            t = tensor_like
        else:
            d = copy.copy(torch_to_numpy_dtype_dict)
            d[mindtorch.bfloat16] = np.float32
            a = np.array(tensor_like, dtype=d[dtype])
            t = mindtorch.tensor(tensor_like, device=device, dtype=dtype)
        ...
        if isinstance(np_result, np.ndarray):
            try:
                np_result = mindtorch.from_numpy(np_result)
            except Exception:
                # NOTE: copying an array before conversion is necessary when,
                #   for example, the array has negative strides.
                np_result = mindtorch.from_numpy(np_result.copy())
            if t.dtype is mindtorch.bfloat16 and torch_result.dtype is mindtorch.bfloat16 and np_result.dtype is mindtorch.float:
                torch_result = torch_result.to(mindtorch.float)
    ...
    def assertEqualBroadcasting(self, x, y, *args, **kwargs) -> None:
        ...
        if not isinstance(y, Iterable):
            ...
            y = mindtorch.ones_like(x) * y
        if not isinstance(y, mindtorch.Tensor):
            # iterable, but not a tensor
            y = mindtorch.ones_like(x) * mindtorch.tensor(y)
    ...
    def assertEqual(
        ...
        if any(
            ...
            def to_list(input):
                return input.tolist() if isinstance(input, (mindtorch.Tensor, np.ndarray)) else list(input)
        ...
        elif isinstance(x, mindtorch.Tensor) and isinstance(y, Sequence):
            y = mindtorch.as_tensor(y, dtype=x.dtype, device=x.device)
        elif isinstance(x, Sequence) and isinstance(y, mindtorch.Tensor):
            x = mindtorch.as_tensor(x, dtype=y.dtype, device=y.device)

        # unbind NSTs to compare them; don't do this for NJTs
        if isinstance(x, mindtorch.Tensor) and x.is_nested and x.layout == mindtorch.strided:
            x = x.unbind()
        if isinstance(y, mindtorch.Tensor) and y.is_nested and y.layout == mindtorch.strided:
            y = y.unbind()
        ...
        error_metas = not_close_error_metas(
            ...
            sequence_types=(
                ...
                ParameterList,
                ScriptList,
                mindtorch.utils.data.dataset.Subset,
            ),
    ...
    def assertExpected(self, s, subname=None):
        ...
        expected = expected.replace(
            ...
            f'producer_version: "{mindtorch.onnx.producer_version}"'
        )
    ...
    def assertAtenOp(self, onnx_model, operator, overload_name=""):
        all_aten_nodes = [p for p in onnx_model.graph.node
                          if p.op_type == "ATen" and p.domain == "org.pymindtorch.aten"]
    ...
    def check_nondeterministic_alert(self, fn, caller_name, should_alert=True):
        ...
        expected while `mindtorch.use_deterministic_algorithms(True)` is set.
    ...
    def _attempt_load_from_subprocess(
        ...
        """
        Attempts weights_only `mindtorch.load` in a subprocess. This is used to test that
        weights_only `mindtorch.load` works as expected without global imports.
        ...
        """
        script = f"import torch;{import_string}mindtorch.load(r'{file}', weights_only=True)"
        cm = (
...
#   and review including them in mindtorch.testing
# Methods for matrix generation

def random_square_matrix_of_rank(l, rank, dtype=mindtorch.double, device='cpu'):
    assert rank <= l
    A = mindtorch.randn(l, l, dtype=dtype, device=device)
    u, s, vh = mindtorch.linalg.svd(A, full_matrices=False)
    for i in range(l):
        if i >= rank:
            s[i] = 0
        elif s[i] == 0:
...
def random_well_conditioned_matrix(*shape, dtype, device, mean=1.0, sigma=0.001):
    ...
    """
    primitive_dtype = {
        mindtorch.float: mindtorch.float,
        mindtorch.double: mindtorch.double,
        mindtorch.cfloat: mindtorch.float,
        mindtorch.cdouble: mindtorch.double
    }
    x = mindtorch.rand(shape, dtype=dtype, device=device)
    m = x.size(-2)
    n = x.size(-1)
    u, _, vh = mindtorch.linalg.svd(x, full_matrices=False)
    s = (mindtorch.randn(*(shape[:-2] + (min(m, n),)), dtype=primitive_dtype[dtype], device=device) * sigma + mean) \
...
def random_symmetric_matrix(l, *batches, **kwargs):
    dtype = kwargs.get('dtype', mindtorch.double)
    device = kwargs.get('device', 'cpu')
    A = mindtorch.randn(*(batches + (l, l)), dtype=dtype, device=device)
    A = (A + A.mT).div_(2)
    return A
...
def random_hermitian_psd_matrix(matrix_size, *batch_dims, dtype=mindtorch.double, device='cpu'):
    """
    ...
    """
    A = mindtorch.randn(*(batch_dims + (matrix_size, matrix_size)), dtype=dtype, device=device)
    return A @ A.mH
...
def random_symmetric_pd_matrix(matrix_size, *batch_dims, **kwargs):
    dtype = kwargs.get('dtype', mindtorch.double)
    device = kwargs.get('device', 'cpu')
    A = mindtorch.randn(*(batch_dims + (matrix_size, matrix_size)),
                    dtype=dtype, device=device)
    return mindtorch.matmul(A, A.mT) \
        + mindtorch.eye(matrix_size, dtype=dtype, device=device) * 1e-5
...
def make_symmetric_pd_matrices(*shape, device, dtype):
    assert shape[-1] == shape[-2]
    t = make_tensor(shape, device=device, dtype=dtype)
    i = mindtorch.eye(shape[-1], device=device, dtype=dtype) * 1e-5
    return t @ t.mT + i
...
def random_hermitian_pd_matrix(matrix_size, *batch_dims, dtype, device):
    ...
    A = mindtorch.randn(*(batch_dims + (matrix_size, matrix_size)),
                    dtype=dtype, device=device)
    return A @ A.mH + mindtorch.eye(matrix_size, dtype=dtype, device=device)
...
def make_fullrank_matrices_with_distinct_singular_values(*shape, device, dtype, requires_grad=False):
    with mindtorch.no_grad():
        t = make_tensor(shape, device=device, dtype=dtype)
        u, _, vh = mindtorch.linalg.svd(t, full_matrices=False)
        ...
        # s = [2, 3, ..., k+1]
        s = mindtorch.arange(2, k + 2, dtype=real_dtype, device=device)
        # s = [2, -3, 4, ..., (-1)^k k+1]
        s[1::2] *= -1.
...
def random_matrix(rows, columns, *batch_dims, **kwargs):
    ...
    if silent and not mindtorch._C.has_lapack:
        return mindtorch.ones(rows, columns, dtype=dtype, device=device)

    A = mindtorch.randn(batch_dims + (rows, columns), dtype=dtype, device=device)
    if A.numel() == 0:
        return A
    u, _, vh = mindtorch.linalg.svd(A, full_matrices=False)
    k = min(rows, columns)
    s = mindtorch.linspace(1 / (k + 1), 1, k, dtype=dtype, device=device)
    if singular:
...
def _generate_indices_prefer_all_rows(rows: int, cols: int, num_indices: int) -> mindtorch.Tensor:
    ...
    return mindtorch.tensor(indices[:num_indices])
...
def random_sparse_matrix(rows, columns, density=0.01, **kwargs):
    ...
    values = mindtorch.randn(nonzero_elements, dtype=dtype, device=device)

    # ensure that the diagonal dominates
    values *= mindtorch.tensor([-float(i - j)**2 for i, j in indices], dtype=dtype, device=device).exp()
    A = mindtorch.sparse_coo_tensor(indices.t(), values, (rows, columns), device=device)
    return A.coalesce()
...
def random_sparse_pd_matrix(matrix_size, density=0.01, **kwargs):
    ...
    for (i, j), v in sorted(data.items()):
        ...
        jcoords.append(j)
        values.append(v)
    indices_tensor = mindtorch.tensor([icoords, jcoords])
    return mindtorch.sparse_coo_tensor(indices_tensor, values, (matrix_size, matrix_size), dtype=dtype, device=device)

# FIXME: remove this by updating test suites using it
def do_test_dtypes(self, dtypes, layout, device):
    for dtype in dtypes:
        if dtype != mindtorch.float16:
            out = mindtorch.zeros((2, 3), dtype=dtype, layout=layout, device=device)
...
def do_test_empty_full(self, dtypes, layout, device):
    ...
    def get_int64_dtype(dtype):
        ...
        if not module:
            return mindtorch.int64
        return operator.attrgetter(module)(torch).int64

    default_dtype = mindtorch.get_default_dtype()
    check_value(mindtorch.empty(shape), default_dtype, mindtorch.strided, -1, None, False)
    check_value(mindtorch.full(shape, -5.), default_dtype, mindtorch.strided, -1, None, False)
    for dtype in dtypes:
        for rg in {dtype.is_floating_point, False}:
            int64_dtype = get_int64_dtype(dtype)
            v = mindtorch.empty(shape, dtype=dtype, device=device, layout=layout, requires_grad=rg)
            check_value(v, dtype, layout, device, None, rg)
            out = v.new()
            check_value(mindtorch.empty(shape, out=out, device=device, layout=layout, requires_grad=rg),
                        dtype, layout, device, None, rg)
            check_value(v.new_empty(shape), dtype, layout, device, None, False)
            check_value(v.new_empty(shape, dtype=int64_dtype, device=device, requires_grad=False),
                        int64_dtype, layout, device, None, False)
            check_value(mindtorch.empty_like(v), dtype, layout, device, None, False)
            check_value(mindtorch.empty_like(v, dtype=int64_dtype, layout=layout, device=device, requires_grad=False),
                        int64_dtype, layout, device, None, False)

            if dtype is not mindtorch.float16 and layout != mindtorch.sparse_coo:
                fv = 3
                v = mindtorch.full(shape, fv, dtype=dtype, layout=layout, device=device, requires_grad=rg)
                check_value(v, dtype, layout, device, fv, rg)
                check_value(v.new_full(shape, fv + 1), dtype, layout, device, fv + 1, False)
                out = v.new()
                check_value(mindtorch.full(shape, fv + 2, out=out, device=device, layout=layout, requires_grad=rg),
                            dtype, layout, device, fv + 2, rg)
                check_value(v.new_full(shape, fv + 3, dtype=int64_dtype, device=device, requires_grad=False),
                            int64_dtype, layout, device, fv + 3, False)
                check_value(mindtorch.full_like(v, fv + 4), dtype, layout, device, fv + 4, False)
                check_value(mindtorch.full_like(v, fv + 5,
...
# https://pymindtorch.org/docs/stable/generated/mindtorch.use_deterministic_algorithms.html
#
...
def gradcheck(fn, inputs, **kwargs):
    ...
    return mindtorch.autograd.gradcheck(fn, inputs, **kwargs)
...
def gradgradcheck(fn, inputs, grad_outputs=None, **kwargs):
    ...
    return mindtorch.autograd.gradgradcheck(fn, inputs, grad_outputs, **kwargs)
...
dtype2prec_DONTUSE = {mindtorch.float: 1e-5,
                      mindtorch.double: 1e-5,
                      mindtorch.half: 1e-2,
                      mindtorch.bfloat16: 1e-1}
...
def is_coalesced_indices(s):
    ...
    hash_indices = mindtorch.tensor(hash_coeffs, device=s.device).cumprod(-1).flip(-1)
    if s.sparse_dim() > 1:
    ...
    res = mindtorch.allclose(hash_indices, hash_indices.sort()[0])

    # check if there are no repeated indices
    res = res and mindtorch.allclose(hash_indices, hash_indices.unique())
...
def find_library_location(lib_name: str) -> Path:
    ...
    torch_root = Path(mindtorch.__file__).resolve().parent
...
def bytes_to_scalar(byte_list: list[int], dtype: mindtorch.dtype, device: mindtorch.device):
    dtype_to_ctype: dict[mindtorch.dtype, Any] = {
        mindtorch.int8: ctypes.c_int8,
        mindtorch.uint8: ctypes.c_uint8,
        mindtorch.uint16: ctypes.c_uint16,
        mindtorch.uint32: ctypes.c_uint32,
        mindtorch.uint64: ctypes.c_uint64,
        mindtorch.int16: ctypes.c_int16,
        mindtorch.int32: ctypes.c_int32,
        mindtorch.int64: ctypes.c_int64,
        mindtorch.bool: ctypes.c_bool,
        mindtorch.float32: ctypes.c_float,
        mindtorch.complex64: ctypes.c_float,
        mindtorch.float64: ctypes.c_double,
        mindtorch.complex128: ctypes.c_double,
    }
    ...
    return mindtorch.tensor(res, device=device, dtype=dtype)
...
def dtype_name(dtype):
    """ Returns the pretty name of the dtype (e.g. mindtorch.int64 -> int64). """
    return str(dtype).split('.')[1]


@functools.lru_cache
def get_cycles_per_ms() -> float:
    """Measure and return approximate number of cycles per millisecond for mindtorch.cuda._sleep
    """

    def measure() -> float:
        start = mindtorch.cuda.Event(enable_timing=True)
        end = mindtorch.cuda.Event(enable_timing=True)
        start.record()
        mindtorch.cuda._sleep(1000000)
        end.record()
        end.synchronize()
...
def outs_and_grads(fn, graph_inps, inps):
    ...
    for out in pytree.tree_leaves(outs):
        if isinstance(out, mindtorch.Tensor) and out.requires_grad:
            out.sum().backward(retain_graph=True)
    grads = [inp.grad for inp in pytree.tree_leaves(inps) if isinstance(inp, mindtorch.Tensor)]
    for inp in pytree.tree_leaves(inps):
        if isinstance(inp, mindtorch.Tensor):
            inp.grad = None
    return outs, grads
...
class TestGradients(TestCase):
    ...
    def _check_helper(self, device, dtype, op, variant, check, *, check_forward_ad=False, check_backward_ad=True,
        ...
        if not op.supports_dtype(dtype, mindtorch.device(device).type):
        ...
        for sample in samples:
            ...
            gradcheck_args = tuple(x for x in all_args if (isinstance(x, mindtorch.Tensor) and x.requires_grad))  # type: ignore[union-attr]
            ...
            def _input_recomposition_helper(inputs, inp, input_idx):
                if is_iterable_of_tensors(inp):
                    tensor_list = []
                    for x in inp:
                        if isinstance(x, mindtorch.Tensor) and x.requires_grad:
                            tensor_list.append(inputs[input_idx])
                            input_idx = input_idx + 1
                        else:
                            tensor_list.append(x)
                    return tensor_list, input_idx
                elif isinstance(inp, mindtorch.Tensor) and inp.requires_grad:
    ...
    def _skip_helper(self, op, device, dtype):
        if dtype not in op.supported_backward_dtypes(mindtorch.device(device).type):
...
class NestedTensorTestCase(TestCase):
    def assertEqualIgnoringNestedInts(self, a, b):
        ...
        def _unbind_njts(x):
            if isinstance(x, mindtorch.Tensor) and x.is_nested and x.layout == mindtorch.jagged:
                return x.unbind()
            else:
                return x
    ...
    def assertEqualNoncontigAware(self, a, b):
        ...
        def _get_njt_shapes(x):
            return (
                x.shape
                if isinstance(x, mindtorch.Tensor) and x.is_nested
                else None
            )
    ...
    def branch_nested_state(self):
        ...
        nested_tensor_module = mindtorch.nested._internal.nested_tensor
...
def munge_exc(e, *, suppress_suffix=True, suppress_prefix=True, file=None, skip=0):
    ...
    s = re.sub(_as_posix_path(os.path.join(os.path.dirname(mindtorch.__file__), "")), "", s)
    if suppress_suffix:
...
@contextmanager
def check_leaked_tensors(limit=1, matched_type=mindtorch.Tensor):
...
def recover_orig_fp32_precision(fn):
    @contextlib.contextmanager
    def recover():
        old_mkldnn_conv_p = mindtorch.backends.mkldnn.conv.fp32_precision  # type: ignore[attr-defined]
        old_mkldnn_rnn_p = mindtorch.backends.mkldnn.rnn.fp32_precision  # type: ignore[attr-defined]
        old_mkldnn_matmul_p = mindtorch.backends.mkldnn.matmul.fp32_precision  # type: ignore[attr-defined]
        old_cudnn_conv_p = mindtorch.backends.cudnn.conv.fp32_precision  # type: ignore[attr-defined]
        old_cudnn_rnn_p = mindtorch.backends.cudnn.rnn.fp32_precision  # type: ignore[attr-defined]
        old_cuda_matmul_p = mindtorch.backends.cuda.matmul.fp32_precision
        try:
            yield
        finally:
            mindtorch.backends.mkldnn.conv.fp32_precision = old_mkldnn_conv_p  # type: ignore[attr-defined]
            mindtorch.backends.mkldnn.rnn.fp32_precision = old_mkldnn_rnn_p  # type: ignore[attr-defined]
            mindtorch.backends.mkldnn.matmul.fp32_precision = old_mkldnn_matmul_p  # type: ignore[attr-defined]
            mindtorch.backends.cudnn.conv.fp32_precision = old_cudnn_conv_p  # type: ignore[attr-defined]
            mindtorch.backends.cudnn.rnn.fp32_precision = old_cudnn_rnn_p  # type: ignore[attr-defined]
            mindtorch.backends.cuda.matmul.fp32_precision = old_cuda_matmul_p
