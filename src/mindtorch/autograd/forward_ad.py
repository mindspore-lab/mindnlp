...
def enter_dual_level():
    ...
    new_level = mindtorch._C._enter_dual_level()
...
def exit_dual_level(*, level=None):
    ...
    mindtorch._C._exit_dual_level(level=level)
    _current_level = level - 1


def _maybe_load_decompositions():
    if os.environ.get("PYTORCH_JIT", "1") == "1" and __debug__:
        from mindtorch._decomp import decompositions_for_jvp  # noqa: F401
...
def make_dual(tensor, tangent, *, level=None):
    ...
    Please see the `forward-mode AD tutorial <https://pymindtorch.org/tutorials/intermediate/forward_ad_usage.html>`__
    for detailed steps on how to use this API.

    """
    # See NOTE: [forward-mode AD decompositions mechanism]
    #
    # Import from mindtorch._decomp import decompositions_for_jvp to register
    ...
    #     else:
    #         buffer = z
    #     return min - mindtorch.log1p(z), buffer
    #     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ <--- HERE
    ...
    return mindtorch._VF._make_dual(tensor, tangent, level=level)
...
class UnpackedDualTensor(NamedTuple):
    ...
    primal: mindtorch.Tensor
    tangent: Optional[mindtorch.Tensor]
...
def unpack_dual(tensor, *, level=None):
    ...
    primal, dual = mindtorch._VF._unpack_dual(tensor, level=level)
...
class dual_level(_DecoratorContextManager):
    ...
    .. Note::
        ...
        forward grads, one can use :func:`mindtorch.func.jvp`.

    Example::

        >>> # xdoctest: +SKIP("Undefined variables")
        >>> x = mindtorch.tensor([1])
        >>> x_t = mindtorch.tensor([1])
        >>> with dual_level():
        ...     inp = make_dual(x, x_t)
...
# _is_fwd_grad_enabled = mindtorch._C._is_fwd_grad_enabled
...
class _set_fwd_grad_enabled(_DecoratorContextManager):
    def __init__(self, mode: bool) -> None:
        ...
        mindtorch._C._set_fwd_grad_enabled(mode)

    def __enter__(self) -> None:
        pass

    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        mindtorch._C._set_fwd_grad_enabled(self.prev)