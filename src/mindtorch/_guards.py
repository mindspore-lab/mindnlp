...
from mindtorch.utils._backport_slots import dataclass_slots
from mindtorch.utils._traceback import CapturedTraceback, format_frame
from mindtorch.utils.weak import WeakTensorKeyDictionary
...
if TYPE_CHECKING:
    ...
    from mindtorch._dynamo.codegen import PyCodegen
    from mindtorch._funcmindtorch._aot_autograd.schemas import ViewAndMutationMeta
    from mindtorch._subclasses.fake_tensor import FakeTensorMode


"""
mindtorch._guards is the definitional source of truth for general purpose guard structures.
...
class CompileId:
    ...
    # mindtorch.compiling a compiled autograd graph
...
class GuardSource(enum.Enum):
    ...
    def is_specialized_nn_module(self) -> bool:
        import mindtorch._dynamo.config as config
...
class Guard:
    ...
    def sort_key(self) -> tuple[bool, int, int, str, int]:
        ...
        is_duplicate_input = (
            ...
            and self.create_fn.func is mindtorch._dynamo.guards.GuardBuilder.DUPLICATE_INPUT
        )
        return (
            is_duplicate_input,
...
mindtorch._dynamo.guards._parse_guard_env_guards to avoid a RuntimeError.
"""
...
class ModuleContextCheckpointState:
    nn_modules: dict[str, mindtorch.nn.Module] = {}

    def __init__(self, nn_modules: dict[str, mindtorch.nn.Module]) -> None:
...
class GlobalContext(Checkpointable[GlobalContextCheckpointState]):
    ...
    For example, mindtorch.is_grad_enabled.
    """
...
class HopSubgraphCache:
    ...
    def add_lazy_bwd_entry(
        ...
        gmod: mindtorch.fx.GraphModule,
    ) -> int: ...

    @abstractmethod
    def get_lazy_bwd_entry(
        self, identifier: str, tangent_metadata: tuple[object]
    ) -> tuple[Optional[mindtorch.fx.GraphModule], Optional[int]]: ...
...
class InvokeSubgraphCache(HopSubgraphCache):
    def __init__(self) -> None:
        ...
        self.lazy_bwd_cache: dict[
            str, dict[tuple[object], tuple[mindtorch.fx.GraphModule, int]]
        ] = defaultdict(dict)
...
class HopDispatchSetCache:
    def __init__(self) -> None:
        ...
        from mindtorch._higher_order_ops.invoke_subgraph import invoke_subgraph

        self.hop_cache_map = {invoke_subgraph: InvokeSubgraphCache()}

    def get_cache(
        self, op: mindtorch._ops.HigherOrderOperator
...
def detect_fake_mode(inputs: Any = None) -> Optional[FakeTensorMode]:
    ...
    """
    from mindtorch._subclasses.fake_tensor import FakeTensor, FakeTensorMode
    ...
    from mindtorch.utils._python_dispatch import _get_current_dispatch_mode_stack
