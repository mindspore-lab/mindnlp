...
from mindtorch.overrides import is_tensor_like
from mindtorch.types import _dtype, _Number, Device, Number
...
def broadcast_all(*values: Union[Tensor, Number]) -> tuple[Tensor, ...]:
    ...
    Args:
        values (list of `Number`, `mindtorch.*Tensor` or objects implementing __torch_function__)
    ...
    if not all(is_tensor_like(v) or isinstance(v, _Number) for v in values):
        raise ValueError(
            ...
            "mindtorch.Tensor or objects implementing __torch_function__."
        )
    if not all(is_tensor_like(v) for v in values):
        options: dict[str, Any] = dict(dtype=mindtorch.get_default_dtype())
        for value in values:
            if isinstance(value, mindtorch.Tensor):
                options = dict(dtype=value.dtype, device=value.device)
                break
        new_values = [
            v if is_tensor_like(v) else mindtorch.tensor(v, **options) for v in values
        ]
        return mindtorch.broadcast_tensors(*new_values)
    return mindtorch.broadcast_tensors(*values)
...
def _standard_normal(
    ...
    # if mindtorch._C._get_tracing_state():
    #     # [JIT WORKAROUND] lack of support for .normal_()
    #     return mindtorch.normal(
    #         mindtorch.zeros(shape, dtype=dtype, device=device),
    #         mindtorch.ones(shape, dtype=dtype, device=device),
    #     )
    return mindtorch.empty(shape, dtype=dtype, device=device).normal_()
...
def logits_to_probs(logits: Tensor, is_binary: bool = False) -> Tensor:
    ...
    """
    if is_binary:
        return mindtorch.sigmoid(logits)
...
def clamp_probs(probs: Tensor) -> Tensor:
    ...
    Examples:
        >>> probs = mindtorch.tensor([0.0, 0.5, 1.0])
        >>> clamp_probs(probs)
        tensor([1.1921e-07, 5.0000e-01, 1.0000e+00])

        >>> probs = mindtorch.tensor([0.0, 0.5, 1.0], dtype=mindtorch.float64)
        >>> clamp_probs(probs)
        tensor([2.2204e-16, 5.0000e-01, 1.0000e+00], dtype=mindtorch.float64)

    """
    eps = mindtorch.finfo(probs.dtype).eps
...
def probs_to_logits(probs: Tensor, is_binary: bool = False) -> Tensor:
    ...
    if is_binary:
        return mindtorch.log(ps_clamped) - mindtorch.log1p(-ps_clamped)
    return mindtorch.log(ps_clamped)
...
def tril_matrix_to_vec(mat: Tensor, diag: int = 0) -> Tensor:
    ...
    arange = mindtorch.arange(n, device=mat.device)
...
def vec_to_tril_matrix(vec: Tensor, diag: int = 0) -> Tensor:
    ...
    eps = mindtorch.finfo(vec.dtype).eps
    ...
    n = round(n.item()) if isinstance(n, mindtorch.Tensor) else round(n)
    mat = vec.new_zeros(vec.shape[:-1] + mindtorch.Size((n, n)))
    arange = mindtorch.arange(n, device=vec.device)
    tril_mask = arange < arange.view(-1, 1) + (diag + 1)
    mat[..., tril_mask] = vec
    return mat