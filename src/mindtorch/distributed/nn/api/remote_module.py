...
from mindtorch import device, dtype, nn, Tensor
from mindtorch.distributed import _remote_device
from mindtorch.distributed.nn.jit import instantiator
from mindtorch.distributed.rpc.internal import _internal_rpc_pickler
...
def _create_module_with_interface(
    ...
    if module_interface_cls is not None:
        module = mindtorch.jit.script(module)
...
class _RemoteModule(nn.Module):
    def __new__(cls, *args, **kwargs):
        ...
        mindtorch._C._log_api_usage_once("mindtorch.distributed.nn.api.remote_module")
    ...
    def __init__(
        ...
        It can be shared across processors using `RPC framework <https://pymindtorch.org/docs/stable/rpc.html>`__,
        without incurring any overheads of copying the actual module,
        which is equivalent to an :class:`~mindtorch.distributed.rpc.RRef`
        ...
        Args:
            ...
            _module_interface_cls (type, optional): The TorchScript interface type for the module
                to be created. The type object should be decorated by @mindtorch.jit.interface.
        ...
        Example::
            ...
            >>> import mindtorch
            >>> import mindtorch.distributed.rpc as rpc
            >>> from mindtorch import nn, Tensor
            >>> from mindtorch.distributed.nn.api.remote_module import RemoteModule
            >>>
    ...
    def remote_parameters(self, recurse: bool = True) -> List[rpc.RRef[Parameter]]:
        """
        Return a list of :class:`~mindtorch.distributed.rpc.RRef` pointing to the remote module's parameters.

        This can typically be used in conjunction
        with :class:`~mindtorch.distributed.optim.DistributedOptimizer`.
        ...
        Returns:
            A list of :class:`~mindtorch.distributed.rpc.RRef` (``List[RRef[nn.Parameter]]``)
    ...
    def get_module_rref(self) -> rpc.RRef[nn.Module]:
        """Return an :class:`~mindtorch.distributed.rpc.RRef` (``RRef[nn.Module]``) pointing to the remote module."""
        return self.module_rref

    @mindtorch.jit.export
    ...
    def _prepare_init(self, remote_device_str: str) -> bool:
        ...
        enable_moving_cpu_tensors_to_cuda = mindtorch.device(self.device).type == "cuda"
    ...
    def _install_generated_methods(self):
        for method in self.generated_methods:
            ...
            method = mindtorch.jit.export(method)
...
class RemoteModule(_RemoteModule):
    ...
    Args:
        remote_device (str): Device on the destination worker where we'd like to place this module.
            The format should be "<workername>/<device>", where the device field can be parsed as mindtorch.device type.
    ...
    Example::
        ...
        `DistributedDataParallel <https://pymindtorch.org/docs/stable/nn.html#mindtorch.nn.parallel.DistributedDataParallel>`__ (DDP)
        can be found in this `tutorial <https://pymindtorch.org/tutorials/advanced/rpc_ddp_tutorial.html>`__.
    """
...
def _recursive_script_module_reducer(recursive_script_module):
    ...
    f = io.BytesIO()
    mindtorch.jit.save(recursive_script_module, f)
...
_internal_rpc_pickler._register_reducer(
    mindtorch.jit.RecursiveScriptModule, _recursive_script_module_reducer
)
