...
from mindtorch.distributed._state_dict_utils import (
...
from mindtorch.distributed.fsdp import (
...
from mindtorch.nn.modules.module import _IncompatibleKeys
...
FQNS_T = Set[str]
PrimitiveType = Union[DTensor, ShardedTensor, mindtorch.Tensor, int, float, str]
ValueType = Union[
...
class _StateDictInfo(StateDictOptions):
    fqn_param_mapping: Dict[
        Union[str, mindtorch.Tensor], Union[FQNS_T, mindtorch.Tensor]
...
def _get_fqns(
    ...
    for i, curr_obj_name in enumerate(obj_names):
        ...
        elif isinstance(curr_obj, mindtorch._dynamo.eval_frame.OptimizedModule):
...
def _verify_options(
    model: nn.Module,
    optims: Tuple[mindtorch.optim.Optimizer, ...],
    optim_only: bool,
    *,
    ...
    fqn_param_mapping: Dict[
        Union[str, mindtorch.Tensor], Union[Set[str], mindtorch.Tensor]
    ] = {}
    shared_params_mapping: Dict[
        Union[str, mindtorch.Tensor], Union[Set[str], mindtorch.Tensor]
    ] = {}
    ...
    for param_, fqns_ in list(shared_params_mapping.items()):
        for fqn in fqns_:
            shared_params_mapping[fqn] = cast(mindtorch.Tensor, param_)
...
def _state_dict_fn(obj: Union[nn.Module, mindtorch.optim.Optimizer], api: str) -> Callable:
...
def _maybe_full_or_cpu_state_dict(
    ...
    if info.full_state_dict:
        ranks_only = (
            ()
            if (not info.cpu_offload or not mindtorch.distributed.is_initialized())
            else (0,)
        )
...
def _get_model_state_dict(
    ...
    for key, p in list(state_dict.items()):
        if mindtorch.is_tensor(p) and p.is_meta:
            state_dict.pop(key)
...
def _load_model_state_dict(
    ...
    if info.broadcast_from_rank0 or info.full_state_dict:
        ...
        if device == mindtorch.device("meta"):
...
def _init_optim_state(optim: mindtorch.optim.Optimizer) -> None:
    """
    ...
    for param_group in optim.param_groups:
        for param in param_group[_PARAMS]:
            if param.requires_grad:
                param.grad = mindtorch.zeros_like(param)
...
def _flatten_optim_state_dict(state_dict: OptimizerStateType) -> Dict[str, ValueType]:
    ...
    def _raise_if_type_not_supported(v):
        if not isinstance(v, (mindtorch.Tensor, int, float)):
...
def _get_optim_state_dict(
    model: nn.Module,
    optimizers: Tuple[mindtorch.optim.Optimizer, ...],
    info: _StateDictInfo,
) -> OptimizerStateType:
...
def _split_optim_state_dict(
    ...
    Args:
        ...
        optim (mindtorch.optim.Optimizer): the optimizer.
...
def _load_optim_state_dict(
    ...
    for optim in optimizers:
        ...
        if info.fsdp_modules:
            ...
            _ = tree_map_only(mindtorch.Tensor, _device, local_state_dict)
            assert device is not None
...
def get_optimizer_state_dict(
    model: nn.Module,
    optimizers: Union[mindtorch.optim.Optimizer, Iterable[mindtorch.optim.Optimizer]],
    *,
    submodules: Optional[Set[nn.Module]] = None,
    options: Optional[StateDictOptions] = None,
) -> OptimizerStateType:
    ...
    with _gc_context():
        optimizers = (
            (optimizers,)
            if isinstance(optimizers, mindtorch.optim.Optimizer)
...
def get_state_dict(
    ...
    *,
    ...
    Example:
        ...
        >>> from mindtorch.nn.parallel import DistributedDataParallel as DDP
        >>> from mindtorch.distributed.checkpoint.state_dict import get_state_dict

        >>> fsdp_model = FSDP(copy.deepcopy(model))
        >>> fsdp_optim = mindtorch.optim.Adam(model.parameters(), lr=1e-3)
        >>> ddp_model = DDP(copy.deepcopy(model))
        >>> ddp_optim = mindtorch.optim.Adam(model.parameters(), lr=1e-3)
...
def _patch_model_state_dict(
    ...
    *,
    ...
    Example:
        from mindtorch.distributed.fsdp import FullyShardedDataParallel as FSDP
        from mindtorch.distributed.checkpoint.state_dict import patch_model_state_dict
