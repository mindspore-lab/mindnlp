...
mindtorch.save() and mindtorch.load() is not recommended when checkpointing sharded models.
"""
...
from mindtorch.distributed.checkpoint.optimizer import load_sharded_optimizer_state_dict
from mindtorch.distributed.fsdp import FullyShardedDataParallel as FSDP
from mindtorch.distributed.fsdp.fully_sharded_data_parallel import StateDictType
...
def init_model():
    model = FSDP(mindtorch.nn.Linear(4, 4).cuda(dist.get_rank()))
    optim = mindtorch.optim.Adam(model.parameters(), lr=0.1)
    model(mindtorch.rand(4, 4)).sum().backward()
    optim.step()
...
if __name__ == "__main__":
    world_size = mindtorch.cuda.device_count()
