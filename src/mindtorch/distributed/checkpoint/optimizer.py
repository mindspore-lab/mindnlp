...
from mindtorch.distributed._shard.sharded_tensor.metadata import (
...
from mindtorch.distributed.checkpoint._nested_dict import unflatten_state_dict
from mindtorch.distributed.checkpoint.default_planner import DefaultLoadPlanner
...
from mindtorch.distributed.checkpoint.planner import LoadPlan, LoadPlanner
from mindtorch.distributed.checkpoint.planner_helpers import (
    _create_read_items,
    create_read_items_for_chunk_list,
)
from mindtorch.distributed.checkpoint.state_dict_loader import load_state_dict
from mindtorch.distributed.checkpoint.storage import StorageReader
from mindtorch.distributed.checkpoint.utils import (
    _element_wise_add,
    _element_wise_sub,
...
def _is_nested_tensor(val: mindtorch.Tensor) -> bool:
...
def _alloc_tensor(
    ...
    if device_type == "cpu":
        device = cast(mindtorch.device, _get_device_module(device_type).current_device())
    else:
...
class _ReaderWithOffset(DefaultLoadPlanner):
    ...
    def create_local_plan(self) -> LoadPlan:
        ...
        for fqn, obj in self.state_dict.items():
            ...
            local_chunks = [
                ChunkStorageMetadata(
                    offsets=mindtorch.Size(
                        _element_wise_add(original_shard.metadata.shard_offsets, offset)
                    ),
                    sizes=mindtorch.Size(original_shard.metadata.shard_sizes),
                )
            ]
            ...
            for ri in reqs:
                ...
                original_index = dataclasses.replace(
                    ri.dest_index, offset=mindtorch.Size(original_offset)
                )
    ...
    def lookup_tensor(self, index: MetadataIndex) -> mindtorch.Tensor:
...
def load_sharded_optimizer_state_dict(
    ...
    >>> # xdoctest: +SKIP
    >>> import mindtorch.distributed.checkpoint as dist_cp
    >>> # Save
    >>> model: mindtorch.nn.Model
    >>> optim_params = model.parameters()
    >>> optim = mindtorch.optim.SGD(optim_params, lr=0.01)
    >>> # Save
    ...
    for key, value in metadata.state_dict_metadata.items():
        ...
        if value.size.numel() == 1:
            ...
            st_md = sharding_spec.build_metadata(mindtorch.Size(alloc_size), properties)
            local_shards = []
