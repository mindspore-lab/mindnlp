...
class _NamedOptimizer(optim.Optimizer):
    ...
    Args:
        named_parameters (Mapping[str, Union[mindtorch.Tensor, ShardedTensor]]):
    ...
    Example::
        ...
        >>> from mindtorch import optim
        >>> from mindtorch.distributed.optim import _NamedOptimizer
        >>>
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mindtorch._C._log_api_usage_once("mindtorch.distributed.optim._NamedOptimizer")
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        This will call :meth:`mindtorch.optim.Optimizer.step` on the wrapped
        optimizer.
        """
        return self._optimizer.step(closure=closure)

    @property
    def state(self) -> Mapping[mindtorch.Tensor, Any]:  # type: ignore[override]
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any]) -> None:
        ...
        for idx, param_key in enumerate(self.ordered_param_keys):
            ...
            for state_key, state_val in new_state[idx].items():
                ...
                elif isinstance(state_val, mindtorch.Tensor):
                    assert isinstance(src_state_val, mindtorch.Tensor)
    ...
    def init_state(self) -> None:
        ...
        for param in self.named_parameters.values():
            if param.requires_grad:
                t = mindtorch.zeros_like(param)
                param.grad = mindtorch.autograd.Variable(t)
