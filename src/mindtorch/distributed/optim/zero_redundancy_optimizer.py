...
from mindtorch.distributed.optim.utils import functional_optim_map
from mindtorch.optim import Optimizer
...
def _is_trainable(param: mindtorch.Tensor) -> bool:
...
def _broadcast_object(
    ...
    device: mindtorch.device = mindtorch.device("cpu"),
) -> Any:
    r"""
    ...
    Arguments:
        ...
        device (``mindtorch.device``, optional): device to send from or receive
            to (default: ``mindtorch.device("cpu")``).
    ...
    if dist.get_rank() == src_rank:
        # Send the object
        buffer = io.BytesIO()
        mindtorch.save(obj, buffer)
        data = bytearray(buffer.getbuffer())
        length_tensor = mindtorch.LongTensor([len(data)]).to(device)
        data_send_tensor = mindtorch.ByteTensor(data).to(device)
        dist.broadcast(length_tensor, src=src_rank, group=group, async_op=False)
        dist.broadcast(data_send_tensor, src=src_rank, group=group, async_op=False)
    else:
        # Receive the object
        length_tensor = mindtorch.LongTensor([0]).to(device)
        dist.broadcast(length_tensor, src=src_rank, group=group, async_op=False)
        data_recv_tensor = mindtorch.empty(
            [int(length_tensor.item())], dtype=mindtorch.uint8, device=device
        )
        dist.broadcast(data_recv_tensor, src=src_rank, group=group, async_op=False)
        buffer = io.BytesIO(data_recv_tensor.cpu().numpy())
        obj = mindtorch.load(buffer, map_location=device, weights_only=False)
    return obj
...
class _DDPBucketAssignment:
    ...
    Attributes:
        ...
        parameters (List[mindtorch.Tensor]): model parameters in the bucket
        ...
        device (mindtorch.device): device on which the parameters are stored.
        tensor (mindtorch.Tensor): flattened tensor giving the data of the
    ...
    def __init__(
        self,
        bucket_index: int,
        parameters: List[mindtorch.Tensor],
        offset: int,
    ):
        ...
        self.device: mindtorch.device = self.parameters[0].device
        self.tensor: Optional[mindtorch.Tensor] = None
...
class _OverlapInfo:
    ...
    Attributes:
        ...
        params_per_bucket (List[List[mindtorch.Tensor]]): ``params_per_bucket[i]``
            gives the model parameters in the ``i``th bucket.
        params_per_rank (List[List[mindtorch.Tensor]]): ``params_per_rank[i]``
        ...
        bucket_index_to_future (Dict[int, mindtorch.futures.Future]):
    ...
    def __init__(self, world_size) -> None:
        ...
        self.params_per_bucket: List[List[mindtorch.Tensor]] = []
        self.params_per_rank: List[List[mindtorch.Tensor]] = [[] for _ in range(world_size)]
        ...
        self.bucket_index_to_future: Dict[int, mindtorch.futures.Future] = {}
...
class ZeroRedundancyOptimizer(Optimizer, Joinable):
    r"""
    Wrap an arbitrary :class:`optim.Optimizer <mindtorch.optim.Optimizer>` and shards its states across ranks in the group.
    ...
    :class:`mindtorch.nn.parallel.DistributedDataParallel` to reduce per-rank peak
    memory consumption.
    ...
    Arguments:
        params (``Iterable``): an ``Iterable`` of :class:`mindtorch.Tensor` s
            or :class:`dict` s giving all parameters, which will be sharded
            across ranks.

    Keyword Args:
        optimizer_class (:class:`mindtorch.nn.Optimizer`): the class of the local
            optimizer.
        process_group (``ProcessGroup``, optional): ``mindtorch.distributed``
            ``ProcessGroup`` (default: ``dist.group.WORLD`` initialized by
            :meth:`mindtorch.distributed.init_process_group`).
    ...
    Example::
        ...
        >>> from mindtorch.distributed.optim import ZeroRedundancyOptimizer
        >>> from mindtorch.nn.parallel import DistributedDataParallel as DDP
        >>> model = nn.Sequential(*[nn.Linear(2000, 2000).to(rank) for _ in range(20)])
        >>> ddp = DDP(model, device_ids=[rank])
        >>> opt = ZeroRedundancyOptimizer(
        >>>     ddp.parameters(),
        >>>     optimizer_class=mindtorch.optim.Adam,
        >>>     lr=0.01
        >>> )
    ...
    def __init__(
        ...
        **defaults: Any,
        ...
        self._param_to_rank_cache: Dict[mindtorch.Tensor, int] = {}
        self._param_to_index_cache: Dict[mindtorch.Tensor, int] = {}
        self._partition_parameters_cache: List[List[Dict]] = []
        self._index_to_param_cache: List[mindtorch.Tensor] = []
        self._device_to_params_per_rank_cache: Dict[
            mindtorch.device, List[List[mindtorch.Tensor]]
        ] = {}
        ...
        self._buckets: List[List[mindtorch.Tensor]] = []
    ...
    def consolidate_state_dict(self, to: int = 0) -> None:
        ...
        empty_messenger = mindtorch.tensor(
            [0], dtype=mindtorch.uint8, device=self._default_device
        )
    ...
    def _verify_params_per_rank(
        self,
        params_per_rank: List[List[mindtorch.Tensor]],
    ) -> None:
        r"""
    ...
    def _partition_param_group(
        self, param_group: Dict[str, Any], params_per_rank: List[List[mindtorch.Tensor]]
    ) -> None:
        r"""
        ...
        Arguments:
            ...
            params_per_rank (list[list[mindtorch.Tensor]]): a :class:`list` of
    ...
    def _partition_parameters(
        self,
        params_per_rank: Optional[List[List[mindtorch.Tensor]]] = None,
    ) -> List[List[Dict]]:
        r"""
        Partitions parameters across distributed data parallel ranks.

        Arguments:
            params_per_rank (list[list[mindtorch.Tensor]], optional): a
    ...
    @property
    def _param_to_rank(self) -> Dict[mindtorch.Tensor, int]:
    ...
    @property
    def _param_to_index(self) -> Dict[mindtorch.Tensor, int]:
        r"""
    ...
    @property
    def _index_to_param(self) -> List[mindtorch.Tensor]:
    ...
    def _device_to_params_per_rank(
        self,
    ) -> Dict[mindtorch.device, List[List[mindtorch.Tensor]]]:
        r"""
    ...
    def _assign_bucket_subset_to_rank(
        self,
        bucket_index: int,
        bucket_params: List[mindtorch.Tensor],
        bucket_offset: int,
        assigned_rank: int,
        ...
        Arguments:
            bucket_index (int): index of the :class:`DistributedDataParallel`
                gradient bucket.
            bucket_params (List[mindtorch.Tensor]): subset of the parameters
    ...
    def _local_step(
        self,
        gradients: Optional[List[Optional[mindtorch.Tensor]]] = None,
        ...
        **kwargs: Any,
        ...
        Arguments:
            gradients (list[Optional[mindtorch.Tensor]], optional): a :class:`list`
        ...
        for index, value in state_dict["state"].items():
            ...
            if self._param_to_rank[param] != self.rank:
                ...
                for state_name, state_value in self.optim.state[param].items():
                    if mindtorch.is_tensor(state_value) and state_value.dim() == 0:
        ...
        for dev_i, (device, params_per_rank) in enumerate(
            ...
            for params in params_per_rank:
                ...
                if bucket_size == 0:
                    ...
                    bucket = mindtorch.zeros(1, device=device)
                else:
                    # Construct the bucket (assuming all dense and same dtype)
                    bucket = mindtorch.empty(bucket_size, dtype=dtype, device=device)
                    offset = 0
    ...
    def _verify_and_init_params(
        self,
        params: Any,
    ) -> Union[List[mindtorch.Tensor], List[dict]]:
        r"""
        ...
        if isinstance(params, mindtorch.Tensor):
            raise TypeError(
                ...
                f"Tensors, but got {mindtorch.typename(params)}"
            )
        try:
            ...
            raise TypeError(
                ...
                f" or dicts, but got {mindtorch.typename(params)}"
            ) from e
        if len(all_params) == 0:
    ...
    def _verify_same_dense_param_type(self) -> None:
        ...
        """
        typename = mindtorch.typename(self._all_params[0])
        ...
        for param in self._all_params[1:]:
            other_typename = mindtorch.typename(param)
