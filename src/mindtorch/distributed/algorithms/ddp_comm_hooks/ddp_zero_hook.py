...
from mindtorch.distributed.optim.zero_redundancy_optimizer import _OverlapStatus
from mindtorch.nn.parallel.distributed import DistributedDataParallel
...
def _perform_local_step(
    ...
    gradients: List[Optional[mindtorch.Tensor]] = [
...
def hook_with_zero_step(
    hook: Callable[[Any, dist.GradBucket], mindtorch.futures.Future],
    ...
    Arguments:
        hook (Callable[[Any, dist.GradBucket], mindtorch.futures.Future]): the hook
            to modify.
...
def hook_with_zero_step_interleaved(
    ...
    Arguments:
        hook (Any * dist.GradBucket -> mindtorch.futures.Future): the hook to
            modify.
        ddp (DistributedDataParallel): the :class:`DistributedDataParallel`
            instance to use.
        zero (ZeroRedundancyOptimizer): the :class:`ZeroRedundancyOptimizer`
    ...
    def hook_with_zero_interleaved_fn(
        ...
        def zero_step(fut: mindtorch.futures.Future) -> mindtorch.Tensor:
            r"""
            Perform partial :class:`ZeroRedundancyOptimizer` :meth:`step` using gradients in the :class:`DistributedDataParallel`.

            Returns:
                A :class:`mindtorch.Tensor` representing the contents of the
                gradient bucket.
            """
            overlap_info = zero._overlap_info
            bucket_index = bucket.index()
