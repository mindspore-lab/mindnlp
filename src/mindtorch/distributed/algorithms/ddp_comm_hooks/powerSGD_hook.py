...
def _orthogonalize(matrices, epsilon=0):
    ...
    if rank <= 2 or dtype in [mindtorch.float16, mindtorch.bfloat16]:
...
def _orthogonalize_gram_schmidt(matrices, epsilon=0):
    ...
    If epsilon is 0, this is equivalent to `mindtorch.qr(matrices, out=(matrices, _))`,
    """
    ...
    for i in range(num_cols):
        ...
        if epsilon == 0:
            ...
            try:
                col /= mindtorch.norm(col, dim=1, keepdim=True)
            ...
            col /= mindtorch.norm(col, dim=1, keepdim=True) + epsilon
        # Project it on the rest and remove it.
        if i + 1 < num_cols:
            rest = matrices[:, :, i + 1 :]
            rest -= mindtorch.sum(col * rest, dim=1, keepdim=True) * col
...
class PowerSGDState:
    ...
    def __init__(
        ...
        self.error_dict: Dict[int, mindtorch.Tensor] = {}
        self.p_memory_dict: Dict[int, mindtorch.Tensor] = {}
        self.q_memory_dict: Dict[int, mindtorch.Tensor] = {}
...
def powerSGD_hook(
...
) -> mindtorch.futures.Future[mindtorch.Tensor]:
    r"""
    ...
    if state.use_error_feedback:
        if bucket_index in state.error_dict:
            ...
            state.error_dict[bucket_index] = mindtorch.zeros(
        ...
        input_tensor_cp = mindtorch.clone(input_tensor).detach()
    ...
    uncompressed_tensors_memory = (
        mindtorch.cat([tensor.view(-1) for tensor in uncompressed_tensors])
        if uncompressed_tensors
        else mindtorch.tensor([], device=device, dtype=dtype)
    )
    ...
    if not state.warm_start or bucket_index not in state.p_memory_dict:
        ...
        state.p_memory_dict[bucket_index] = mindtorch.empty(
            total_Ps_size, device=device, dtype=dtype
        )
        state.q_memory_dict[bucket_index] = mindtorch.empty(
    ...
    if not need_randomize_qs:
        ...
        with mindtorch.random.fork_rng(devices=[]):
            ...
            mindtorch.manual_seed(state.rng.randint(1_000_000_000))
            for q in qs:
                q.copy_(
                    mindtorch.randn(
                        *q.shape,
    ...
    def compute_qs(fut):
        ...
        for tensor, p, q in zip(tensors_to_compress, ps, qs):
            mindtorch.bmm(tensor.transpose(1, 2), p, out=q)
    ...
    def decompress(fut):
        ...
        for p, q, tensor in zip(ps, qs, tensors_to_compress):
            mindtorch.bmm(p, q.transpose(1, 2), out=tensor)
        ...
        if mindtorch.cuda.is_available():
            mindtorch.cuda.synchronize(device)
        ...
        def create_low_rank_tensor(fill_random_values, rng):
            ...
            if fill_random_values:
                with mindtorch.random.fork_rng(devices=[]):
                    ...
                    mindtorch.manual_seed(rng.randint(1_000_000_000))
                    return mindtorch.randn(
                        square_side_length,
                        state.matrix_approximation_rank,
                        device="cpu",
                        dtype=input_tensor.dtype,
