...
def _allreduce_fut(
    process_group: dist.ProcessGroup, tensor: mindtorch.Tensor
...
def _compress_hook(
    ...
    buffer = (
        cast(Tuple[mindtorch.Tensor, ...], bucket)[0]
    ...
    def decompress(fut):
        ...
        value = fut if isinstance(fut, mindtorch.Tensor) else fut.value()[0]
        decompressed_tensor.copy_(value)
        return decompressed_tensor

    if mindtorch._utils.is_compiling():
...
def fp16_compress_hook(
    ...
    """
    Compress by casting ``GradBucket`` to ``mindtorch.float16`` divided by process group size.

    This DDP communication hook implements a simple gradient compression
    approach that casts ``GradBucket`` tensor to half-precision floating-point format (``mindtorch.float16``)
    ...
    """
    return _compress_hook(mindtorch.float16, process_group, bucket)
...
def bf16_compress_hook(
    ...
    `Brain floating point format <https://en.wikipedia.org/wiki/Bfloat16_floating-point_format>`_ (``mindtorch.bfloat16``)
    and then divides it by the process group size.
    ...
    """
    return _compress_hook(mindtorch.bfloat16, process_group, bucket)


def fp16_compress_wrapper(
    hook: Callable[[Any, dist.GradBucket], mindtorch.futures.Future[mindtorch.Tensor]]
) -> Callable[[Any, dist.GradBucket], mindtorch.futures.Future[mindtorch.Tensor]]:
    """
    Cast input tensor to ``mindtorch.float16``, cast result of hook back to input dtype.

    This wrapper casts the input gradient tensor of a given DDP communication hook to half-precision
    floating point format (``mindtorch.float16``), and casts the resulting tensor of the given hook back to
    ...
    def fp16_compress_wrapper_hook(
        ...
        bucket.set_buffer(bucket.buffer().to(mindtorch.float16))
...
def bf16_compress_wrapper(
    ...
    def bf16_compress_wrapper_hook(
        ...
        bucket.set_buffer(bucket.buffer().to(mindtorch.bfloat16))
