...
class LowPrecisionState(DefaultState):
    ...
    parameter precision specified by ``parameter_type`` (default: mindtorch.float32).
    Builds on top of the :class:`DefaultState`.

    Args:
        parameter_type (mindtorch.dtype): The precision of model's parameters.
    ...
    def __init__(
        self,
        process_group,
        parameter_type=mindtorch.float32,
    ):
        super().__init__(process_group)
        self.parameter_type = parameter_type


def _decompress(state: LowPrecisionState, grad: mindtorch.Tensor):
    """
    ...
    try:
        ...
        raise AttributeError(
            f"Device {grad.device}  does not have a \
                corresponding backend registered as 'mindtorch.device_type'."
        ) from e
...
def allreduce_hook(state: DefaultState, grad: mindtorch.Tensor):
    r"""
    ...
    Args:
        ...
        grad (mindtorch.Tensor): A gradient for the local batch that needs to be communicated across ranks.
    """
...
def reduce_scatter_hook(state: DefaultState, grad: mindtorch.Tensor, output: mindtorch.Tensor):
    r"""
    ...
    Args:
        state (DefaultState): State information, configures pre- and post-division factors.
        grad (mindtorch.Tensor): An unsharded gradient for the local batch that needs to be
        communicated across ranks.
        output (mindtorch.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
...
def _low_precision_hook(
    prec: mindtorch.dtype,
    state: LowPrecisionState,
    grad: mindtorch.Tensor,
    output: Optional[mindtorch.Tensor],
):
    if grad.dtype != prec:
...
def fp16_compress_hook(
    state: LowPrecisionState, grad: mindtorch.Tensor, output: Optional[mindtorch.Tensor] = None
):
    r"""
    Implement FSDP communication hook for a simple gradient compression approach.
    Casts ``grad`` to half-precision floating-point format (``mindtorch.float16``).
    ...
    Args:
        ...
        grad (mindtorch.Tensor): A gradient for the local batch that needs to be communicated across ranks in a lower precision.
        output (mindtorch.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
    fp16_hook = functools.partial(_low_precision_hook, mindtorch.float16)
...
def bf16_compress_hook(
    ...
    Args:
        ...
        output (mindtorch.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
    bf16_hook = functools.partial(_low_precision_hook, mindtorch.bfloat16)
