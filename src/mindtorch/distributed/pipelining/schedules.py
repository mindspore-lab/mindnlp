...
from mindtorch.distributed._composable.fsdp.fully_shard import FSDPModule, UnshardHandle
...
if TYPE_CHECKING:
    from mindtorch.distributed import Work
...
class _PipelineSchedule(ABC):
    def __init__(
        self,
        n_microbatches: int,
        loss_fn: Optional[Callable[..., mindtorch.Tensor]] = None,
        ...
        self._internal_losses: List[mindtorch.Tensor] = []
...
def _batch_p2p(p2p_ops: List[dist.P2POp], desc: Optional[str] = None):
    """
    Simple wrapper over batch_isend_irecv from mindtorch.distributed, which just adds a descriptive logger on top.
    """
    if len(p2p_ops) == 0:
        return None
...
class PipelineScheduleSingle(_PipelineSchedule):
    ...
    def step(self, *args, target=None, losses: Optional[List] = None, **kwargs):
        ...
        if target is not None:
            targets_split = list(mindtorch.tensor_split(target, self._n_microbatches))
        else:
            targets_split = None
