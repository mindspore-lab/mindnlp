...
sum_reducer = _LossReducer(mindtorch.tensor(0.0), lambda a, b: a + b)
...
def _shard_dict_of_args(
    ...
    for arg_key, arg in args_dict.items():
        ...
        for v, chunk_v in zip(flat, chunk_spec_flat):
            if chunk_v is _Replicate or not isinstance(v, mindtorch.Tensor):
                ...
                # Throw an error
                assert isinstance(v, mindtorch.Tensor), f"{v} is not a tensor"
                ...
                chunk_tensors = mindtorch.tensor_split(
                ...
                if _debug_mask_minibatches:
                    ...
                    for chunk_tensor in chunk_tensors:
                        new_val = mindtorch.zeros_like(v)
    ...
    for arg_idx, arg in enumerate(spec_flattened):
        if isinstance(arg, TensorChunkSpec):
            ...
            if _debug_mask_minibatches:
                ...
                meta_chunks = mindtorch.tensor_split(
                    mindtorch.empty(*overall_shape, device="meta"),
            ...
            args_flattened.append(mindtorch.cat(values_to_cat, dim=arg.split_dim))
