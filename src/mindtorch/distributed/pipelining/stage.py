...
import mindtorch.fx as fx
import mindtorch.nn as nn
from mindtorch._subclasses.fake_tensor import FakeTensor
from mindtorch.distributed._composable.fsdp.fully_shard import FSDPModule, fully_shard
from mindtorch.fx.node import map_aggregate
from mindtorch.nn.parallel import DistributedDataParallel
...
def _normalize_model_output_as_tuple(output: Any) -> Tuple[Any]:
    ...
    mindtorch.export tracing, compiled models may also return a list instead of a Tuple, which we will normalize back to a
...
def _make_tensor_from_meta(
    example: Union[mindtorch.Tensor, FakeTensor],
...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        """
        Args:
            submodule (mindtorch.nn.Module): The module to be executed in this stage.
            stage_index (int): The index of this stage.
            num_stages (int): The total number of stages in this pipeline.
            device (mindtorch.device): The device to run this stage on.
        ...
        # Run time states
        self._outputs_meta: Optional[Tuple[mindtorch.Tensor, ...]] = None
        # map microbatch ID to list of forward tensor args
        self.fwd_cache: Dict[int, Tuple[Any, List[mindtorch.Tensor]]] = {}
        # map microbatch ID to list of backward grad tensor args
        self.bwd_cache: Dict[int, Tuple[Optional[mindtorch.Tensor], ...]] = {}
    ...
    def _configure_outputs_meta(self, outputs_meta: Tuple[mindtorch.Tensor, ...]):
        """
    ...
    def get_outputs_meta(self) -> Tuple[mindtorch.Tensor, ...]:
    ...
    def set_local_bwd_input(
        self, next_stage_bwd_outputs: Tuple[Optional[mindtorch.Tensor], ...], mb_index: int
    ) -> None:
        """
    ...
    def get_bwd_send_ops(self, bwd_chunk_id: int) -> List[dist.P2POp]:
        ...
        for grad, grad_recv_stage in zip(grads_input, self.grad_send_info):
            if isinstance(grad, mindtorch.Tensor) and grad_recv_stage is not None:
                logger.debug(
    ...
    def clear_runtime_states(self) -> None:
        ...
        # `mindtorch.autograd.backward()` will accumulate gradients into leaf
    ...
    ) -> Tuple[Tuple[Optional[mindtorch.Tensor], ...], Optional[List[Dict[str, Any]]]]:
        """
        ...
        if isinstance(self.submod, DistributedDataParallel):
            if last_backward:
                ...
                self.submod.reducer.prepare_for_backward(  # type: ignore[union-attr, operator]
                    list(
                        mindtorch.nn.parallel.distributed._find_tensors(  # type: ignore[attr-defined]
                            bwd_kwargs["stage_output"]
                        )
                    )
                )
    ...
    def backward_one_chunk(
        ...
        if self.is_last:
            ...
            # `mindtorch.autograd.backward` will accumulate the gradient into the
        ...
        grads_input: Tuple[Optional[mindtorch.Tensor], ...] = ()
        ...
        if self.dw_builder:
            ...
            if full_backward:
                ...
                if not self.is_first:
                    if isinstance(bwd_kwargs["stage_output"], mindtorch.Tensor):
    ...
    def _validate_fwd_outputs(self, outputs: Tuple[mindtorch.Tensor, ...]):
...
class _PipelineStage(_PipelineStageBase):
    def __init__(
        self,
        stage_module: mindtorch.nn.Module,
        stage_index: int,
        pipe_info: PipeInfo,
        device: mindtorch.device,
        group: Optional[dist.ProcessGroup] = None,
        ...
        Args:
            stage_module (mindtorch.nn.Module): the module to be wrapped by this stage
            ...
            device (mindtorch.device): the device to be used by this stage
    ...
    def _create_act_send_info(self):
        ...
        output_vals: Tuple[mindtorch.Tensor] = tuple(
...
class PipelineStage(_PipelineStageBase):
    ...
    Args:
        ...
        device (mindtorch.device): The device where this stage is located.
        input_args (Union[mindtorch.Tensor, Tuple[mindtorch.tensor]], optional): The input arguments for the submodule.
        output_args (Union[mindtorch.Tensor, Tuple[mindtorch.tensor]], optional): The output arguments for the submodule.
    ...
    def __init__(
        ...
        num_stages: int,
        device: mindtorch.device,
        input_args: Optional[Union[mindtorch.Tensor, Tuple[mindtorch.Tensor, ...]]] = None,
        output_args: Optional[Union[mindtorch.Tensor, Tuple[mindtorch.Tensor, ...]]] = None,
        group: Optional[dist.ProcessGroup] = None,
        dw_builder: Optional[Callable[[], Callable[..., None]]] = None,
    ):
        super().__init__(submodule, stage_index, num_stages, device, group, dw_builder)
        self.inputs: Optional[List[mindtorch.Tensor]] = None
        self.inputs_meta: Optional[Tuple[mindtorch.Tensor, ...]] = None
        ...
        if input_args is None:
            ...
            self.inputs_meta = (
                (input_args,) if isinstance(input_args, mindtorch.Tensor) else input_args
            )
            if output_args is None:
                ...
                try:
                    ...
                    output_args = tree_map_only(
                        mindtorch.Tensor, lambda x: x.to("meta"), output_args
                    )
                except Exception as e:
                    raise RuntimeError(
            ...
            self._configure_outputs_meta(
                (output_args,) if isinstance(output_args, mindtorch.Tensor) else output_args
            )

        # these are the buffers used in backwards send/recv, they are allocated later
        self.outputs_grad: List[mindtorch.Tensor] = []
    ...
    def _shape_inference(
        ...
        if (
            ...
            args = tree_map_only(mindtorch.Tensor, lambda x: x.to("meta"), args)
        else:
            assert (
                len(args) == 0
        ...
        args = tree_map_only(
            mindtorch.Tensor, lambda x: mindtorch.zeros_like(x, device=self.device), args
        )
        ...
        outputs_meta = tuple(
            tree_map_only(mindtorch.Tensor, lambda x: x.to("meta"), outputs)
        )
    ...
    def _init_p2p_neighbors(self):
        ...
        """
        ops = []
        recv_tensor = mindtorch.zeros(1, device="cuda")
        send_tensor = mindtorch.ones(1, device="cuda")
        # forward
        if not self.is_first:
