...
from mindtorch._C._distributed_rpc import _get_current_rpc_agent


__all__ = ["RPCExecMode", "serialize", "deserialize", "PythonUDF", "RemoteException"]

# Thread local tensor tables to store tensors while pickling mindtorch.Tensor
# objects
...
class _InternalRPCPickler:
    ...
    def __init__(self):
        ...
        self._dispatch_table[mindtorch.Tensor] = self._tensor_reducer
    ...
    def _script_module_receiver(cls, script_module_serialized):
        """
        Given a serialized representation of a ScriptModule created with mindtorch.jit.save,
    ...
    def _script_module_reducer(self, script_module):
        ...
        """
        f = io.BytesIO()
        mindtorch.jit.save(script_module, f)
    ...
    def serialize(self, obj):
        ...
        if isinstance(obj, mindtorch.jit.ScriptModule):
            # Ignore type error because dispatch_table is defined in third-party package
...
def _start_record_function(exec_type, func_name, current_worker_name, dest_worker_name):
    ...
    Returns:
        An instance of `mindtorch.autograd._RecordFunction`.
    """
    assert mindtorch.autograd._profiler_enabled(), "Autograd profiler should be enabled."
    profile_key = f"rpc_{exec_type.value}#{str(func_name)}({current_worker_name} -> {dest_worker_name})"
    rf = mindtorch.autograd._RecordFunction()  # type: ignore[attr-defined]
    mindtorch.autograd._run_before_callbacks(rf, profile_key)  # type: ignore[attr-defined]
    return rf
