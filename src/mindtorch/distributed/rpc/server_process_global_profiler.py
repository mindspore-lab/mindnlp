...
import mindtorch
from mindtorch.autograd.profiler_legacy import profile
...
class _server_process_global_profile(profile):
    """
    It has the same API as ``mindtorch.autograd.profiler.profile`` class,
    ...
    def __enter__(self):
        ...
        profiler_kind = (
            mindtorch.autograd.ProfilerState.CUDA
            if self.use_cuda
            else mindtorch.autograd.ProfilerState.CPU
        )
        profiler_config = mindtorch.autograd.ProfilerConfig(
            profiler_kind,
            self.record_shapes,
            self.profile_memory,
            False,
            False,
            False,
            mindtorch.profiler._ExperimentalConfig(),
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        Attributes:
            function_events (mindtorch.autograd.profiler.EventList).  It's a list that has helper
            methods, like 1) show record items in a pretty-print table.
            2) do averaging by grouping on keys. 3) and more.

            process_global_function_events (List[mindtorch.autograd.profiler.FunctionEvent]).
        ...
        for thread_local_events in process_global_events:
            ...
            thread_local_function_events = (
                mindtorch.autograd.profiler_legacy._parse_legacy_records(
                    thread_local_events
                )
            )
        ...
        self.function_events = mindtorch.autograd.profiler_util.EventList(
