...
def async_execution(fn):
    ...
    is guaranteed to be a :class:`~mindtorch.futures.Future` object and this
    function can run asynchronously on the RPC callee. More specifically, the
    callee extracts the :class:`~mindtorch.futures.Future` returned by the wrapped
    function and installs subsequent processing steps as a callback to that
    :class:`~mindtorch.futures.Future`. The installed callback will read the value
    from the :class:`~mindtorch.futures.Future` when completed and send the
    value back as the RPC response. That also means the returned
    :class:`~mindtorch.futures.Future` only exists on the callee side and is never
    ...
    :meth:`~mindtorch.distributed.rpc.rpc_async` or waiting for other signals.
    ...
    Example::
        The returned :class:`~mindtorch.futures.Future` object can come from
        :meth:`~mindtorch.distributed.rpc.rpc_async`,
        :meth:`~mindtorch.futures.Future.then`, or :class:`~mindtorch.futures.Future`
        constructor. The example below shows directly using the
        :class:`~mindtorch.futures.Future` returned by
        :meth:`~mindtorch.futures.Future.then`.

        >>> from mindtorch.distributed import rpc
        >>>
        ...
        >>>     return rpc.rpc_async(to, mindtorch.add, args=(x, y)).then(
        ...
        >>>     "worker1",
        >>>     async_add_chained,
        >>>     args=("worker2", mindtorch.ones(2), 1, 1)
        >>> )
        ...
        >>>     "worker1",
        >>>     async_add,
        >>>     args=("worker2", mindtorch.ones(2), 1)
        >>> )
        ...
        >>>         ret_fut = mindtorch.futures.Future()
        >>>         rpc.rpc_async(to, mindtorch.add, args=(x, y)).then(
        ...
        >>>     args=("worker2", mindtorch.ones(2), 1, 2)
        >>> )
        ...
        :meth:`mindtorch.distributed.rpc.RRef.rpc_sync`,
        :meth:`mindtorch.distributed.rpc.RRef.rpc_async`, and
        :meth:`mindtorch.distributed.rpc.RRef.remote`.

        >>> from mindtorch.distributed import rpc
        >>>
        >>> # reuse the AsyncExecutionClass class above
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.rpc_sync().static_async_add("worker2", mindtorch.ones(2), 1, 2)
        >>> print(ret)  # prints tensor([4., 4.])
        >>>
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.rpc_async().static_async_add("worker2", mindtorch.ones(2), 1, 2).wait()
        >>> print(ret)  # prints tensor([4., 4.])
        >>>
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.remote().static_async_add("worker2", mindtorch.ones(2), 1, 2).to_here()
        >>> print(ret)  # prints tensor([4., 4.])
    """
