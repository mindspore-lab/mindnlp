...
import mindtorch
from mindtorch._C._distributed_rpc import _TensorPipeRpcBackendOptionsBase

from . import constants as rpc_contants


DeviceType = Union[int, str, mindtorch.device]

__all__ = ["TensorPipeRpcBackendOptions"]


def _to_device(device: DeviceType) -> mindtorch.device:
...
) -> Dict[mindtorch.device, mindtorch.device]:
    full_device_map: Dict[mindtorch.device, mindtorch.device] = {}
    reverse_map: Dict[mindtorch.device, mindtorch.device] = {}
    for k, v in device_map.items():
        k, v = mindtorch.device(k), mindtorch.device(v)
        if v in reverse_map:
            raise ValueError(
...
def _to_device_list(devices: List[DeviceType]) -> List[mindtorch.device]:
...
class TensorPipeRpcBackendOptions(_TensorPipeRpcBackendOptionsBase):
    ...
    :class:`~mindtorch.distributed.rpc.TensorPipeAgent`, derived from
    :class:`~mindtorch.distributed.rpc.RpcBackendOptions`.

    Args:
        num_worker_threads (int, optional): The number of threads in the
            thread-pool used by
            :class:`~mindtorch.distributed.rpc.TensorPipeAgent` to execute
            requests (default: 16).
        rpc_timeout (float, optional): The default timeout, in seconds,
            ...
            RPCs in :meth:`~mindtorch.distributed.rpc.rpc_sync` and
            :meth:`~mindtorch.distributed.rpc.rpc_async` if necessary.
        init_method (str, optional): The URL to initialize the distributed
            ...
            same argument of :meth:`~mindtorch.distributed.init_process_group`
            (default: ``env://``).
        device_maps (Dict[str, Dict], optional): Device placement mappings from
            ...
            the dictionary (``Dict`` of ``int``, ``str``, or ``mindtorch.device``)
            that maps this worker's devices to the callee worker's devices.
            (default: ``None``)
        devices (List[int, str, or ``mindtorch.device``], optional): all local
    ...
    def set_device_map(self, to: str, device_map: Dict[DeviceType, DeviceType]):
        ...
        Args:
            to (str): Callee name.
            device_map (Dict of int, str, or mindtorch.device): Device placement
    ...
    def set_devices(self, devices: List[DeviceType]):
        ...
        Args:
            devices (List of int, str, or mindtorch.device): local devices used by
