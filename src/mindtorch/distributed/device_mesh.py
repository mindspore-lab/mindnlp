...
import mindtorch
from mindtorch.distributed import is_available
...
if not is_available():
    ...
    # since it would try to import ``mindtorch.distributed.device_mesh`` or
    # ``mindtorch.distributed.init_device_mesh`` but cannot find them.
    ...
    sys.modules["mindtorch.distributed.device_mesh"].DeviceMesh = _DeviceMeshStub  # type: ignore[attr-defined]
    sys.modules[
        "mindtorch.distributed.device_mesh"
    ...
    def _get_device_handle(device_type: str = "cuda"):
        ...
        For example, when the device_type is cuda, the module `mindtorch.cuda` is returned.
    ...
    class DeviceMesh:
        ...
        def __init__(
            self,
            device_type: str,
            mesh: Union[mindtorch.Tensor, "ArrayLike"],
            *,
            mesh_dim_names: Optional[Tuple[str, ...]] = None,
            _init_backend: bool = True,
        ) -> None:
            self.device_type = device_type
            if isinstance(mesh, mindtorch.Tensor) and mesh.device.type != "cpu":
                raise ValueError(f"`mesh` must be a CPU tensor, got {mesh}")
            self.mesh = (
                mesh.detach().to(dtype=mindtorch.int)
                if isinstance(mesh, mindtorch.Tensor)
                else mindtorch.tensor(mesh, device="cpu", dtype=mindtorch.int)
            )
        ...
        def from_group(
            ...
            device_type: str,
            mesh: Optional[Union[mindtorch.Tensor, "ArrayLike"]] = None,
            *,
            mesh_dim_names: Optional[Tuple[str, ...]] = None,
        ) -> "DeviceMesh":
            """
            ...
            if isinstance(group, ProcessGroup):
                ...
                if (
                    isinstance(mesh, mindtorch.Tensor) and mesh.tolist() != group_ranks
                ) or (
                    mesh is not None
                    and not isinstance(mesh, mindtorch.Tensor)
                    and mesh != group_ranks
                ):
                    raise ValueError(
                        f"Invalid mesh {str(mesh)} for ProcessGroup with ranks {group_ranks}"
                    )
                mesh = mindtorch.tensor(group_ranks, device="cpu", dtype=mindtorch.int)
                device_mesh = DeviceMesh(
                    device_type,
                    mesh,
            ...
            mesh = (
                mesh.detach().to(dtype=mindtorch.int, device="cpu")
                if isinstance(mesh, mindtorch.Tensor)
                else mindtorch.tensor(mesh, device="cpu", dtype=mindtorch.int)
            )
        ...
        *,
        ...
        if device_type and not device_type.isalpha():
            raise RuntimeError(
                ...
                "If you maintained a 'mindtorch.device' object, it's recommended to pass in 'device.type'.",
            )

        # Always initialize the mesh's tensor on CPU, regardless of what the
        # external device type has been set to be (e.g. meta)
        with mindtorch.device("cpu"):
            mesh = mindtorch.arange(math.prod(mesh_shape), dtype=mindtorch.int).view(mesh_shape)
        device_mesh = DeviceMesh(
