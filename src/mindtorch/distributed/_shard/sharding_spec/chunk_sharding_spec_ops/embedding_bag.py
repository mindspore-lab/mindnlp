...
from mindtorch.distributed._shard.sharding_spec import ChunkShardingSpec
from mindtorch.distributed._shard.sharding_spec.api import custom_sharding_spec_op
from mindtorch.distributed.nn.functional import all_gather, reduce_scatter
...
# @custom_sharding_spec_op(ChunkShardingSpec, mindtorch.nn.functional.embedding_bag)
def sharded_embedding_bag(types, args, kwargs, pg):
    """
    Handles ``__torch_function__`` dispatch for ``mindtorch.nn.functional.embedding_bag``.
...
def _validate_embedding_bag_param(args, kwargs):
    ...
    if not isinstance(input, mindtorch.Tensor):
        raise TypeError("input need to be mindtorch.Tensor")
    if offsets is not None and not isinstance(offsets, mindtorch.Tensor):
        raise TypeError("offsets need to be mindtorch.Tensor")
    if per_sample_weights is not None and not isinstance(
        per_sample_weights, mindtorch.Tensor
    ):
        raise TypeError("per_sample_weights need to be mindtorch.Tensor")
    ...
    if int(mindtorch.min(input).item()) < 0:
        raise ValueError(
            "Index out of range in Input %d %d",
            int(mindtorch.min(input).item()),
            weight_size[1],
        )
    if int(mindtorch.max(input).item()) >= weight_size[0]:
        raise ValueError(
            "Index out of range in Input %d %d",
            int(mindtorch.max(input).item()),
            weight_size[1],
        )
...
def _handle_col_wise_sharding(
    ...
    output = _handle_col_wise_sharding_base(
        mindtorch.nn.functional.embedding_bag,
        1,
        input,
        world_size,
        weight,
...
def _handle_row_wise_sharding(
    ...
    if input.dim() > 1 and per_sample_weights is None:
        ...
        gather_inp = mindtorch.cat(gathered_inputs, dim=cat_dim)
        if per_sample_weights is not None:
            per_sample_weights = mindtorch.cat(gathered_per_sample_weights, dim=cat_dim)
        offset_add = 0 if input.dim() > 1 else input.size(0)
        if offsets is not None:
            offsets_list = mindtorch.cat(
    ...
    if max_norm is not None:
        mindtorch.nn.functional.embedding_bag(
            mindtorch.unique(lookup_input)[:-1],
            local_shard,
            offsets=mindtorch.tensor([0], device=local_shard.device, dtype=mindtorch.long),
            mode=mode,
        ...
        max_norm = None
    result = mindtorch.nn.functional.embedding_bag(
        lookup_input,
        mindtorch.cat([local_shard, padding_row]),
    ...
    result = reduce_scatter(
        mindtorch.empty_like(local_shards[0]),
        list(local_shards),
        op=op,
        group=pg,
    )
    ...
    if mode == "mean":
        if input.dim() > 1:
            ...
            split_sizes = mindtorch.sum(
                mindtorch.ne(input, padding_idx), dim=-1, dtype=local_shard.dtype
            )
        else:
            split_sizes = mindtorch.cat(
                (
        ...
        return mindtorch.div(result, split_sizes.unsqueeze(1))
...
def _all_gather_embedding_bag_input(input, per_sample_weights, offsets, pg):
    ...
    gathered_inputs = all_gather(mindtorch.stack(input_to_gather), group=pg)
