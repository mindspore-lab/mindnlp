...
from mindtorch.distributed._shard.sharded_tensor._ops._common import _sharded_op_common
...
from mindtorch.distributed.nn.functional import (
    _all_gather_base,
    all_reduce,
    all_to_all_single,
)
...
def _register_sharded_op_on_local_tensor(
    ...
    ``mindtorch.nn.functional.softmax`` or ``mindtorch.Tensor.view``.
...
def _handle_col_wise_sharding_base(
    ...
    for i, inp in enumerate(gathered_inputs):
        if op_func == mindtorch.nn.functional.embedding_bag:
            result = op_func(
                inp,
                local_shard,
        ...
        elif op_func == mindtorch.nn.functional.embedding:
            result = op_func(
                inp,
                local_shard,
                padding_idx=padding_idx,
        ...
        results.append(mindtorch.transpose(result, 0, col_dim))
    ...
    return mindtorch.transpose(output, 0, col_dim)
...
def _result_distribute_with_col_rearrange(results, input, world_size, weight, pg):
    ...
    combined_results = mindtorch.cat(results)
    output = mindtorch.empty(
    ...
    return output.index_select(0, mindtorch.tensor(indices, device=output.device))
...
def _handle_max_norm_col_wise(
    ...
    unique_inp = mindtorch.unique(mindtorch.cat(gathered_inputs))
    local_shard_sum = mindtorch.sum(
        mindtorch.pow(mindtorch.abs(local_shard), norm_type), dim=1, dtype=local_shard.dtype
    )
    ...
    local_shard_norm = mindtorch.pow(local_shard_sum, 1.0 / norm_type)
    max_norm_tensor = mindtorch.full(
        (local_shard.size(0),),
        float("inf"),
    ...
    normalized_tensor = mindtorch.where(
    ...
    local_shard_norm_renormed = (
        mindtorch.div(mindtorch.mul(local_shard_t, normalized_tensor), local_shard_norm)
        .t()
        .contiguous()
    )
...
def _all_gather_base_input(input, pg):
    ...
    gather_inp = mindtorch.empty(gather_inp_size, device=input.device, dtype=input.dtype)
