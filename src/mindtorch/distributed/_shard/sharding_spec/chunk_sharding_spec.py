...
import mindtorch.distributed._shard.sharded_tensor.metadata as sharded_tensor_meta
import mindtorch.distributed.distributed_c10d as distributed_c10d
from mindtorch.distributed._shard._utils import narrow_tensor
...
from mindtorch.distributed._shard.sharded_tensor.utils import (
...
class ChunkShardingSpec(ShardingSpec):
    ...
    along a single dimension into equal chunks (similar to :meth:`mindtorch.chunk`).

    The semantics of how a tensor is partitioned is inline with
    :meth:`mindtorch.chunk`, where ``dim`` in mindtorch.chunk corresponds to the
    specified ``dim`` and ``chunks`` in mindtorch.chunk is the number of elements
    ...
    Args:
        ...
        placement(List[Union[_remote_device, str]]):
            ...
            be a list of
            :class:`mindtorch.distributed._remote_device`'s. This list
            could also contain a string which represents remote
            device as accepted by
            :class:`mindtorch.distributed._remote_device`
    """

    ShardingDim = Union[int, str]

    dim: ShardingDim
    placements: List[Union[mindtorch.distributed._remote_device, str]]

    def __post_init__(self):
        self._verify_dim(self.dim)
        for i, remote_device in enumerate(self.placements):
            if not isinstance(remote_device, mindtorch.distributed._remote_device):
                self.placements[i] = mindtorch.distributed._remote_device(remote_device)
    ...
    def shard(
        self, tensor: mindtorch.Tensor, src_rank: int = 0, process_group=None
    ) -> "ShardedTensor":
        """
        Args:
        ...
        tensors_to_scatter = cast(
            List[Optional[mindtorch.Tensor]],
        ...
        tensors_to_scatter_: Optional[List[mindtorch.Tensor]] = None
        if current_rank == src_rank:
            tensors_to_scatter_ = []
            for t in tensors_to_scatter:
                assert isinstance(t, mindtorch.Tensor)
