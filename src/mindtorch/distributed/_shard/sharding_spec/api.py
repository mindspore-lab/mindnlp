...
class DevicePlacementSpec(PlacementSpec):
    ...
    Args:
        device(:class:`mindtorch.distributed._remote_device`): The device to place the entity on.
    """

    device: mindtorch.distributed._remote_device

    def __post_init__(self):
        if not isinstance(self.device, mindtorch.distributed._remote_device):
            self.device = mindtorch.distributed._remote_device(self.device)
...
class ShardingSpec(ABC):
    ...
    def build_metadata(
        ...
        Args:
            tensor_sizes (:class:`mindtorch.Size`):
                The tensor shape to shard on, a `mindtorch.Size` object that represents the
                tensor shape to be sharded according to the ShardingSpec.
            tensor_properties(:class:`mindtorch.distributed._shard.sharded_tensor.TensorProperties):
...
def custom_sharding_spec_op(sharding_spec_class, func):
    ...
    Args:
        ...
        func(Callable): The op to override (ex: mindtorch.bmm)
    """
...
def _infer_sharding_spec_from_shards_metadata(shards_metadata):
    ...
    Returns:
        A :class:`mindtorch.distributed._shard.sharding_spec.ShardingSpec` object of sharding
