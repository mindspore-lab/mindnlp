...
import mindtorch
from mindtorch.distributed._shard.op_registry_utils import _decorator_func
...
if TYPE_CHECKING:
    from mindtorch.distributed._shard.sharding_spec import ShardingSpec
else:
...
def full(
    ...
    *,
    ...
    Args:
        sharding_spec (:class:`mindtorch.distributed._sharding_spec.ShardingSpec`): The specification
            describing how to shard the Tensor.
        size (int...):  a list, tuple, or `mindtorch.Size` of integers defining the shape of the
            output tensor.
    ...
    mindtorch.nn.init.constant_(sharded_tensor, fill_value)  # type: ignore[arg-type]
...
def rand(
    ...
    *size,
    ...
    mindtorch.nn.init.uniform_(sharded_tensor, 0, 1)  # type: ignore[arg-type]
    return sharded_tensor
...
def randn(
    ...
    *size,
    ...
    mindtorch.nn.init.normal_(sharded_tensor, 0, 1)  # type: ignore[arg-type]
    return sharded_tensor
...
def init_from_local_shards(
    ...
    Args:
        local_shards (List[:class `mindtorch.distributed._shard.sharded_tensor.Shard`]): A list
            of shards that represent the local shards on this rank.
        global_size (int...):  a list, tuple, or `mindtorch.Size` of integers defining the
    ...
    Examples:
        ...
        >>> )
        >>> local_shards = [Shard(mindtorch.randn(5, 5), local_shard_metadata)]
...
def state_dict_hook(module, destination, prefix, local_metadata):
    ...
    :meth:`mindtorch.nn.Module._register_state_dict_hook`.
    """
...
def custom_sharded_op_impl(func):
    ...
    Example::
        >>> # xdoctest: +SKIP
        >>> @custom_sharded_op_impl(mindtorch.nn.functional.linear)
        >>> def my_custom_sharded_linear(types, args, kwargs, process_group):
        >>>     ...
        >>> # xdoctest: +SKIP("Undefined variables")
        >>> input = mindtorch.rand(10, 32)
        >>> weight = sharded_tensor.rand(32, 16)
        >>> bias = mindtorch.rand(16)
        >>> # This will call 'my_custom_sharded_linear'
        >>> mindtorch.nn.functional.linear(input, weight, bias)

    The types, args and kwargs parameters are the same parameters that are
    passed to ``__torch_function__`` dispatch API
    (https://pymindtorch.org/docs/stable/notes/extending.html#extending-torch).
    ...
    Args:
        func(Callable): Torch function for which we want to provide a sharded
            implementation (ex: mindtorch.nn.functional.linear)
    """
