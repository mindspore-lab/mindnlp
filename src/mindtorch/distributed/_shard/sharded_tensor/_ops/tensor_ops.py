...
import mindtorch
from mindtorch.distributed._shard.common_op_utils import _register_default_op
...
_register_default_op(mindtorch.Tensor.shape.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
_register_default_op(mindtorch.Tensor.dtype.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
# _register_default_op(mindtorch.Tensor.layout.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
_register_default_op(mindtorch.Tensor.size, _sharded_op_impl)
_register_default_op(mindtorch.Tensor.dim, _sharded_op_impl)
_register_default_op(mindtorch.Tensor.ndim.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
_register_default_op(mindtorch.Tensor.is_contiguous, _sharded_op_impl)
_register_default_op(mindtorch.Tensor.contiguous, _sharded_op_impl)
_register_default_op(mindtorch.Tensor.is_floating_point, _sharded_op_impl)

# __reduce_ex__ to dispatch to get_state/set_state
_register_default_op(mindtorch.Tensor.__reduce_ex__, _sharded_op_impl)

# autograd related properties
_register_default_op(mindtorch.Tensor.requires_grad.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
# TODO: set grad with a ShardedTensor that consists of all local grads
_register_default_op(mindtorch.Tensor.grad.__get__, _sharded_op_impl)  # type: ignore[union-attr]
# _register_default_op(mindtorch.Tensor.grad_fn.__get__, _sharded_op_impl)  # type: ignore[union-attr]
# _register_default_op(mindtorch.Tensor.is_leaf.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
...
# @_sharded_op_impl(mindtorch.Tensor.device.__get__)
def tensor_device(types, args=(), kwargs=None, pg=None):
    self_st = args[0]
    # Validate types
    if not isinstance(self_st, ShardedTensor):
        raise TypeError("input needs to be a ShardedTensor")
    dev: mindtorch.device
    if self_st._local_shards:
        dev = self_st._local_shards[0].tensor.device
    elif pg and pg._get_backend_name() == "gloo":
        dev = mindtorch.device("cpu")
    else:
        dev = mindtorch.device(mindtorch.cuda.current_device())
    return dev


# @_sharded_op_impl(mindtorch.Tensor.is_meta.__get__)  # type: ignore[attr-defined]
...
def sharded_type_as_check(*args, **kwargs):
    ...
    Args: same as ``mindtorch.Tensor.type_as``.

    Return: None
    """
    if len(args) < 2:
        raise ValueError("Needs to give a tensor to cast type as!")
    if not isinstance(args[1], mindtorch.Tensor) and not isinstance(args[1], ShardedTensor):
...
def sharded_type_as(args, kwargs, pg):
    """
    Handles ``__torch_function__`` dispatch for the ``mindtorch.Tensor.type_as`` op.
...
_register_sharded_op_on_local_shards(
    mindtorch.Tensor.__deepcopy__,
    customized_func=sharded_deepcopy,
)


@_sharded_op_impl(mindtorch.Tensor.copy_)
...
def sharded_clone(args, kwargs, pg):
    ...
    if desire_memory_format and desire_memory_format != mindtorch.preserve_format:
        raise RuntimeError("Only support mindtorch.preserve_format for ShardedTensor!")
    cloned_local_shards = [
        Shard(
...
# _register_sharded_op_on_local_shards(
#     mindtorch.Tensor.detach,
#     customized_func=sharded_detach,
# )


# @_sharded_op_impl(mindtorch.Tensor.requires_grad_)
def tensor_requires_grad_set(types, args=(), kwargs=None, pg=None):
    ...
    with mindtorch._C.DisableTorchFunctionSubclass():
