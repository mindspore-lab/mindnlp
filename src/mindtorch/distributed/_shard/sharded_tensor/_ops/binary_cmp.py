...
from mindtorch.distributed._shard.sharded_tensor import _sharded_op_impl, ShardedTensor


def _communicate_result(result, pg):
    # Gather results from all ranks.
    if result:
        result_tensor = mindtorch.ones(1, device=mindtorch.device(mindtorch.cuda.current_device()))
    else:
        result_tensor = mindtorch.zeros(1, device=mindtorch.device(mindtorch.cuda.current_device()))

    dist.all_reduce(result_tensor, group=pg)

    expected_result = mindtorch.ones(
        1, device=mindtorch.device(mindtorch.cuda.current_device())
    ) * dist.get_world_size(pg)

    return mindtorch.equal(result_tensor, expected_result)


def binary_cmp(cmp_fun, types, args, kwargs=None, process_group=None):
    if len(args) != 2:
        raise ValueError(f"Expected two arguments for mindtorch.{cmp_fun.__name__}")

    st1 = args[0]
    st2 = args[1]
    if not (isinstance(st1, ShardedTensor) and isinstance(st2, ShardedTensor)):
        raise TypeError(
            f"Both arguments to mindtorch.{cmp_fun.__name__} need to be of type ShardedTensor"
        )
...
@_sharded_op_impl(mindtorch.equal)
def equal(types, args, kwargs, process_group):
    return binary_cmp(mindtorch.equal, types, args, kwargs, process_group)


@_sharded_op_impl(mindtorch.allclose)
def allclose(types, args, kwargs, process_group):
    return binary_cmp(mindtorch.allclose, types, args, kwargs, process_group)
