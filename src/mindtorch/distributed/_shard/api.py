...
def _shard_tensor(
    tensor: mindtorch.Tensor, sharding_spec: ShardingSpec, src_rank=0, process_group=None
) -> ShardedTensor:
    """
    Given a :class:`mindtorch.Tensor`, it shards that tensor according to the provided
    ``sharding_spec``. ``src_rank`` denotes the source rank which would be
    used as the ground truth of the data which would be scattered as shards
    across the rest of the ranks.

    Args:
        tensor (:class:`mindtorch.Tensor`): Tensor needs to be sharded.
    ...
    .. warning::
        Only :class:`mindtorch.distributed._shard.sharding_spec.ChunkShardingSpec` is
...
def shard_parameter(
    ...
    """
    Given a :class:`mindtorch.nn.Module`, a ``param_name`` for a parameter in that
    ...
    :class:`mindtorch.distributed._sharded_tensor.ShardedTensor`

    Args:
        module (:class:`mindtorch.nn.Module`): Module whose parameter needs to be sharded.
    ...
    if not isinstance(tensor, mindtorch.Tensor):
        raise ValueError(
...
def _reshard_output(
    module: mindtorch.nn.Module, resharding_spec: ShardingSpec
) -> mindtorch.nn.Module:
    """
    ...
    Args:
        module (:class:`mindtorch.nn.Module`): Module whose output needs to be resharded.
        resharding_spec (:class:`mindtorch.distributed._shard.sharding_spec.ShardingSpec`):
            The specification describing how the output of the module will be resharded.

    Returns:
        A :class:`mindtorch.nn.Module` object with reshard API hooked.
    """
...
def _collect_local_shard(module: mindtorch.nn.Module) -> mindtorch.nn.Module:
    """
    ...
    Args:
        module (:class:`mindtorch.nn.Module`): Module whose output is ShardedTensor and the
            local tensor value needs to be returned.

    Returns:
        A :class:`mindtorch.nn.Module` object with collection API hooked.
    """
...
def shard_module(module: nn.Module, plan: ShardingPlan, src_rank=0, process_group=None):
    ...
    Args:
        module (:class:`mindtorch.nn.Module`): The module to apply sharding to
        plan (:class:`mindtorch.distributed._shard.sharding_plan.ShardingPlan`):
