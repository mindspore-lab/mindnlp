...
def contract(state_cls: Type[_State] = _State):
    ...
    Example::
        ...
        >>> my_feature(model.l2)
        >>> model(mindtorch.randn(2, 10)).sum().backward()
    """

    # wraps will make functions decorated with contract() pickleable - needed for integration with mindtorch.package
    @wraps(state_cls)
    def inner(func):
        @wraps(func)
        def wrapper(
            ...
            all_orig_named_buffers: List[Dict[str, mindtorch.Tensor]] = []
            ...
            all_new_named_buffers: List[Dict[str, mindtorch.Tensor]] = []
