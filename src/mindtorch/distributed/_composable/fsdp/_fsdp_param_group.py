...
from mindtorch.distributed.device_mesh import _get_device_handle
from mindtorch.distributed.fsdp._common_utils import _named_parameters_with_duplicates
from mindtorch.distributed.tensor import Shard
from mindtorch.profiler import record_function
...
logger = logging.getLogger("mindtorch.distributed._composable.fsdp")
...
class FSDPCommContext:
    ...
    def lazy_init(self, device: mindtorch.device):
    ...
    ) -> Tuple[mindtorch.Stream, mindtorch.Stream]:
...
class AllGatherState(NamedTuple):
    ...
    event: mindtorch.Event  # all-gather copy-out


class ReduceScatterState(NamedTuple):
    reduce_scatter_input: mindtorch.Tensor
    event: mindtorch.Event  # reduce-scatter event


class AllReduceState(NamedTuple):
    all_reduce_input: mindtorch.Tensor
    event: mindtorch.Event  # all-reduce event


class FSDPParamGroup:
    """This class represents a parameter group to communicate together."""

    _orig_dtype: mindtorch.dtype
    _reduce_dtype: Optional[mindtorch.dtype]
    ...
    def __init__(
        ...
        self._post_reduce_event: Optional[mindtorch.Event] = None
        # Holds the reshard-after-forward CUDA event when resharding to a
        # different world size, which should be waited on in the next unshard
        self._reshard_after_forward_event: Optional[mindtorch.Event] = None

        # Only for HSDP, if accumulating gradients without all-reduce, save the
        # partial reduce output (only reduce-scattered but not all-reduced)
        self._partial_reduce_output: Optional[mindtorch.Tensor] = None
    ...
    def _wait_all_gather_streams_on_event(self, event: mindtorch.Event):
    ...
    def post_backward(self, *unused: Any):
        ...
        with record_function(self._with_fqn("FSDP::post_backward_reshard")):
            ...
            unsharded_grads: List[mindtorch.Tensor] = []
            for fsdp_param in self.fsdp_params:
    ...
    def finalize_backward(self):
        ...
        if self._all_gather_result is not None:
            ...
            if (event := self._all_gather_result.all_gather_event) is not None:
                mindtorch.cuda.current_stream().wait_event(event)
    ...
    def _register_post_backward_hook(
        ...
        if (not mindtorch._dynamo.config.skip_fsdp_hooks) or compiled_autograd_enabled():
            return args, kwargs
        ...
        inp_tensors: List[mindtorch.Tensor] = []
        for i, obj in enumerate(args_kwargs_list):
            if mindtorch.is_tensor(obj) and obj.requires_grad:
...
class RegisterPostBackwardFunction(mindtorch.autograd.Function):
    @staticmethod
...
compile the forward part if you want to use Traceable FSDP2."""
            mindtorch._dynamo.comptime.comptime.print(msg)
            raise RuntimeError(msg)

    @staticmethod
    def forward(ctx, param_group: FSDPParamGroup, *inputs: mindtorch.Tensor):
    ...
    @staticmethod
    def backward(ctx, *grads: mindtorch.Tensor):
        RegisterPostBackwardFunction._assert_not_tracing_fsdp()
