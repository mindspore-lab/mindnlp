...
from mindtorch.distributed.tensor import DeviceMesh, DTensor, init_device_mesh
...
def _init_default_fully_shard_mesh() -> DeviceMesh:
    ...
    device = mindtorch._C._get_accelerator()
    mesh = init_device_mesh(device.type, mesh_shape=(default_pg.size(),))
    return mesh


def _get_device_from_mesh(mesh: DeviceMesh) -> mindtorch.device:
    if mesh.device_type == "cpu":
        return mindtorch.device("cpu")
    device_handle = _get_device_handle(mesh.device_type)
    return mindtorch.device(mesh.device_type, device_handle.current_device())
...
def _get_managed_states(
    modules: List[nn.Module],
) -> Tuple[List[nn.Parameter], List[mindtorch.Tensor]]:
...
def _move_states_to_device(
    ...
    for tensor in itertools.chain(params, buffers):
        ...
        if is_traceable_wrapper_subclass(tensor_):
            with mindtorch.no_grad():  # avoid autograd increasing C++ refcount by 1
                tensor_on_device = nn.Parameter(tensor.to(device))
            mindtorch.utils.swap_tensors(tensor, tensor_on_device)
        else:
