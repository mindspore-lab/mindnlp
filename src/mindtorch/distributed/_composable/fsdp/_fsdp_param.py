...
from mindtorch._prims_common import make_contiguous_strides_for
...
from mindtorch.distributed.tensor.device_mesh import _mesh_resources
from mindtorch.distributed.tensor.placement_types import _StridedShard, Placement
...
- All-gather inputs: the ``mindtorch.Tensor`` or ``Tensor`` s passed to all-gather,
  derived from the sharded parameter
- All-gather output: the ``mindtorch.Tensor`` or ``Tensor`` s resulting from
...
For the default ``mindtorch.Tensor`` case, there is only one all-gather input, and
...
all-gather output and unsharded parameter. For non-``mindtorch.Tensor`` extensions,
...
it in-place thereafter. For the default ``mindtorch.Tensor` original parameter
case, the all-gather output and unsharded parameter share the same
data, so we use storage resizing on the all-gather output.
"""

lib = mindtorch.library.Library("fsdp", "FRAGMENT")  # noqa: TOR901

lib.define("copy_(Tensor(a!) tensor, Tensor data) -> ()")


@mindtorch.library.impl(lib, "copy_", "Meta")
@mindtorch.library.impl(lib, "copy_", "CUDA")
@mindtorch.library.impl(lib, "copy_", "CPU")
...
@mindtorch.library.impl(lib, "copy_", "Functionalize")
def copy__functionalize(tensor, data):
    mindtorch._sync(tensor)
    mindtorch._sync(data)
    tensor_inner = mindtorch._from_functional_tensor(tensor)
    data_inner = mindtorch._from_functional_tensor(data)
    with mindtorch._C._ExcludeDispatchKeyGuard(
        mindtorch._C.DispatchKeySet(mindtorch._C.DispatchKey.Functionalize)
    ):
        mindtorch.ops.fsdp.copy_.default(tensor_inner, data_inner)


mindtorch.fx.node.has_side_effect(mindtorch.ops.fsdp.copy_.default)
...
class ExtensionsData:
    ...
    all_gather_input_sizes: Sequence[mindtorch.Size] = ()  # ND
...
class FSDPParam:
    ...
    orig_dtype: mindtorch.dtype
    param_dtype: Optional[mindtorch.dtype]
    reduce_dtype: Optional[mindtorch.dtype]
    _orig_size: mindtorch.Size  # ND
    sharded_size: mindtorch.Size  # ND
    contiguous_sharded_stride: Tuple[int, ...]
    padded_sharded_param_size: mindtorch.Size  # ND
    sharded_post_forward_size: mindtorch.Size  # ND
    contiguous_sharded_post_forward_stride: Tuple[int, ...]
    _sharded_param_data: mindtorch.Tensor  # 1D
    sharded_param: nn.Parameter  # ND
    _sharded_post_forward_param_data: Optional[mindtorch.Tensor]  # 1D
    _sharded_post_forward_param: Optional[nn.Parameter]  # ND
    _unsharded_param: nn.Parameter  # ND
    unsharded_accumulated_grad: Optional[mindtorch.Tensor]  # ND
    _sharding_spec: DTensorSpec
    # DTensor attributes (only defined for DTensor `param`):
    _tp_spec: DTensorSpec
    all_gather_outputs: List[mindtorch.Tensor]  # 1D
    # All-gather extension attributes
    _extensions_data: ExtensionsData
    _unsharded_inner_tensors: List[mindtorch.Tensor]
    ...
    def __init__(
        ...
        self.grad_offload_event: Optional[mindtorch.Event] = None
        self._init_sharded_param(param, device, shard_placement_fn)
        if self.post_forward_mesh_info:
            self._init_sharded_post_forward_param_metadata(param)
        self._init_extensions()
        self.all_gather_outputs: List[mindtorch.Tensor] = []
    ...
    def _init_sharded_post_forward_param_metadata(self, param: mindtorch.Tensor) -> None:
    ...
    def _init_extensions(self) -> None:
        ...
        self._unsharded_inner_tensors: List[mindtorch.Tensor] = []

    def init_all_gather_outputs(
        self,
        all_gather_input_numels: List[int],
        all_gather_input_dtypes: List[mindtorch.dtype],
        world_size: int,
        device: mindtorch.device,
        ...
        self.all_gather_outputs = [
            mindtorch.empty(mindtorch.Size([numel * world_size]), dtype=dtype, device=device)
            for numel, dtype in zip(all_gather_input_numels, all_gather_input_dtypes)
        ]

    def init_unsharded_param(self):
        """
        [Note: Invariants for mindtorch.compile Traceable FSDP2]
        ...
        unsharded_param = mindtorch.as_strided(
            unsharded_tensor,
            self._orig_size,
        ...
        if hasattr(self, "_unsharded_param"):
            ...
            with mindtorch.no_grad(), mindtorch.autograd._unsafe_preserve_version_counter(
                self._unsharded_param
            ):
                ...
                mindtorch.ops.fsdp.copy_(self._unsharded_param, unsharded_param)
        else:
            self._unsharded_param = nn.Parameter(
                unsharded_param, requires_grad=self.sharded_param.requires_grad
            )

    def _unflatten_all_gather_outputs(self) -> Tuple[mindtorch.Tensor, ...]:
        return tuple(
            t.view(-1, *s[1:])
            for t, s in zip(
    ...
    def to_sharded_post_forward(self) -> None:
        ...
        sharded_post_forward_tensor = mindtorch.as_strided(
            self._sharded_post_forward_param_data,
    ...
    def to_sharded_dtensor(self, tensor: mindtorch.Tensor) -> DTensor:
        """
    ...
    def to_sharded_post_forward_dtensor(self, tensor: mindtorch.Tensor) -> DTensor:
    ...
    @property
    def all_gather_inputs(self) -> List[mindtorch.Tensor]:  # 1D
        ...
        if self.sharded_state == ShardedState.SHARDED:
            ...
            all_gather_input = _to_dtype_if_needed(
                cast(mindtorch.Tensor, self._sharded_post_forward_param_data),
                self.param_dtype,
            )
            return [all_gather_input]
        return [mindtorch.empty(0)]  # mypy
    ...
    @property
    def unsharded_grad_data(self) -> mindtorch.Tensor:
        grad = self.unsharded_param.grad
        assert grad is not None, "Expects unsharded_param.grad to not be None"
        return self._get_grad_inner_tensor(grad)

    @property
    def unsharded_accumulated_grad_data(self) -> mindtorch.Tensor:
        grad = self.unsharded_accumulated_grad
        assert grad is not None, "Expects unsharded_accumulated_grad to not be None"
        return self._get_grad_inner_tensor(grad)

    def _get_grad_inner_tensor(self, grad: mindtorch.Tensor) -> mindtorch.Tensor:
        if self.is_dtensor:
    ...
    @property
    def _sharded_local_tensor(self) -> mindtorch.Tensor:
    ...
    def reset_sharded_param(self):
        ...
        if new_param is not self.sharded_param:
            if mindtorch.__future__.get_swap_module_params_on_conversion():
                raise AssertionError(
...
def alloc_storage(tensor: mindtorch.Tensor) -> None:
    size = tensor.numel() * tensor.itemsize
    if (storage := tensor.untyped_storage()).size() != size:
        storage.resize_(size)


def free_storage(tensor: mindtorch.Tensor) -> None:
...
def set_requires_grad_if_needed(
    src_tensor: mindtorch.Tensor, dst_tensor: mindtorch.Tensor
) -> None:
