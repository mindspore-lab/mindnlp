...
from mindtorch._logging import warning_once
from mindtorch.autograd import Variable
from mindtorch.autograd.graph import _MultiHandle
from mindtorch.distributed._composable_state import (
    _get_module_state,
    _insert_module_state,
    _State,
)
from mindtorch.distributed.device_mesh import _get_device_handle
from mindtorch.distributed.utils import _to_kwargs
from mindtorch.utils._pytree import tree_flatten, tree_map
...
class FSDPStateContext:
    ...
    def __init__(self) -> None:
        ...
        self.post_optim_event: Optional[mindtorch.Event] = None


def disable_if_config_true(func):
    @functools.wraps(func)
    def fsdp_hook_wrapper(*args, **kwargs):
        if mindtorch._dynamo.config.skip_fsdp_hooks:
            return mindtorch._dynamo.disable(func, recursive=True)(*args, **kwargs)
        else:
...
class FSDPState(_State):
    ...
    def _root_pre_forward(
        ...
        with mindtorch.profiler.record_function("FSDP::root_pre_forward"):
            ...
            if self._device.type in ["cuda", "hpu"]:
                with mindtorch.profiler.record_function("FSDP::inputs_to_device"):
    ...
    def _pre_forward(
        ...
        if self._mp_policy.cast_forward_inputs and self._mp_policy.param_dtype:
            with mindtorch.profiler.record_function("FSDP::cast_forward_inputs"):
    ...
    def _post_forward(self, module: nn.Module, input: Any, output: Any) -> Any:
        ...
        if self._mp_policy.output_dtype is not None:
            with mindtorch.profiler.record_function("FSDP::cast_forward_outputs"):
                output = tree_map(
    ...
    def _pre_backward(self, grad: mindtorch.Tensor) -> mindtorch.Tensor:
    ...
    def _root_post_backward_final_callback(self) -> None:
        ...
        with mindtorch.profiler.record_function("FSDP::root_post_backward_callback"):
            for state in self._state_ctx.all_states:
    ...
    def _register_pre_backward_hook(self, output: Any) -> Any:
        ...
        for t in flat_outputs:
            if mindtorch.is_tensor(t) and t.requires_grad:
