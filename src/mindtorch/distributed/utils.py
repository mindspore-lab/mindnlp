...
from mindtorch.nn.utils.rnn import PackedSequence
...
def _cast_forward_inputs(
    dtype: Optional[mindtorch.dtype],
    *args: Any,
    **kwargs: Any,
) -> Tuple[Any, Any]:
    """
    ...
    def cast_fn(x: mindtorch.Tensor) -> mindtorch.Tensor:
...
T = TypeVar("T", mindtorch.Tensor, PackedSequence)


@overload
def _recursive_to(
    inputs: S, target_device: mindtorch.device, use_side_stream_for_tensor_copies: bool
) -> List[S]:
    ...
...
@overload
def _recursive_to(
    inputs: T, target_device: mindtorch.device, use_side_stream_for_tensor_copies: bool
) -> Tuple[T]:
    ...
...
def _recursive_to(inputs, target_device, use_side_stream_for_tensor_copies):
    ...
    def to_map(obj):
        if isinstance(obj, (mindtorch.Tensor, PackedSequence)):
            ...
            if not use_side_stream_for_tensor_copies:
                ...
                from mindtorch.nn.parallel._functions import _get_stream
                ...
                with device_mod.device(target_device.index):
                    ...
                    if isinstance(obj, PackedSequence):
                        ...
                        assert isinstance(output, mindtorch.Tensor)
                        output.record_stream(current_stream)  # type: ignore[arg-type]
                return (output,)

        from mindtorch.nn.parallel.scatter_gather import _is_namedtuple
...
def _alloc_storage(tensor: mindtorch.Tensor, size: mindtorch.Size) -> None:
    """
...
def _free_storage(tensor: mindtorch.Tensor):
    """
...
@overload
def _apply_to_tensors(fn: Callable[[mindtorch.Tensor], Q], container: mindtorch.Tensor) -> Q:
    ...


@overload
def _apply_to_tensors(fn: Callable[[mindtorch.Tensor], Any], container: R) -> R:
    ...
...
def _apply_to_tensors(fn, container):
    ...
    def apply(x):
        ...
        if isinstance(x, mindtorch.Tensor):
            return fn(x)
...
def _to_kwargs(
    ...
    target_device: mindtorch.device,
...
def _replace_by_prefix(
    ...
    Usage::
        ...
        state_dict = {"layer.xyz": mindtorch.tensor(1)}
        replace_by_prefix_(state_dict, "layer.", "module.layer.")
        assert state_dict == {"module.layer.xyz": mindtorch.tensor(1)}
    """
...
def _data_ptr_allocated(tensor: mindtorch.Tensor) -> bool:
