...
class _Orthogonal(Module):
    ...
    @no_grad()
    def right_inverse(self, Q: mindtorch.Tensor) -> mindtorch.Tensor:
        ...
        if not hasattr(self, "base"):
            ...
            # Y = mindtorch.cat([X.tril(), X.new_zeros(n, n - k).expand(*X.shape[:-2], -1, -1)], dim=-1)
            # A = Y - Y.mH
            # cayley(A)[:, :k]
            ...
            # householder(mindtorch.zeros(m,n)) == mindtorch.eye(m,n)
...
def orthogonal(
    ...
    *,
    ...
    - ``"matrix_exp"``/``"cayley"``:
      the :func:`~mindtorch.matrix_exp` :math:`Q = \exp(A)` and the `Cayley map`_
    ...
    - ``"householder"``: computes a product of Householder reflectors
      (:func:`~mindtorch.linalg.householder_product`).
    ...
    and it is orthogonalized via the QR decomposition otherwise (see :func:`mindtorch.linalg.qr`).
    ...
    .. note::
        ...
        in :func:`~mindtorch.nn.utils.parametrize.register_parametrization`.
    ...
    Example::
        ...
        >>> mindtorch.dist(Q.T @ Q, mindtorch.eye(20))
        tensor(4.9332e-07)
    """
...
class _SpectralNorm(Module):
    ...
    def _reshape_weight_to_matrix(self, weight: mindtorch.Tensor) -> mindtorch.Tensor:
        # Precondition
    ...
    @no_grad()
    def _power_method(self, weight_mat: mindtorch.Tensor, n_power_iterations: int) -> None:
    ...
    def forward(self, weight: mindtorch.Tensor) -> mindtorch.Tensor:
        if weight.ndim == 1:
    ...
    def right_inverse(self, value: mindtorch.Tensor) -> mindtorch.Tensor:
...
def spectral_norm(
    ...
    .. note::
        This function is implemented using the parametrization functionality
        in :func:`~mindtorch.nn.utils.parametrize.register_parametrization`. It is a
        reimplementation of :func:`mindtorch.nn.utils.spectral_norm`.
    ...
    Example::
        ...
        >>> mindtorch.linalg.matrix_norm(snm.weight, 2)
