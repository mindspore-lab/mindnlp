...
#
# A local torch import is retained inside get_transformers_info() because
# that function introspects HuggingFace transformers models (torch-based).
...
def get_model_tensor_names(model: ModelReference, options: MergeOptions) -> List[str]:
    loader = model.lazy_loader(
        cache_dir=options.transformers_cache, lazy_loader=options.lazy_loader
    )
    return list(loader.index.tensor_paths.keys())


def get_transformers_info(model: ModelReference, options: MergeOptions) -> tuple:
    import torch  # needed for HuggingFace transformers model introspection
