...
def rectify_embed_sizes(weight_info: WeightInfo, tensors: List[mindspore.Tensor]):
...
def take_common_submatrix(tensors: List[mindspore.Tensor]) -> bool:
    min_size = [None, None]
    for t in tensors:
        for idx in range(2):
    ...
    if not all(t.shape == tuple(min_size) for t in tensors):
