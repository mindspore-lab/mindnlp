...
from ..architecture.auto import infer_architecture_info
from ..common import ModelReference, set_config_value
from ..io.tasks import (
    LoaderCache,
)
from ..io.tensor_writer import TensorWriter
from ..options import MergeOptions, PrettyPrintHelp, add_merge_options
from ..tokenizer.normalization import (
    NormalizedToken,
    normalized_vocabulary,
    token_prefixes,
)
from ..tokensurgeon import (
    SubwordMethod,
    WeightingScheme,
    batch_mp_rope,
    batch_omp,
...
from ..tokensurgeon.common_interpolation import DistanceMetric
...
def get_stuff(
    ...
    get_tied: bool = False,
    device: str = "cpu",
) -> Tuple[Dict[NormalizedToken, int], Optional[mindspore.Tensor], Optional[mindspore.Tensor]]:
    if arch_info is None:
...
def john_hewitt_init(orig_embed: mindspore.Tensor, num_new_tokens: int) -> mindspore.Tensor:
    orig_embed_f32 = orig_embed.to(mindspore.float32)
    mean = orig_embed_f32.mean(axis=0)
    centered = orig_embed_f32 - mean
    covariance = ops.matmul(centered.T, centered) / orig_embed_f32.shape[0]

    try:
        ops.cholesky(covariance)
        is_pd = True
    except Exception as exc:
        LOG.debug(
            "Cholesky check failed in john_hewitt_init (%s: %s)",
            type(exc).__name__,
            exc,
        )
        is_pd = False
    ...
    cov_np = covariance.asnumpy()
    samples_np = np.random.multivariate_normal(mean_np, cov_np, size=num_new_tokens)
    new_embeds = mindspore.Tensor(samples_np, dtype=orig_embed.dtype)
    return new_embeds


def compute_new_embeddings(  # pylint: disable=too-many-positional-arguments
    orig_embed: mindspore.Tensor,
    donor_embed: mindspore.Tensor,
    orig_vocab: Dict[NormalizedToken, int],
    donor_vocab: Dict[NormalizedToken, int],
    target_tokens: List[NormalizedToken],
    is_lm_head: bool,
    token_basis: Optional[Tuple[mindspore.Tensor, mindspore.Tensor]],
    ...
    if options.method == ApproximationMethod.MEAN:
        mean = orig_embed.mean(axis=0)
        return mean.unsqueeze(0).broadcast_to((len(target_tokens), -1))
        ...
        donor_shared_embeds = donor_embed[
            mindspore.Tensor([donor_vocab[t] for t in shared_vocab])
        ]

        orig_shared_embeds = orig_embed[
            mindspore.Tensor([orig_vocab[t] for t in shared_vocab])
        ]
        res = None
        in_donor = None
        targets = donor_embed[mindspore.Tensor([donor_vocab[t] for t in target_tokens])]
        ...
        if res is None:
            res = (
                ops.bmm(
                    coeffs.unsqueeze(1), orig_shared_embeds[indices].to(mindspore.float32)
                )
                .squeeze(1)
                .to(orig_embed.dtype)
            )
        ...
        donor_basis = donor_basis.to(mindspore.float32)
        orig_basis = orig_basis.to(mindspore.float32)
        target_donor_embeds = donor_embed[
            mindspore.Tensor([donor_vocab[t] for t in target_tokens])
        ].to(mindspore.float32) - donor_embed.mean(axis=0)

        import numpy as np
        coeffs_np = np.linalg.lstsq(
            donor_basis.T.asnumpy(),
            target_donor_embeds.T.asnumpy(),
            rcond=None,
        )[0].T
        coeffs = mindspore.Tensor(coeffs_np, dtype=mindspore.float32)

        if LOG.isEnabledFor(logging.DEBUG):
            donor_rt = ops.matmul(coeffs, donor_basis)
            err = (donor_rt - target_donor_embeds).norm(axis=1)
            err_rel = err / ops.clamp(target_donor_embeds.norm(axis=1), min=1e-6)
            sim = ops.cosine_similarity(
                donor_rt, target_donor_embeds, dim=1
            )
            LOG.debug(f"Reconstruction error: {err.mean().asnumpy().item():.4f}")
            LOG.debug(f"Relative reconstruction error: {err_rel.mean().asnumpy().item():.4f}")
            LOG.debug(f"Cosine similarity: {sim.mean().asnumpy().item():.4f}")

        return ops.matmul(coeffs, orig_basis) + orig_embed.mean(axis=0)
    else:
        raise ValueError(f"Unknown approximation method: {options.method}")


def build_embedding_matrix(  # pylint: disable=too-many-positional-arguments
    ...
    res = ops.zeros(
        (out_vocab_size, orig_embed.shape[1]),
        dtype=orig_embed.dtype,
    )
    new_tokens = []
    ...
    if new_tokens:
        ...
        for base_idx in tqdm.tqdm(
            ...
            if options.new_vocab_noise:
                new_embeds += ops.randn_like(new_embeds) * options.new_vocab_noise
...
@click.command("wizard-tokensurgeon", cls=PrettyPrintHelp)
