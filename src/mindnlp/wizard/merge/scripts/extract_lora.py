...
from ..architecture import WeightInfo, arch_info_for_config
from ..card import generate_card_lora
from ..common import ModelReference, get_auto_cls
from ..graph import Executor, Task
from ..io.tasks import FinalizeModel, LoadTensor, SaveTensor, TensorWriterTask
...
def _get_npu_device():
    """Get available Ascend/NPU device string."""
    try:
        import mindspore  # pylint: disable=reimported,import-error
        mindspore.set_context(device_target="Ascend")
        return "Ascend"
    except Exception as exc:
        LOG.debug(
            "Ascend context is unavailable for extract_lora (%s: %s); using CPU fallback",
            type(exc).__name__,
            exc,
        )
        return "CPU"
...
def _build_executor(tasks: List[Task], merge_options: MergeOptions, fallback_device: str):
    if merge_options.multi_npu:
        return MultiDeviceExecutor(
            tasks,
            storage_device="CPU" if not merge_options.low_cpu_memory else None,
        )
    return Executor(
        tasks,
        math_device=merge_options.device or fallback_device,
        storage_device=(
...
@click.command("wizard-extract-lora", cls=PrettyPrintHelp)
@click.option(
    "--model",
    required=True,
...
def main(  # pylint: disable=too-many-positional-arguments
    ...
    executor = _build_executor(tasks, merge_options, device)
...
class TaskVectorDecompositionTask(Task[Tuple[mindspore.Tensor, mindspore.Tensor]]):
    ...
    def execute(self, task_vector: mindspore.Tensor) -> Tuple[mindspore.Tensor, mindspore.Tensor]:
        if self.transpose:
            task_vector = task_vector.T
        out_dtype = task_vector.dtype
        u, s, vh = ops.svd(
            task_vector.astype(mindspore.float32), full_matrices=False
        )
        rank = min(self.max_rank, s.shape[0])
        if self.sv_epsilon > 0:
            rank = min(int((s > self.sv_epsilon).sum()), rank)
        if self.distribute_scale:
            sqrt_s = ops.diag(ops.sqrt(s[:rank]))
            scale_a = sqrt_s
            scale_b = sqrt_s
        else:
            scale_a = ops.diag(s[:rank])
            scale_b = ops.eye(rank, rank, dtype=mindspore.float32)
        sqrt_s = ops.diag(ops.sqrt(s[:rank]))
        weight_a = ops.matmul(scale_a, vh[:rank])
        weight_b = ops.matmul(u[:, :rank], scale_b)

        return weight_a.astype(out_dtype), weight_b.astype(out_dtype)
...
class TaskVectorTask(Task[mindspore.Tensor]):
    base_tensor: Task
    model_tensor: Task
    ...
    def execute(self, base: mindspore.Tensor, model: mindspore.Tensor) -> mindspore.Tensor:
        return model - base
...
class LoRAModuleSaveTask(Task):
    ...
    def execute(
        self, writer: TensorWriter, decomp: Tuple[mindspore.Tensor, mindspore.Tensor]
    ) -> None:
...
def plan_extraction(  # pylint: disable=too-many-positional-arguments
...
def plan_lora_module(  # pylint: disable=too-many-positional-arguments
    base_model_ref: ModelReference,
    model_ref: ModelReference,
    wi: WeightInfo,
