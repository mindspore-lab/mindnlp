...
from ..io.lazy_tensor_loader import ShardedTensorIndex
from ..io.tensor_writer import TensorWriter

DEFAULT_SHARD_SIZE = 5 * 1024**3


def load_tensor_from_file(tensor_name: str, tensor_file: str = None) -> mindspore.Tensor:
    """
    ...
    :return: The loaded tensor as a MindSpore tensor.
    """
    with safe_open(tensor_file, framework="numpy", device="cpu") as f:
        if tensor_name in f.keys():
            np_tensor = f.get_tensor(tensor_name)
...
def load_tensor_from_index(tensor_name: str, index: ShardedTensorIndex) -> mindspore.Tensor:
    """
...
def copy_and_fill_missing_params(
    ...
    facilitating testing after generating submodel parameters through merging.
    ...
    Returns:
        pathlib.Path: The path to the directory where the final merged model is saved.
    """
    output_dir = (
    ...
    writer = TensorWriter(
        out_path=str(output_dir), max_shard_size=max_shard_size, output_format="safetensors"
    )
    ...
    for name, tensor_path in tqdm(
        ...
        if name in common_param_names:
            ...
            if submodel_tensor.shape != tensor.shape:
                logging.warning(
                    f"Size mismatch for tensor '{name}': {tensor.shape} vs {submodel_tensor.shape}"
                )

            tensor = submodel_tensor

        writer.save_tensor(name, tensor.copy())
...
class ParameterNamesUtils:
    ...
    def tensor_shape(name, index) -> Tuple[int]:
        from safetensors import safe_open  # pylint: disable=reimported

        with safe_open(
            Path(index.base_path) / index.tensor_paths[name], framework="numpy"
        ) as f:
