...
#
# Renamed from merge_raw_pytorch.py → merge_raw.py (framework-neutral name).

"""
Merge arbitrary weight files (safetensors / bin) without architecture awareness.

Unlike ``wizard-merge`` which requires HuggingFace model directories with
``config.json``, this tool works directly on individual weight files.
"""
...
from mindnlp.wizard.merge import merge_methods
from ..architecture import WeightInfo
from ..common import ImmutableMap, ModelReference, dtype_from_name
from ..config import ParameterSetting, evaluate_setting
from ..graph import Executor, Task
from ..io import LazyTensorLoader, ShardedTensorIndex
from ..io.tasks import FinalizeModel, SaveTensor, TensorWriterTask
from ..merge_methods.base import MergeMethod, TensorDictWrapper
...
class RawMergeConfig(BaseModel, frozen=True):
    merge_method: str
    ...
    def get(self, model: str) -> LazyTensorLoader:
        if model not in self.loaders:
            self.loaders[model] = LazyTensorLoader(
                ShardedTensorIndex.from_file(model),
                lazy_loader=self.lazy_loader,
            )
        return self.loaders[model]


class SimpleLoadTensor(Task[mindspore.Tensor]):
    model: str
    tensor_name: str
    dtype: Optional[str] = None
    ...
    def execute(self) -> Optional[mindspore.Tensor]:
        ...
        tensor = loader.get_tensor(
            self.tensor_name, device=self.device or "CPU"
        )
        if tensor is None:
            return None
        if dt := dtype_from_name(self.dtype):
            tensor = tensor.astype(dt)
        return tensor


def plan_flat_merge(
    config: RawMergeConfig,
    out_path: str,
    tensor_union: bool,
    ...
    loaders.lazy_loader = options.lazy_loader
    all_tensor_names: set = set()
    for model_def in tqdm.tqdm(
        config.models, desc="Preparing model loaders"
    ):
    ...
    for tensor_name in tqdm.tqdm(
        list(all_tensor_names), desc="Planning operations"
    ):
        inputs = {
        ...
        if (
            config.base_model is not None
            and config.base_model not in inputs
        ):
        ...
        has_tensor = [
            ...
            if lt.tensor_name
            in loaders.get(lt.model).index.tensor_paths
        ]
        if len(has_tensor) < len(inputs):
            ...
            if tensor_union:
                ...
                logging.warning(
                    f"Tensor {tensor_name} not found in models:"
                )
        ...
        tensor_task = merge_method.make_task(
            ...
            tensor_parameters=ImmutableMap(
                data={
                    key: ImmutableMap(data=tensor_params[key])
                    for key in tensor_params
                }
            ),
            base_model=(
...
def construct_param_dicts(
    ...
    for param_def in merge_method.parameters():
        if config.parameters and param_def.name in config.parameters:
            value = evaluate_setting(
                tensor_name, config.parameters[param_def.name]
            )
        ...
        if param_def.name not in global_params:
            if param_def.required:
                raise RuntimeError(
                    ...
                    f"for merge method {merge_method}"
                )
            else:
    ...
    for param_def in merge_method.tensor_parameters():
        for model_def in config.models:
            mr = ModelReference.model_validate(
                {"model": {"path": model_def.model}}
            )
            tensor_params[mr] = tensor_params.get(mr, {})
            if value := evaluate_setting(
                tensor_name,
                model_def.parameters.get(param_def.name, []),
            ):
                ...
                raise RuntimeError(
                    f"Missing required parameter {param_def.name} "
                    f"for model {mr} tensor {tensor_name}"
                )
            else:
...
@click.command("wizard-merge-raw", cls=PrettyPrintHelp)
...
@click.option(
    ...
    default=False,
    is_flag=True,
    help="Only merge tensors present in all input models",
)
@click.option(
    "--tensor-union",
    "-u",
...
def main(
    ...
    """Merge arbitrary weight files (safetensors / bin).

    Uses similar configuration syntax to ``wizard-merge``, minus the
    ``slices`` sections.  Each input model should be the path on disk to
    a safetensors or bin file."""
    merge_options.apply_global_options()

    with open(config_path, "r", encoding="utf-8") as file:
        config_source = file.read()

    config = RawMergeConfig.model_validate(yaml.safe_load(config_source))
    ...
    executor = Executor(
        tasks,
        math_device=merge_options.device or "CPU",
        storage_device=(
            merge_options.device
            if merge_options.low_cpu_memory
            else "CPU"
        ),
    )
    executor.execute()
