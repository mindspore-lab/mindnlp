...
from ..evo.genome import ModelGenome
from ..evo.strategy import (
...
@click.option("--num-npus", type=int, help="Number of NPUs to use across all nodes")
@click.option("--merge-npu/--no-merge-npu", is_flag=True, default=True)
...
def main(  # pylint: disable=too-many-positional-arguments
    ...
    if use_wandb:
        ...
        run = wandb.init(
            project=wandb_project or "wizard-evolve",
            entity=wandb_entity,
    ...
    merge_options = MergeOptions(
        ...
        quiet=True,
        read_to_npu=merge_npu and not in_memory,
        copy_tokenizer=True,
    ...
    x0 = genome.initial_genotype(random=config.random_init).view(-1).asnumpy()
    xbest = x0
    xbest_cost = np.inf
