...
from ..moe import ALL_OUTPUT_ARCHITECTURES, MoEOutputArchitecture
from ..moe.config import MoEMergeConfig, is_bad_config
from ..moe.router import get_gate_params, warn_degenerate_gates
from ..options import PrettyPrintHelp, add_merge_options
...
@click.command("wizard-moe", cls=PrettyPrintHelp)
...
def main(  # pylint: disable=too-many-positional-arguments
    ...
    if merge_options.write_model_card:
        with open(
            os.path.join(out_path, "wizard_moe_config.yml"), "w", encoding="utf-8"
        ) as fp:
