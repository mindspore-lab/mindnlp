...
class PermutedEmbeddings(Task[Dict[ModelReference, mindspore.Tensor]]):
    ...
    def execute(
        ...
        for token, token_id in vocab.items():
            embed = mindspore.ops.zeros(embed_size, dtype=dtype)
        ...
        for model in models:
            ...
            new_embed = mindspore.ops.zeros(
                (vocab_size, embed_size), dtype=dtype
            )
            for token, token_id in vocab.items():
                force = False
            ...
            if vocab_size > len(vocab):
                avg_embed = mindspore.ops.mean(new_embed[: len(vocab), :], axis=0)
    ...
    def compute_default_embedding(  # pylint: disable=too-many-positional-arguments
        self,
        tokenizer_info: TokenizerInfo,
        tensors: Dict[ModelReference, mindspore.Tensor],
        permutations: Dict[ModelReference, Dict[int, int]],
