...
from ..common import ModelPath, ModelReference, get_config_value
...
def build_union_tokenizer(
    ...
    for model, tokenizer in tokenizers_map.items():
        vocab_size = (
...
def build_tokenizer(
    ...
    tokenizers_loaded = {base_model: tokenizer_base}
    for model in referenced_models:
        ...
        try:
            ...
            continue
        tokenizers_loaded[model] = model_tok
    ...
    if isinstance(tokenizer_source, ModelReference):
        ...
        tokenizer_out = build_union_tokenizer(
            tokenizer_base, tokenizers_loaded, trust_remote_code=trust_remote_code
        )
    else:
    ...
    for model in (
        ...
        if model in tokenizers_loaded:
            model_vocab = tokenizers_loaded[model].get_vocab()
        else:
            model_vocab = tokenizers_loaded[base_model].get_vocab()
    ...
    return TokenizerInfo(
        ...
        original_vocabs={
            model: tok.get_vocab() for model, tok in tokenizers_loaded.items()
        },
    )
