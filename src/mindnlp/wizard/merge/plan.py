...
"""Merge planner: module/slice/layer/tensor planning."""
...
from .tokenizer import BuildTokenizer, PermutedEmbeddings
...
class MergePlanner:
    ...
    def plan_tensor(
        ...
        if weight.optional:
            ...
            if not any_weight:
                logging.info("Skipping optional weight %s", weight.name)
                return
        ...
        gather_tensors = GatherTensors(
            ...
            device=self.options.device if self.options.read_to_npu else None,
        )
        ...
        if "split_pieces" in make_task_sig.parameters:
            make_task_kwargs["split_pieces"] = self.options.split_pieces
        if "max_tensor_mem_gb" in make_task_sig.parameters:
            make_task_kwargs["max_tensor_mem_gb"] = self.options.max_tensor_mem_gb

        tensor_task = tensor_merge_method.make_task(**make_task_kwargs)
        self._tensors.append((weight, tensor_task))

    def plan_layer(  # pylint: disable=too-many-positional-arguments
        self,
    ...
    def plan_to_disk(self, out_path: str) -> List[Task]:
        ...
        writer_task = TensorWriterTask(
            ...
            write_threads=self.options.write_threads,
        )
        save_tasks = []
        for weight, tensor_task in self._tensors:
            save_tasks.append(
                SaveTensor(
                    ...
                    dtype=weight.force_dtype or self.config.out_dtype or self.config.dtype,
                )
            )
    ...
    def plan_in_memory(self) -> List[ReturnTensor]:
        ...
        return [
            ReturnTensor(
                weight_info=w,
                tensor_task=t,
                dtype=w.force_dtype or self.config.out_dtype or self.config.dtype,
            )
