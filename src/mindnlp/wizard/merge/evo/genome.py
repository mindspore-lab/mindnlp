...
class ModelGenome:
    ...
    def initial_genotype(self, random: bool = False) -> mindspore.Tensor:
        ...
        shape = (n_layer_groups, n_models, n_param_sets, n_params)
        if random:
            return ops.rand(*shape)
        else:
            x0_t = ops.zeros(*shape)
            x0_t[:, :, :, 0] = 1 / n_models
            if n_params > 1:
                x0_t[:, :, :, 1:] = 1
            return x0_t
    ...
    def genotype_merge_config(
        self, genotype: Union[mindspore.Tensor, np.ndarray]
    ) -> MergeConfiguration:
        """Convert a genotype tensor to a mergekit configuration."""

        genotype = self._to_mindspore(genotype)
        ...
        if self.definition.merge_method == "slerp":
            ...
            for param_idx, param in enumerate(
                ...
                if param == "density":
                    values = ops.clamp(ops.abs(values), 0, 1)
    ...
    def _discrete_config_slices(
        ...
        n_layer_groups: int,
        n_param_sets: int,
        param_arrays: Dict[str, mindspore.Tensor],
    ) -> List[Dict]:
    ...
    def _smooth_config_models(
        self, n_param_sets: int, param_arrays: Dict[str, mindspore.Tensor]
    ) -> List[Dict]:
                ...
                for param, values in param_arrays.items():
                    ...
                    for set_idx in range(n_param_sets):
                        ...
                        params[param].append(
                            {
                                "filter": filter_,
                                "value": _unpack_single_element(value.asnumpy().tolist()),
                            }
                        )
            else:
                for param, values in param_arrays.items():
                    params[param] = _unpack_single_element(
                        values[:, model_idx, 0].asnumpy().tolist()
                    )
    ...
    def _slerp_slices(self, genotype: mindspore.Tensor) -> List[Dict]:
        ...
        for slice_idx in range(n_layer_groups):
            ...
            chosen_values, chosen_indices = ops.topk(
                genotype[slice_idx, :, 0, 0], 2
            )
            t = ops.softmax(chosen_values, axis=-1)[1].item()
            s["parameters"] = {"t": t}
            s["base_model"] = self.definition.models[chosen_indices[0].item()]
            s["sources"] = [
                s["sources"][chosen_indices[0].item()],
                s["sources"][chosen_indices[1].item()],
            ]
    ...
    def _to_mindspore(
        ...
        """Convert a genotype to a MindSpore tensor of the correct shape."""
        if not isinstance(genotype, mindspore.Tensor):
            genotype = mindspore.Tensor(genotype, dtype=mindspore.float32)
