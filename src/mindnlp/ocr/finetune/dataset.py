...
class OCRDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        Returns:
            包含 pixel_values, input_ids, attention_mask, labels 的字典
        """
        ...
        if Path(image_path_str).is_absolute() or Path(image_path_str).exists():
            ...
            # 如果已经是"images/xxx.png"格式，说明路径相对于数据集根目录
        ...
        try:
            ...
            inputs = self.processor(
                ...
                truncation=False  # 禁用截断以避免image token不匹配
            )
        finally:
            # 恢复原设备设置
        ...
        # 提取单个样本 (去除 batch 维度) - 强制所有tensor都在CPU上
        import torch
        result = {
        ...
        # 构建 labels (用于计算 loss)
        # 对于训练,只计算 assistant 回复部分的 loss
        labels = result['input_ids'].clone()
        
        # 找到 assistant 开始的位置,之前的 token 设为 -100
        # 这是一个简化实现,实际可能需要根据 tokenizer 的特殊 token 来判断
