...
"""
# 必须在最开始阻止mindnlp patch的应用,确保使用纯PyTorch路径
import sys
import os
...
def evaluate_table_accuracy(references: List[str], hypotheses: List[str]) -> Dict[str, float]:
    ...
    表格识别特定指标：
    - Cell Accuracy: 单元格级别的准确率
    - Row Accuracy: 行级别的准确率
    - Structure Accuracy: 表格结构准确率
    
    Args:
        references: 参考文本列表 (应包含表格标记，如 |, -, \n)
        hypotheses: 识别结果列表
...
def evaluate_formula_accuracy(references: List[str], hypotheses: List[str]) -> Dict[str, float]:
    ...
    公式识别特定指标：
    - Symbol Accuracy: 符号级别准确率
    - LaTeX Structure Accuracy: LaTeX 结构准确率
    - Exact Match: 完全匹配率
    
    Args:
        references: 参考公式文本 (LaTeX 格式)
        hypotheses: 识别结果
...
def load_lora_model(base_model_path: str, lora_path: str, device: Optional[str] = None):
    ...
    if hasattr(processor, 'tokenizer'):
        ...
        # 确保 pad_token_id 是 int 而不是 tensor
