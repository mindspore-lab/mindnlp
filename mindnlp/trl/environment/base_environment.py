...
class StringStoppingCriteria(StoppingCriteria):
    ...
    def __call__(self, input_ids, scores, **kwargs):
        ...
        for i, decoded_generation in enumerate(decoded_generations):
            sequence_complete = any(stop_string in decoded_generation 
...
class TextHistory:
    """The TextHistory class keeps track of the history of an interaction 
    between the language model and the environment."""
    ...
    def __init__(self, text, tokens, system=True):
        ...
        args:
            ...
            tokens (`ms.Tensor`): The tokens of the first segment.
            system (`bool`, *optional*): Whether the first segment is a system or user segment.
        """
        self.system_spans = []
        self.text_spans = []
        self.token_spans = []
        self.token_masks = ms.Tensor([], dtype=ms.Tensor)
        self.text = ""
        self.tokens = ms.Tensor([], dtype=ms.Tensor)
    ...
    def append_segment(self, text, tokens, system=True):
        ...
        args:
            ...
            tokens (`ms.Tensor`): The tokens of the new segment.
        ...
        self.tokens = ops.cat((self.tokens, tokens))
        if system:
            self.token_masks = ops.cat((self.token_masks, ops.zeros_like(tokens)))
        else:
            self.token_masks = ops.cat((self.token_masks, ops.ones_like(tokens)))
...
class TextEnvironment:
    ...
    def __init__(
        ...
        Args:
            ...
            generation_kwargs (Optional[dict]): A dictionary of keyword arguments to pass to 
    ...
    def run(self, queries, **rewards_kwargs):
        ...
        queries_tokens = [
            self.tokenizer(query, return_tensors="pt").input_ids[0]
        ...
        queries, responses, masks = map(list, 
                                        zip(*[history.split_query_response_tokens() 
    ...
    def _generate_batched(
        ...
        args:
            query_tensors (list[ms.Tensor]): A list of query tensors to generate responses for.
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            batch_mask = [ops.ones_like(element) for element in batch]
            ...
            for mask, generated_tokens in zip(
                padded_inputs["attention_mask"], stopping_criteria.generated_tokens
            ):
                if not self.is_encoder_decoder:
                    output = output[(mask).sum() :]  # remove prompt
