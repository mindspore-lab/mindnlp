...
class BestOfNSampler:
    ...
    def generate(
        self,
        tokenized_query: Union[List[int], ms.Tensor, List[ms.Tensor], List[List[int]]],
        ...
        **generation_kwargs,
        ...
        Args:
            tokenized_query (`List[int]` or `ms.Tensor` or `List[ms.Tensor]` or `List[int]`):
        ...
        if isinstance(tokenized_query, ms.Tensor) and tokenized_query.ndim == 1:
            queries = tokenized_query.unsqueeze(0)
        elif isinstance(tokenized_query, List):
            element_type = type(tokenized_query[0])
            if element_type == int:
                queries = ms.Tensor(tokenized_query).unsqueeze(0)
            elif element_type == ms.Tensor:
                queries = [tensor.reshape((1, -1)) for tensor in tokenized_query]
            else:
                queries = [ms.Tensor(query).reshape((1, -1)) for query in tokenized_query]
        ...
        for query in queries:
            ...
            scores = ms.Tensor(self.queries_to_scores(output))
