...
from ...peft import (
        ...
        #prepare_model_for_kbit_training 的作用是为量化的transformers模型准备训练环境,以确保模型在进行量化训练前可以正确运行。
    )
...
class PreTrainedModelWrapper(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if reward_adapter is not None and not isinstance(reward_adapter, str):
            raise ValueError(
                '''
                The `reward_adapter` argument
                should be a string representing the name of local path
                or the Hub id to the Reward Modeling adapter.
                '''
            )
        ...
        if isinstance(pretrained_model_name_or_path, str):
            is_loaded_in_8bit = pretrained_kwargs["load_in_8bit"]\
                if "load_in_8bit" in pretrained_kwargs else False
            is_loaded_in_4bit = pretrained_kwargs["load_in_4bit"]\
                if "load_in_4bit" in pretrained_kwargs else False
        else:
            is_loaded_in_8bit = getattr(pretrained_model_name_or_path, \
                "is_loaded_in_8bit", False)
            is_loaded_in_4bit = getattr(pretrained_model_name_or_path, \
                "is_loaded_in_4bit", False)

        if (is_loaded_in_8bit or is_loaded_in_4bit) and "device_map" not in pretrained_kwargs:
            # warn users
            logging.warning(
                '''The `device_map` argument is not provided.
                We will override the device_map argument."
                " to set the entire"
                " model on the current device. If you want to set the model on multiple devices,
                please provide"
        ...
        if peft_config is not None and not isinstance(peft_config, PeftConfig):
            raise ValueError("The `peft_config` argument \
                should be an instance of `peft.PeftConfig` class.")
        ...
        if isinstance(pretrained_model_name_or_path, str):
            ...
            #     remote_adapter_config = None
            try:
                # If there is a trained peft adapter in the hub,
                # load its config.
                remote_adapter_config = hf_hub_download(
                    pretrained_model_name_or_path,
                    "adapter_config.json",
            ...
            local_adapter_present = os.path.exists(os.path.join\
                (pretrained_model_name_or_path, "adapter_config.json"))

            if (local_adapter_present or remote_adapter_config is not None):
                if peft_config is not None:
                    logging.warning(
                        "`peft_config` argument ignored since a peft config file was found in %s",
                        pretrained_model_name_or_path
                    )
                ...
                if local_adapter_present:
                    trained_adapter_config = \
                        PeftConfig.from_pretrained(pretrained_model_name_or_path)
                else:
        ...
        elif isinstance(pretrained_model_name_or_path, \
            cls.supported_pretrained_model_architectures):
        ...
        if isinstance(pretrained_model_name_or_path, str):
            ...
            filename = os.path.join(pretrained_model_name_or_path, "mindnlp.bin")
            ...
            if not (os.path.exists(filename) or os.path.exists(safe_filename)):
                ...
                if filename is None and files_to_download is None:
                    safe_filename, files_to_download, is_sharded, is_resuming_training \
                        = cls._get_checkpoint_from_hub(
                        pretrained_model,
                        pretrained_model_name_or_path,
    ...
    def _get_checkpoint_from_hub(
        ...
        index_filename,
        token=None,
        model_name="mindnlp.bin",
        model_index_name="mindnlp.bin.index.json",
    ):
        ...
        # sharded
            ...
            if os.path.exists(index_filename):
                ...
                try:
                    ...
                    logging.warning(
                        "A %s model is loaded from '%s', and no v_head weight is found. "
                        "This IS expected if you are not resuming PPO training.",
...
##################################-Done-############################################################
    ...
    def add_and_load_reward_modeling_adapter(
        ...
        model is a `PeftModel` and if you have initialized
        the model with the `reward_modeling_adapter_id`
        argument, pointing to the id of the reward modeling adapter.
        The latest needs also to contain the
        score head in order to produce the reward.
        """
        pretrained_model.load_adapter(adapter_model_id,
                                      adapter_name,
                                      is_trainable=False)
    ...
    def save_pretrained(self, *args, **kwargs):
        ...
        if self.is_peft_model:
            save_path = args[0]
            save_path = os.path.join(save_path, "mindnlp_model.bin")
            ms.save_checkpoint(state_dict, save_path)
    ...
    def post_init(self, *args, **kwargs):
        ...
        instantiated and loaded from a checkpoint.
    ...
    def compute_reward_score(self, input_ids, attention_mask=None, **kwargs):
        ...
        # 查询当前的上下文模式
        init_mode = ms.context.get_context("mode")
        ...
        # 恢复到 GRAPH_MODE（如有需要）
        ms.context.set_context(mode = init_mode)
...
def create_reference_model(
    ...
    Args:
        ...
        The number of initial layers that are shared between
        both models and kept frozen.
        pattern (`str`, *optional*):
        The shared layers are selected with a string pattern
            (e.g. "transformer.h.{layer}" for GPT2) and
            if a custom pattern is necessary it can be passed here.
    ...
    # with `create_reference_model()`.
    # Please instantiate your reference model directly
    # with `AutoCausalLM.from_pretrained()`."
    #     )
    #DeepSpeed Python 是一种前沿数据并行技术，通过减少内存开销来高效训练大规模模型
    ...
    if pattern is not None and len(unshared_param_list) == 0:
        logging.warning("Pattern passed or found, but no layers\
            matched in the model. Check for a typo.")
