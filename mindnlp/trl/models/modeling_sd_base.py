'''Copyright 2023 DDPO-pytorch authors (Kevin Black),
The HuggingFace Team, metric-space. All rights reserved.
...
##when peft is available, reopen the imports below
...
class DDPOSchedulerOutput:
    """
    Output class for the diffusers scheduler to be finetuned
    with the DDPO trainer
...
class DDPOStableDiffusionPipeline:
    """
    Main class for the diffusers pipeline to be finetuned
    with the DDPO trainer
    """
    ...
    def vae(self):
        """
        Returns the Variational Autoencoder model used
        from mapping images to and from the latent space
        """
        raise NotImplementedError
    ...
    def scheduler(self):
        """
        Returns the scheduler associated with the pipeline used
    ...
    def save_checkpoint(self, *args, **kwargs):
        """
        Light wrapper around accelerate's
        register_save_state_pre_hook which is run before saving state
        """
        raise NotImplementedError
    ...
    def load_checkpoint(self, *args, **kwargs):
        """
        Light wrapper around accelerate's
        register_lad_state_pre_hook which is run before loading state
        """
        raise NotImplementedError

def _left_broadcast(input_tensor, shape):
    """
    As opposed to the default direction of broadcasting
    (right to left), this function broadcasts
    from left to right
        Args:
            input_tensor (`torch.FloatTensor`): is the tensor
            to broadcast
    ...
    if input_ndim > len(shape):
        raise ValueError(
            """The number of dimensions of the tensor tobroadcast
            cannot be greater than the length of the shape
            to broadcast to"""
        )
    return input_tensor.reshape(input_tensor.shape\
        + (1,) * (len(shape) - input_ndim)).broadcast_to(shape)


def get_variance(alphas_cumprod,
                 final_alpha_cumprod,
                 timestep,
                 prev_timestep):
    ...
    alpha_prod_t = ops.gather_elements(alphas_cumprod, 0, timestep)
    alpha_prod_t_prev = ops.where(
        prev_timestep >= 0,
        alphas_cumprod.gather_elements(0, prev_timestep),
        final_alpha_cumprod,
    )
...
class DefaultDDPOStableDiffusionPipeline(DDPOStableDiffusionPipeline):
    '''need to be rewrited for second time.'''
    def __init__(self, pretrained_model_name: str,
        ...
        try:
            ...
            if use_lora:
                warnings.warn(
                    """If you are aware that the pretrained model
                    has no lora weights to it, ignore this message. """
                    """Otherwise please check the if
    ...
    def get_trainable_layers(self):
        ...
        #             param.data = param.to(ms.float32)
        #     return self.sd_pipeline.unet
        # else:
        #     return self.sd_pipeline.unet
    ...
    #     if len(models) != 1:
    #         raise ValueError("Given how the trainable params were set,\
    #             this should be of length 1")
    #     if self.use_lora and hasattr(models[0], "peft_config")\
    ...
    #         )
    #         self.sd_pipeline.load_lora_into_unet(lora_state_dict,
...
#         if `True`, compute "corrected" `model_output` from the clipped
#             predicted original sample. Necessary because predicted
#             original sample is clipped to [-1, 1] when
#             `self.config.clip_sample` is `True`. If no clipping has happened,
#             "corrected" `model_output` would
#             coincide with the one provided as input
#             and `use_clipped_model_output` will have not effect.
...
#     Returns:
#         `DDPOSchedulerOutput`: the predicted sample
#          at the previous timestep and the log probability of the sample
#     """
