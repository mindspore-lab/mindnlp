...
def convert_to_scalar(stats: Dict) -> Dict:
    ...
    for k, v in stats.items():
        ...
        if (isinstance(v, Tensor) or isinstance(v, np.ndarray)) and (
...
def masked_mean(values: Tensor, mask: Tensor, axis: Optional[bool] = None) -> Tensor:
...
def masked_var(values: Tensor, mask: Tensor, unbiased: bool = True, axis: Optional[bool] = None) -> Tensor:
...
def clip_by_value(x: Tensor, tensor_min: float, tensor_max: float) -> Tensor:
    ...
    """
    clipped = ops.maximum(ops.minimum(x, Tensor(tensor_max, dtype=x.dtype)), Tensor(tensor_min, dtype=x.dtype))
    return clipped
...
def set_seed(seed: int) -> None:
    """
    Helper function for reproducible behavior to set the seed in `random`, `numpy`, and `mindspore`.
