...
def _get_unpad_data(attention_mask: mindspore.Tensor) -> Tuple[mindspore.Tensor, mindspore.Tensor, int]:
    """
...
def _upad_input(
    ...
    This function is used instead of `unpad_input` in order to avoid the recomputation of the same intermediary
    ...
    if query_length == kv_seq_len:
        ...
        cu_seqlens_q = ops.arange(batch_size + 1, dtype=mindspore.int32)  # There is a memcpy here, that is very bad.
...
def prepare_fa2_from_position_ids(
    ...
    key = key.contiguous().view(-1, key.shape[-2], key.shape[-1])
    value = value.contiguous().view(-1, value.shape[-2], value.shape[-1])
    position_ids = position_ids.flatten()
    indices_q = ops.arange(position_ids.shape[0], dtype=mindspore.int32)

    cu_seq_lens = ops.cat(
        (indices_q[position_ids == 0],
         mindspore.tensor(position_ids.shape, dtype=mindspore.int32)
        )
    )
...
def fa_peft_integration_check(
    ...
    key: mindspore.Tensor,
    value: mindspore.Tensor,
    target_dtype: Optional[mindspore.dtype.TensorType] = None,
):
    """
    ...
    Args:
        ...
        target_dtype (`mindspore.dtype`, *optional*):
...
def _flash_attention_forward(
    ...
    cu_seq_lens_q: Optional[mindspore.Tensor] = None,
    cu_seq_lens_k: Optional[mindspore.Tensor] = None,
    ...
    **kwargs,
    ...
    Args:
        ...
        use_top_left_mask (`bool`, defaults to `False`):
            ...
            while what is needed here is bottom-right alignment, that was made default for flash_attn>=2.1.
    ...
    # Use `flash_attn_varlen_func` to prevent cross-example attention and also allow padding free approach
        ...
        max_length_q is not None or (query_length != 1 and not (position_ids.diff(dim=-1) >= 0).all())
    ):
