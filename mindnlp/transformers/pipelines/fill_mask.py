...
class FillMaskPipeline(Pipeline):
    ...
    def get_masked_index(self, input_ids: GenericTensor) -> np.ndarray:
        masked_index = ops.nonzero(input_ids == self.tokenizer.mask_token_id, as_tuple=False)
        return masked_index
    ...
    def postprocess(self, model_outputs, top_k=5, target_ids=None):
        ...
        masked_index = ops.nonzero(input_ids == self.tokenizer.mask_token_id, as_tuple=False).squeeze(-1)
        # Fill mask pipeline supports only one ${mask_token} per sample

        logits = outputs[0, masked_index, :]
        probs = ops.softmax(logits, dim=-1)
        if target_ids is not None:
            probs = probs[..., mindspore.Tensor(target_ids)]
