...
PATCHTSMIXER_INPUTS_DOCSTRING = r"""
    Args:
        past_values (`mindspore.Tensor` of shape `(batch_size, seq_length, num_input_channels)`):
...
class PatchTSMixerPositionalEncoding(nn.Module):
    ...
    def __init__(self, config: PatchTSMixerConfig):
        ...
        if config.use_positional_encoding:
            ...
            self.position_enc = nn.Parameter(ops.zeros(config.num_patches, config.d_model))

    @staticmethod
    def _init_pe(config: PatchTSMixerConfig) -> nn.Parameter:
        # Positional encoding
        if config.positional_encoding_type == "random":
            position_enc = nn.Parameter(ops.randn(config.num_patches, config.d_model), requires_grad=True)
        elif config.positional_encoding_type == "sincos":
            position_enc = ops.zeros(config.num_patches, config.d_model)
            position = ops.arange(0, config.num_patches).unsqueeze(1)
...
class PatchTSMixerNormLayer(nn.Module):
    ...
    def forward(self, inputs: mindspore.Tensor):
        """
        Args:
            inputs (`mindspore.Tensor` of shape `((batch_size, num_channels, num_patches, d_model))`):
        ...
        if "batch" in self.norm_mlp.lower():
            ...
            inputs = ops.reshape(inputs_reshaped, inputs.shape)
...
class PatchTSMixerMLP(nn.Module):
    ...
    def forward(self, inputs: mindspore.Tensor):
        ...
        Returns:
            `mindspore.Tensor` of the same shape as `inputs`
        """
...
class PatchTSMixerAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = ops.transpose(ops.bmm(query_states, key_states), 1, 2)
...
class PatchMixerBlock(nn.Module):
    ...
    def forward(self, hidden_state):
        """
        Args:
            hidden_state (`mindspore.Tensor`): Input tensor.

        Returns:
            `mindspore.Tensor`: Transformed tensor.
        """
        ...
        hidden_state = ops.transpose(hidden_state,2, 3)
...
class FeatureMixerBlock(nn.Module):
    ...
    def forward(self, hidden: mindspore.Tensor):
        """
        Args:
            hidden (`mindspore.Tensor` of shape `(batch_size, num_patches, d_model)`):
...
class PatchTSMixerBlock(nn.Module):
    ...
    def forward(self, hidden_state, output_hidden_states: bool = False):
        """
        Args:
            hidden_state (`mindspore.Tensor`): The input tensor.
            output_hidden_states (`bool`, *optional*, defaults to False.):
                Whether to output the hidden states as well.

        Returns:
            `mindspore.Tensor`: The embedding. `list`: List of all hidden states if `output_hidden_states` is set to
            `True`.
        """
        all_hidden_states = []
...
class PatchTSMixerForPredictionHead(nn.Module):
    ...
    def forward(self, hidden_features):
        ...
        Args:
            hidden_features (`mindspore.Tensor` of shape `(batch_size, num_patch, d_model)` in `flatten` mode
                or `(batch_size, n_vars, num_patch, d_model)` in `common_channel`/`mix_channel` mode.): Input hidden
                features.

        Returns:
            `mindspore.Tensor` of shape `(batch_size, prediction_length, nvars)`.
        ...
        if isinstance(forecast, tuple):
            forecast = tuple(ops.transpose(z,-1, -2) for z in forecast)
        else:
            forecast = ops.transpose(forecast,-1, -2)  # [batch_size x prediction_length x n_vars]
        if self.prediction_channel_indices is not None:
            if isinstance(forecast, tuple):
...
class PatchTSMixerLinearHead(nn.Module):
    ...
    def forward(self, hidden_features):
        """
        Args:
            hidden_features (`mindspore.Tensor` of shape `(batch_size x num_patch x d_model)` in `flatten` mode
                or `(batch_size x n_vars x num_patch x d_model)` in `common_channel`/`mix_channel` mode.): Input hidden
                features.

        Returns:
            `mindspore.Tensor` of shape `(batch_size x num_targets)`.
        """

        # batch_size x d_model x num_patch or batch_size x n_vars x d_model x num_patch
        hidden_features = ops.transpose(hidden_features,-1, -2)
        ...
        if (self.distribution_output is None) and (self.output_range is not None):
            hidden_features = (
                ops.sigmoid(hidden_features) * (self.output_range[1] - self.output_range[0]) + self.output_range[0]
            )
...
class PatchTSMixerPretrainHead(nn.Module):
    ...
    def forward(self, hidden_features):
        ...
        Returns:
            `mindspore.Tensor` of shape `(batch_size x n_vars x num_patch x patch_length)`.
        """
...
def random_masking(
    ...
    mask = ops.gather(mask, -1, index=ids_restore)
    ...
    def forward(
        ...
        Returns:
            tuple of `mindspore.Tensor` of shapes
                ...
                `(batch_size, 1, num_input_channels)`)
        """
        scale = ops.ones_like(data).mean(dim=self.dim, keepdim=self.keepdim)
        loc = ops.zeros_like(data).mean(dim=self.dim, keepdim=self.keepdim)
...
class PatchTSMixerEncoderOutput(ModelOutput):
    ...
    Args:
        last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_channels, num_patches, d_model)`):
...
class PatchTSMixerEncoder(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        Returns:
            `mindspore.Tensor` of shape `(batch_size, n_vars, num_patches, d_model)`
        """
...
class PatchTSMixerModel(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        self,
        past_values: mindspore.Tensor,
        observed_mask: Optional[mindspore.Tensor] = None,
        output_hidden_states: Optional[bool] = False,
        return_dict: Optional[bool] = None,
    ) -> PatchTSMixerModelOutput:
        r"""
        observed_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_input_channels)`, *optional*):
        ...
        if observed_mask is None:
            observed_mask = ops.ones_like(past_values)
        ...
        if isinstance(encoder_output, tuple):
            encoder_output = PatchTSMixerEncoderOutput(
...
class PatchTSMixerForPreTrainingOutput(ModelOutput):
    ...
    Args:
        prediction_outputs (`mindspore.Tensor` of shape `(batch_size, num_input_channels, num_patches, patch_length)`):
            Prediction output from the pretrain head.
        hidden_states (`tuple(mindspore.Tensor)`, *optional*):
            Hidden-states of the model at the output of each layer.
        last_hidden_state (`mindspore.Tensor` of shape `(batch_size, num_input_channels, num_patches, d_model)`):
            Backbone embeddings before passing through the head.
        loss (*optional*, returned when `y` is provided, `mindspore.Tensor` of shape `()`):
            Total loss
    """
...
class PatchTSMixerForPretraining(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        if isinstance(model_output, tuple):
            model_output = PatchTSMixerModelOutput(
            last_hidden_state=model_output[0],
            hidden_states=model_output[1] if len(model_output) > 1 else None,
            patch_input=model_output[2] if len(model_output) > 2 else None,
            mask=model_output[3] if len(model_output) > 3 else None,
            loc=model_output[4] if len(model_output) > 4 else None,
            scale=model_output[5] if len(model_output) > 5 else None,
        )
        ...
        if self.masked_loss is True and loss_val is not None:
            loss_val = (ops.mean(loss_val,dim=-1) * model_output.mask).sum() / (model_output.mask.sum() + 1e-10)
...
class PatchTSMixerForPredictionOutput(ModelOutput):
    ...
    Args:
        prediction_outputs (`mindspore.Tensor` of shape `(batch_size, prediction_length, num_input_channels)`):
        ...
        loss (*optional*, returned when `y` is provided, `mindspore.Tensor` of shape `()`):
            Total loss.
        loc (`mindspore.Tensor`, *optional* of shape `(batch_size, 1, num_input_channels)`):
            Input mean
        scale (`mindspore.Tensor`, *optional* of shape `(batch_size, 1, num_input_channels)`):
            Input std dev
...
class SamplePatchTSMixerPredictionOutput(ModelOutput):
    ...
    Args:
        sequences (`mindspore.Tensor` of shape `(batch_size, num_samples, prediction_length, number_channels)`):
...
class SamplePatchTSMixerRegressionOutput(ModelOutput):
    ...
    Args:
        sequences (`mindspore.Tensor` of shape `(batch_size, num_samples, num_targets)`
                Sampled values from the chosen distribution.
    """
...
class PatchTSMixerForPrediction(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        observed_mask (`mindspore.Tensor` of shape `(batch_size, sequence_length, num_input_channels)`, *optional*):
            ...
            in `[0, 1]`:
                - 1 for values that are **observed**,
                - 0 for values that are **missing** (i.e. NaNs that were replaced by zeros).
        future_values (`mindspore.Tensor` of shape `(batch_size, target_len, num_input_channels)` for forecasting,:
    ...
    def generate(
        ...
        # stack tensors
        samples = ops.stack(samples, dim=1)  # [batch_size x num_samples x prediction_length x num_channels]
...
class PatchTSMixerForTimeSeriesClassificationOutput(ModelOutput):
    ...
    Args:
        prediction_outputs (`mindspore.Tensor` of shape `(batch_size, num_labels)`):
...
class PatchTSMixerForTimeSeriesClassification(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        r"""
        target_values (`mindspore.Tensor` of shape `(batch_size, target_len, num_input_channels)` for forecasting,
            `(batch_size, num_targets)` for regression, or `(batch_size,)` for classification, *optional*): Target
...
class InjectScalerStatistics4D(nn.Module):
    ...
    def forward(self, inputs: mindspore.Tensor, loc: mindspore.Tensor, scale: mindspore.Tensor):
        """
        Args:
            inputs (`mindspore.Tensor` of shape `(batch_size, num_input_channels, num_patch, d_model)`)
            loc (`mindspore.Tensor` of shape `(batch_size, 1, num_input_channels)`)
            scale (`mindspore.Tensor` of shape `(batch_size, 1, num_input_channels)`)
        Returns:
            `mindspore.Tensor` of shape `(batch_size, num_input_channels, num_patch, d_model)`
        """

        mean = ops.transpose(loc,-1, -2)  # [batch_size x n_channels x 1 ]
        mean = mean.unsqueeze(-2)  # [batch_size x n_channels x 1 x 1]
        mean = mean.tile((1, 1, self.num_patches, 1))  # [batch_size x n_channels x num_patch x 1]

        stdev = ops.transpose(scale,-1, -2)  # [batch_size x n_channels x 1 ]
        stdev = stdev.unsqueeze(-2)  # [batch_size x n_channels x 1 x 1]
        stdev = stdev.tile((1, 1, self.num_patches, 1))  # [batch_size x n_channels x num_patch x 1]

        concat_stats = ops.cat([mean, stdev], dim=-1)  # [batch_size x n_channels x num_patch x 2]
        ...
        inputs = ops.cat([inputs, concat_stats], dim=-1)  # [batch_size x channels x num_patch x d_model+2]
...
class PatchTSMixerForRegression(PatchTSMixerPreTrainedModel):
    ...
    def forward(
        ...
        if target_values is not None and return_loss is True:
            if self.distribution_output:
                if self.distribution_output == "negative_binomial" and ops.any(target_values < 0):
                ...
                y_hat = ([item.view(-1, self.config.num_targets) for item in y_hat])
...
__all__ =[  "PatchTSMixerPreTrainedModel",
        "PatchTSMixerModel",
