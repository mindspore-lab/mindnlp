...
from mindspore.common.initializer import initializer, Normal, Zero, One, Uniform
...
class MimiConv1d(nn.Cell):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1d(
            ...
            group=groups,
            has_bias=bias,
            pad_mode='valid'  # MindSpore需要显式padding
        )

        self.stride = ms.Parameter(Tensor(stride, ms.int64), requires_grad=False)
        self.kernel_size = ms.Parameter(Tensor((kernel_size - 1) * dilation + 1, ms.int64), requires_grad=False)
        self.padding_total = ms.Parameter(Tensor(self.kernel_size.value() - self.stride.value(), ms.int64), requires_grad=False)
    ...
    def _get_extra_padding_for_conv1d(self, hidden_states):
        ...
        n_frames = ops.ceil(n_frames).astype(ms.int64) - 1
        ideal_length = n_frames * self.stride + self.kernel_size - self.padding_total
        return ideal_length - length

    def _pad1d(self, hidden_states, paddings, mode="zero", value=0.0):
        length = hidden_states.shape[-1]
        padding_left, padding_right = paddings
        
        if mode != "reflect":
            return ops.pad(hidden_states, ((0,0), (0,0), (padding_left, padding_right)), mode=mode, constant_values=value)
            
        max_pad = max(padding_left, padding_right)
        extra_pad = 0
        if length <= max_pad:
            extra_pad = max_pad - length + 1
            hidden_states = ops.pad(hidden_states, ((0,0), (0,0), (0, extra_pad)))
            
        padded = ops.pad(hidden_states, ((0,0), (0,0), (padding_left, padding_right)), mode=mode)
...
class MimiLayerScale(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.scale = nn.Parameter(Tensor(initial_scale, channels), requires_grad=True)
...
class MimiRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        with ops.autocast(device_type=device_type, enabled=False):
            ...
            emb = ops.concat((freqs, freqs), dim=-1)
            cos = emb.cos()
            sin = emb.sin()
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    """
    cos = ops.expand_dims(cos, unsqueeze_dim)  # [batch, 1, seq_len, dim]
...
def repeat_kv(hidden_states: Tensor, n_rep: int) -> Tensor:
    """MindSpore版本的repeat_kv实现"""
    batch, num_key_value_heads, slen, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    
    # 使用reshape和broadcast_to替代expand
    hidden_states = ops.reshape(hidden_states, 
                              (batch, num_key_value_heads, 1, slen, head_dim))
    hidden_states = ops.broadcast_to(hidden_states,
                                   (batch, num_key_value_heads, n_rep, slen, head_dim))
    return ops.reshape(hidden_states,
                      (batch, num_key_value_heads * n_rep, slen, head_dim))
...
class MimiAttention(nn.Cell):
    ...
    def __init__(self, config, layer_idx=None):
        ...
        self.o_proj = nn.Dense(self.num_heads * self.head_dim, config.hidden_size, has_bias=config.attention_bias)
    ...
    def apply_rotary_pos_emb(self, q, k, cos, sin, position_ids=None):
        # 实现RoPE
        cos = ops.expand_dims(cos, 1)  # [batch, 1, seq_len, dim]
        sin = ops.expand_dims(sin, 1)
        q_embed = (q * cos) + (rotate_half(q) * sin)
        k_embed = (k * cos) + (rotate_half(k) * sin)
        return q_embed, k_embed

    def construct(self, hidden_states, attention_mask=None, position_ids=None,
        ...
        # 计算注意力分数
        attention_scores = ops.matmul(query_states, 
                                    ops.transpose(key_states, (0, 1, 3, 2)))
        ...
        attention_probs = ops.cast(attention_scores, ms.float32)
        attention_probs = ops.softmax(attention_probs, axis=-1)
        attention_probs = ops.cast(attention_probs, query_states.dtype)
        
        if self.training:
            attention_probs = ops.dropout(attention_probs, self.attention_dropout)

        context_layer = ops.matmul(attention_probs, value_states)
        context_layer = ops.transpose(context_layer, (0, 2, 1, 3))
        context_layer = ops.reshape(context_layer, (bsz, q_len, -1))

        output = self.o_proj(context_layer)
        
        outputs = (output, None, None)  # output, attention_weights, past_key_value
        if output_attentions:
            outputs = (output, attention_probs, None)
        if use_cache:
            outputs = (output, attention_probs if output_attentions else None, (key_states, value_states))
...
class MimiTransformerModel(nn.Cell):
    ...
    def construct(
        ...
        """
        Args:
            hidden_states (`Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
...
class MimiEuclideanCodebook(nn.Cell):
    ...
    def __init__(self, config: MimiConfig, epsilon: float = 1e-5):
        ...
        self.initialized = ms.Parameter(ops.ones((1,)), requires_grad=False)
        self.cluster_usage = ms.Parameter(ops.ones((config.codebook_size,)), 
                                        requires_grad=False)
        self.embed_sum = ms.Parameter(embed, requires_grad=False)
        self._embed = None
    ...
    def embed(self) -> Tensor:
        if self._embed is None:
            # 使用clip替代clamp
            self._embed = self.embed_sum / ops.clip_by_value(
                self.cluster_usage, clip_value_min=self.epsilon)[:, None]
        return self._embed

    def quantize(self, hidden_states):
        # 使用MindSpore的距离计算
        dists = ops.cdist(hidden_states[None], self.embed[None], p=2)[0]
        embed_ind = ops.argmin(dists, axis=-1)
        return embed_ind
...
class MimiResidualVectorQuantizer(nn.Cell):
    ...
    def decode(self, codes: Tensor) -> Tensor:
        ...
        quantized_out = ops.zeros((), codes.dtype)
        codes = ops.transpose(codes, (1, 0))
...
class MimiSplitResidualVectorQuantizer(nn.Cell):
    ...
    def encode(self, embeddings: Tensor, num_quantizers: Optional[float] = None) -> Tensor:
        ...
        if num_quantizers > self.num_semantic_quantizers:
            ...
            codes = ops.concat([codes, acoustic_codes], axis=0)
...
class MimiPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Dense):  # Linear -> Dense
            module.weight.set_data(initializer(Normal(self.config.initializer_range),
                                             module.weight.shape,
                                             module.weight.dtype))
            if module.bias is not None:
                module.bias.set_data(initializer(Zero(),
                                               module.bias.shape,
                                               module.bias.dtype))
                                               
        elif isinstance(module, (nn.LayerNorm, nn.GroupNorm)):
            module.gamma.set_data(initializer(One(),
                                            module.gamma.shape,
                                            module.gamma.dtype))
            module.beta.set_data(initializer(Zero(),
                                           module.beta.shape, 
                                           module.beta.dtype))
            ...
            weight_data = initializer(Normal(a), weight_shape, module.weight.dtype)
            module.weight.set_data(weight_data)
            
            if module.bias is not None:
                bias_data = initializer(Uniform(a=-k, b=k), 
                                      module.bias.shape,
                                      module.bias.dtype)
                module.bias.set_data(bias_data)
                
        elif isinstance(module, nn.Embedding):
            module.embedding_table.set_data(
                initializer(Normal(self.config.initializer_range),
...
class MimiModel(MimiPreTrainedModel):
    ...
    def _encode_frame(
        ...
        encoder_outputs = self.encoder_transformer(
            ops.transpose(embeddings, (0, 2, 1)), past_key_values=past_key_values, return_dict=return_dict
        )
        if return_dict:
            past_key_values = encoder_outputs.get("past_key_values")
        elif len(encoder_outputs) > 1:
            past_key_values = encoder_outputs[1]
        embeddings = ops.transpose(encoder_outputs[0], (0, 2, 1))
    ...
    def encode(
        ...
        Args:
            input_values (`Tensor` of shape `(batch_size, channels, sequence_length)`):
    ...
    def _decode_frame(
        ...
        embeddings = ops.transpose(decoder_outputs[0], (0, 2, 1))
