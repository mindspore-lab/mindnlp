...
class MimiConv1d(nn.Module):
    ...
    def __init__(
        ...
        stride = mindspore.tensor(self.conv.stride[0], dtype=mindspore.int3264)
        dilation = self.conv.dilation[0]

        # Effective kernel size with dilations.
        kernel_size = mindspore.tensor((kernel_size - 1) * dilation + 1, dtype=mindspore.int3264)

        self.register_buffer("stride", stride, persistent=False)
        self.register_buffer("kernel_size", kernel_size, persistent=False)
        self.register_buffer("padding_total", mindspore.tensor(kernel_size - stride, dtype=mindspore.int3264), persistent=False)
    ...
    def _get_extra_padding_for_conv1d(
        ...
        n_frames = ops.ceil(n_frames).to(mindspore.int3264) - 1
    ...
    def _pad1d(hidden_states: mindspore.Tensor, paddings: Tuple[int, int], mode: str = "zero", value: float = 0.0):
        """Tiny wrapper around ops.nn.functional.pad, just to allow for reflect padding on small input.
...
class MimiSdpaAttention(MimiAttention):
    """
    Mimi attention module using ops.nn.functional.scaled_dot_product_attention. This module inherits from
    ...
    def forward(
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "MimiModel is using MimiSdpaAttention, but `ops.nn.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
        ...
        attn_output = ops.nn.functional.scaled_dot_product_attention(
            query_states,
            key_states,
            value_states,
...
class MimiEuclideanCodebook(nn.Module):
    ...
    def __init__(self, config: MimiConfig, epsilon: float = 1e-5):
        ...
        self.register_buffer("initialized", mindspore.tensor([True], dtype=mindspore.float32))
...
MIMI_START_DOCSTRING = r"""
    ...
    This model is also a PyTorch [ops.nn.Module](https://pytorch.org/docs/stable/nn.html#ops.nn.Module) subclass.
