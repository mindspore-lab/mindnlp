...
class MimiConvTranspose1d(nn.Cell):
    ...
    def __init__(
        ...
        self.conv = nn.Conv1dTranspose(in_channels, out_channels, kernel_size, stride, group=groups, has_bias=bias)
...
class MimiLayerScale(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.scale = mindspore.Parameter(ops.full((channels,),initial_scale),"scale", requires_grad=True)
...
class MimiEuclideanCodebook(nn.Cell):
    ...
    def __init__(self, config: MimiConfig, epsilon: float = 1e-5):
        super().__init__()
        embed = ops.zeros(config.codebook_size, mindspore.float32)

        self.codebook_size = config.codebook_size

        self.initialized= mindspore.Tensor([True], dtype=mindspore.float32)
