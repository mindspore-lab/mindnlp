...
class MimiOutput(ModelOutput):
    """
    Args:
        audio_codes (`ms.LongTensor`  of shape `(batch_size, num_quantizers, codes_length)`, *optional*):
            Discret code embeddings computed using `model.encode`.
        audio_values (`ms.FloatTensor` of shape `(batch_size, sequence_length)`, *optional*)
    ...
    audio_codes: ms.LongTensor = None
    audio_values: ms.FloatTensor = None
    encoder_past_key_values: Optional[Union[Cache, List[ms.FloatTensor]]] = None
    decoder_past_key_values: Optional[Union[Cache, List[ms.FloatTensor]]] = None
...
class MimiDecoderOutput(ModelOutput):
    """
    Args:
        audio_values (`ms.FloatTensor`  of shape `(batch_size, segment_length)`, *optional*):
            Decoded audio values, obtained using the decoder part of Mimi.
        decoder_past_key_values (`Cache`, *optional*):
            Pre-computed hidden-states (key and values in the self-attention blocks) that can be used to speed up sequential decoding of the decoder transformer.
            This typically consists in the `past_key_values` returned by the model at a previous stage of decoding, when `use_cache=True` or `config.use_cache=True`.
...
class MimiConv1d(nn.Module):
    ...
    def _get_extra_padding_for_conv1d(
        ...
        n_frames = ms.ceil(n_frames).to(ms.int64) - 1
...
class MimiLayerScale(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.scale = nn.Parameter(ms.full((channels,), initial_scale, requires_grad=True))
...
class MimiRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        with ms.autocast(device_type=device_type, enabled=False):
...
class MimiAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = ms.matmul(query_states, key_states.transpose(2, 3)) * self.scaling
...
class MimiSdpaAttention(MimiAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Reference: https://github.com/pyms/pyms/issues/112577.
        ...
        attn_output = ms.nn.functional.scaled_dot_product_attention(
            query_states,
            key_states,
            value_states,
...
class MimiTransformerLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Args:
            hidden_states (`ms.FloatTensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
            attention_mask (`ms.FloatTensor`, *optional*):
            ...
            use_cache (`bool`, *optional*):
                ...
                (see `past_key_values`).
            past_key_value (`Tuple(ms.FloatTensor)`, *optional*): cached past key and value projection states
            cache_position (`ms.LongTensor` of shape `(sequence_length)`, *optional*):
...
class MimiTransformerModel(nn.Module):
    ...
    def forward(
        self,
        hidden_states: ms.LongTensor = None,
        attention_mask: Optional[ms.Tensor] = None,
        position_ids: Optional[ms.LongTensor] = None,
        past_key_values: Optional[Union[Cache, List[ms.FloatTensor]]] = None,
        ...
        """
        Args:
            hidden_states (`ms.FloatTensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            ...
            position_ids (`ms.LongTensor` of shape `(batch_size, sequence_length)`, *optional*):
                Indices of positions of each input sequence tokens in the position embeddings. Selected in the range `[0,
                config.n_positions - 1]`.

                [What are position IDs?](../glossary#position-ids)
            past_key_values (`Cache` or `tuple(tuple(ms.FloatTensor))`, *optional*):
                ...
                - Tuple of `tuple(ms.FloatTensor)` of length `config.n_layers`, with each tuple having 2 tensors of
    ...
    def _update_causal_mask(
        ...
        if (
            ...
            # Details: https://github.com/pyms/pyms/issues/110213
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            diagonal_attend_mask = ms.arange(target_length, device=device) > cache_position.reshape(-1, 1)
            if config.sliding_window is not None:
                # if we have sliding window, we should not attend to tokens beyond sliding window length, so we mask them out also
                # the check is needed to verify is current checkpoint was trained with sliding window or not
                if not isinstance(past_key_values, SlidingWindowCache) or sequence_length > target_length:
                    sliding_attend_mask = ms.arange(target_length, device=device) <= (
...
class MimiEuclideanCodebook(nn.Module):
    ...
    def quantize(self, hidden_states):
        ...
        dists = ms.cdist(hidden_states[None], self.embed[None], p=2)[0]
...
class MimiResidualVectorQuantizer(nn.Module):
    ...
    def encode(self, embeddings: ms.Tensor, num_quantizers: Optional[int] = None) -> ms.Tensor:
        ...
        out_indices = ms.stack(all_indices)
        return out_indices
...
class MimiSplitResidualVectorQuantizer(nn.Module):
    ...
    def encode(self, embeddings: ms.Tensor, num_quantizers: Optional[float] = None) -> ms.Tensor:
        ...
        if num_quantizers > self.num_semantic_quantizers:
            ...
            codes = ms.cat([codes, acoustic_codes], dim=0)
...
MIMI_START_DOCSTRING = r"""
    ...
    This model is also a PyTorch [ms.nn.Module](https://pyms.org/docs/stable/nn.html#ms.nn.Module) subclass.
...
MIMI_INPUTS_DOCSTRING = r"""
    Args:
        input_values (`ms.FloatTensor` of shape `(batch_size, channels, sequence_length)`, *optional*):
