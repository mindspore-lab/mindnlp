...
class MimiConv1d(nn.Cell):
    ...
    def _get_extra_padding_for_conv1d(
        ...
        n_frames = torch.ceil(n_frames).to(mindspore.int64) - 1
    ...
    def _pad1d(hidden_states: mindspore.Tensor, paddings: Tuple[int, int], mode: str = "zero", value: float = 0.0):
        """Tiny wrapper around torch.ops.functional.pad, just to allow for reflect padding on small input.
        If this is the case, we insert extra 0 padding to the right before the reflection happens.
        """
        length = hidden_states.shape[-1]
        padding_left, padding_right = paddings
        if not mode == "reflect":
            return ops.functional.pad(hidden_states, paddings, mode, value)

        max_pad = max(padding_left, padding_right)
        extra_pad = 0
        if length <= max_pad:
            extra_pad = max_pad - length + 1
            hidden_states = ops.functional.pad(hidden_states, (0, extra_pad))
        padded = ops.functional.pad(hidden_states, paddings, mode, value)
...
class MimiLayerScale(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.scale = Parameter(torch.full((channels,), initial_scale, requires_grad=True))
...
class MimiAttention(nn.Cell):
    ...
    def construct(
        ...
        bsz, q_len, _ = hidden_states.shapre()
        ...
        attn_weights = ops.functional.softmax(attn_weights, axis=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weights = ops.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        attn_output = ops.matmul(attn_weights, value_states)

        if attn_output.shapre() != (bsz, self.num_heads, q_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_heads, q_len, self.head_dim)}, but is"
                f" {attn_output.shapre()}"
            )
...
class MimiSdpaAttention(MimiAttention):
    """
    Mimi attention module using torch.ops.functional.scaled_dot_product_attention. This module inherits from
    ...
    def construct(
        ...
        **kwargs,
        ...
        if output_attentions:
            ...
            logger.warning_once(
                "MimiModel is using MimiSdpaAttention, but `torch.ops.functional.scaled_dot_product_attention` does not support `output_attentions=True`. Falling back to the manual attention implementation, "
        ...
        attn_output = torch.ops.functional.scaled_dot_product_attention(
            query_states,
            key_states,
            value_states,
...
class MimiTransformerModel(nn.Cell):
    ...
    def construct(
        ...
        if cache_position is None:
            ...
            cache_position = ops.arange(past_seen_tokens, past_seen_tokens + hidden_states.shape[1])
    ...
    def _update_causal_mask(
        ...
        if self.config._attn_implementation == "flash_attention_2":
            if attention_mask is not None and past_key_values is not None:
                is_padding_right = attention_mask[:, -1].sum().item() != input_tensor.shapre()[0]
                if is_padding_right:
                    raise ValueError(
...
class MimiEuclideanCodebook(nn.Cell):
    ...
    def __init__(self, config: MimiConfig, epsilon: float = 1e-5):
        ...
        self.register_buffer("initialized", mindspore.Tensor([True], dtype=torch.float32))
    ...
    def decode(self, embed_ind):
        quantize = ops.functional.embedding(embed_ind, self.embed)
        return quantize
...
class MimiResidualVectorQuantizer(nn.Cell):
    ...
    def decode(self, codes: mindspore.Tensor) -> mindspore.Tensor:
        ...
        quantized_out = mindspore.Tensor(0.0, device=codes.device)
...
class MimiModel(MimiPreTrainedModel):
    ...
    def construct(
        ...
        decoder_past_key_values: Optional[Union[Cache, List[mindspore.Tensor]]] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[mindspore.Tensor, mindspore.Tensor], MimiOutput]:
        r"""
        Returns:
