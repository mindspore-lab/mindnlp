...
class RagModel(RagPreTrainedModel):
    ...
    def construct(
        ...
        if decoder_attention_mask is not None:
            decoder_attention_mask = decoder_attention_mask.to(mindspore.int32).repeat_interleave(n_docs, dim=0)
...
class RagSequenceForGeneration(RagPreTrainedModel):
    ...
    def get_nll(
        ...
        # shift tokens left
        target = ops.cat(
            [target[:, 1:], ops.full((target.shape[0], 1),self.config.generator.pad_token_id)], 1
        )
        ...
        seq_logprobs = ops.log_softmax(seq_logits, axis=-1).view(
            seq_logits.shape[0] // n_docs, n_docs, -1, seq_logits.shape[-1]
        )  # batch_size x n_docs x tgt_len x #vocab_size
        doc_logprobs = ops.log_softmax(doc_scores, axis=1).unsqueeze(-1).unsqueeze(-1)

        # RAG-sequence marginalization
        first_token_scores = seq_logprobs[:, :, :1, :]
        second_token_scores = seq_logprobs[:, :, 1:2, :]
        remainder = seq_logprobs[:, :, 2:, :]
        rag_logprobs = ops.cat([first_token_scores, second_token_scores + doc_logprobs, remainder], axis=2)

        # calculate loss
        target = target.unsqueeze(1).unsqueeze(-1).repeat(1, n_docs, 1, 1)
        assert target.dim() == rag_logprobs.dim()

        ll = rag_logprobs.gather_elements(dim=-1, index=target)
