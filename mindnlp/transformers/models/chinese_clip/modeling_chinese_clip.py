...
# Copied from transformers.models.bert.modeling_bert.BertEmbeddings with Bert->ChineseCLIPText zyftest
        ...
        if self.position_embedding_type in ('relative_key', 'relative_key_query'):
            ...
            if use_cache:
                ...
                position_ids_l = ops.arange(
                    query_length, dtype=ms.int64).view(-1, 1)
...
class ChineseCLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, ChineseCLIPVisionEmbeddings):
            factor = self.config.initializer_factor
        ...
        elif isinstance(cell, ChineseCLIPTextEmbeddings):
            word_embeddings = np.random.normal(0.0, self.config.initializer_range, cell.word_embeddings.weight.shape)
            if cell.word_embeddings.padding_idx:
                word_embeddings[cell.word_embeddings.padding_idx] = 0

            cell.word_embeddings.weight.set_data(ms.Tensor(word_embeddings, cell.word_embeddings.weight.dtype))

            token_type_embeddings = np.random.normal(0.0, self.config.initializer_range, cell.token_type_embeddings.weight.shape)
            if cell.token_type_embeddings.padding_idx:
                token_type_embeddings[cell.token_type_embeddings.padding_idx] = 0

            cell.token_type_embeddings.weight.set_data(ms.Tensor(token_type_embeddings, cell.token_type_embeddings.weight.dtype))

            position_embeddings = np.random.normal(0.0, self.config.initializer_range, cell.position_embeddings.weight.shape)
            if cell.position_embeddings.padding_idx:
                position_embeddings[cell.position_embeddings.padding_idx] = 0

            cell.position_embeddings.weight.set_data(ms.Tensor(position_embeddings, cell.position_embeddings.weight.dtype))

            # cell.word_embeddings.weight.data.initialize(
            #     Normal(self.config.initializer_range))
            # cell.position_embeddings.weight.data.initialize(
            #     Normal(self.config.initializer_range))
            # cell.token_type_embeddings.weight.data.initialize(
            #     Normal(self.config.initializer_range))

            # for embedding in [cell.word_embeddings, cell.position_embeddings, cell.token_type_embeddings]:
            #     if embedding.padding_idx is not None:
            #         embedding.weight.data[embedding.padding_idx].initialize(
            #             'zeros')

        elif isinstance(cell, ChineseCLIPVisionAttention):
            factor = self.config.initializer_factor
        ...
        elif isinstance(cell, ChineseCLIPVisionMLP):
            factor = self.config.initializer_factor
        ...
        elif isinstance(cell, ChineseCLIPModel):
...
class ChineseCLIPTextEncoder(nn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.CellList([ChineseCLIPTextLayer(config)
...
class ChineseCLIPVisionEncoder(nn.Cell):
    ...
    def __init__(self, config: ChineseCLIPConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([ChineseCLIPVisionLayer(
...
class ChineseCLIPTextModel(ChineseCLIPPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.pooler = ChineseCLIPTextPooler(
...
class ChineseCLIPVisionModel(ChineseCLIPPreTrainedModel):
    ...
    _no_split_modules = ["ChineseCLIPVisionEmbeddings",
...
class ChineseCLIPModel(ChineseCLIPPreTrainedModel):
    ...
    def construct(
        ...
        image_embeds = image_embeds / \
            image_embeds.norm(ord=2, dim=-1, keepdim=True)
