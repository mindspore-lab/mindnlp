...
def point_wise_feed_construct_network(d_model_size, dff):
...
class EncoderLayer(nn.Cell):
    def __init__(self, d_model_size, num_heads, dff, rate=0.1):
        ...
        self.ffn = point_wise_feed_construct_network(d_model_size, dff)
    ...
    def construct(
        ...
        if attention_mask is not None:
            ...
            attention_mask = (1.0 - attention_mask) * np.finfo(
                mindspore.dtype_to_nptype(self.dtype)
            ).min
...
class CTRLForSequenceClassification(CTRLPreTrainedModel):
    ...
    def construct(
        ...
        >>> num_labels = len(model.config.id2label)
        >>> labels = torch.nn.functional.one_hot(mindspore.Tensor([predicted_class_id]), num_classes=num_labels).to(
        ...     torch.float
        ... )
        >>> loss = model(**inputs, labels=labels).loss
...
__all__ = ["CTRLForSequenceClassification", "CTRLLMHeadModel", "CTRLModel"]
