...
class CTRLModel(CTRLPreTrainedModel):
    ...
    def forward(
        ...
        mask = ops.triu(ops.ones(seq_len + past_length, seq_len + past_length), 1).to(
            device
        )
...
class CTRLForSequenceClassification(CTRLPreTrainedModel):
    ...
    def forward(
        ...
        >>> mindspore.manual_seed(42)  # doctest: +IGNORE_RESULT
        ...
        >>> num_labels = len(model.config.id2label)
        >>> labels = mindspore.nn.functional.one_hot(mindspore.tensor([predicted_class_id]), num_classes=num_labels).to(
        ...     mindspore.float
        ... )
        >>> loss = model(**inputs, labels=labels).loss
