...
class MaskFormerPixelLevelModuleOutput(ModelOutput):
    ...
    decoder_last_hidden_state: Optional[mindspore.Tensor] = None
...
class MaskFormerModelOutput(ModelOutput):
    ...
    Args:
        ...
        transformer_decoder_last_hidden_state (`mindspore.Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
        ...
        hidden_states `tuple(mindspore.Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `mindspore.Tensor` containing `encoder_hidden_states`, `pixel_decoder_hidden_states` and
    ...
    encoder_last_hidden_state: Optional[mindspore.Tensor] = None
    pixel_decoder_last_hidden_state: Optional[mindspore.Tensor] = None
    transformer_decoder_last_hidden_state: Optional[mindspore.Tensor] = None
    encoder_hidden_states: Optional[Tuple[mindspore.Tensor]] = None
...
class MaskFormerForInstanceSegmentationOutput(ModelOutput):
    ...
    auxiliary_logits: mindspore.Tensor = None
    encoder_last_hidden_state: Optional[mindspore.Tensor] = None
    pixel_decoder_last_hidden_state: Optional[mindspore.Tensor] = None
    transformer_decoder_last_hidden_state: Optional[mindspore.Tensor] = None
    encoder_hidden_states: Optional[Tuple[mindspore.Tensor]] = None
...
def pair_wise_sigmoid_focal_loss(inputs: Tensor, labels: Tensor, alpha: float = 0.25, gamma: float = 2.0) -> Tensor:
    ...
    loss = ops.matmul(focal_pos, labels.T) + ops.matmul(focal_neg, (1 - labels).T)
...
class MaskFormerHungarianMatcher(nn.Module):
    ...
    def forward(self, masks_queries_logits, class_queries_logits, mask_labels, class_labels) -> List[Tuple[Tensor]]:
        ...
        for pred_probs, pred_mask, target_mask, labels in zip(preds_probs, preds_masks, mask_labels, class_labels):
            ...
            pred_mask_flat = pred_mask.flatten(start_dim=1)  # [num_queries, height*width]
            # same for target_mask "c h w -> c (h w)"
            target_mask_flat = target_mask[:, 0].flatten(start_dim=1)  # [num_total_labels, height*width]
...
class MaskFormerLoss(nn.Module):
    ...
    def loss_masks(
        ...
        pred_masks = pred_masks[:, 0].flatten(start_dim=1)
...
class MaskFormerTransformerModule(nn.Module):
    ...
    def forward(
        ...
        inputs_embeds = ops.zeros_like(queries_embeddings)
...
class MaskFormerForInstanceSegmentation(MaskFormerPreTrainedModel):
    ...
    def get_logits(self, outputs: MaskFormerModelOutput) -> Tuple[Tensor, Tensor, Dict[str, Tensor]]:
        ...
        if self.config.use_auxiliary_loss:
            stacked_transformer_decoder_outputs = ops.stack(outputs.transformer_decoder_hidden_states)
            ...
            binaries_masks = ops.einsum("lbqc, bchw -> lbqhw", mask_embeddings, pixel_embeddings)
            ...
            masks_queries_logits = ops.einsum("bqc, bchw -> bqhw", mask_embeddings, pixel_embeddings)
