...
class PatchTSTAttention(nn.Cell):
...
class PatchTSTBatchNorm(nn.Cell):
    """
    ...
    def construct(self, inputs: mindspore.Tensor):
        ...
        """
        output = inputs.swapaxes(1, 2)  # output: (batch_size, d_model, sequence_length)
...
def random_masking(
    ...
    ids_restore = ops.argsort(ids_shuffle, axis=-1)  # ids_restore: [bs x num_channels x L]
...
class PatchTSTPatchify(nn.Cell):
    """
    ...
    def construct(self, past_values: mindspore.Tensor):
        """
        Parameters:
...
class PatchTSTMasking(nn.Cell):
    ...
    def construct(self, patch_input: mindspore.Tensor):
        """
        Parameters:
...
class PatchTSTEncoderLayer(nn.Cell):
    """
    ...
    def __init__(self, config: PatchTSTConfig):
        ...
        self.dropout_path1 = nn.Dropout(p=config.path_dropout) if config.path_dropout > 0 else nn.Identity()
        if config.norm_type == "batchnorm":
            self.norm_sublayer1 = PatchTSTBatchNorm(config)
        elif config.norm_type == "layernorm":
            self.norm_sublayer1 = nn.LayerNorm([config.d_model], epsilon=config.norm_eps)
        else:
            raise ValueError(f"{config.norm_type} is not a supported norm layer type.")

        # Add & Norm of the sublayer 2
        if self.channel_attention:
            self.dropout_path2 = nn.Dropout(p=config.path_dropout) if config.path_dropout > 0 else nn.Identity()
            if config.norm_type == "batchnorm":
                self.norm_sublayer2 = PatchTSTBatchNorm(config)
            elif config.norm_type == "layernorm":
                self.norm_sublayer2 = nn.LayerNorm([config.d_model], epsilon=config.norm_eps)
            else:
                raise ValueError(f"{config.norm_type} is not a supported norm layer type.")

        # Position-wise Feed-Forward
        self.ff = nn.SequentialCell(
            nn.Dense(config.d_model, config.ffn_dim, has_bias=config.bias),
            nn.GELU(),
            # ACT2CLS[config.activation_function](),
            nn.Dropout(p=config.ff_dropout) if config.ff_dropout > 0 else nn.Identity(),
            nn.Dense(config.ffn_dim, config.d_model, has_bias=config.bias),
        )

        # Add & Norm of sublayer 3
        self.dropout_path3 = nn.Dropout(p=config.path_dropout) if config.path_dropout > 0 else nn.Identity()
        if config.norm_type == "batchnorm":
            self.norm_sublayer3 = PatchTSTBatchNorm(config)
        elif config.norm_type == "layernorm":
            self.norm_sublayer3 = nn.LayerNorm([config.d_model], epsilon=config.norm_eps)
        else:
            raise ValueError(f"{config.norm_type} is not a supported norm layer type.")

        self.pre_norm = config.pre_norm

    def construct(self, hidden_state: mindspore.Tensor, output_attentions: Optional[bool] = None):
        """
        Parameters:
        ...
        if self.channel_attention:
            ...
            hidden_state = hidden_state.swapaxes(2, 1)
            ...
            # hidden_state: [bs x num_channels x sequence_length x d_model]
            hidden_state = hidden_state.swapaxes(1, 2)
...
class PatchTSTPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        Initialize weights
        """
        if isinstance(cell, PatchTSTPositionalEncoding):
            # initialize cls_token
            if self.config.use_cls_token:
                cell.cls_token.set_data(initializer(Normal(sigma=0.02),
                                             cell.cls_token.shape, cell.cls_token.dtype))
            # initialize positional encoding
            if self.config.positional_encoding_type == "random":
                cell.position_enc.set_data(initializer(Normal(mean=0.0, sigma=0.02),
                                             cell.position_enc.shape, cell.position_enc.dtype))
        elif isinstance(cell, nn.LayerNorm):
            cell.weight.set_data(initializer('ones', cell.weight.shape, cell.weight.dtype))
            cell.bias.set_data(initializer('zeros', cell.bias.shape, cell.bias.dtype))
        elif isinstance(cell, PatchTSTBatchNorm):
            cell.batchnorm.bias.set_data(initializer('zeros', cell.batchnorm.bias.shape, cell.batchnorm.bias.dtype))
            cell.batchnorm.weight.set_data(initializer('ones', cell.batchnorm.weight.shape, cell.batchnorm.weight.dtype))
        elif isinstance(cell, (nn.Dense, nn.Conv1d)):
            cell.weight.set_data(initializer(Normal(mean=0.0, sigma=self.config.init_std),
...
class PatchTSTEmbedding(nn.Cell):
    def __init__(self, config: PatchTSTConfig):
        super().__init__()
        ...
        if self.share_embedding:
            self.input_embedding = nn.Dense(config.patch_length, config.d_model)
        else:
            self.input_embedding = nn.CellList()
            for _ in range(config.num_input_channels):
                self.input_embedding.append(nn.Dense(config.patch_length, config.d_model))
    ...
    def construct(self, patch_input: mindspore.Tensor):
        ...
        if self.share_embedding:
            ...
            embeddings = ops.stack(embeddings, axis=1)
        return embeddings


class PatchTSTPositionalEncoding(nn.Cell):
    """
    ...
    def __init__(self, config: PatchTSTConfig, num_patches: int):
        ...
        if config.use_cls_token:
            ...
            self.cls_token = Parameter(ops.zeros(1, 1, 1, config.d_model), 'cls_token')
            num_patches += 1
        # postional encoding: [num_patches x d_model]
        self.position_enc = self._init_pe(config, num_patches)
        # Positional dropout
        self.positional_dropout = (
            nn.Dropout(p=config.positional_dropout) if config.positional_dropout > 0 else nn.Identity()
        )

    @staticmethod
    def _init_pe(config: PatchTSTConfig, num_patches: int) -> Parameter:
        # Positional encoding
        if config.positional_encoding_type == "random":
            position_enc = Parameter(ops.randn(num_patches, config.d_model), 'position_enc', requires_grad=True)
            ...
            position_enc = Parameter(position_enc, 'position_enc', requires_grad=False)
        else:
            raise ValueError(
    ...
    def construct(self, patch_input: mindspore.Tensor):
        if self.use_cls_token:
            ...
            cls_tokens = cls_token.broadcast_to ((patch_input.shape[0], self.num_input_channels, -1, -1))
            # hidden_state: [bs x num_channels x (num_patches+1) x d_model]
            hidden_state = ops.cat((cls_tokens, patch_input), axis=2)
        else:
...
class PatchTSTEncoder(PatchTSTPreTrainedModel):
    ...
    def __init__(self, config: PatchTSTConfig, num_patches: int):
        ...
        # Encoder
        self.layers = nn.CellList([PatchTSTEncoderLayer(config) for i in range(config.num_hidden_layers)])
...
class PatchTSTStdScaler(nn.Cell):
    """
    ...
    def construct(
        ...
        denominator = ops.clamp(denominator, min=1.0, max=None)
...
class PatchTSTMeanScaler(nn.Cell):
    """
...
class PatchTSTNOPScaler(nn.Cell):
    """
    ...
    def construct(
        ...
        Returns:
            tuple of `mindspore.Tensor` of shapes
                ...
                `(batch_size, 1, num_input_channels)`)
        """
        scale = ops.ones_like(data, requires_grad=False).mean(dim=self.dim, keep_dims=self.keepdim)
        loc = ops.zeros_like(data, requires_grad=False).mean(dim=self.dim, keep_dims=self.keepdim)
        return data, loc, scale


class PatchTSTScaler(nn.Cell):
    def __init__(self, config: PatchTSTConfig):
        super().__init__()
...
class PatchTSTMaskPretrainHead(nn.Cell):
    """
    ...
    def __init__(self, config: PatchTSTConfig):
        ...
        self.linear = nn.Dense(config.d_model, config.patch_length)
        self.use_cls_token = config.use_cls_token

    def construct(self, embedding: mindspore.Tensor) -> mindspore.Tensor:
        """
        Parameters:
...
class PatchTSTForPretraining(PatchTSTPreTrainedModel):
    ...
    def construct(
        ...
        loss_val = ops.mse_loss(x_hat, model_output.patch_input, reduction="none" )
        masked_loss = (loss_val.mean(axis=-1) * model_output.mask).sum() / (model_output.mask.sum() + 1e-10)
...
class PatchTSTClassificationHead(nn.Cell):
    def __init__(self, config: PatchTSTConfig):
        super().__init__()
        self.use_cls_token = config.use_cls_token
        self.pooling_type = config.pooling_type
        self.flatten = nn.Flatten(start_dim=1)
        self.dropout = nn.Dropout(p=config.head_dropout) if config.head_dropout > 0 else nn.Identity()
        self.linear = nn.Dense(config.num_input_channels * config.d_model, config.num_targets)

    def construct(self, embedding: mindspore.Tensor):
        """
        Parameters:
            embedding (`mindspore.Tensor` of shape `(bs, num_channels, num_patches, d_model)` or
                     `(bs, num_channels, num_patches+1, d_model)` if `cls_token` is set to True, *required*):
        ...
        if self.use_cls_token:
            ...
            pooled_embedding = embedding.mean(axis=2)
...
class PatchTSTForClassification(PatchTSTPreTrainedModel):
    ...
    def construct(
        ...
        if target_values is not None:
            ...
            loss_val = ops.cross_entropy(y_hat, target_values)
...
class PatchTSTPredictionHead(nn.Cell):
        ...
        if not self.share_projection:
            ...
            for i in range(self.num_input_channels):
                ...
                if distribution_output is None:
                    # use linear head
                    self.projections.append(nn.Dense(head_dim, config.prediction_length))
                else:
                    # use distribution head
                    self.projections.append(distribution_output.get_parameter_projection(head_dim))
                self.dropouts.append(nn.Dropout(p=config.head_dropout) if config.head_dropout > 0 else nn.Identity())
        else:
            # all the channels share the same head
            self.flatten = nn.Flatten(start_dim=2)
            if distribution_output is None:
                # use linear head
                self.projection = nn.Dense(head_dim, config.prediction_length)
            else:
                # use distribution head
    ...
    def construct(self, embedding: mindspore.Tensor):
        ...
        if self.use_cls_token:
            ...
            if self.pooling_type == "mean":
                ...
                # pooled_embedding: [bs x num_channels x d_model]
                pooled_embedding = embedding.max(axis=2).values
            else:
        ...
        if isinstance(output, tuple):
            ...
            output = tuple(z.swapaxes(2, 1) for z in output)
        else:
            output = output.swapaxes(2, 1)  # [bs x forecast_len x num_channels]
        return output
...
class PatchTSTForPrediction(PatchTSTPreTrainedModel):
    ...
    def construct(
        ...
        if future_values is not None:
            if self.distribution_output:
                ...
                loss_val = ops.mse_loss(y_hat_out, future_values, reduction="mean")
    ...
    def generate(
        ...
        if self.distribution_output:
            ...
            samples = ops.stack(samples, axis=1)
        else:
...
class PatchTSTRegressionHead(nn.Cell):
    """
    Regression head
    """
    ...
    def __init__(self, config: PatchTSTConfig, distribution_output=None):
        ...
        if distribution_output is None:
            self.projection = nn.Dense(head_dim, config.num_targets)
        else:
            self.projection = distribution_output.get_parameter_projection(head_dim)
...
class PatchTSTForRegression(PatchTSTPreTrainedModel):
    ...
    def construct(
        ...
        if target_values is not None:
            if self.distribution_output:
                ...
                loss = ops.mse_loss(y_hat, target_values, reduction="mean")
    ...
    def generate(
        ...
        samples = ops.stack(samples, axis=1).view(-1, num_parallel_samples, self.config.num_targets)
        return SamplePatchTSTOutput(sequences=samples)
