...
from mindspore.dataset.vision import CenterCrop, Compose, Normalize, Resize, ToTensor
...
def convert_git_checkpoint(model_name, pytorch_dump_folder_path, push_to_hub=False):
    ...
    if "large" in model_name and not is_video and "large-r" not in model_name:
        ...
        param_dict = mindspore.load_checkpoint(model_name)
        mindspore.load_param_into_net(model, param_dict)

    # rename keys
    new_param_dict = {}
    for name, param in param_dict.items():
    # 示例：将 "module.encoder." 替换为 "encoder."
        if name.startswith("module.encoder."):
            new_name = name.replace("module.encoder.", "encoder.", 1)
    ...
    missing_keys = ["git.embeddings.position_ids", "git.image_encoder.vision_model.embeddings.position_ids"]
    for key in missing_keys:
        if key in model.parameters_dict():  # 检查模型中是否存在该参数
            print(f"Warning: Missing key '{key}' in checkpoint but exists in model. It will be initialized randomly.")
        else:
            print(f"Info: Missing key '{key}' in both checkpoint and model. It is not used.")

    # 处理意外的键
    unexpected_keys = ["git.image_encoder.visual_projection.weight"]
    for key in unexpected_keys:
        if key in param_dict:
            print(f"Warning: Unexpected key '{key}' in checkpoint. It will be ignored.")
            del param_dict[key]  # 从参数字典中移除
    ...
    if is_video:
        ...
        assert ops.allclose(pixel_values, original_pixel_values)

    input_ids = mindspore.tensor([[101]])
    outputs = model(input_ids, pixel_values=pixel_values)
    logits = outputs.logits
    print("Logits:", logits[0, -1, :3])

    if model_name == "git-base":
        expected_slice_logits = mindspore.tensor([-1.2832, -1.2835, -1.2840])
    elif model_name == "git-base-coco":
        expected_slice_logits = mindspore.tensor([-0.9925, -0.9930, -0.9935])
    elif model_name == "git-base-textcaps":
        expected_slice_logits = mindspore.tensor([-1.2980, -1.2983, -1.2985])
    elif model_name == "git-base-vqav2":
        expected_slice_logits = mindspore.tensor([-0.8570, -0.8568, -0.8561])
    elif model_name == "git-base-textvqa":
        expected_slice_logits = mindspore.tensor([-1.4085, -1.4083, -1.4082])
    elif model_name == "git-base-vatex":
        expected_slice_logits = mindspore.tensor([-1.3451, -1.3447, -1.3447])
    elif model_name == "git-base-msrvtt-qa":
        expected_slice_logits = mindspore.tensor([-0.8554, -0.8550, -0.8540])
    elif model_name == "git-large":
        expected_slice_logits = mindspore.tensor([-1.1708, -1.1707, -1.1705])
    elif model_name == "git-large-coco":
        expected_slice_logits = mindspore.tensor([-1.0425, -1.0423, -1.0422])
    elif model_name == "git-large-textcaps":
        expected_slice_logits = mindspore.tensor([-1.2705, -1.2708, -1.2706])
    elif model_name == "git-large-vqav2":
        expected_slice_logits = mindspore.tensor([-0.7042, -0.7043, -0.7043])
    elif model_name == "git-large-textvqa":
        expected_slice_logits = mindspore.tensor([-0.8590, -0.8592, -0.8590])
    elif model_name == "git-large-vatex":
        expected_slice_logits = mindspore.tensor([-1.0113, -1.0114, -1.0113])
    elif model_name == "git-large-msrvtt-qa":
        expected_slice_logits = mindspore.tensor([0.0130, 0.0134, 0.0131])
    elif model_name == "git-large-r":
        expected_slice_logits = mindspore.tensor([-1.1283, -1.1285, -1.1286])
    elif model_name == "git-large-r-coco":
        expected_slice_logits = mindspore.tensor([-0.9641, -0.9641, -0.9641])
    elif model_name == "git-large-r-textcaps":
        expected_slice_logits = mindspore.tensor([-1.1121, -1.1120, -1.1124])

    assert ops.allclose(logits[0, -1, :3], expected_slice_logits, atol=1e-4)
    print("Looks ok!")
