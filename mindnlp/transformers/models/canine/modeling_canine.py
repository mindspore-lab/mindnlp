...
class CanineModelOutputWithPooling(ModelOutput):
    ...
    last_hidden_state: mindspore.Tensor = None  # type: ignore
    pooler_output: mindspore.Tensor = None  # type: ignore
    ...
    def construct(
        ...
        if attention_mask is not None:
            if attention_mask.ndim == 3:
                ...
                attention_mask = (1.0 - attention_mask.float()) * finfo(
                    attention_scores.dtype, "min"
                )
...
class CanineModel(CaninePreTrainedModel):
    ...
    def _downsample_attention_mask(
        self, char_attention_mask: mindspore.Tensor, downsampling_rate: int
    ):
        ...
        poolable_char_mask = ops.reshape(
        ...
        pooled_molecule_mask = nn.MaxPool1d(
        ...
        molecule_attention_mask = ops.squeeze(pooled_molecule_mask, axis=-1)

        return molecule_attention_mask

    def _repeat_molecules(
        self, molecules: mindspore.Tensor, char_seq_length: mindspore.Tensor
    ) -> mindspore.Tensor:
        ...
        repeated = ops.repeat_interleave(
            molecules_without_extra_cls, repeats=rate, axis=-2
        )
        ...
        remainder_length = ops.fmod(
            mindspore.tensor(char_seq_length, dtype=mindspore.int32),
            mindspore.tensor(rate, dtype=mindspore.int32),
        ).item()
    ...
    def construct(
        ...
        extended_molecule_attention_mask: mindspore.Tensor = (
