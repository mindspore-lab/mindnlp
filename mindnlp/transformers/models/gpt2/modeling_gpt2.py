...
class GPT2Attention(nn.Cell):
    ...
    Methods:
        ...
        - `construct(hidden_states, layer_past=None, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, use_cache=False, output_attentions=False)`: Constructs the attention mechanism.
...
class GPT2Block(nn.Cell):
    ...
    - construct(self, hidden_states, layer_past=None, attention_mask=None, head_mask=None, encoder_hidden_states=None, encoder_attention_mask=None, use_cache=False, output_attentions=False): Performs the forward pass of the GPT2Block.
      - Parameters:
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def construct(
        ...
        Args:
            ...
            past_key_values (:obj:`Optional[Tuple[Tuple[mindspore.Tensor]]]`, `optional`): Tuple of :obj:`(layer_num, batch_size, num_heads, past_sequence_length, hidden_size)` tensors containing the previous hidden states (key and values of the attention blocks) if they were cached, used for faster decoding. Defaults to :obj:`None`.
            attention_mask (:obj:`Optional[mindspore.Tensor]`, `optional`): Mask to avoid performing attention on padding token indices. Mask values selected in ``[0, 1]`` where 1 indicates the token is not masked and 0 indicates the token is masked. Defaults to :obj:`None`.
            token_type_ids (:obj:`Optional[mindspore.Tensor]`, `optional`): Input tensor of shape :obj:`(batch_size, sequence_length)` indicating the token types to differentiate between different sentences in the input. Defaults to :obj:`None`.
            ...
            head_mask (:obj:`Optional[mindspore.Tensor]`, `optional`): Mask to nullify selected heads of the self-attention modules. Mask values selected in ``[0, 1]``, where 1 indicates the head is kept and 0 indicates the head is nullified. Defaults to :obj:`None`.
            ...
            encoder_hidden_states (:obj:`Optional[mindspore.Tensor]`, `optional`): The encoded input sequence of shape :obj:`(batch_size, sequence_length, hidden_size)` to be used in the cross-attention layer. Defaults to :obj:`None`.
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    This class provides methods for initializing the model, getting and setting the output embeddings, preparing inputs for generation, and constructing the model. It also includes a static method for reordering the cache when using beam search or beam sampling.
    ...
    Methods:
        ...
        - prepare_inputs_for_generation(self, input_ids, past_key_values=None, inputs_embeds=None, **kwargs): Prepares inputs for generation by adjusting the input_ids, token_type_ids, attention_mask, and position_ids.
        - construct(self, input_ids, past_key_values=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None, encoder_hidden_states=None, encoder_attention_mask=None, labels=None, use_cache=None, output_attentions=None, output_hidden_states=None, return_dict=None): Constructs the GPT2LMHeadModel and returns the model outputs.
...
class GPT2DoubleHeadsModel(GPT2PreTrainedModel):
    ...
    The class includes methods for initializing the model, setting and getting output embeddings, preparing inputs for text generation, and constructing the model for inference or training. It also provides a method for reordering cache during beam search or beam sampling.
...
class GPT2ForSequenceClassification(GPT2PreTrainedModel):
    ...
    The GPT2ForSequenceClassification class provides a method 'construct' for constructing the sequence classification model. The 'construct' method accepts input tensors such as input_ids, past_key_values, attention_mask, token_type_ids, position_ids, head_mask, inputs_embeds, labels, use_cache, output_attentions, output_hidden_states, and return_dict.
    
    The 'construct' method returns a tuple containing the sequence classification loss, logits, past_key_values, hidden_states, and attentions. If the return_dict parameter is set to False, the output is a tuple of pooled_logits and transformer_outputs. The sequence classification loss is computed based on the given labels and the model configuration.
...
class GPT2ForTokenClassification(GPT2PreTrainedModel):
    ...
    """
    This class represents a GPT-2 model for token classification, inheriting from GPT2PreTrainedModel. It includes methods for initialization and construction of the model for token classification tasks. The model utilizes a transformer architecture with configurable dropout and classifier layers for classification or regression loss computation based on the number of labels specified in the configuration. The construct method processes input data through the transformer, applies dropout, generates logits using the classifier layer, and computes the loss if labels are provided. The method returns the loss and output based on the specified return format. 
    """
    def __init__(self, config):
