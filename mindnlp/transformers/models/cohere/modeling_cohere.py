...
def _get_unpad_data(attention_mask):
    ...
    indices = ops.flatten(ops.nonzero(attention_mask.flatten()), start_dim=0)
    max_seqlen_in_batch = seqlens_in_batch.max().item()
    cu_seqlens = ops.pad(ops.cumsum(seqlens_in_batch, dim=0, dtype=ms.int32), (1, 0))
    return (
        indices,
        cu_seqlens,
        max_seqlen_in_batch,
...
class CohereLayerNorm(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        mean = ops.ReduceMean(True)(hidden_states, -1)
...
class CohereRotaryEmbedding(nn.Cell):
    ...
    def construct(self, x, position_ids):
        if self.inv_freq is None:
            self.inv_freq = 1.0 / (
                self.base ** (ops.arange(0, self.dim, 2, dtype=ms.int64).float() / self.dim)
            )
...
def rotate_half(x):
    ...
    x1 = x[..., ::2]
    x2 = x[..., 1::2]
    rot_x = ops.flatten(ops.stack([-x2, x1], axis=-1), start_dim=-2)
    return rot_x
...
class CohereAttention(nn.Cell):
    ...
    def construct(
        ...
        **kwargs,
        ...
        query_states = P.Reshape()(query_states, (bsz, q_len, self.num_heads, self.head_dim))
        key_states = P.Reshape()(key_states, (bsz, q_len, self.num_key_value_heads, self.head_dim))
        if self.use_qk_norm:
        ...
        value_states = P.Reshape()(value_states, (bsz, q_len, self.num_key_value_heads, self.head_dim)).swapaxes(1, 2)
        ...
        if P.Shape()(attn_output) != (bsz, self.num_heads, q_len, self.head_dim):
            raise ValueError(
                f"`attn_output` should be of size {(bsz, self.num_heads, q_len, self.head_dim)}, but is"
                f" {P.Shape()(attn_output)}"
            )

        attn_output = attn_output.swapaxes(1, 2)

        attn_output = P.Reshape()(attn_output, (bsz, q_len, self.hidden_size,))
...
class CohereModel(CoherePreTrainedModel):
    ...
    def _update_causal_mask(self, attention_mask, input_tensor):
        ...
        if attention_mask is not None and attention_mask.dim() == 2:
            ...
            padding_mask = causal_mask[..., :mask_length].eq(0.0).astype(ms.int32) * attention_mask[:, None,
                                                                                            None, :].eq(0.0).astype(
                ms.int32)
            causal_mask[..., :mask_length] = causal_mask[..., :mask_length].masked_fill(
                padding_mask.astype(ms.bool_), finfo(dtype, 'min')
            )
...
class CohereForCausalLM(CoherePreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            shift_logits = P.Reshape()(shift_logits, (-1, self.config.vocab_size))
            shift_labels = P.Reshape()(shift_labels, (-1))
