...
class SPTokenizer:
    ...
    def tokenize(self, s: str, encode_special_tokens=False):
        ...
        Returns:
            list: A list of tokenized pieces generated by the SentencePiece model. If encode_special_tokens is False, the method returns the tokenized pieces of the input string 's'. If encode_special_tokens is True, the method returns the tokenized pieces with special tokens encoded.
...
class ChatGLM3Tokenizer(PreTrainedTokenizer):
    ...
    """
    The 'ChatGLM3Tokenizer' class represents a tokenizer for a chat model that inherits from PreTrainedTokenizer. It provides methods for tokenizing, converting tokens to IDs, converting IDs to tokens, building model inputs with special tokens, padding, and saving the vocabulary. The class also provides methods for constructing chat inputs, building single messages, and extracting prefix tokens. Additionally, it offers properties for accessing special tokens and their IDs, as well as the vocabulary size. Furthermore, it provides a method for converting tokens to a string.
    ...
    def pad_token(self, value):
        ...
        This method sets the pad_token value for the ChatGLM3Tokenizer object. The pad_token value is used during tokenization to represent padding tokens. If this method is called, a warning message will be logged indicating that setting pad_token is not supported and the default pad_token value will be used instead.
    ...
    def build_chat_input(self, query, history=None, role="user"):
        ...
        Args:
            ...
            history (list): A list of dictionaries representing the chat history. Each dictionary should have the keys 'role' (str), 'metadata' (str), and 'content' (str). The 'role' key specifies the role of the participant in the conversation (either 'user' or 'system'). The 'metadata' key contains optional metadata for the message. The 'content' key contains the actual text content of the message.
