...
def _prepare_cross_attention_mask(
    cross_attention_mask: mindspore.Tensor,
    ...
    cross_attention_mask = inverted_cross_attn_mask.masked_fill(
        inverted_cross_attn_mask.to(mindspore.bool_), float(ops.finfo(dtype).min)
    )

    # apply full-row bias, which return 4D tensor of shape [B, H, S1, 1] where value is 0 if the a full row in cross attn mask's
    # last dimension contains negative infinity values, otherwise it's 1
    negative_inf_value = float(ops.finfo(dtype).min)
    full_text_row_masked_out_mask = (
        (cross_attention_mask != negative_inf_value).any(-1).type_as(cross_attention_mask)[..., None]
    )
...
def _prepare_aspect_ratio_attention_mask(
    aspect_ratio_mask: mindspore.Tensor,
    num_patches: int,
    target_length: int,
    dtype: mindspore.dtype,
) -> mindspore.Tensor:
    ...
    attention_mask = attention_mask @ ops.transpose(attention_mask, -1, -2) * float(ops.finfo(dtype).min)
...
class MllamaPrecomputedAspectRatioEmbedding(nn.Module):
    def __init__(self, config: MllamaVisionConfig, is_gated: bool = True):
        ...
        if is_gated:
            self.gate = nn.Parameter(ops.zeros(1))

    def forward(self, hidden_state: mindspore.Tensor, aspect_ratio_ids: mindspore.Tensor) -> mindspore.Tensor:
...
class MllamaPrecomputedPositionEmbedding(nn.Module):
    def __init__(self, config: MllamaVisionConfig):
        ...
        # position embedding
        position_embedding = ops.randn(self.num_patches, self.hidden_size)
...
class MllamaVisionAttention(nn.Module):
    ...
    def forward(
        ...
        query = ops.transpose(query.view(batch_size, q_seq_len, self.num_heads, self.head_dim), 1, 2)
        key = ops.transpose(key.view(batch_size, kv_seq_len, self.num_heads, self.head_dim), 1, 2)
        value = ops.transpose(value.view(batch_size, kv_seq_len, self.num_heads, self.head_dim), 1, 2)

        attn_weights = ops.matmul(query, ops.transpose(key, 2, 3)) / math.sqrt(self.head_dim)
        ...
        attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=mindspore.float32).to(query.dtype)
...
MLLAMA_VISION_ATTENTION_CLASSES = {"eager": MllamaVisionAttention}
...
class MllamaVisionEncoderLayer(nn.Module):
    def __init__(self, config: MllamaVisionConfig, is_gated: bool = False):
        ...
        if is_gated:
            self.gate_attn = nn.Parameter(ops.ones(1) * math.pi / 4)
            self.gate_ffn = nn.Parameter(ops.ones(1) * math.pi / 4)
...
class MllamaTextCrossAttention(nn.Module):
    ...
    def forward(
        self,
        hidden_states: mindspore.Tensor,
        cross_attention_states: Optional[mindspore.Tensor] = None,
        ...
        if cross_attention_states is not None:
            ...
            key_states = ops.transpose(key_states.view(bsz, -1, self.num_key_value_heads, self.head_dim), 1, 2)
            value_states = ops.transpose(value_states.view(bsz, -1, self.num_key_value_heads, self.head_dim), 1, 2)
...
class MllamaTextSelfAttention(nn.Module):
    ...
    def forward(
        ...
        hidden_states: mindspore.Tensor,
        attention_mask: mindspore.Tensor,
        position_embeddings: mindspore.Tensor,
        output_attentions: bool = False,
...
class MllamaSelfAttentionDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states: mindspore.Tensor,
        cross_attention_states: Optional[mindspore.Tensor] = None,
        cross_attention_mask: Optional[mindspore.Tensor] = None,
        attention_mask: Optional[mindspore.Tensor] = None,
        full_text_row_masked_out_mask: Optional[Tuple[mindspore.Tensor, mindspore.Tensor]] = None,
...
class MllamaCrossAttentionDecoderLayer(nn.Module):
    ...
    def __init__(self, config: MllamaTextConfig, layer_idx: int) -> None:
        ...
        self.input_layernorm = MllamaTextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.cross_attn_attn_gate = nn.Parameter(ops.zeros(1))

        self.mlp = MllamaTextMLP(config)
        self.post_attention_layernorm = MllamaTextRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.cross_attn_mlp_gate = nn.Parameter(ops.zeros(1))

    def forward(
        self,
        hidden_states: mindspore.Tensor,
        cross_attention_states: mindspore.Tensor,
        cross_attention_mask: mindspore.Tensor,
        attention_mask: mindspore.Tensor,
        full_text_row_masked_out_mask: Tuple[mindspore.Tensor, mindspore.Tensor],
        position_ids: Optional[mindspore.Tensor] = None,
        past_key_value: Optional[Cache] = None,
        output_attentions: Optional[bool] = False,
        use_cache: Optional[bool] = False,
...
class MllamaPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, nn.Conv2d)):
            ...
            nn.init.normal_(module.data, mean=0.0, std=std)
...
class MllamaVisionModel(MllamaPreTrainedModel):
    ...
    def __init__(self, config: MllamaVisionConfig):
        ...
        self.class_embedding = nn.Parameter(self.scale * ops.randn(self.hidden_size))
    ...
    def apply_class_embedding(self, hidden_state: mindspore.Tensor) -> mindspore.Tensor:
        batch_size, _, hidden_size = hidden_state.shape
        class_embedding = self.class_embedding.broadcast_to((batch_size, 1, hidden_size))
        hidden_state = ops.cat([class_embedding, hidden_state], dim=1)
        return hidden_state

    def forward(
        self,
        pixel_values: mindspore.Tensor,
        aspect_ratio_ids: mindspore.Tensor,
        aspect_ratio_mask: mindspore.Tensor,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        ...
        hidden_state = ops.transpose(ops.flatten(patch_embeds, 2), 1, 2)
        ...
        intermediate_hidden_states = ops.stack(all_intermediate_hidden_states, dim=-1)
        ...
        hidden_state = ops.cat([hidden_state, intermediate_hidden_states], dim=-1)
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        aspect_ratio_mask: Optional[mindspore.Tensor] = None,
        aspect_ratio_ids: Optional[mindspore.Tensor] = None,
        attention_mask: Optional[mindspore.Tensor] = None,
        cross_attention_mask: Optional[mindspore.Tensor] = None,
        cross_attention_states: Optional[mindspore.Tensor] = None,
        position_ids: Optional[mindspore.Tensor] = None,
    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs, is_encoder_decoder, **kwargs):
        ...
        if cross_attention_mask_prev is not None:
            model_kwargs["cross_attention_mask"] = ops.cat(
