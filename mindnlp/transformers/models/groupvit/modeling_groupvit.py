...
def hard_softmax(logits: ms.Tensor, dim: int):
    ...
    # Straight through zyftest.
    index = ops.max(y_soft, dim, keepdims=True)[1]
    y_hard = ops.tensor_scatter_elements(ops.zeros_like(logits), axis=dim,
                                         indices=index, updates=ms.Tensor(np.ones(index.shape), dtype=logits.dtype))
...
def get_grouping_from_attentions(attentions, hw_shape):
    ...
    for attn_masks in attentions:
        ...
        cur_attn_map = resize_attention_map(
            prev_attn_masks.permute(0, 2, 1), *hw_shape)
...
class GroupViTAssignAttention(nn.Cell):
    ...
    def get_attn(self, attn, gumbel=True, hard=True):
        if gumbel and self.training:
            attn = ops.gumbel_softmax(attn, dim=len(
                attn.shape)-2, hard=hard, tau=float(1.0))
        else:
            if hard:
...
class GroupViTTokenAssign(nn.Cell):
    ...
    def construct(self, image_tokens, group_tokens):
        ...
        new_image_tokens = new_image_tokens + \
            self.mlp_channels(self.norm_new_x(new_image_tokens))
...
class GroupViTModelOutput(ModelOutput):
    ...
    segmentation_logits: ms.Tensor = None
...
class GroupViTPatchEmbeddings(nn.Cell):
    ...
    def __init__(
        ...
        self.projection = nn.Conv2d(
            num_channels, embed_dim, kernel_size=patch_size, stride=patch_size, has_bias=True)
...
class GroupViTVisionEmbeddings(nn.Cell):
    def __init__(self, config: GroupViTVisionConfig):
        ...
        self.position_embeddings = ms.Parameter(
            ops.zeros(1, num_patches, config.hidden_size))
    ...
    def interpolate_pos_encoding(self, embeddings: ms.Tensor, height: int, width: int) -> ms.Tensor:
        ...
        scale_factor = (feat_height / original_height,
...
class GroupViTStage(nn.Cell):
    ...
    def __init__(
        ...
        if num_group_token > 0:
            self.group_token = ms.Parameter(
                ops.zeros(1, num_group_token, config.hidden_size))
        else:
        ...
        if num_prev_group_token > 0 and num_group_token > 0:
            self.group_projector = nn.SequentialCell([
                ...
                GroupViTMixerMLP(config, num_prev_group_token,
                                 config.hidden_size // 2, num_group_token),
            ])
        else:
        ...
        if self.with_group_token:
            ...
            if self.group_projector is not None:
                group_token = group_token + \
                    self.group_projector(prev_group_token)
        else:
            group_token = None
...
class GroupViTPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, (nn.Dense, nn.Conv2d)):
            ...
            cell.weight.data.initialize(Normal(init_range))
        ...
        self.stages = nn.CellList(
            [
                GroupViTStage(
                    ...
                    num_prev_group_token=config.num_output_groups[i -
                                                                  1] if i > 0 else 0,
                )
...
class GroupViTTextEncoder(nn.Cell):
    ...
    def __init__(self, config: GroupViTTextConfig):
        super().__init__()
        self.config = config
        self.layers = nn.CellList([GroupViTEncoderLayer(
    ...
    def construct(
        ...
        if self.eos_token_id == 2:
            ...
            pooled_output = last_hidden_state[
                ...
                input_ids.astype(dtype=ms.int32).argmax(axis=-1),
            ]
        else:
            ...
            pooled_output = last_hidden_state[
                ...
                (input_ids.astype(dtype=ms.int32) == self.eos_token_id)
                .astype(ms.int32)
                .argmax(axis=-1),
            ]
...
class GroupViTModel(GroupViTPreTrainedModel):
    ...
    def __init__(self, config: GroupViTConfig):
        ...
        self.visual_projection = nn.SequentialCell([
            nn.Dense(self.vision_embed_dim,
                     self.projection_intermediate_dim, has_bias=True),
            nn.BatchNorm1d(self.projection_intermediate_dim),
            nn.ReLU(),
            nn.Dense(self.projection_intermediate_dim,
                     self.projection_dim, has_bias=True),
        ])
        self.text_projection = nn.SequentialCell([
            nn.Dense(self.text_embed_dim,
                     self.projection_intermediate_dim, has_bias=True),
            nn.BatchNorm1d(self.projection_intermediate_dim),
            nn.ReLU(),
            nn.Dense(self.projection_intermediate_dim,
    ...
    def construct(
        ...
        if output_segmentation:
            ...
            # normalized features
            image_group_embeds = image_group_embeds / \
                image_group_embeds.norm(dim=-1, keepdim=True)
            # [batch_size_image x num_group, batch_size_text]
            logits_per_image_group = ops.matmul(
                image_group_embeds, text_embeds.t()) * logit_scale
            ...
            seg_logits = ops.matmul(
                logits_per_image_group, flatten_grouping) * logit_scale
