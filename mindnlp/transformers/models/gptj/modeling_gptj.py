...
def _get_unpad_data(attention_mask):
    ...
    indices = ms.nonzero(attention_mask.flatten(), as_tuple=False).flatten()
...
def create_sinusoidal_positions(num_pos: int, dim: int) -> ms.Tensor:
    inv_freq = 1.0 / (10000 ** (ops.arange(0, dim, 2, dtype=ms.float32) / dim))
    sinusoid_inp = ops.einsum("i , j -> i j", ops.arange(num_pos, dtype=ms.int64).float(), inv_freq).float()
...
class GPTJAttention(nn.Cell):
    def __init__(self, config):
        ...
        self.causal_mask = ops.tril(ops.ones((max_positions, max_positions), dtype=ms.bool_)).view(
                1, 1, max_positions, max_positions)
        self.masked_bias = ms.Tensor(-1e9, ms.float32)
        ...
        self.scale_attn = ops.sqrt(ms.Tensor(self.head_dim, dtype=ms.float32)).to(ms.float32)
    ...
    def construct(
        ...
        # print("hello")
        # print(embed_positions.shape)
        # print(position_ids.shape)


        repeated_position_ids = position_ids.unsqueeze(-1).repeat(1, 1, embed_positions.shape[-1])
        # sincos = ops.gather(embed_positions, repeated_position_ids, 1)
        sincos = ops.gather_elements(
                    embed_positions, dim=1, index=repeated_position_ids)
        sin, cos = ops.split(sincos, sincos.shape[-1] // 2, axis=-1)

        # print("hello2")
        # print(embed_positions.shape)
        # print(repeated_position_ids.shape)
        # print(sincos.shape)
        # print(sin.shape, cos.shape)
