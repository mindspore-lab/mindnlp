...
import numpy as np
import mindspore
from mindnlp.core import nn, ops, Parameter
from mindnlp.core.nn import BCEWithLogitsLoss, CrossEntropyLoss, MSELoss
from mindnlp.core.nn.init import initializer, _calculate_fan_in_and_fan_out
...
class SiglipVisionEmbeddings(nn.Module):
    ...
    def interpolate_pos_encoding(self, embeddings: mindspore.Tensor, height: int, width: int) -> mindspore.Tensor:
        ...
        if num_patches == num_positions and height == width: # and not mindspore.jit.is_tracing()
    ...
    def forward(self, pixel_values: mindspore.Tensor, interpolate_pos_encoding=False) -> mindspore.Tensor:
        ...
        embeddings = ops.transpose(ops.flatten(patch_embeds, 2), 1, 2)
...
class SiglipPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SiglipVisionEmbeddings):
            ...
            module.logit_scale.assign_value(ops.log(initializer('ones', module.logit_scale.shape, module.logit_scale.dtype)))
...
class SiglipMultiheadAttentionPoolingHead(nn.Module):
    ...
    def forward(self, hidden_state):
        ...
        probe = self.probe.repeat(batch_size, axis=0)
...
class SiglipModel(SiglipPreTrainedModel):
    ...
    def forward(
        ...
        # normalized features
        image_embeds = image_embeds / mindspore.ops.norm(image_embeds, 2, dim=-1, keepdim=True)
        text_embeds = text_embeds / mindspore.ops.norm(text_embeds, 2, dim=-1, keepdim=True)

        # cosine similarity as logits
        logits_per_text = (
            ops.matmul(text_embeds, image_embeds.t()) * ops.exp(self.logit_scale)
            + self.logit_bias
        )
...
class SiglipForImageClassification(SiglipPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = ops.mean(sequence_output, dim=1)
        # apply classifier
