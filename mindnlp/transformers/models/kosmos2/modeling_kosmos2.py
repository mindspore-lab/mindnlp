...
#     return inverted_mask.masked_fill(inverted_mask.to(mindspore.bool_),finfo(dtype).min)
...
class Kosmos2TextTransformer(nn.Cell):
    ...
    def construct(
        ...
        hidden_states = self.construct_embedding(
            input_ids=input_ids,
            inputs_embeds=inputs_embeds,
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, Kosmos2VisionEmbeddings):
            ...
            if cell.embed_tokens.padding_idx is not None:
                weights = np.zeros(shape=cell.embed_tokens.weight.data[cell.embed_tokens.padding_idx].shape)

                cell.embed_tokens.weight.data[cell.embed_tokens.padding_idx] =Tensor(weights,cell.embed_tokens.weight.data[cell.embed_tokens.padding_idx].dtype)
...
class Kosmos2TextForCausalLM(Kosmos2PreTrainedModel):
    ...
    def __init__(self, config: Kosmos2TextConfig):
        ...
        self.model = Kosmos2TextTransformer(config)
        self.lm_head = nn.Dense(in_channels=config.embed_dim, out_channels=config.vocab_size, has_bias=False)
    ...
    def construct(
        ...
        if labels is not None:
            ...
            # Flatten the tokens
            loss = ops.cross_entropy(shift_logits.view(batch_size * seq_length, vocab_size), shift_labels.view(batch_size * seq_length))
...
class Kosmos2ImageToTextProjection(nn.Cell):
    ...
    def __init__(self, config: Kosmos2Config):
        super().__init__()
        self.dense = nn.Dense(config.vision_config.hidden_size, config.text_config.embed_dim)
