...
from .mhaf import multi_head_attention_forward
...
class WavLMAttention(nn.Cell):
    ...
    def __init__(
        ...
        self.gru_rel_pos_const = Parameter(ops.ones((1, self.num_heads, 1, 1)))
    ...
    def construct(
        ...
        attn_output, attn_weights = self.multi_head_self_attention(
    ...
    def multi_head_self_attention(
        self,
        hidden_states: ms.Tensor,
        attention_mask: Union[ms.Tensor, ms.Tensor],
        gated_position_bias: ms.Tensor,
    ...
    def compute_bias(self, query_length: int, key_length: int) -> ms.Tensor:
    ...
    def _relative_positions_bucket(self, relative_positions: ms.Tensor) -> ms.Tensor:
        num_buckets = self.num_buckets // 2

        relative_buckets = (relative_positions > 0).to(ms.int64) * num_buckets
        ...
        relative_position_if_large = (max_exact + relative_positions_if_large).to(ms.int64)

        relative_position_if_large = ops.stack([relative_position_if_large, ops.full_like(relative_position_if_large, num_buckets - 1)])
        relative_position_if_large = ops.min(relative_position_if_large, axis=0)[0]
...
class WavLMGumbelVectorQuantizer(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        if self.training:
            ...
            codevector_probs = ops.gumbel_softmax(hidden_states.float(), tau=float(self.temperature), hard=True)
...
class WavLMPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, WavLMGumbelVectorQuantizer):
            cell.weight_proj.weight.set_data(
                initializer(Normal(1.0), cell.weight_proj.weight.shape, cell.weight_proj.weight.dtype))
            cell.weight_proj.bias.set_data(
                initializer('zeros', cell.weight_proj.bias.shape, cell.weight_proj.bias.dtype))
...
class WavLMModel(WavLMPreTrainedModel):
    def __init__(self, config: WavLMConfig):
        ...
        if config.mask_time_prob > 0.0 or config.mask_feature_prob > 0.0:
            self.masked_spec_embed = Parameter(initializer(Uniform(), (config.hidden_size,), dtype=ms.float32))
...
class WavLMForCTC(WavLMPreTrainedModel):
    ...
    def construct(
        ...
        if labels is not None:
            ...
            attention_mask = (
                attention_mask if attention_mask is not None else ops.ones_like(input_values, dtype=ms.int64)
            )
...
class WavLMForXVector(WavLMPreTrainedModel):
    ...
    def freeze_base_model(self):
        ...
        be updated during training. Only the classification head will be updated.
        """
        for named, param in self.wavlm.parameters_and_names():
            param.requires_grad = False
