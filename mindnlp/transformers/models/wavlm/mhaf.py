...
linear = mindspore.ops.function.nn_func.linear
scaled_dot_product_attention = mindspore.ops.function.nn_func._scaled_dot_product_attention
pad = mindspore.ops.function.nn_func.pad


def _mha_shape_check(query: ms.Tensor, key: ms.Tensor, value: ms.Tensor,
                     key_padding_mask: Optional[ms.Tensor], attn_mask: Optional[ms.Tensor], num_heads: int):
...
def _canonical_mask(
    ...
    if mask is not None:
        ...
        _mask_is_float = mindspore.ops.is_floating_point(mask)
...
def _none_or_dtype(input: Optional[ms.Tensor]):
    if input is None:
        return None
...
def multi_head_attention_forward(
    ...
    if need_weights:
        ...
        attn_output = scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, is_causal, training, q.dtype)[0]
