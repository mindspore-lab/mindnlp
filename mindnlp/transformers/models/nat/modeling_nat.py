...
class NatPatchEmbeddings(nn.Cell):
    ...
    def construct(self, pixel_values: Optional[mindspore.Tensor]) -> mindspore.Tensor:
...
class NatLayer(nn.Cell):
    def __init__(self, config, dim, num_heads, drop_path_rate=0.0):
        ...
        self.layer_scale_parameters = (
            Parameter(config.layer_scale_init_value * ops.ones((2, dim)), requires_grad=True)
...
class NatForImageClassification(NatPreTrainedModel):
    def __init__(self, config):
        ...
        # Classifier head
        self.classifier = (
            nn.Dense(self.nat.num_features, config.num_labels) if config.num_labels > 0 else nn.Identity()
        )
