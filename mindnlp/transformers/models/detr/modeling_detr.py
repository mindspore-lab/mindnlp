...
class DetrObjectDetectionOutput(ModelOutput):
    ...
    Args:
        loss (`mindspore.Tensor` of shape `(1,)`, *optional*, returned when `labels` are provided)):
            Total loss as a Dense combination of a negative log-likehood (cross-entropy) for class prediction and a
            bounding box loss. The latter is defined as a Dense combination of the L1 loss and the generalized
...
class DetrConvEncoder(nn.Cell):
    ...
    def construct(self, pixel_values: mindspore.Tensor, pixel_mask: mindspore.Tensor):
        ...
        for feature_map in features:
            ...
            mask = ops.interpolate(pixel_mask[None].float(), size=feature_map.shape[-2:]).to(ops.bool)[0]
...
class DetrSinePositionEmbedding(nn.Cell):
    ...
    def __init__(self, embedding_axis=64, temperature=10000, normalize=False, scale=None):
        super().__init__()
    ...
    def construct(self, pixel_values, pixel_mask):
        ...
        y_embed = pixel_mask.cumsum(1, dtype=ops.float32)
        x_embed = pixel_mask.cumsum(2, dtype=ops.float32)
        if self.normalize:
            y_embed = y_embed / (y_embed[:, -1:, :] + 1e-6) * self.scale
            x_embed = x_embed / (x_embed[:, :, -1:] + 1e-6) * self.scale

        axis_t = ops.arange(self.embedding_axis, dtype=ops.int64).float()
        axis_t = self.temperature ** (2 * ops.div(axis_t, 2, rounding_mode="floor") / self.embedding_axis)

        pos_x = x_embed[:, :, :, None] / axis_t
        pos_y = y_embed[:, :, :, None] / axis_t
...
class DetrLearnedPositionEmbedding(nn.Cell):
    ...
    def __init__(self, embedding_axis=256):
        super().__init__()
        self.row_embeddings = nn.Embedding(50, embedding_axis)
        self.column_embeddings = nn.Embedding(50, embedding_axis)
...
class DetrAttention(nn.Cell):
    ...
    def __init__(
        ...
        super().__init__()
        self.embed_axis = embed_axis
        self.num_heads = num_heads
        self.dropout = dropout
        self.head_axis = embed_axis // num_heads
        if self.head_axis * num_heads != self.embed_axis:
            raise ValueError(
                f"embed_axis must be divisible by num_heads (got `embed_axis`: {self.embed_axis} and `num_heads`:"
                f" {num_heads})."
            )
        self.scaling = self.head_axis**-0.5

        self.k_proj = nn.Dense(embed_axis, embed_axis, has_bias=bias)
        self.v_proj = nn.Dense(embed_axis, embed_axis, has_bias=bias)
        self.q_proj = nn.Dense(embed_axis, embed_axis, has_bias=bias)
        self.out_proj = nn.Dense(embed_axis, embed_axis, has_bias=bias)

    def _shape(self, tensor: mindspore.Tensor, seq_len: int, batch_size: int):
        # return tensor.view(batch_size, seq_len, self.num_heads, self.head_axis).transpose(1, 2).contiguous()
        return tensor.view(batch_size, seq_len, self.num_heads, self.head_axis).transpose(1, 2)

    def with_pos_embed(tensor: mindspore.Tensor, object_queries: Optional[Tensor]):
    ...
    def construct(
        ...
        batch_size, target_len, embed_axis = hidden_states.shape
        ...
        proj_shape = (batch_size * self.num_heads, -1, self.head_axis)
        ...
        if attn_output.shape != (batch_size * self.num_heads, target_len, self.head_axis):
            raise ValueError(
                f"`attn_output` should be of size {(batch_size, self.num_heads, target_len, self.head_axis)}, but is"
                f" {attn_output.shape}"
            )

        attn_output = attn_output.view(batch_size, self.num_heads, target_len, self.head_axis)
        attn_output = attn_output.transpose(1, 2)
        attn_output = attn_output.reshape(batch_size, target_len, embed_axis)
...
class DetrEncoderLayer(nn.Cell):
    def __init__(self, config: DetrConfig):
        super().__init__()
        self.embed_axis = config.d_model
        self.self_attn = DetrAttention(
            embed_axis=self.embed_axis,
            num_heads=config.encoder_attention_heads,
            dropout=config.attention_dropout,
        )
        self.self_attn_layer_norm = nn.LayerNorm([self.embed_axis])
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.fc1 = nn.Dense(self.embed_axis, config.encoder_ffn_axis)
        self.fc2 = nn.Dense(config.encoder_ffn_axis, self.embed_axis)
        self.final_layer_norm = nn.LayerNorm([self.embed_axis])
    ...
    def construct(
        ...
        """
        Args:
            hidden_states (`mindspore.Tensor`): input to the layer of shape `(batch, seq_len, embed_axis)`
...
class DetrDecoderLayer(nn.Cell):
    def __init__(self, config: DetrConfig):
        ...
        self.encoder_attn_layer_norm = nn.LayerNorm([self.embed_axis])
        self.fc1 = nn.Dense(self.embed_axis, config.decoder_ffn_axis)
        self.fc2 = nn.Dense(config.decoder_ffn_axis, self.embed_axis)
        self.final_layer_norm = nn.LayerNorm([self.embed_axis])
    ...
    def construct(
        ...
        Args:
            ...
            encoder_hidden_states (`mindspore.Tensor`):
                cross attention input to the layer of shape `(batch, seq_len, embed_axis)`
...
class DetrPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, cell):
        ...
        if isinstance(cell, DetrMHAttentionMap):
            cell.k_linear.weight.set_data(initializer('xavier_uniform', cell.k_linear.weight.shape, cell.k_linear.weight.dtype))
            cell.q_linear.weight.set_data(initializer('xavier_uniform', cell.q_linear.weight.shape, cell.q_linear.weight.dtype))
...
class DetrModel(DetrPreTrainedModel):
    ...
    def construct(
        ...
        # Second, apply 1x1 convolution to reduce the channel axisension to d_model (256 by default)
...
class DetrForObjectDetection(DetrPreTrainedModel):
    def __init__(self, config: DetrConfig):
        ...
        self.bbox_predictor = DetrMLPPredictionHead(
            input_axis=config.d_model, hidden_axis=config.d_model, output_axis=4, num_layers=3
        )
    ...
    def construct(
        ...
        >>> target_sizes = mindspore.Tensor([image.shape[::-1]])
...
class DetrMaskHeadSmallConv(nn.Cell):
    ...
    def __init__(self, axis, fpn_axiss, context_axis):
        super().__init__()
        ...
        inter_axiss = [axis, context_axis // 2, context_axis // 4, context_axis // 8, context_axis // 16, context_axis // 64]

        self.lay1 = nn.Conv2d(axis, axis, 3, padding=1)
        self.gn1 = nn.GroupNorm(8, axis)
        self.lay2 = nn.Conv2d(axis, inter_axiss[1], 3, padding=1)
        self.gn2 = nn.GroupNorm(min(8, inter_axiss[1]), inter_axiss[1])
        self.lay3 = nn.Conv2d(inter_axiss[1], inter_axiss[2], 3, padding=1)
        self.gn3 = nn.GroupNorm(min(8, inter_axiss[2]), inter_axiss[2])
        self.lay4 = nn.Conv2d(inter_axiss[2], inter_axiss[3], 3, padding=1)
        self.gn4 = nn.GroupNorm(min(8, inter_axiss[3]), inter_axiss[3])
        self.lay5 = nn.Conv2d(inter_axiss[3], inter_axiss[4], 3, padding=1)
        self.gn5 = nn.GroupNorm(min(8, inter_axiss[4]), inter_axiss[4])
        self.out_lay = nn.Conv2d(inter_axiss[4], 1, 3, padding=1)

        self.axis = axis

        self.adapter1 = nn.Conv2d(fpn_axiss[0], inter_axiss[1], 1)
        self.adapter2 = nn.Conv2d(fpn_axiss[1], inter_axiss[2], 1)
        self.adapter3 = nn.Conv2d(fpn_axiss[2], inter_axiss[3], 1)
...
class DetrMHAttentionMap(nn.Cell):
    ...
    def __init__(self, query_axis, hidden_axis, num_heads, dropout=0.0, bias=True, std=None):
        super().__init__()
        self.num_heads = num_heads
        self.hidden_axis = hidden_axis
        self.dropout = nn.Dropout(dropout)

        self.q_linear = nn.Dense(query_axis, hidden_axis, has_bias=bias)
        self.k_linear = nn.Dense(query_axis, hidden_axis, has_bias=bias)

        self.normalize_fact = float(hidden_axis / self.num_heads) ** -0.5
    ...
    def construct(self, q, k, mask: Optional[Tensor] = None):
        ...
        queries_per_head = q.view(q.shape[0], q.shape[1], self.num_heads, self.hidden_axis // self.num_heads)
        keys_per_head = k.view(k.shape[0], self.num_heads, self.hidden_axis // self.num_heads, k.shape[-2], k.shape[-1])
        weights = ops.einsum("bqnc,bnchw->bqnhw", queries_per_head * self.normalize_fact, keys_per_head)

        if mask is not None:
            weights.masked_fill_(mask.unsqueeze(1).unsqueeze(1), ops.finfo(weights.dtype).min)
...
class DetrLoss(nn.Cell):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        """
        Classification loss (NLL) targets dicts must contain the key "class_labels" containing a tensor of axis
        [nb_target_boxes]
        """
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        Targets dicts must contain the key "boxes" containing a tensor of axis [nb_target_boxes, 4]. The target boxes
    ...
    def _get_target_permutation_idx(indices):
    ...
    def construct(self, outputs, targets):
        ...
        num_boxes = ops.as_tensor([num_boxes], dtype=mindspore.float)
        world_size = 1
...
class DetrMLPPredictionHead(nn.Cell):
    ...
    def __init__(self, input_axis, hidden_axis, output_axis, num_layers):
        super().__init__()
        self.num_layers = num_layers
        h = [hidden_axis] * (num_layers - 1)
        self.layers = nn.ModuleList(nn.Dense(n, k) for n, k in zip([input_axis] + h, h + [output_axis]))
    ...
    def construct(self, outputs, targets):
        ...
        Args:
            outputs (`dict`):
                ...
                * "logits": Tensor of axis [batch_size, num_queries, num_classes] with the classification logits
                * "pred_boxes": Tensor of axis [batch_size, num_queries, 4] with the predicted box coordinates.
            targets (`List[dict]`):
                A list of targets (len(targets) = batch_size), where each target is a dict containing:
                * "class_labels": Tensor of axis [num_target_boxes] (where num_target_boxes is the number of
                  ground-truth
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    if tensor_list[0].naxis == 3:
        ...
        raise ValueError("Only 3-axisensional tensors are supported")
