...
def safe_squeeze(arr: np.ndarray, axis: Optional[int] = None) -> np.ndarray:
    """
    Squeezes an array, but only if the axis specified has axis 1.
    """
    if axis is None:
        return arr.squeeze()
...
def score_labels_from_class_probabilities(logits: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    scores = np.take_along_dim(probs, labels, axis=-1)
...
def binary_mask_to_rle(mask):
    ...
    Args:
        mask (`ops.Tensor` or `numpy.array`):
...
def convert_segmentation_to_rle(segmentation):
    ...
    Args:
        segmentation (`ops.Tensor` or `numpy.array`):
...
def remove_low_and_no_objects(masks, scores, labels, object_mask_threshold, num_labels):
    ...
    Returns:
        `Tuple[`ops.Tensor`, `ops.Tensor`, `ops.Tensor`]`: The `masks`, `scores` and `labels` without the region
...
class DetrImageProcessor(BaseImageProcessor):
        ...
        Args:
            ...
            return_tensors (`str` or `TensorType`, *optional*):
                The type of tensors to return. Can be one of:
                    ...
                    - `TensorType.PYTORCH` or `'pt'`: Return a batch of type `ops.Tensor`.
        ...
        **kwargs,
        ...
        if not valid_images(images):
            raise ValueError(
                ...
                "ops.Tensor, tf.Tensor or jax.ndarray."
            )
    ...
    def post_process(outputs, target_sizes):
        ...
        bottom_right_x, bottom_right_y) format. Only supports Pyops.

        Args:
            outputs ([`DetrObjectDetectionOutput`]):
                Raw outputs of the model.
            target_sizes (`ops.Tensor` of shape `(batch_size, 2)`):
    ...
    def post_process_segmentation(outputs, target_sizes, threshold=0.9, mask_threshold=0.5):
        """
        Converts the output of [`DetrForSegmentation`] into image segmentation predictions. Only supports Pyops.

        Args:
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            target_sizes (`ops.Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`):
    ...
    def post_process_panoptic(outputs, processed_sizes, target_sizes=None, is_thing_map=None, threshold=0.85):
        """
        Converts the output of [`DetrForSegmentation`] into actual panoptic predictions. Only supports Pyops.

        Args:
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            processed_sizes (`ops.Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`):
                Torch Tensor (or list) containing the size (h, w) of each image of the batch, i.e. the size after data
                augmentation but before batching.
            target_sizes (`ops.Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`, *optional*):
            ...
            is_thing_map (`ops.Tensor` of shape `(batch_size, 2)`, *optional*):
        ...
        for cur_logits, cur_masks, cur_boxes, size, target_size in zip(
            ...
            def get_ids_area(masks, scores, h=h, w=w, stuff_equiv_classes=stuff_equiv_classes, target_size=target_size, dedup=False):
                ...
                if dedup:
                    ...
                    for equiv in stuff_equiv_classes.values():
                        if len(equiv) > 1:
                            first_equiv = equiv[0]
                            for eq_id in equiv:
                                m_id.masked_fill_(m_id.eq(eq_id), first_equiv)
                ...
                np_seg_img = ops.ByteTensor(ops.ByteStorage.from_buffer(seg_img.tobytes()))
                np_seg_img = np_seg_img.view(final_h, final_w, 3)
                np_seg_img = np_seg_img.numpy()

                m_id = ops.from_numpy(rgb_to_id(np_seg_img))
            ...
            area, seg_img = get_ids_area(cur_masks, cur_scores, dedup=True, h=h, w=w, stuff_equiv_classes=stuff_equiv_classes, target_size=target_size)
            if cur_labels.numel() > 0:
                # We know filter empty masks as long as we find some
                while True:
                    filtered_small = ops.as_tensor(
                        [area[i] <= 4 for i, c in enumerate(cur_labels)], dtype=mindspore.bool
                    )
                    if filtered_small.any().item():
                        cur_scores = cur_scores[~filtered_small]
                        cur_labels = cur_labels[~filtered_small]
                        cur_masks = cur_masks[~filtered_small]
                        area, seg_img = get_ids_area(cur_masks, cur_scores, dedup=False, h=h, w=w, stuff_equiv_classes=stuff_equiv_classes, target_size=target_size)
                    else:
                        break
    ...
    def post_process_object_detection(
        ...
        Args:
            ...
            target_sizes (`ops.Tensor` or `List[Tuple[int, int]]`, *optional*):
    ...
    def post_process_semantic_segmentation(outputs, target_sizes: List[Tuple[int, int]] = None):
        """
        Converts the output of [`DetrForSegmentation`] into semantic segmentation maps. Only supports Pyops.
        ...
        Returns:
            `List[ops.Tensor]`:
                ...
                `ops.Tensor` correspond to a semantic class id.
        """
        ...
        if target_sizes is not None:
            ...
            for idx in range(batch_size):
                resized_logits = ops.interpolate(
                    segmentation[idx].unsqueeze(axis=0), size=target_sizes[idx], mode="bilinear", align_corners=False
                )
    ...
    def post_process_instance_segmentation(
        ...
        """
        Converts the output of [`DetrForSegmentation`] into instance segmentation predictions. Only supports Pyops.
