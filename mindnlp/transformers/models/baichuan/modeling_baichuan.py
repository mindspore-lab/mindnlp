...
class TextIterStreamer:
    ...
    Note:
    - The put method processes the input value and adds it to the token list and text queue. If skip_prompt is set to True, prompts are skipped during processing. The end method signals the end of text data processing, and the __next__ method retrieves the next processed text data. 
    ...
    def __init__(self, tokenizer, skip_prompt=False, skip_special_tokens=False):
        ...
        This method initializes the TextIterStreamer class by setting the tokenizer, skip_prompt, and skip_special_tokens attributes. It also initializes the tokens and text_queue attributes. The tokenizer object is used for tokenization. The skip_prompt flag is used to determine whether the prompt should be skipped during tokenization. The skip_special_tokens flag is used to determine whether special tokens should be skipped during tokenization. The tokens attribute is an empty list that will store the tokens. The text_queue attribute is a queue used for storing the text.
        """
...
class RotaryEmbedding(nn.Cell):
    ...
    def construct(self, x, seq_len=None):
        ...
        If the input sequence length (seq_len) is greater than the maximum sequence length cached (self.max_seq_len_cached), the method computes the cosine and sine values for the updated sequence length. It then updates the cached cosine (self.cos_cached) and sine (self.sin_cached) values accordingly.
        
        The rotary embedding is computed using the inverse frequency tensor (self.inv_freq). The frequency tensor is generated by multiplying the sequence length with the inverse frequency tensor. The resulting tensor is concatenated with itself along the last axis to create the embedding tensor (emb).
        
        The cosine and sine values are computed from the embedding tensor using the cos and sin functions, respectively. The resulting cosine and sine tensors are then stored in the cached variables (self.cos_cached and self.sin_cached) to be reused in subsequent calls.
        ...
        Note: The maximum sequence length cached (self.max_seq_len_cached) is updated only when the input sequence length (seq_len) is greater than it. This ensures that the cached values are reused for shorter sequences, improving efficiency.
        """
...
class BaiChuanAttention(nn.Cell):
    ...
    """
    BaiChuanAttention class represents an attention mechanism component used in neural network models. It is designed to calculate attention weights and apply them to the input hidden states to generate the final output. This class inherits from nn.Cell.
    ...
    Methods:
        ...
        construct(self, hidden_states: mindspore.Tensor, attention_mask: Optional[mindspore.Tensor] = None, past_key_value: Optional[Tuple[mindspore.Tensor]] = None, output_attentions: bool = False, use_cache: bool = False) -> Tuple[mindspore.Tensor, Optional[mindspore.Tensor], Optional[Tuple[mindspore.Tensor]]]: Constructs the attention mechanism by calculating attention weights and output based on the input hidden states and additional parameters.
    
    Raises:
        ValueError: If the hidden size is not divisible by the number of attention heads.
    
    Returns:
        Tuple[mindspore.Tensor, Optional[mindspore.Tensor], Optional[Tuple[mindspore.Tensor]]]: A tuple containing the attention output tensor, attention weights (if requested), and past key-value states (if caching is enabled).
    """
    def __init__(self, config: BaiChuanConfig):
...
class BaiChuanLayer(nn.Cell):
    ...
    Methods:
        ...
        construct(self, hidden_states: mindspore.Tensor, attention_mask: Optional[mindspore.Tensor] = None, past_key_value: Optional[Tuple[mindspore.Tensor]] = None, output_attentions: Optional[bool] = False, use_cache: Optional[bool] = False) -> Tuple[mindspore.Tensor, Optional[Tuple[mindspore.Tensor, mindspore.Tensor]]]: Constructs the BaiChuanLayer with the given parameters and returns the hidden states with optional present key value computed during attention.
...
class BaiChuan7bModel(BaiChuanPreTrainedModel):
    ...
    def construct(
        ...
        Raises:
        - ValueError: Raised if both input_ids and inputs_embeds are specified simultaneously, if neither decoder_input_ids nor decoder_inputs_embeds are specified, or if an invalid configuration is encountered during model construction.
        """
...
class BaiChuan13bModel(BaiChuanPreTrainedModel):
    ...
    Methods:
        ...
        - construct(self, input_ids, attention_mask, past_key_values, inputs_embeds, use_cache, output_attentions, output_hidden_states, return_dict): Constructs the model with the given inputs and returns the model output.
    ...
    def __init__(self, config: BaiChuanConfig):
        ...
        Args:
            ...
            config (BaiChuanConfig): An object of type BaiChuanConfig containing configuration parameters for the model. It specifies the configuration parameters such as pad_token_id, vocab_size, num_attention_heads, hidden_size, num_hidden_layers, rms_norm_eps, and model_max_length.
...
class BaiChuanForCausalLM(BaiChuanPreTrainedModel):
    ...
    def construct(
        ...
        Returns:
            Union[Tuple, CausalLMOutputWithPast]: The model outputs. If `return_dict` is False, returns a tuple containing the logits and the various model outputs. If `return_dict` is True, returns an instance of `CausalLMOutputWithPast` containing the loss, logits, past key values, hidden states, and attentions.
