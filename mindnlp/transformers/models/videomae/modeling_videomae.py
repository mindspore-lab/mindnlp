...
def get_sinusoid_encoding_table(n_position, d_hid):
    ...
    # TODO: make it with mindspore instead of numpy
...
class VideoMAEEmbeddings(nn.Cell):
    ...
    def construct(self, pixel_values, bool_masked_pos):
        ...
        embeddings = embeddings + self.position_embeddings.type_as(embeddings).copy()
...
class VideoMAESelfAttention(nn.Cell):
    def __init__(self, config: VideoMAEConfig) -> None:
        ...
        if config.qkv_bias:
            self.q_bias = Parameter(ops.zeros(self.all_head_size), name='q_bias')
            self.v_bias = Parameter(ops.zeros(self.all_head_size), name='v_bias')
        else:
            self.q_bias = None
            self.v_bias = None
    ...
    def construct(
        ...
        k_bias = ops.zeros_like(self.v_bias) if self.q_bias is not None else None
        keys = ops.dense(input=hidden_states, weight=self.key.weight, bias=k_bias)
        values = ops.dense(input=hidden_states, weight=self.value.weight, bias=self.v_bias)
        queries = ops.dense(input=hidden_states, weight=self.query.weight, bias=self.q_bias)
...
class VideoMAEModel(VideoMAEPreTrainedModel):
    ...
    def construct(
        ...
        r"""
        bool_masked_pos (`mindspore.Tensor` of shape `(batch_size, sequence_length)`, *optional*):
...
class VideoMAEForPreTraining(VideoMAEPreTrainedModel):
    def __init__(self, config):
        ...
        self.mask_token = Parameter(ops.zeros(1, 1, config.decoder_hidden_size), name='mask_token')
    ...
    def construct(
        ...
        r"""
        bool_masked_pos (`mindspore.Tensor` of shape `(batch_size, sequence_length)`):
            Boolean masked positions. Indicates which patches are masked (1) and which aren't (0). Each video in the
        ...
        >>> bool_masked_pos = ops.randint(0, 2, (1, seq_length)).bool()
        ...
        expanded_position_embeddings = self.position_embeddings.broadcast_to((batch_size, -1, -1)).type_as(pixel_values)
        ...
        with mindspore._no_grad():
            ...
            if self.config.num_channels != 3:
                ...
                mean = mindspore.Tensor(IMAGENET_DEFAULT_MEAN, dtype=dtype)[None, None, :, None, None]
                std = mindspore.Tensor(IMAGENET_DEFAULT_STD, dtype=dtype)[None, None, :, None, None]
            ...
            if self.config.norm_pix_loss:
                ...
                frames_norm = (frames - frames.mean(axis=-2, keep_dims=True)) / (
                    frames.var(axis=-2, keepdims=True).sqrt() + 1e-6
                )
