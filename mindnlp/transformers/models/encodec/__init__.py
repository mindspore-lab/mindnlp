...
"""
<<<<<<< HEAD
<<<<<<<< HEAD:mindnlp/transformers/models/encodec/__init__.py
...
class BarkSelfAttention(nn.Cell):
    ...
    def construct(self,
        ...
        # # print(query.view(B, T, self.num_heads, C // self.num_heads).shape)
        # query = query.view(B, T, self.num_heads, C // self.num_heads).swapaxes(1, 2)
        # key = key.view(B, T, self.num_heads, C // self.num_heads).swapaxes(1, 2)
        # value = value.view(B, T, self.num_heads, C // self.num_heads).swapaxes(1, 2)
...
class BarkLayerNorm(nn.Cell):
    ...
    def __init__(self, hidden_size, bias=True):
        ...
        self.layer_norm = nn.LayerNorm(self.weight.shape, gamma_init=self.weight, beta_init=self.bias, epsilon=1e-5)
        ...
        **kwargs,
        ...
        if history_prompt is not None:
            ...
            semantic_history = mindspore.Tensor(
                [semantic_generation_config.semantic_pad_token] * max_input_semantic_length, dtype=mindspore.int
            ).to(self.device)

        semantic_history = ops.repeat_interleave(semantic_history[None], batch_size, axis=0)

        infer_array = mindspore.Tensor(
            [[semantic_generation_config.semantic_infer_token]] * batch_size, dtype=mindspore.int
        ).to(self.device)
        ...
        semantic_output = super().generate(
            ops.ones((batch_size, max_input_semantic_length + 1), dtype=mindspore.int).to(self.device),
            input_embeds=input_embeds,
...
class BarkCoarseModel(BarkCausalModel):
    ...
    def preprocess_histories(
        ...
        if history_prompt is not None:
            ...
            # shape: (batch_size, 0)
            x_semantic_history = mindspore.Tensor([[]] * batch_size, dtype=mindspore.int).to(self.device)
            x_coarse_history = mindspore.Tensor([[]] * batch_size, dtype=mindspore.int).to(self.device)
        ...
        **kwargs,
        ...
        for _ in range(n_window_steps):
            ...
            input_coarse = ops.hstack(
                [
                    input_coarse,
                    mindspore.Tensor([[coarse_generation_config.coarse_infer_token]] * batch_size).to(self.device),
...
class BarkModel(BarkPreTrainedModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        return audio
>>>>>>>> 523e2f0cc3d30248fac725064d16125a9d23a063:mindnlp/transformers/models/bark/bark.py
=======
