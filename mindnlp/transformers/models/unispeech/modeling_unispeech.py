...
class UniSpeechGumbelVectorQuantizer(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.register_buffer('codevectors', mindspore.Tensor(1, self.num_groups * self.num_vars, config.codevector_dim // self.num_groups))
...
class UniSpeechModel(UniSpeechPreTrainedModel):
    def __init__(self, config: UniSpeechConfig):
        ...
        if config.mask_time_prob > 0.0 or config.mask_feature_prob > 0.0:
            ...
            self.register_buffer('masked_spec_embed', mindspore.Tensor(config.hidden_size))
...
class UniSpeechForPreTraining(UniSpeechPreTrainedModel):
    ...
    def forward(
        ...
        prob_replace_matrix = ops.empty(transformer_features.shape[0], transformer_features.shape[1]).fill(
            self.config.replace_prob
        )
        prob_replace_matrix = ops.transpose(prob_replace_matrix, 0, 1)
        # prob_replace_matrix = prob_replace_matrix.transpose(0, 1)
        sampled_replace_matrix = ops.bernoulli(prob_replace_matrix).bool()
        sampled_replace_matrix = ops.transpose(sampled_replace_matrix, 0, 1)
...
class UniSpeechForCTC(UniSpeechPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            log_probs = ops.transpose(F.log_softmax(logits, dim=-1, dtype=mindspore.float32), 0, 1)
...
class UniSpeechForSequenceClassification(UniSpeechPreTrainedModel):
    def __init__(self, config):
        ...
        if config.use_weighted_layer_sum:
            ...
            self.register_buffer('layer_weights', ops.ones(num_layers) / num_layers)
