...
if is_mindspore_available():
    ...
    from mindnlp.transformers import JukeboxModel, JukeboxPrior, JukeboxTokenizer


@require_mindspore
class Jukebox1bModelTester(unittest.TestCase):
    all_model_classes = (JukeboxModel,) if is_mindspore_available() else ()
    ...
    def test_sampling(self):
        model = JukeboxModel.from_pretrained(self.model_id, min_duration=0).set_train(False)
        ...
        self.assertIn(zs[0][0].detach().tolist(), [self.EXPECTED_OUTPUT_2, self.EXPECTED_OUTPUT_2_PT_2])

        set_seed(0)
        zs = model._sample(zs, labels, [1], sample_length=40 * model.priors[1].raw_to_tokens, save_results=False)
        self.assertIn(zs[1][0].detach().tolist(), [self.EXPECTED_OUTPUT_1, self.EXPECTED_OUTPUT_1_PT_2])

        set_seed(0)
        zs = model._sample(zs, labels, [2], sample_length=40 * model.priors[2].raw_to_tokens, save_results=False)
        self.assertIn(zs[2][0].detach().tolist(), [self.EXPECTED_OUTPUT_0, self.EXPECTED_OUTPUT_0_PT_2])
    ...
    def test_conditioning(self):
        ...
        self.assertTrue(mindnlp.core.ops.allclose(
            audio_conditioning[0][0][:30].detach(), mindspore.tensor(self.EXPECTED_AUDIO_COND), atol=1e-4, rtol=1e-4
        ))
        self.assertTrue(mindnlp.core.ops.allclose(
            metadata_conditioning[0][0][:30].detach(), mindspore.tensor(self.EXPECTED_META_COND), atol=1e-4, rtol=1e-4
        ))
        self.assertTrue(mindnlp.core.ops.allclose(
            lyric_tokens[0, :30].detach(), mindspore.tensor(self.EXPECTED_LYRIC_COND), atol=1e-4, rtol=1e-4
        ))
    ...
    def test_primed_sampling(self):
        ...
        self.assertTrue(mindnlp.core.ops.allclose(zs[0][0][:40], mindspore.tensor(self.EXPECTED_PRIMED_0)))

        upper_2 = ops.cat((zs[0], ops.zeros(1, 2048 - zs[0].shape[-1])), dim=-1).long()
        ...
        self.assertTrue(mindnlp.core.ops.allclose(zs[1][0][:40], mindspore.tensor(self.EXPECTED_PRIMED_1)))

        upper_1 = ops.cat((zs[1], ops.zeros(1, 2048 - zs[1].shape[-1])), dim=-1).long()
        ...
        self.assertTrue(mindnlp.core.ops.allclose(zs[2][0][:40], mindspore.tensor(self.EXPECTED_PRIMED_2)))
    ...
    def test_vqvae(self):
        ...
        self.assertTrue(mindnlp.core.ops.allclose(zs[0][0], mindspore.tensor(self.EXPECTED_VQVAE_ENCODE)))

        x = model.vqvae.decode(zs, start_level=2, bs_chunks=x.shape[0])
        self.assertTrue(mindnlp.core.ops.allclose(x[0, :40, 0], mindspore.tensor(self.EXPECTED_VQVAE_DECODE), atol=1e-4, rtol=1e-4))
...
class Jukebox5bModelTester(unittest.TestCase):
    ...
    def test_slow_sampling(self):
        ...
        zs = [ops.zeros((1, 0), dtype=mindspore.int64) for _ in range(3)]
        zs = model._sample(zs, labels, [0], sample_length=60 * model.priors[0].raw_to_tokens, save_results=False)
        self.assertTrue(mindnlp.core.ops.allclose(zs[0][0], mindspore.tensor(self.EXPECTED_GPU_OUTPUTS_2)))
        model.priors[0]

        set_seed(0)
        model.priors[1]
        zs = model._sample(zs, labels, [1], sample_length=60 * model.priors[1].raw_to_tokens, save_results=False)
        self.assertTrue(mindnlp.core.ops.allclose(zs[1][0], mindspore.tensor(self.EXPECTED_GPU_OUTPUTS_1)))
        model.priors[1]

        set_seed(0)
        model.priors[2]
        zs = model._sample(zs, labels, [2], sample_length=60 * model.priors[2].raw_to_tokens, save_results=False)
        self.assertTrue(mindnlp.core.ops.allclose(zs[2][0], mindspore.tensor(self.EXPECTED_GPU_OUTPUTS_0)))

    #@slow
    def test_fp16_slow_sampling(self):
        prior_id = "ArthurZ/jukebox_prior_0"
        model = JukeboxPrior.from_pretrained(prior_id, min_duration=0).set_train(False).half()
        ...
        self.assertIn(outputs[0].tolist(), [self.EXPECTED_GPU_OUTPUTS_2, self.EXPECTED_GPU_OUTPUTS_2_PT_2])
