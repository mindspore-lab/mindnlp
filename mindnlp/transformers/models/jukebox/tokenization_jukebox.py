...
class JukeboxTokenizer(PreTrainedTokenizer):
    ...
    def __call__(self, artist, genres, lyrics="", return_tensors="ms") -> BatchEncoding:
        ...
        input_ids_list = [input_ids + [artists_id[i]] + genres_ids[i] + full_tokens[i] for i in range(len(self.version))]
        
        # 手动转换为MindSpore张量以避免异构数据的问题
        tensor_input_ids = []
        for ids in input_ids_list:
            tensor_input_ids.append(mindspore.Tensor(np.array([ids])))
        
        return BatchEncoding({"input_ids": tensor_input_ids, "attention_masks": attention_masks})
