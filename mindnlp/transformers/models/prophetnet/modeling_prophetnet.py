...
PROPHETNET_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (`ms.int64Tensor` of shape `(batch_size, sequence_length)`):
        ...
        decoder_input_ids (`ms.int64Tensor` of shape `(batch_size, target_sequence_length)`, *optional*):
...
def ngram_attention_bias(sequence_length, ngram, dtype):
    ...
    dtype_min = ms.Tensor(info.min, dtype)
    left_block = (
        ops.ones((ngram, sequence_length, sequence_length), dtype=dtype) * dtype_min
    )
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Dense):
            module.weight.set_data(
                initializer(
                    Normal(sigma=self.config.init_std, mean=0.0),
                    module.weight.shape,
                    module.weight.dtype,
                )
            )
            ...
            module.weight.set_data(initializer(
                Normal(mean=0.0, sigma=self.config.init_std), module.weight.shape, module.weight.dtype))
    ...
    def construct(self, inputs_shape, attention_mask=None, past_key_values=None, position_ids=None):
        ...
        if position_ids is None:
            if past_key_values is not None:
                ...
                ).astype(ms.int64)+ self.padding_idx
...
class ProphetNetNgramSelfAttention(nn.Cell):
    ...
    def construct(
        ...
        if extended_predict_attention_mask is not None:
            ...
            extended_predict_attention_mask = extended_predict_attention_mask.astype(predict_attn_weights.dtype)
    ...
    def get_main_relative_pos_embeddings(
        ...
        main_relative_pos_embeddings = ops.gather(rel_pos_embeddings, axis=1, input_indices=main_relative_position_buckets)
    ...
    def get_predict_relative_pos_embeddings(
        ...
        predict_relative_pos_embeddings = ops.gather(
            rel_pos_embeddings, axis=1, input_indices=predict_relative_position_buckets
        )
...
class ProphetNetEncoder(ProphetNetPreTrainedModel):
    ...
    def construct(
        ...
        info = np.finfo(ms.dtype_to_nptype(self.dtype))
        dtype_min = ms.Tensor(info.min, self.dtype)
...
class ProphetNetDecoder(ProphetNetPreTrainedModel):
    ...
    def prepare_attention_mask(self, hidden_states, attention_mask):
        ...
        # get causal mask
        info = np.finfo(ms.dtype_to_nptype(hidden_states.dtype))
        dtype_min = ms.Tensor(info.min, hidden_states.dtype)
        ...
        dtype_min = ms.Tensor(info.min, self.dtype)
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask[:, None, None, :]) * dtype_min
    ...
    def prepare_predict_attention_mask(self, hidden_states, attention_mask):
        ...
        dtype_min = ms.Tensor(info.min, self.dtype)
        if attention_mask is not None:
            extended_attention_mask = (1.0 - attention_mask[:, None, None, None, :]) * dtype_min
...
class ProphetNetForConditionalGeneration(ProphetNetPreTrainedModel):
    ...
    def construct(
        ...
        r"""
        labels (`ms.int64Tensor` of shape `(batch_size,)`, *optional*):
...
class ProphetNetForCausalLM(ProphetNetPreTrainedModel):
    ...
    def construct(
        ...
        labels (`ms.int64Tensor` of shape `(batch_size, sequence_length)`, *optional*):
...
__all__ = ["ProphetNetPreTrainedModel", "ProphetNetForConditionalGeneration", "ProphetNetModel", "ProphetNetDecoder", "ProphetNetEncoder", "ProphetNetForCausalLM",]
