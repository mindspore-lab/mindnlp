...
class TimeSeriesFeatureEmbedder(nn.Cell):
    ...
    def construct(self, features: mindspore.Tensor) -> mindspore.Tensor:
        if self.num_features > 1:
            ...
            cat_feature_slices = ops.chunk(features, self.num_features, axis=-1)
        else:
...
class TimeSeriesValueEmbedding(nn.Cell):
    def __init__(self, feature_size, d_model):
        super().__init__()
        self.value_projection = nn.Dense(in_channels=feature_size, out_channels=d_model, has_bias=False)
...
class TimeSeriesTransformerModel(TimeSeriesTransformerPreTrainedModel):
    ...
    def create_network_inputs(
        ...
        # transformer inputs
        transformer_inputs = ops.cat((reshaped_lagged_sequence, features), axis=-1)
...
__all__ = ['TimeSeriesTransformerConfig','TimeSeriesTransformerPreTrainedModel','TimeSeriesTransformerModel','TimeSeriesTransformerForPrediction']