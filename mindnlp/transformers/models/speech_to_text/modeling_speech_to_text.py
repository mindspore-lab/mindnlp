...
class Conv1dSubsampler(nn.Module):
    ...
    def forward(self, input_features):
        # hidden_states = input_features.swapaxes(1, 2)  # -> B x (C x D) x T
        hidden_states = ops.transpose(input_features, 1, 2)
        for conv in self.conv_layers:
            hidden_states = conv(hidden_states)
            hidden_states = nn.functional.glu(hidden_states, dim=1)
        # hidden_states = hidden_states.swapaxes(1, 2)  # -> T x B x (C x D)
...
class Speech2TextSinusoidalPositionalEmbedding(nn.Module):
    ...
    def forward(self, input_ids: mindspore.Tensor, past_key_values_length: int = 0):
        ...
        # print("position weights", self.weights.index_select(0, position_ids.view(-1)).view(bsz, seq_len, -1))
...
class Speech2TextAttention(nn.Module):
    ...
    def forward(
        ...
        # print("start")
        # print("Attention softmax before",attn_weights.dtype,attn_weights)
        # with open('mindnlp_data.txt','a+') as f:
        #     import json
        #     attn_weights_list = attn_weights.tolist()
        #     json.dump(attn_weights_list, f, indent=4)
        attn_weights = attn_weights.type(mindspore.float64)
        attn_weights = F.softmax(attn_weights, dim=-1)
        attn_weights = attn_weights.type(mindspore.float32)
        # print("Attention softmax after",attn_weights)
        ...
        # print("Attention2-0",attn_weights.dtype,attn_weights)
        attn_probs = F.dropout(attn_weights, p=self.dropout, training=self.training)

        # ensure the same data type
        if value_states.dtype == mindspore.float16:
            attn_probs = attn_probs.type(mindspore.float16)

        attn_output = ops.bmm(attn_probs, value_states)
        # print("Attention3", attn_output)
        # print("end")
...
class Speech2TextEncoder(Speech2TextPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            padding_mask = ops.zeros(inputs_embeds.shape[:2], dtype=mindspore.int64)
