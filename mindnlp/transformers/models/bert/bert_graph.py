...
class MSBertAttention(nn.Cell):
    ...
    def __init__(self, config):
        super().__init__()
        self.self_attn = MSBertSelfAttention(config)
...
class MSBertLayer(nn.Cell):
    ...
    def __init__(self, config):
        super().__init__()
        self.attention = MSBertAttention(config)
...
class MSBertEncoder(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.layer = nn.CellList([MSBertLayer(config) for _ in range(config.num_hidden_layers)])
...
__all__ = [
    'MSBertEmbeddings', 'MSBertAttention', 'MSBertEncoder', 'MSBertIntermediate', 'MSBertLayer',
    'MSBertModel', 'MSBertForPretraining', 'MSBertLMPredictionHead', 'MSBertForSequenceClassification'
]
