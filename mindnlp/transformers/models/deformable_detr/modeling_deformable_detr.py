...
_CHECKPOINT_FOR_DOC = "sensetime/deformable_detr"
...
def inverse_sigmoid(x, eps=1e-5):
    ...
    return mindspore.ops.log(x1 / x2)
...
class DeformableDetrConvEncoder(nn.Cell):
    ...
    def __init__(self, config):
        ...
        backbone_model_type = config.backbone if config.backbone is not None else config.backbone_config.model_type
...
class DeformableDetrConvModel(nn.Cell):
    """
...
class DeformableDetrSinePositionEmbedding(nn.Cell):
    """
    ...
    def construct(self, pixel_values, pixel_mask):
        ...
        pos_x = mindspore.ops.stack((pos_x[:, :, :, 0::2].sin(), pos_x[:, :, :, 1::2].cos()), axis=4).flatten(3)
        pos_y = mindspore.ops.stack((pos_y[:, :, :, 0::2].sin(), pos_y[:, :, :, 1::2].cos()), axis=4).flatten(3)
        pos = mindspore.ops.cat((pos_y, pos_x), axis=3).permute(0, 3, 1, 2)
        return pos


# Copied from transformers.models.detr.modeling_detr.DetrLearnedPositionEmbedding
class DeformableDetrLearnedPositionEmbedding(nn.Cell):
    """
...
def multi_scale_deformable_attention(
    ...
    for level_id, (height, width) in enumerate(value_spatial_shapes):
        ...
        sampling_value_l_ = mindspore.ops.grid_sample(
    ...
    output = (
        (mindspore.ops.stack(sampling_value_list, axis=-2).flatten(-2) * attention_weights)
        .sum(-1)
...
class DeformableDetrEncoderLayer(nn.Cell):
    ...
    def construct(
        ...
        if self.training:
            if mindspore.ops.isinf(hidden_states).any() or mindspore.ops.isnan(hidden_states).any():
...
class DeformableDetrEncoder(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.dropout = config.dropout
        self.layers = nn.CellList([DeformableDetrEncoderLayer(config) for _ in range(config.encoder_layers)])
    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            ref = mindspore.ops.stack((ref_x, ref_y), -1)
            reference_points_list.append(ref)
        reference_points = mindspore.ops.cat(reference_points_list, 1)
...
class DeformableDetrDecoder(DeformableDetrPreTrainedModel):
    ...
    def __init__(self, config: DeformableDetrConfig):
        ...
        self.dropout = config.dropout
        self.layers = nn.CellList([DeformableDetrDecoderLayer(config) for _ in range(config.decoder_layers)])
        self.gradient_checkpointing = False
    ...
    def construct(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if num_coordinates == 4:
                reference_points_input = (
                    reference_points[:, :, None] * mindspore.ops.cat([valid_ratios, valid_ratios], -1)[:, None]
                )
        ...
        intermediate = mindspore.ops.stack(intermediate, axis=1)
        intermediate_reference_points = mindspore.ops.stack(intermediate_reference_points, axis=1)
...
class DeformableDetrModel(DeformableDetrPreTrainedModel):
    ...
    def get_valid_ratio(self, mask, dtype=mindspore.float32):
        ...
        valid_height = mindspore.ops.sum(mask[:, :, 0], 1)
        valid_width = mindspore.ops.sum(mask[:, 0, :], 1)
        valid_ratio_height = valid_height.to(dtype) / height
        valid_ratio_width = valid_width.to(dtype) / width
        valid_ratio = mindspore.ops.stack([valid_ratio_width, valid_ratio_height], -1)
        return valid_ratio
    ...
    def get_proposal_pos_embed(self, proposals):
        ...
        pos = mindspore.ops.stack((pos[:, :, :, 0::2].sin(), pos[:, :, :, 1::2].cos()), axis=4).flatten(2)
        return pos
    ...
    def gen_encoder_output_proposals(self, enc_output, padding_mask, spatial_shapes):
        ...
        for level, (height, width) in enumerate(spatial_shapes):
            ...
            valid_height = mindspore.ops.sum(~mask_flatten_[:, :, 0, 0], 1)
            ...
            scale = mindspore.ops.cat([valid_width.unsqueeze(-1), valid_height.unsqueeze(-1)], 1).view(batch_size, 1, 1, 2)
            grid = (grid.unsqueeze(0).expand(batch_size, -1, -1, -1) + 0.5) / scale
            width_heigth = mindspore.ops.ones_like(grid) * 0.05 * (2.0**level)
            proposal = mindspore.ops.cat((grid, width_heigth), -1).view(batch_size, -1, 4)
            proposals.append(proposal)
            _cur += height * width
        output_proposals = mindspore.ops.cat(proposals, 1)
        output_proposals_valid = ((output_proposals > 0.01) & (output_proposals < 0.99)).all(-1, keepdim=True)
        output_proposals = mindspore.ops.log(output_proposals / (1 - output_proposals))  # inverse sigmoid
    ...
    def construct(
        ...
        if pixel_mask is None:
            pixel_mask = mindspore.ops.ones(((batch_size, height, width)), dtype=mindspore.int64)
        ...
        if self.config.num_feature_levels > len(sources):
            ...
            for level in range(_len_sources, self.config.num_feature_levels):
                ...
                mask = mindspore.ops.interpolate(pixel_mask[None].float(), size=source.shape[-2:]).to(mindspore.bool_)[0]
        ...
        source_flatten = mindspore.ops.cat(source_flatten, 1)
        mask_flatten = mindspore.ops.cat(mask_flatten, 1)
        lvl_pos_embed_flatten = mindspore.ops.cat(lvl_pos_embed_flatten, 1)
        spatial_shapes = mindspore.Tensor(spatial_shapes, dtype=mindspore.int64)
        level_start_index = mindspore.ops.cat((spatial_shapes.new_zeros((1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
        valid_ratios = mindspore.ops.stack([self.get_valid_ratio(m, dtype=source_flatten.dtype) for m in masks], 1)
        ...
        if self.config.two_stage:
            ...
            query_embed, target = mindspore.ops.split(pos_trans_out, num_channels, axis=2)
        else:
            query_embed, target = mindspore.ops.split(query_embeds, num_channels, axis=1)
...
class DeformableDetrForObjectDetection(DeformableDetrPreTrainedModel):
    ...
    def construct(
        ...
        outputs_class = mindspore.ops.stack(outputs_classes)
        outputs_coord = mindspore.ops.stack(outputs_coords)
...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2):
    ...
    """
    prob = inputs.sigmoid()
    ce_loss = mindspore.ops.binary_cross_entropy_with_logits(inputs, targets, reduction="none")
...
class DeformableDetrLoss(nn.Cell):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        target_classes_o = ops.concat([t["class_labels"][J] for t, (_, J) in zip(targets, indices)])
        target_classes = ops.full(source_logits.shape[:2], self.num_classes, dtype=mindspore.int64)
        target_classes[idx] = target_classes_o

        target_classes_onehot = ops.zeros((source_logits.shape[0], source_logits.shape[1], source_logits.shape[2] + 1), dtype=source_logits.dtype)
        target_classes_onehot = ops.tensor_scatter_elements(target_classes_onehot, target_classes.unsqueeze(-1), ops.ones_like(target_classes.unsqueeze(-1)), axis=2)
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        target_lengths = Tensor([len(v["class_labels"]) for v in targets], dtype=mindspore.float32)
        card_pred = (logits.argmax(-1) != logits.shape[-1] - 1).sum(1)
        card_err = ops.l1_loss(card_pred.astype(mindspore.float32), target_lengths)
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        idx = self._get_source_permutation_idx(indices)
        source_boxes = ops.concat(outputs["pred_boxes"][idx])
    ...
    def _get_source_permutation_idx(self, indices):
        batch_idx = ops.concat([ops.full_like(source, i) for i, (source, _) in enumerate(indices)])
        source_idx = ops.concat([source for (source, _) in indices])
        return batch_idx, source_idx

    def _get_target_permutation_idx(self, indices):
        batch_idx = ops.concat([ops.full_like(target, i) for i, (_, target) in enumerate(indices)])
        target_idx = ops.concat([target for (_, target) in indices])
    ...
    def construct(self, outputs, targets):
        ...
        num_boxes = ops.clamp(num_boxes / world_size, min=1).asnumpy().item()
...
class DeformableDetrMLPPredictionHead(nn.Cell):
    """
    ...
    def construct(self, x):
        for i, layer in enumerate(self.layers):
            x = mindspore.ops.relu(layer(x)) if i < self.num_layers - 1 else layer(x)
        return x


class DeformableDetrHungarianMatcher(nn.Cell):
    """
    ...
    def construct(self, outputs, targets):
        ...
        target_ids = mindspore.ops.cat([v["class_labels"] for v in targets])
        target_bbox = mindspore.ops.cat([v["boxes"] for v in targets])
        ...
        bbox_cost = mindspore.ops.cdist(out_bbox, target_bbox, p=1)
        ...
        return [(mindspore.Tensor(i, dtype=mindspore.int64), mindspore.Tensor(j, dtype=mindspore.int64)) for i, j in indices]
...
def box_iou(boxes1, boxes2):
    ...
    left_top = mindspore.ops.max(boxes1[:, None, :2], boxes2[:, :2])  # [N,M,2]
    right_bottom = mindspore.ops.min(boxes1[:, None, 2:], boxes2[:, 2:])  # [N,M,2]
...
def generalized_box_iou(boxes1, boxes2):
    ...
    top_left = mindspore.ops.min(boxes1[:, None, :2], boxes2[:, :2])
    bottom_right = mindspore.ops.max(boxes1[:, None, 2:], boxes2[:, 2:])
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    if tensor_list[0].ndim == 3:
        ...
        tensor = mindspore.ops.zeros(batch_shape, dtype=dtype)
        mask = mindspore.ops.ones((batch_size, height, width), dtype=mindspore.bool_)
