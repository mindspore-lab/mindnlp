...
class LayoutLMv3TextEmbeddings(nn.Cell):
    ...
    def create_position_ids_from_input_ids(self, input_ids, padding_idx):
        ...
        incremental_indices = (ops.cumsum(mask, axis=1).to(mask.dtype)) * mask
...
class LayoutLMv3SelfAttention(nn.Cell):
    ...
    def cogview_attention(self, attention_scores, alpha=32):
        ...
        max_value = ops.amax(scaled_attention_scores, axis=-1).unsqueeze(-1)
    ...
    def construct(
        ...
        attention_scores = ops.matmul(query_layer / math.sqrt(self.attention_head_size), key_layer.swapaxes(-1, -2))
...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):
    def __init__(self, config):
        ...
        if config.visual_embed:
            ...
            self.pos_embed = Parameter(ops.zeros((1, size * size + 1, config.hidden_size)), name="pos_embed")
    ...
    def init_visual_bbox(self, image_size=(14, 14), max_len=1000):
        ...
        visual_bbox_x = ops.div(
            ops.arange(0, max_len * (image_size[1] + 1), max_len).to(ms.float64), image_size[1], rounding_mode="trunc"
        )
        visual_bbox_y = ops.div(
            ops.arange(0, max_len * (image_size[0] + 1), max_len).to(ms.float64), image_size[0], rounding_mode="trunc"
        )
        visual_bbox = ops.stack(
            [
                ops.tile(visual_bbox_x[:-1], (image_size[0], 1)),
                ops.tile(visual_bbox_y[:-1], (image_size[1], 1)).swapaxes(0, 1),
                ops.tile(visual_bbox_x[1:], (image_size[0], 1)),
                ops.tile(visual_bbox_y[1:], (image_size[1], 1)).swapaxes(0, 1)
            ],
            axis=-1,
        ...
        self.visual_bbox = ops.cat([cls_token_box.to(visual_bbox.dtype), visual_bbox], axis=0)

    def calculate_visual_bbox(self, dtype, batch_size):
        visual_bbox = ops.tile(self.visual_bbox, (batch_size, 1, 1))
    ...
    def construct(
        ...
        if input_ids is not None or inputs_embeds is not None:
            ...
            if token_type_ids is None:
                token_type_ids = ops.zeros(input_shape, dtype=ms.int64)
            if bbox is None:
                bbox = ops.zeros(tuple(list(input_shape) + [4]), dtype=ms.int64)
        ...
        if pixel_values is not None:
            ...
            visual_attention_mask = ops.ones(
                (batch_size, visual_embeddings.shape[1]), dtype=ms.int64
            )
            if attention_mask is not None:
                attention_mask = ops.cat([attention_mask, visual_attention_mask.to(attention_mask.dtype)], axis=1)
            else:
            ...
            if self.config.has_relative_attention_bias or self.config.has_spatial_attention_bias:
                ...
                visual_position_ids = ops.tile(ops.arange(0, visual_embeddings.shape[1], dtype=ms.int64), (batch_size, 1))
                if input_ids is not None or inputs_embeds is not None:
            ...
            if self.config.has_relative_attention_bias:
                ...
                position_ids = ops.broadcast_to(position_ids, input_ids.shape)
...
class LayoutLMv3ClassificationHead(nn.Cell):
    ...
    def __init__(self, config, pool_feature=False):
        ...
        if pool_feature:
            self.dense = nn.Dense(config.hidden_size * 3, config.hidden_size)
        else:
            self.dense = nn.Dense(config.hidden_size, config.hidden_size)
        classifier_dropout = (
