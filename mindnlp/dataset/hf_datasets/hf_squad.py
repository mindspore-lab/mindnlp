...
# ============================================================================
"""
Hugging Face SQuAD load function
"""
...
class HFSquad:
    """
    Hugging Face squad dataset source
    """
...
def HF_SQuAD(
    ...
    Examples:
        ...
        >>> dataset_train, dataset_validation = HF_SQuAD(root, split, shuffle=False)
        >>> train_iter = dataset_train.create_tuple_iterator()
        >>> print(next(train_iter))
    """

    cache_dir = os.path.join(root, "datasets", "hf_datasets", "SQuAD")
    ...
    ds_list = hf_load('squad', split=mode_list, data_dir=cache_dir)
    for every_ds in ds_list:
        datasets_list.append(GeneratorDataset(
            source=HFSquad(every_ds),
...
@process.register
def HF_SQuAD_Process(dataset, tokenizer, vocab, batch_size=64, max_context_len=1000, max_qa_len=30,
                      bucket_boundaries=None, drop_remainder=False):
    """
    the process of the SQuAD dataset

    Args:
        dataset (GeneratorDataset): SQuADdataset.
    ...
    Examples:
        >>> dataset_train = HF_SQuAD(split = ('train',))
        ...
        >>> squad_train = HF_SQuAD_Process(dataset_train, tokenizer=tokenizer, vocab=vocab, \
