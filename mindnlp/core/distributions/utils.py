...
from mindnlp.core.overrides import is_tensor_like
from mindnlp.core.types import _dtype, _Number, Device, Number
...
def broadcast_all(*values: Union[Tensor, Number]) -> tuple[Tensor, ...]:
    ...
    value is broadcasted based on the following rules:
      - `core.*Tensor` instances are broadcasted as per :ref:`_broadcasting-semantics`.
      - Number instances (scalars) are upcast to tensors having
        the same size and type as the first tensor passed to `values`.  If all the
        values are scalars, then they are upcasted to scalar Tensors.

    Args:
        values (list of `Number`, `core.*Tensor` or objects implementing __torch_function__)

    Raises:
        ValueError: if any of the values is not a `Number` instance,
            a `core.*Tensor` instance, or an instance implementing __torch_function__
    """
    if not all(is_tensor_like(v) or isinstance(v, _Number) for v in values):
        raise ValueError(
            "Input arguments must all be instances of Number, "
            "core.Tensor or objects implementing __torch_function__."
        )
    if not all(is_tensor_like(v) for v in values):
        options: dict[str, Any] = dict(dtype=core.get_default_dtype())
        for value in values:
            if isinstance(value, core.Tensor):
                options = dict(dtype=value.dtype, device=value.device)
                break
        new_values = [
            v if is_tensor_like(v) else core.tensor(v, **options) for v in values
        ]
        return core.broadcast_tensors(*new_values)
    return core.broadcast_tensors(*values)
...
def clamp_probs(probs: Tensor) -> Tensor:
    ...
    Examples:
        >>> probs = core.tensor([0.0, 0.5, 1.0])
        >>> clamp_probs(probs)
        tensor([1.1921e-07, 5.0000e-01, 1.0000e+00])

        >>> probs = core.tensor([0.0, 0.5, 1.0], dtype=core.float64)
        >>> clamp_probs(probs)
        tensor([2.2204e-16, 5.0000e-01, 1.0000e+00], dtype=core.float64)

    """
    eps = core.finfo(probs.dtype).eps
...
def probs_to_logits(probs: Tensor, is_binary: bool = False) -> Tensor:
    ...
    if is_binary:
        return core.log(ps_clamped) - core.log1p(-ps_clamped)
    return core.log(ps_clamped)
...
def tril_matrix_to_vec(mat: Tensor, diag: int = 0) -> Tensor:
    ...
    """
    n = mat.shape[-1]
    if not core._C._get_tracing_state() and (diag < -n or diag >= n):
        raise ValueError(f"diag ({diag}) provided is outside [{-n}, {n - 1}].")
    arange = core.arange(n, device=mat.device)
...
def vec_to_tril_matrix(vec: Tensor, diag: int = 0) -> Tensor:
    ...
    eps = core.finfo(vec.dtype).eps
    if not core._C._get_tracing_state() and (round(n) - n > eps):
        raise ValueError(
    ...
    n = round(n.item()) if isinstance(n, core.Tensor) else round(n)
    mat = vec.new_zeros(vec.shape[:-1] + core.Size((n, n)))
    arange = core.arange(n, device=vec.device)
    tril_mask = arange < arange.view(-1, 1) + (diag + 1)
    mat[..., tril_mask] = vec
    return mat