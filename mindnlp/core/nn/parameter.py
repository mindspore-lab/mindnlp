...
class UninitializedTensorMixin:
    _allowed_methods = [
        core.Tensor.__hash__,
        core.Tensor.size,
        core.Tensor.copy_,
        core.Tensor.is_complex,
        core.Tensor.is_floating_point,
        core.Tensor.half,
        core.Tensor.float,
        core.Tensor.double,
        core.Tensor.char,
        ...
        core.Tensor.cpu,
        core.Tensor.to,
        core.Tensor.get_device,
        core._has_compatible_shallow_copy_type,
    ]
    ...
    def materialize(self, shape, device=None, dtype=None):
        ...
        Args:
            ...
            dtype (:class:`core.dtype`): the desired floating point type of
        ...
        self.data = core.empty(shape, device=device, dtype=dtype)
    ...
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        ...
        raise ValueError(
            ...
            f"explicitly manipulating `core.nn.parameter.{cls.__name__}` "
...
class UninitializedParameter(UninitializedTensorMixin, Parameter):
    ...
    def __new__(cls, requires_grad=True, device=None, dtype=None) -> None:
        ...
        data = core.empty(0, **factory_kwargs)
        return core.Tensor._make_subclass(cls, data, requires_grad)
...
class Buffer(Tensor):
    ...
    Buffers are :class:`~core.Tensor` subclasses, that have a
    very special property when used with :class:`Module` s -- when they're
    assigned as Module attributes they are automatically added to the list of
    its buffers, and will appear e.g. in :meth:`~core.nn.Module.buffers` iterator.
    Assigning a Tensor doesn't have such effect. One can still assign a Tensor as explicitly by using
    the :meth:`~core.nn.Module.register_buffer` function.
...
class UninitializedBuffer(UninitializedTensorMixin, core.Tensor):
    r"""A buffer that is not initialized.

    Uninitialized Buffer is a a special case of :class:`core.Tensor`
    where the shape of the data is still unknown.

    Unlike a :class:`core.Tensor`, uninitialized parameters
    hold no data and attempting to access some properties, like their shape,
    will throw a runtime error. The only operations that can be performed on a uninitialized
    parameter are changing its datatype, moving it to a different device and
    converting it to a regular :class:`core.Tensor`.

    The default device or dtype to use when the buffer is materialized can be set
    during construction using e.g. ``device='cuda'``.
    """

    cls_to_become = core.Tensor
    ...
    def __new__(
        ...
        factory_kwargs = {"device": device, "dtype": dtype}
        data = core.empty(0, **factory_kwargs)
        ret = core.Tensor._make_subclass(cls, data, requires_grad)
