...
def cached():
    ...
    .. code-block:: python
        ...
        from mindnlp import core.nn.utils.parametrize as P
        ...
        with P.cached():
            output = model(inputs)
...
class ParametrizationList(ModuleList):
    r"""A sequential container that holds and manages the original parameters or buffers of a parametrized :class:`core.nn.Module`.
...
def _inject_new_class(module: Module) -> None:
    ...
    def getstate(self):
        raise RuntimeError(
            ...
            "https://pycore.org/tutorials/beginner/saving_loading_models.html"
...
def register_parametrization(
    ...
    *,
    ...
    .. warning::
        ...
        to the optimizer. See :meth:`core.Optimizer.add_param_group`.
    ...
    Examples:
        ...
        >>> print(core.allclose(m.weight, m.weight.T))  # m.weight is now symmetric
        True
        >>> A = core.rand(5, 5)
        >>> A = A + A.T   # A is now symmetric
        >>> m.weight = A  # Initialize the weight to be the symmetric matrix A
        >>> print(core.allclose(m.weight, A))
        True
        ...
        >>>         U, S, Vh = core.linalg.svd(Z, full_matrices=False)
        >>>         # Return rescaled singular vectors
        >>>         s0_sqrt = S[0].sqrt().unsqueeze(-1)
        >>>         return U[..., :, 0] * s0_sqrt, Vh[..., 0, :] * s0_sqrt
        >>>
        >>> linear_rank_one = P.register_parametrization(nn.Linear(4, 4), "weight", RankOne())
        >>> print(core.linalg.matrix_rank(linear_rank_one.weight).item())
        1
...
def remove_parametrizations(
    ...
    if parametrizations.is_tensor:
        ...
        if leave_parametrized:
            ...
            with no_grad():
                if type(original) is core.Tensor:
                    ...
                    try:
                        ...
                        raise RuntimeError(
                            ...
                            "core.__future__.set_swap_module_params_on_conversion(True)."
                        ) from e
    else:
        if leave_parametrized:
