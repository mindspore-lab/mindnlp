...
class BasePruningMethod(ABC):
    ...
    def compute_mask(self, t, default_mask):
        ...
        Args:
            t (core.Tensor): tensor representing the importance scores of the
            parameter to prune.
            default_mask (core.Tensor): Base mask from previous pruning
            iterations, that need to be respected after the new mask is
            applied. Same dims as ``t``.

        Returns:
            mask (core.Tensor): mask to apply to ``t``, of same dims as ``t``
        """
    ...
    def apply_mask(self, module):
        ...
        Returns:
            pruned_tensor (core.Tensor): pruned version of the input tensor
        """
    ...
    def apply(cls, module, name, *args, importance_scores=None, **kwargs):
        ...
        Args:
            ...
            importance_scores (core.Tensor): tensor of importance scores (of
        ...
        if not isinstance(method, PruningContainer):
            ...
            default_mask = core.ones_like(orig)  # temp
        ...
        # to go
            ...
            default_mask = (
                ...
                .detach()
                .clone(memory_format=core.contiguous_format)
            )
    ...
    def prune(self, t, default_mask=None, importance_scores=None):
        ...
        Args:
            t (core.Tensor): tensor to prune (of same dimensions as
                ``default_mask``).
            importance_scores (core.Tensor): tensor of importance scores (of
            ...
            default_mask (core.Tensor, optional): mask from previous pruning
        ...
        if importance_scores is not None:
            ...
            importance_scores = t
        default_mask = default_mask if default_mask is not None else core.ones_like(t)
...
class PruningContainer(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        * for 'global', the mask will be computed across all entries.
        ...
        Args:
            t (core.Tensor): tensor representing the parameter to prune
                (of same dimensions as ``default_mask``).
            default_mask (core.Tensor): mask from previous pruning iteration.

        Returns:
            mask (core.Tensor): new mask that combines the effects
        ...
        def _combine_masks(method, t, mask):
            ...
            Args:
                ...
                mask (core.Tensor): mask from previous pruning iteration

            Returns:
                new_mask (core.Tensor): new mask that combines the effects
...
class RandomUnstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        mask = default_mask.clone(memory_format=core.contiguous_format)

        if nparams_toprune != 0:  # k=0 not supported by core.kthvalue
            prob = core.rand_like(t)
            topk = core.topk(prob.view(-1), k=nparams_toprune)
...
class L1Unstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        if nparams_toprune != 0:  # k=0 not supported by core.kthvalue
            ...
            topk = core.topk(core.abs(t).view(-1), k=nparams_toprune, largest=False)
    ...
    def apply(cls, module, name, amount, importance_scores=None):  # type: ignore[override]
        ...
        Args:
            ...
            amount (int or float): quantity of parameters to prune.
                ...
                fraction of parameters to prune. If ``int``, it represents the
                absolute number of parameters to prune.
            importance_scores (core.Tensor): tensor of importance scores (of same
...
class LnStructured(BasePruningMethod):
    ...
    Args:
        ...
        n (int, float, inf, -inf, 'fro', 'nuc'): See documentation of valid
            entries for argument ``p`` in :func:`core.norm`.
        dim (int, optional): index of the dim along which we define
            channels to prune. Default: -1.
    """
    ...
    def compute_mask(self, t, default_mask):
        ...
        topk = core.topk(norm, k=nparams_tokeep, largest=True)
        # topk will have .indices and .values
...
def random_structured(module, name, amount, dim):
    ...
    Examples:
        ...
        >>> columns_pruned = int(sum(core.sum(m.weight, dim=0) == 0))
...
def ln_structured(module, name, amount, n, dim, importance_scores=None):
    ...
    Examples:
        >>> from core.nn.utils import prune
...
def global_unstructured(parameters, pruning_method, importance_scores=None, **kwargs):
    ...
    Examples:
        ...
        ...     amount=10,
        ... )
        >>> print(sum(core.nn.utils.parameters_to_vector(net.buffers()) == 0))
        tensor(10)
    ...
    relevant_importance_scores = core.nn.utils.parameters_to_vector(
        [
    ...
    default_mask = core.nn.utils.parameters_to_vector(
        [
            getattr(module, name + "_mask", core.ones_like(getattr(module, name)))
            for (module, name) in parameters
        ]
    )
...
def custom_from_mask(module, name, mask):
    ...
    Examples:
        ...
        ...     nn.Linear(5, 3), name="bias", mask=core.tensor([0, 1, 0])
        ... )
        >>> print(m.bias_mask)
        tensor([0., 1., 0.])
...
def _compute_norm(t, n, dim):
    ...
    Args:
        t (core.Tensor): tensor representing the parameter to prune
        n (int, float, inf, -inf, 'fro', 'nuc'): See documentation of valid
            entries for argument p in core.norm
        dim (int): dim identifying the channels to prune

    Returns:
        norm (core.Tensor): L_n norm computed across all dimensions except
