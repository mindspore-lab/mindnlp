...
class _Orthogonal(Module):
    ...
    @no_grad()
    def right_inverse(self, Q: core.Tensor) -> core.Tensor:
        ...
        if not hasattr(self, "base"):
            ...
            # Y = core.cat([X.tril(), X.new_zeros(n, n - k).expand(*X.shape[:-2], -1, -1)], dim=-1)
            # A = Y - Y.mH
            # cayley(A)[:, :k]
            ...
            # householder(core.zeros(m,n)) == core.eye(m,n)
...
def orthogonal(
    ...
    *,
    ...
    - ``"matrix_exp"``/``"cayley"``:
      the :func:`~core.matrix_exp` :math:`Q = \exp(A)` and the `Cayley map`_
    ...
    - ``"householder"``: computes a product of Householder reflectors
      (:func:`~core.linalg.householder_product`).
    ...
    and it is orthogonalized via the QR decomposition otherwise (see :func:`core.linalg.qr`).
    ...
    .. note::
        ...
        in :func:`~core.nn.utils.parametrize.register_parametrization`.
...
class _SpectralNorm(Module):
    ...
    def _reshape_weight_to_matrix(self, weight: core.Tensor) -> core.Tensor:
        # Precondition
    ...
    @no_grad()
    def _power_method(self, weight_mat: core.Tensor, n_power_iterations: int) -> None:
    ...
    def forward(self, weight: core.Tensor) -> core.Tensor:
        if weight.ndim == 1:
    ...
    def right_inverse(self, value: core.Tensor) -> core.Tensor:
...
def spectral_norm(
    ...
    .. note::
        This function is implemented using the parametrization functionality
        in :func:`~core.nn.utils.parametrize.register_parametrization`. It is a
        reimplementation of :func:`core.nn.utils.spectral_norm`.
    ...
    Example::
        ...
        >>> core.linalg.matrix_norm(snm.weight, 2)
