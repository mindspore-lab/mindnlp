...
_tensor_or_tensors = Union[core.Tensor, Iterable[core.Tensor]]
...
def clip_grad_norm_(
        ...
        error_if_nonfinite: bool = False, foreach: Optional[bool] = None) -> core.Tensor:
    ...
    """
    if isinstance(parameters, core.Tensor):
        parameters = [parameters]
    grads = [p.grad for p in parameters if p.grad is not None]
    max_norm = float(max_norm)
    norm_type = float(norm_type)
    if len(grads) == 0:
        return core.tensor(0.)
    if norm_type == inf:
