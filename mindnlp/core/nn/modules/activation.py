...
class ReLU(Module):
    ...
    Examples::
      ...
      An implementation of CReLU - https://arxiv.org/abs/1603.05201
        ...
        >>> m = nn.ReLU()
        >>> input = core.randn(2).unsqueeze(0)
        >>> output = core.cat((m(input), m(-input)))
    """
...
class MultiheadAttention(Module):
    ...
    - Either autograd is disabled (using ``core.inference_mode`` or ``core.no_grad``) or no tensor argument ``requires_grad``
    - training is disabled (using ``.eval()``)
    - ``add_bias_kv`` is ``False``
    - ``add_zero_attn`` is ``False``
    - ``batch_first`` is ``True`` and the input is batched
    - ``kdim`` and ``vdim`` are equal to ``embed_dim``
    - if a `NestedTensor <https://pycore.org/docs/stable/nested.html>`_ is passed, neither ``key_padding_mask``
      nor ``attn_mask`` is passed
    - autocast is disabled

    If the optimized inference fastpath implementation is in use, a
    `NestedTensor <https://pycore.org/docs/stable/nested.html>`_ can be passed for
    ``query``/``key``/``value`` to represent padding more efficiently than using a
    padding mask. In this case, a `NestedTensor <https://pycore.org/docs/stable/nested.html>`_
...
class SELU(Module):
    ...
    .. warning::
        ...
        See :func:`core.nn.init.calculate_gain` for more information.
