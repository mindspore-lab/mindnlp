...
class Fold(Module):
    ...
    same specification as the output shape of :class:`~core.nn.Unfold`.) This
    operation combines these local blocks into the large :attr:`output` tensor
    of shape :math:`(N, C, \text{output\_size}[0], \text{output\_size}[1], \dots)`
    by summing the overlapping values. Similar to :class:`~core.nn.Unfold`, the
    ...
    * For the case of two output spatial dimensions this operation is sometimes
    ...
    .. note::
        :class:`~core.nn.Fold` calculates each combined value in the resulting
        large tensor by summing all values from all containing blocks.
        :class:`~core.nn.Unfold` extracts the values in the local blocks by
        copying from the large tensor. So, if the blocks overlap, they are not
        inverses of each other.

        In general, folding and unfolding operations are related as
        follows. Consider :class:`~core.nn.Fold` and
        :class:`~core.nn.Unfold` instances created with the same
        parameters:
        ...
        >>> # xdoctest: +SKIP
        >>> input_ones = core.ones(input.shape, dtype=input.dtype)
...
class Unfold(Module):
    ...
    * For the case of two input spatial dimensions this operation is sometimes
    ...
    Examples::
        ...
        >>> inp = core.randn(1, 3, 10, 12)
        >>> w = core.randn(2, 3, 4, 5)
        >>> inp_unf = core.nn.functional.unfold(inp, (4, 5))
        >>> out_unf = inp_unf.transpose(1, 2).matmul(w.view(w.size(0), -1).t()).transpose(1, 2)
        >>> out = core.nn.functional.fold(out_unf, (7, 8), (1, 1))
        >>> # or equivalently (and avoiding a copy),
        >>> # out = out_unf.view(1, 2, 7, 8)
        >>> (core.nn.functional.conv2d(inp, w) - out).abs().max()
        tensor(1.9073e-06)
