...
class Flatten(Module):
    ...
    For use with :class:`~nn.Sequential`, see :meth:`core.flatten` for details.
...
class Unflatten(Module):
    ...
    * :attr:`unflattened_size` is the new shape of the unflattened dimension of the tensor and it can be
      a `tuple` of ints or a `list` of ints or `core.Size` for `Tensor` input;  a `NamedShape`
    ...
    Args:
        ...
        unflattened_size (Union[core.Size, Tuple, List, NamedShape]): New shape of the unflattened dimension

    Examples:
        >>> input = core.randn(2, 50)
        >>> # With tuple of ints
        >>> m = nn.Sequential(
        >>>     nn.Linear(50, 50),
        ...
        >>> m = nn.Sequential(
        >>>     nn.Linear(50, 50),
        >>>     nn.Unflatten(1, core.Size([2, 5, 5]))
        >>> )
        >>> output = m(input)
        >>> output.size()
        core.Size([2, 2, 5, 5])
        >>> # With namedshape (tuple of tuples)
        >>> input = core.randn(2, 50, names=('N', 'features'))
