...
class L1Loss(_Loss):
    ...
    Examples::
        ...
        >>> loss = nn.L1Loss()
        >>> input = core.randn(3, 5, requires_grad=True)
        >>> target = core.randn(3, 5)
...
class NLLLoss(_WeightedLoss):
    ...
    Examples::
        ...
        >>> N, C = 5, 4
        >>> loss_fn = nn.NLLLoss()
        >>> data = core.randn(N, 16, 10, 10)
        >>> conv = nn.Conv2d(16, C, (3, 3))
        >>> log_softmax = nn.LogSoftmax(dim=1)
        >>> # output of conv forward is of shape [N, C, 8, 8]
        >>> output = log_softmax(conv(data))
        >>> # each element in target must have 0 <= value < C
        >>> target = core.empty(N, 8, 8, dtype=core.long).random_(0, C)
...
@deprecated(
    ...
    "https://pycore.org/docs/main/nn.html#core.nn.NLLLoss for more details.",
...
class PoissonNLLLoss(_Loss):
    ...
    Examples::
        ...
        >>> log_input = core.randn(5, 2, requires_grad=True)
        >>> target = core.randn(5, 2)
...
class GaussianNLLLoss(_Loss):
    ...
    Examples::
        ...
        >>> input = core.randn(5, 2, requires_grad=True)
        >>> target = core.randn(5, 2)
        >>> var = core.ones(5, 2, requires_grad=True)  # heteroscedastic
        ...
        >>> input = core.randn(5, 2, requires_grad=True)
        >>> target = core.randn(5, 2)
        >>> var = core.ones(5, 1, requires_grad=True)  # homoscedastic
        >>> output = loss(input, target, var)
        >>> output.backward()
...
class KLDivLoss(_Loss):
    ...
    Examples::
        ...
        >>> input = F.log_softmax(core.randn(3, 5, requires_grad=True), dim=1)
        >>> # Sample a batch of distributions. Usually this would come from the dataset
        >>> target = F.softmax(core.rand(3, 5), dim=1)
        >>> output = kl_loss(input, target)

        >>> kl_loss = nn.KLDivLoss(reduction="batchmean", log_target=True)
        >>> log_target = F.log_softmax(core.rand(3, 5), dim=1)
...
class BCELoss(_WeightedLoss):
    ...
    Examples::
        ...
        >>> m = nn.Sigmoid()
        >>> loss = nn.BCELoss()
        >>> input = core.randn(3, 2, requires_grad=True)
        >>> target = core.rand(3, 2, requires_grad=False)
...
class BCEWithLogitsLoss(_Loss):
    ...
    Examples::
        ...
        >>> target = core.ones([10, 64], dtype=core.float32)  # 64 classes, batch size = 10
        >>> output = core.full([10, 64], 1.5)  # A prediction (logit)
        >>> pos_weight = core.ones([64])  # All weights are equal to 1
        >>> criterion = core.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
    ...
    Shape:
     ...
     Examples::
        ...
        >>> input = core.randn(3, requires_grad=True)
        >>> target = core.empty(3).random_(2)
        >>> output = loss(input, target)
        >>> output.backward()
    """
...
class MultiLabelMarginLoss(_Loss):
    ...
    Examples::
        ...
        >>> x = core.FloatTensor([[0.1, 0.2, 0.4, 0.8]])
        >>> # for target y, only consider labels 3 and 0, not after label -1
        >>> y = core.LongTensor([[3, 0, -1, 1]])
...
class SmoothL1Loss(_Loss):
    ...
    It is less sensitive to outliers than :class:`core.nn.MSELoss` and in some cases
...
class CrossEntropyLoss(_WeightedLoss):
    ...
    - Class indices in the range :math:`[0, C)` where :math:`C` is the number of classes; if
      ...
      Note that this case is equivalent to applying :class:`~core.nn.LogSoftmax`
      on an input, followed by :class:`~core.nn.NLLLoss`.
    ...
    Examples::
        ...
        >>> input = core.randn(3, 5, requires_grad=True)
        >>> target = core.empty(3, dtype=core.long).random_(5)
        >>> output = loss(input, target)
        >>> output.backward()
        >>>
        >>> # Example of target with class probabilities
        >>> input = core.randn(3, 5, requires_grad=True)
        >>> target = core.randn(3, 5).softmax(dim=1)
        >>> output = loss(input, target)
        >>> output.backward()
    """
...
class CosineEmbeddingLoss(_Loss):
    ...
    Examples::
        ...
        >>> input1 = core.randn(3, 5, requires_grad=True)
        >>> input2 = core.randn(3, 5, requires_grad=True)
        >>> target = core.ones(3)
...
class MarginRankingLoss(_Loss):
    ...
    Examples::
        ...
        >>> input1 = core.randn(3, requires_grad=True)
        >>> input2 = core.randn(3, requires_grad=True)
        >>> target = core.randn(3).sign()
        >>> output = loss(input1, input2, target)
        >>> output.backward()
    """
    __constants__ = ['margin', 'reduction']
...
class TripletMarginLoss(_Loss):
    ...
    See also :class:`~core.nn.TripletMarginWithDistanceLoss`, which computes the
    ...
    >>> anchor = core.randn(100, 128, requires_grad=True)
    >>> positive = core.randn(100, 128, requires_grad=True)
    >>> negative = core.randn(100, 128, requires_grad=True)
...
class TripletMarginWithDistanceLoss(_Loss):
    ...
    See also :class:`~core.nn.TripletMarginLoss`, which computes the triplet
    ...
    >>> anchor_ids = core.randint(0, 1000, (1,))
    >>> positive_ids = core.randint(0, 1000, (1,))
    >>> negative_ids = core.randint(0, 1000, (1,))
    ...
    >>>     return core.max(core.abs(x1 - x2), dim=1).values
    >>>
...
class CTCLoss(_Loss):
    ...
    Shape:
        - Log_probs: Tensor of size :math:`(T, N, C)` or :math:`(T, C)`,
          ...
          :func:`core.nn.functional.log_softmax`).
    ...
    Examples::
        ...
        >>> input = core.randn(T, N, C).log_softmax(2).detach().requires_grad_()
        >>>
        >>> # Initialize random batch of targets (0 = blank, 1:C = classes)
        >>> target = core.randint(low=1, high=C, size=(N, S), dtype=core.long)
        >>>
        >>> input_lengths = core.full(size=(N,), fill_value=T, dtype=core.long)
        >>> target_lengths = core.randint(low=S_min, high=S, size=(N,), dtype=core.long)
        >>> ctc_loss = nn.CTCLoss()
        ...
        >>>
        >>> # Initialize random batch of targets (0 = blank, 1:C = classes)
        >>> target_lengths = core.randint(low=1, high=T, size=(N,), dtype=core.long)
        >>> target = core.randint(low=1, high=C, size=(sum(target_lengths),), dtype=core.long)
        >>> ctc_loss = nn.CTCLoss()
        >>> loss = ctc_loss(input, target, input_lengths, target_lengths)
        >>> loss.backward()
        >>>
        ...
        >>> input = core.randn(T, C).log_softmax(1).detach().requires_grad_()
        >>> input_lengths = core.tensor(T, dtype=core.long)
        >>>
        >>> # Initialize random batch of targets (0 = blank, 1:C = classes)
        >>> target_lengths = core.randint(low=1, high=T, size=(), dtype=core.long)
        >>> target = core.randint(low=1, high=C, size=(target_lengths,), dtype=core.long)
        >>> ctc_loss = nn.CTCLoss()
        >>> loss = ctc_loss(input, target, input_lengths, target_lengths)
        >>> loss.backward()
    ...
    Note:
        ...
        dtype :attr:`core.int32`.

        The regular implementation uses the (more common in PyTorch) `core.long` dtype.
    ...
    Note:
        ...
        a performance cost) by setting ``core.backends.cudnn.deterministic =
        True``.
