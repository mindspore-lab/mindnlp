...
class Sequential(Module):
    ...
    :class:`core.nn.ModuleList`? A ``ModuleList`` is exactly what it
...
class ModuleList(Module):
    ...
    :class:`~core.nn.ModuleList` can be indexed like a regular Python list, but
    modules it contains are properly registered, and will be visible by all
    :class:`~core.nn.Module` methods.
...
class ModuleDict(Module):
    ...
    :class:`~core.nn.ModuleDict` can be indexed like a regular Python dictionary,
    but modules it contains are properly registered, and will be visible by all
    :class:`~core.nn.Module` methods.

    :class:`~core.nn.ModuleDict` is an **ordered** dictionary that respects

    * the order of insertion, and

    * in :meth:`~core.nn.ModuleDict.update`, the order of the merged
      ``OrderedDict``, ``dict`` (started from Python 3.6) or another
      :class:`~core.nn.ModuleDict` (the argument to
      :meth:`~core.nn.ModuleDict.update`).

    Note that :meth:`~core.nn.ModuleDict.update` with other unordered mapping
    ...
    def update(self, modules: Mapping[str, Module]) -> None:
        r"""Update the :class:`~core.nn.ModuleDict` with key-value pairs from a mapping, overwriting existing keys.

        .. note::
            If :attr:`modules` is an ``OrderedDict``, a :class:`~core.nn.ModuleDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            modules (iterable): a mapping (dictionary) from string to :class:`~core.nn.Module`,
                or an iterable of key-value pairs of type (string, :class:`~core.nn.Module`)
        """
        if not isinstance(modules, container_abcs.Iterable):
...
class ParameterList(Module):
    ...
    Arguments:
        modules (list, optional): a list of :class:`~core.nn.Parameter`` to add
    ...
    Example::
        ...
        class MyModule(nn.Module):
            def __init__(self):
                ...
                self.params = nn.ParameterList([nn.Parameter(core.randn(10, 10)) for i in range(10)])
...
class ParameterDict(Module):
    ...
    :class:`~core.nn.ParameterDict` is an **ordered** dictionary.
    :meth:`~core.nn.ParameterDict.update` with other unordered mapping
    types (e.g., Python's plain ``dict``) does not preserve the order of the
    merged mapping. On the other hand, ``OrderedDict`` or another :class:`~core.nn.ParameterDict`
    will preserve their ordering.

    Note that the constructor, assigning an element of the dictionary and the
    :meth:`~core.nn.ParameterDict.update` method will convert any :class:`~core.Tensor` into
    :class:`~core.nn.Parameter`.
        ...
        class MyModule(nn.Module):
            def __init__(self):
                ...
                self.params = nn.ParameterDict({
                        'left': nn.Parameter(core.randn(5, 10)),
                        'right': nn.Parameter(core.randn(5, 10))
                })
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        ...
        self._keys[key] = None
        attr = self._key_to_attr(key)
        if isinstance(value, core.Tensor) and not isinstance(value, Parameter):
    ...
    def copy(self) -> 'ParameterDict':
        """Return a copy of this :class:`~core.nn.ParameterDict` instance."""
    ...
    def update(self, parameters: Union[Mapping[str, Any], 'ParameterDict']) -> None:
        r"""Update the :class:`~core.nn.ParameterDict` with key-value pairs from ``parameters``, overwriting existing keys.

        .. note::
            If :attr:`parameters` is an ``OrderedDict``, a :class:`~core.nn.ParameterDict`, or
            an iterable of key-value pairs, the order of new elements in it is preserved.

        Args:
            parameters (iterable): a mapping (dictionary) from string to
                :class:`~core.nn.Parameter`, or an iterable of
                key-value pairs of type (string, :class:`~core.nn.Parameter`)
        """
