...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for group in self.param_groups:
            ...
            momentum = core.tensor(group['momentum'], dtype=core.float32, device=group['params'][0].device)
            lr = core.tensor(group['lr'], dtype=core.float32, device=group['params'][0].device)
            ...
            for p in group['params']:
                ...
                # assign(p, new_p)
                stat = core.ones_like(p)
                accum = core.zeros_like(p)
