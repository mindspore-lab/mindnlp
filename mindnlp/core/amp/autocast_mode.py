...
class autocast:
    ...
    def __enter__(self):
        self.prev_fastdtype = core.get_autocast_dtype(self.device_type)
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any):  # type: ignore[override]
        pop_amp_strategy()
        core.set_autocast_dtype(self.device_type, self.prev_fastdtype)
        return False
...
def custom_fwd(
    ...
    *,
    ...
    Autograd functions are subclasses of :class:`core.autograd.Function`.
    ...
    Args:
        device_type(str):  Device type to use. 'cuda', 'cpu', 'xpu' and so on.
            The type is the same as the `type` attribute of a :class:`core.device`.
            Thus, you may obtain the device type of a tensor using `Tensor.device.type`.
        cast_inputs (:class:`core.dtype` or None, optional, default=None):  If not ``None``,
    ...
    def decorate_fwd(*args, **kwargs):
        args[0]._dtype = core.get_autocast_dtype(device_type)
        if cast_inputs is None:
            args[0]._fwd_used_autocast = core.is_autocast_enabled(device_type)
            return fwd(*args, **kwargs)
        else:
            autocast_context = core.is_autocast_enabled(device_type)
