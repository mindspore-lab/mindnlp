...
class _MultiDeviceReplicator:
    ...
    def __init__(self, master_tensor: core.Tensor) -> None:
        self.master = master_tensor
        self._per_device_tensors: Dict[core.device, core.Tensor] = {}

    def get(self, device: core.device) -> core.Tensor:
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    Args:
        device (str, optional, default="cuda"): Device type to use. Possible values are: 'cuda' and 'cpu'.
            The type is the same as the `type` attribute of a :class:`core.device`.
    ...
    def __init__(
        ...
        if self._device == "cuda":
            if enabled and core.cuda.amp.common.amp_definitely_not_available():
                warnings.warn(
                    "core.cuda.amp.GradScaler is enabled, but CUDA is not available.  Disabling."
                )
                self._enabled = False
        ...
        if self._enabled:
            ...
            self._scale: Optional[core.Tensor] = None
            self._growth_factor = growth_factor
            self._backoff_factor = backoff_factor
            self._growth_interval = growth_interval
            self._init_growth_tracker = 0
            # self._growth_tracker will be lazily initialized during the first call to scale()
            self._growth_tracker: Optional[core.Tensor] = None
    ...
    def _lazy_init_scale_growth_tracker(self, dev: core.device) -> None:
        assert self._growth_tracker is None, "_growth_tracker initialized before _scale"
        self._scale = core.full((), self._init_scale, dtype=core.float32, device=dev)
        self._growth_tracker = core.full(
            (), self._init_growth_tracker, dtype=core.int32, device=dev
        )

    @overload
    def scale(self, outputs: core.Tensor) -> core.Tensor:
        ...

    @overload
    def scale(self, outputs: List[core.Tensor]) -> List[core.Tensor]:
        ...

    @overload
    def scale(self, outputs: Tuple[core.Tensor, ...]) -> Tuple[core.Tensor, ...]:
        ...

    @overload
    def scale(self, outputs: Iterable[core.Tensor]) -> Iterable[core.Tensor]:
        ...

    def scale(
        self,
        outputs: Union[core.Tensor, Iterable[core.Tensor]],
    ) -> Union[core.Tensor, Iterable[core.Tensor]]:
        """
        ...
        def apply_scale(val: Union[core.Tensor, Iterable[core.Tensor]]):
    ...
    def _unscale_grads_(
        self,
        optimizer: core.optim.Optimizer,
        inv_scale: core.Tensor,
        found_inf: core.Tensor,
        allow_fp16: bool,
    ) -> Dict[core.device, core.Tensor]:
        ...
        per_device_and_dtype_grads: Dict[
            core.device, Dict[core.dtype, List[core.Tensor]]
        ] = defaultdict(lambda: defaultdict(list))
        with core.no_grad():
            for group in optimizer.param_groups:
                for param in group["params"]:
                    assert isinstance(param, core.Tensor)
                    if param.grad is None:
                        continue
                    if (not allow_fp16) and param.grad.dtype == core.float16:
                    ...
                    if param.grad.is_sparse:
                        ...
                        if param.grad.dtype is core.float16:
    ...
    def unscale_(self, optimizer: core.optim.Optimizer) -> None:
        """
        ...
        Simple example, using :meth:`unscale_` to enable clipping of unscaled gradients::
            ...
            core.nn.utils.clip_grad_norm_(model.parameters(), max_norm)
            scaler.step(optimizer)
            scaler.update()

        Args:
            optimizer (core.optim.Optimizer):  Optimizer that owns the gradients to be unscaled.
        ...
        found_inf = core.full((), 0.0, dtype=core.float32, device=self._scale.device)
    ...
    def step(
        self, optimizer: core.optim.Optimizer, *args: Any, **kwargs: Any
    ) -> Optional[float]:
        ...
        Args:
            optimizer (core.optim.Optimizer):  Optimizer that applies the gradients.
            args:  Any arguments.
    ...
    def update(self, new_scale: Optional[Union[float, core.Tensor]] = None) -> None:
        ...
        Args:
            new_scale (float or :class:`core.Tensor`, optional, default=None):  New scale factor.
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = "new_scale should be a float or a 1-element core.cuda.FloatTensor or \
                    core.FloatTensor with requires_grad=False."
    ...
    def _check_inf_per_device(self, optimizer: core.optim.Optimizer) -> Dict[str, Any]:
        _scale, _ = self._check_scale_growth_tracker("_check_inf_per_device")

        dummy_inv_scale = core.full((), 1.0, dtype=core.float32, device=_scale.device)
        found_inf = core.full((), 0.0, dtype=core.float32, device=_scale.device)
    ...
    def _found_inf_per_device(self, optimizer: core.optim.Optimizer) -> Dict[str, Any]:
        return self._per_optimizer_states[id(optimizer)]["found_inf_per_device"]