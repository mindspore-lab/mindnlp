...
class _StorageBase:
    ...
    device: core.device
    # Used when
    # (1) stashing FakeTensor device onto storage in core.serialization.skip_data
    # (2) stashing device onto storage to propagate to FakeTensor when core.load under FakeTensorMode
    _fake_device: _Optional[core.device] = None
    # Used when loading with FakeTensorMode to give information about offset of storage in core.saved-file
    ...
    def cuda(
        ...
        """
        device2 = core.device("cuda", device) if device else core.device("cuda")
    ...
    def hpu(self, device=None, non_blocking=False) -> Union[_StorageBase, TypedStorage]:
        ...
        Args:
            ...
            non_blocking (bool): If ``True`` and the source is in pinned memory,
                ...
                the argument has no effect.
        """
        device2 = core.device("hpu", device) if device else core.device("hpu")
        return self.to(device=device2, non_blocking=non_blocking)
    ...
    def __repr__(self):
        info_str = f"[{core.typename(self)}(device={self.device}) of size {len(self)}]"
    ...
    def __reduce__(self):
        b = io.BytesIO()
        core.save(self, b, _use_new_zipfile_serialization=False)
    ...
    def cpu(self):
        ...
        if self.device.type != "cpu":
            return core.UntypedStorage(self.size()).copy_(self, False)
        return self

    def mps(self):
        """Return a MPS copy of this storage if it's not already on the MPS."""
        if self.device.type != "mps":
            return core.UntypedStorage(self.size(), device="mps").copy_(self, False)
        return self

    def _to(self, dtype):
        if not isinstance(dtype, core.dtype):
            raise TypeError(f"Argument 'dtype' must be core.dtype, not {type(dtype)}")
        storage = (
            core.tensor([], dtype=core.uint8, device=self.device)
    ...
    def double(self):
        ...
        return self._to(core.double)

    def float(self):
        """Casts this storage to float type."""
        return self._to(core.float)

    def half(self):
        """Casts this storage to half type."""
        return self._to(core.half)

    def long(self):
        """Casts this storage to long type."""
        return self._to(core.long)

    def int(self):
        """Casts this storage to int type."""
        return self._to(core.int)

    def short(self):
        """Casts this storage to short type."""
        return self._to(core.short)

    def char(self):
        """Casts this storage to char type."""
        return self._to(core.int8)

    def byte(self):
        """Casts this storage to byte type."""
        return self._to(core.uint8)

    def bool(self):
        """Casts this storage to bool type."""
        return self._to(core.bool)

    def bfloat16(self):
        """Casts this storage to bfloat16 type."""
        return self._to(core.bfloat16)

    def complex_double(self):
        """Casts this storage to complex double type."""
        return self._to(core.cdouble)

    def complex_float(self):
        """Casts this storage to complex float type."""
        return self._to(core.cfloat)

    def float8_e5m2(self):
        """Casts this storage to float8_e5m2 type"""
        return self._to(core.float8_e5m2)

    def float8_e4m3fn(self):
        """Casts this storage to float8_e4m3fn type"""
        return self._to(core.float8_e4m3fn)

    def float8_e5m2fnuz(self):
        """Casts this storage to float8_e5m2fnuz type"""
        return self._to(core.float8_e5m2fnuz)

    def float8_e4m3fnuz(self):
        """Casts this storage to float8_e4m3fnuz type"""
        return self._to(core.float8_e4m3fnuz)

    def is_pinned(self, device: Union[str, core.device] = "cuda"):
        r"""Determine whether the CPU storage is already pinned on device.

        Args:
            device (str or core.device): The device to pin memory on (default: ``'cuda'``).
    ...
    def pin_memory(self, device: Union[str, core.device] = "cuda"):
    ...
    def share_memory_(self):
        """See :meth:`core.UntypedStorage.share_memory_`"""
        from core.multiprocessing import get_sharing_strategy

        if self.device.type in ["cuda", core._C._get_privateuse1_backend_name()]:
    ...
    def _new_shared(cls, size, *, device="cpu"):
        ...
        device = core.device(device)
        if device.type in ["cuda", core._C._get_privateuse1_backend_name(), "hpu"]:
    ...
    def byteswap(self, dtype):
        ...
        elem_size = core._utils._element_size(dtype)
...
class UntypedStorage(_StorageBase):
    ...
    def filename(self) -> _Optional[str]:
        ...
        :meth:`~core.from_file()` with ``shared`` as ``True``. This attribute is ``None`` otherwise.
        """
    ...
    def share_memory_(self, *args, **kwargs):
        ...
        .. note::
            ...
            #. ``share_memory_`` will call ``shm_unlink(3)`` on the object after mapping it to make sure the shared memory
               object is freed when no process has the object open. ``core.from_file(shared=True)`` does not unlink the
    ...
    def from_file(cls, filename, shared, nbytes):
        ...
        return cls(data, device=core.device('cpu'))
...
def _load_from_bytes(b):
    return core.load(io.BytesIO(b), weights_only=False)
...
def _dtype_to_storage_type_map():
    ...
    # be serialized as an UntypedStorage paired with a core.dtype
    return {
        core.double: "DoubleStorage",
        core.float: "FloatStorage",
        core.half: "HalfStorage",
        core.long: "LongStorage",
        core.int: "IntStorage",
        core.int16: "ShortStorage",
        core.int8: "CharStorage",
        core.uint8: "ByteStorage",
        core.bool: "BoolStorage",
        core.bfloat16: "BFloat16Storage",
        core.cdouble: "ComplexDoubleStorage",
        core.cfloat: "ComplexFloatStorage",
        # core.qint8: "QInt8Storage",
        # core.qint32: "QInt32Storage",
        # core.quint8: "QUInt8Storage",
        # core.quint4x2: "QUInt4x2Storage",
        # core.quint2x4: "QUInt2x4Storage",
    }
...
def _get_storage_from_sequence(sequence, dtype, device):
    if dtype in [
        ...
        tmp_tensor = core.tensor(
            sequence, dtype=interpret_dtypes[dtype], device=device
        )

    else:
        tmp_tensor = core.tensor(sequence, dtype=dtype, device=device)
...
def _get_device_from_module(module: str):
    ...
    if last_part in ["cuda", core._C._get_privateuse1_backend_name(), "hpu"]:
        return last_part
    else:
        return "cpu"
...
class TypedStorage:
    is_sparse: _bool = False
    # Used when stashing FakeTensor device onto storage in core.save(metadata_only=True)
    _fake_device: _Optional[core.device] = None
    ...
    def __new__(
        ...
        *args,
        ...
        if cls == core.storage._LegacyStorage:
            raise RuntimeError(
        ...
        if cls == TypedStorage:
            ...
            if wrap_storage is None:
                ...
                if not isinstance(wrap_storage, core.UntypedStorage):
                    raise TypeError(
                        arg_error_msg
    ...
    def __init__(
        ...
        *args,
        ...
        arg_error_msg = (
            ...
            " * (*, UntypedStorage wrap_storage, core.dtype dtype)"
        )
        ...
        if wrap_storage is not None:
            ...
            if not isinstance(dtype, core.dtype):
                raise TypeError(
                    arg_error_msg
                    + f"\nArgument 'dtype' must be core.dtype, not {type(dtype)}"
                )
            ...
            self.dtype = core.get_default_dtype() if dtype is None else dtype
            device = core.device("cpu" if device is None else device)
            ...
            if len(args) == 0:
                self._untyped_storage = core.UntypedStorage(device=device)

            elif len(args) == 1:
                if _isint(args[0]):
                    self._untyped_storage = core.UntypedStorage(
    ...
    def untyped(self):
        """Return the internal :class:`core.UntypedStorage`."""
        _warn_typed_storage_removal()
        return self._untyped_storage

    def _new_wrapped_storage(self, untyped_storage) -> Self:
        assert type(untyped_storage) == core.UntypedStorage
    ...
    def _element_size(self):
        return core._utils._element_size(self.dtype)
    ...
    def __str__(self):
        _warn_typed_storage_removal()
        info_str = (
            f"[{core.typename(self)}(dtype={self.dtype}, "
    ...
    def _new_shared(self, size, *, device=None):
        ...
        if device is None:
            device = "cpu"
        device = core.device(device)
        untyped_storage = core.UntypedStorage._new_shared(
    ...
    def _from_buffer(cls, *args, dtype=None, device=None, **kwargs):
        if cls == TypedStorage:
            dtype = core.get_default_dtype() if dtype is None else dtype
            device = core.device("cpu" if device is None else device)
            if device.type != "cpu":
                raise RuntimeError(
                    f"TypedStorage.from_buffer: Not available for device {device.type}"
                )
            untyped_storage: core.UntypedStorage = core.UntypedStorage.from_buffer(
            ...
            dtype = cls._dtype
            untyped_storage = core.UntypedStorage.from_buffer(
                *args, dtype=dtype, **kwargs
            )
    ...
    def _to(self, dtype):
        if not isinstance(dtype, core.dtype):
            raise TypeError(f"Argument 'dtype' must be core.dtype, not {type(dtype)}")
        storage = (
            core.tensor([], dtype=self.dtype, device=self.device)
            .set_(self)
            .to(dtype)
            ._typed_storage()
        )
    ...
    def from_file(cls, filename, shared, size):
        ...
        untyped_storage = UntypedStorage.from_file(
            filename, shared, size * core._utils._element_size(cls.dtype)
        )
    ...
    def _new_shared_cuda(cls, *args, **kwargs):
        return core.UntypedStorage._new_shared_cuda(*args, **kwargs)
    ...
    def _release_ipc_counter(cls, *args, device=None, **kwargs):
        return core.UntypedStorage._release_ipc_counter_cuda(*args, **kwargs)
...
class _LegacyStorage(TypedStorage, metaclass=_LegacyStorageMeta):
    ...
    def _new_shared(cls, size):
        """Create a new storage in shared memory with the same data type."""
        untyped_storage = core.UntypedStorage._new_shared(size * cls()._element_size())
    ...
    def _new_shared_filename(cls, manager, obj, size):
        bytes_size = size * core._utils._element_size(cls.dtype)
        return cls(
            wrap_storage=core.UntypedStorage._new_shared_filename_cpu(
