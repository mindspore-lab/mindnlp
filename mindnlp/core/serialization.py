...
def _is_zipfile(f) -> bool:
    ...
    # binary. Since we expect the files here to be generated by core.save or
    # core.jit.save, it's safe to only check the start bytes and avoid
...
def _check_seekable(f) -> bool:
    ...
    def raise_err_msg(patterns, e):
        for p in patterns:
            if p in str(e):
                msg = (str(e) + ". You can only core.load from a file that is seekable."
...
def load(f, map_location=None, pickle_module=pickle, *, weights_only=False, mmap=None, **pickle_load_args):
    ...
    Raises:
        ...
        RuntimeError: Raised if mmap argument is used without files saved with `core.save(_use_new_zipfile_serialization=True)`.
    """
    ...
    with _open_file_like(f, 'rb') as opened_file:
        if _is_zipfile(opened_file):
            ...
            # If we want to actually tail call to core.jit.load, we need to
        ...
        if mmap:
            raise RuntimeError("mmap can only be used with files saved with ",
                               "`core.save(_use_new_zipfile_serialization=True), "
                               "please core.save your checkpoint with this option in order to use mmap.")
...
def _legacy_load(f, pickle_module, **pickle_load_args):
    ...
    Raises:
        ...
        RuntimeError: Raised if there is an issue with the file-like object compatibility with core.load.
    """
    ...
    if f_should_read_directly and f.tell() == 0:
        ...
        try:
            ...
            if _is_zipfile(f):
                # .zip is used for core.jit.save and will throw an un-pickling error here
                raise RuntimeError(
                    f"{f.name} is a zip archive (did you mean to use core.jit.load()?)") from None
            # if not a tarfile, reset file offset and proceed
            f.seek(0)

    if not hasattr(f, 'readinto') and (3, 8, 0) <= sys.version_info < (3, 8, 2):
        raise RuntimeError(
            "core.load does not work with file-like objects that do not implement readinto on Python 3.8.0 and 3.8.1. "
    ...
    for k, v in result.items():
        ...
        new_result[k] = core.from_numpy(array)
...
def _load(zip_file, pickle_module, overall_storage=None, pickle_file='data.pkl', **pickle_load_args):
    ...
    if not zip_file.has_record(byteordername) and \
        ...
        warnings.warn("The default load endianness for checkpoints without a byteorder mark "
                      ...
                      "core.serialization.set_default_load_endianness to set "
...
def convert_torch_to_mindspore(pth_file):
    ...
    try:
        from mindnlp import core # pylint: disable=import-error
    except Exception as exc:
        raise ImportError("'from mindnlp import core' failed, please install torch by "
                        "`pip install torch` or instructions from 'https://pycore.org'") \
                        from exc
    if pth_file.endswith(".safetensors"):
        ...
        state_dict = core.load(pth_file, map_location='cpu')
    ...
    for key, value in state_dict.items():
        if value.dtype == core.bfloat16:
            data = core.from_numpy(value.to(core.float).numpy().astype(np.float16))
            if not has_bf16:
                has_bf16 = True
        else:
            data = core.from_numpy(value.numpy())
    ...
    try:
        ...
        for k, v in safeview:
            ...
            if (not SUPPORT_BF16 and dtype != bfloat16) or SUPPORT_BF16:
                result[k] = core.from_numpy(arr)
            else:
                result[k] = core.from_numpy(arr.astype(np.float16))
        return result
    ...
    try:
        ...
        for element_id, element in enumerate(checkpoint_list.value):
            ...
            if data_type == "BFloat16":
                ...
                parameter = core.from_numpy(param_data)
