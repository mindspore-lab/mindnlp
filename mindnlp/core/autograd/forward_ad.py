...
def enter_dual_level():
    ...
    new_level = core._C._enter_dual_level()
...
def exit_dual_level(*, level=None):
    ...
    core._C._exit_dual_level(level=level)
    _current_level = level - 1


def _maybe_load_decompositions():
    if os.environ.get("PYTORCH_JIT", "1") == "1" and __debug__:
        from core._decomp import decompositions_for_jvp  # noqa: F401
...
def make_dual(tensor, tangent, *, level=None):
    ...
    Please see the `forward-mode AD tutorial <https://pycore.org/tutorials/intermediate/forward_ad_usage.html>`__
    for detailed steps on how to use this API.

    """
    # See NOTE: [forward-mode AD decompositions mechanism]
    #
    # Import from core._decomp import decompositions_for_jvp to register
    ...
    #     else:
    #         buffer = z
    #     return min - core.log1p(z), buffer
    #     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ <--- HERE
    ...
    return core._VF._make_dual(tensor, tangent, level=level)
...
def unpack_dual(tensor, *, level=None):
    ...
    primal, dual = core._VF._unpack_dual(tensor, level=level)
...
class dual_level(_DecoratorContextManager):
    ...
    .. Note::
        ...
        forward grads, one can use :func:`core.func.jvp`.
...
# _is_fwd_grad_enabled = core._C._is_fwd_grad_enabled
...
class _set_fwd_grad_enabled(_DecoratorContextManager):
    def __init__(self, mode: bool) -> None:
        ...
        core._C._set_fwd_grad_enabled(mode)

    def __enter__(self) -> None:
        pass

    def __exit__(self, exc_type: Any, exc_value: Any, traceback: Any) -> None:
        core._C._set_fwd_grad_enabled(self.prev)