...
# Some analysis of tolerance by logging tests from test_core.py can be found in
# https://github.com/pytorch/pytorch/pull/32538.
# {dtype: (rtol, atol)}
_DTYPE_PRECISIONS = {
    core.float16: (0.001, 1e-5),
    core.bfloat16: (0.016, 1e-5),
    core.float32: (1.3e-6, 1e-5),
    core.float64: (1e-7, 1e-7),
    # core.complex32: (0.001, 1e-5),
    # core.complex64: (1.3e-6, 1e-5),
    # core.complex128: (1e-7, 1e-7),
}
# # The default tolerances of core.float32 are used for quantized dtypes, because quantized tensors are compared in
...
#     dict.fromkeys(
#         (core.quint8, core.quint2x4, core.quint4x2, core.qint8, core.qint32),
#         _DTYPE_PRECISIONS[core.float32],
#     )
# )


def default_tolerances(
    *inputs: Union[core.Tensor, core.dtype],
    dtype_precisions: Optional[dict[core.dtype, tuple[float, float]]] = None,
) -> tuple[float, float]:
    ...
    for input in inputs:
        ...
        elif isinstance(input, core.dtype):
            dtypes.append(input)
        else:
            raise TypeError(
                f"Expected a core.Tensor or a core.dtype, but got {type(input)} instead."
            )
...
def make_tensor_mismatch_msg(
    ...
    *,
    ...
    Args:
        actual (core.Tensor): Actual tensor.
        expected (core.Tensor): Expected tensor.
        matches (core.Tensor): Boolean mask of the same shape as ``actual`` and ``expected`` that indicates the
            location of matches.
        rtol (float): Relative tolerance.
        atol (float): Absolute tolerance.
    ...
    total_mismatches = number_of_elements - int(core.sum(matches))
    extra = (
    ...
    if actual.dtype.is_floating_point and actual.dtype.itemsize == 1:
        ...
        first_mismatch_idx = tuple(core.nonzero(~matches, as_tuple=False)[0].tolist())
    ...
    if not actual.dtype.is_floating_point and not actual.dtype.is_complex:
        ...
        #  overflow
        actual_flat = actual_flat.to(core.int64)
        expected_flat = expected_flat.to(core.int64)

    abs_diff = core.abs(actual_flat - expected_flat)
    # Ensure that only mismatches are used for the max_abs_diff computation
    abs_diff[matches_flat] = 0
    max_abs_diff, max_abs_diff_flat_idx = core.max(abs_diff, 0)

    rel_diff = abs_diff / core.abs(expected_flat)
    # Ensure that only mismatches are used for the max_rel_diff computation
    rel_diff[matches_flat] = 0
    max_rel_diff, max_rel_diff_flat_idx = core.max(rel_diff, 0)
    return _make_mismatch_msg(
        default_identifier="Tensor-likes",
        identifier=identifier,
        extra=extra,
...
class NumberPair(Pair):
    ...
    The following table displays correspondence between Python number type and the ``core.dtype``'s. See
    :func:`assert_close` for the corresponding tolerances.

    +------------------+-------------------------------+
    | ``type``         | corresponding ``core.dtype`` |
    +==================+===============================+
    | :class:`int`     | :attr:`~core.int64`          |
    +------------------+-------------------------------+
    | :class:`float`   | :attr:`~core.float64`        |
    +------------------+-------------------------------+
    | :class:`complex` | :attr:`~core.complex64`      |
    +------------------+-------------------------------+
    """
...
class TensorLikePair(Pair):
    """Pair for :class:`core.Tensor`-like inputs.
    ...
    Kwargs:
        ...
        check_device (bool): If ``True`` (default), asserts that corresponding tensors are on the same
            :attr:`~core.Tensor.device`. If this check is disabled, tensors on different
            :attr:`~core.Tensor.device`'s are moved to the CPU before being compared.
        check_dtype (bool): If ``True`` (default), asserts that corresponding tensors have the same ``dtype``. If this
            ...
            :func:`core.promote_types`) before being compared.
    ...
    def _to_tensor(self, tensor_like: Any) -> core.Tensor:
        ...
        try:
            return core.as_tensor(tensor_like)
        except Exception:
            self._inputs_not_supported()
    ...
    def _compare_attributes(
        ...
        - the :attr:`~core.Tensor.shape`,
        - whether both inputs are quantized or not,
        - and if they use the same quantization scheme.

        Checks for

        - :attr:`~core.Tensor.layout`,
        - :meth:`~core.Tensor.stride`,
        - :attr:`~core.Tensor.device`, and
        - :attr:`~core.Tensor.dtype`
    ...
    def _equalize_attributes(
        self, actual: core.Tensor, expected: core.Tensor
    ) -> tuple[core.Tensor, core.Tensor]:
        """Equalizes some attributes of two tensors for value comparison.

        If ``actual`` and ``expected`` are ...

        - ... not on the same :attr:`~core.Tensor.device`, they are moved CPU memory.
        - ... not of the same ``dtype``, they are promoted  to a common ``dtype`` (according to
            :func:`core.promote_types`).
        ...
        if actual.dtype != expected.dtype:
            ...
            if actual_dtype in [core.uint64, core.uint32, core.uint16]:
                actual_dtype = core.int64
            if expected_dtype in [core.uint64, core.uint32, core.uint16]:
                expected_dtype = core.int64
            dtype = core.promote_types(actual_dtype, expected_dtype)
            actual = actual.to(dtype)
            expected = expected.to(dtype)

        if actual.layout != expected.layout:
            # These checks are needed, since Tensor.to_dense() fails on tensors that are already strided
            actual = actual.to_dense() if actual.layout != core.strided else actual
            expected = (
                expected.to_dense() if expected.layout != core.strided else expected
            )

        return actual, expected

    def _compare_values(self, actual: core.Tensor, expected: core.Tensor) -> None:
        if actual.dtype.is_floating_point and actual.dtype.itemsize == 1:
    ...
    def _compare_quantized_values(
        ...
        *,
        ...
        """Compares quantized tensors by comparing the :meth:`~core.Tensor.dequantize`'d variants for closeness.
    ...
    def _compare_sparse_compressed_values(
        ...
        *,
        ...
        format_name, compressed_indices_method, plain_indices_method = {
            core.sparse_csr: (
                "CSR",
                core.Tensor.crow_indices,
                core.Tensor.col_indices,
            ),
            core.sparse_csc: (
                "CSC",
                core.Tensor.ccol_indices,
                core.Tensor.row_indices,
            ),
            core.sparse_bsr: (
                "BSR",
                core.Tensor.crow_indices,
        ...
        # Compressed and plain indices in the CSR / CSC / BSR / BSC sparse formats can be `core.int32` _or_
        # `core.int64`. While the same dtype is enforced for the compressed and plain indices of a single tensor, it
        ...
        indices_dtype = core.promote_types(
...
def assert_close(
    ...
    *,
    ...
    - :attr:`~core.Tensor.device` (if ``check_device`` is ``True``),
    ...
    :meth:`~core.Tensor.qscheme` and the result of :meth:`~core.Tensor.dequantize` is close according to the
    definition above.

    ``actual`` and ``expected`` can be :class:`~core.Tensor`'s or any tensor-or-scalar-likes from which
    :class:`core.Tensor`'s can be constructed with :func:`core.as_tensor`. Except for Python scalars the input types
    ...
    Args:
        ...
        rtol (Optional[float]): Relative tolerance. If specified ``atol`` must also be specified. If omitted, default
            values based on the :attr:`~core.Tensor.dtype` are selected with the below table.
        atol (Optional[float]): Absolute tolerance. If specified ``rtol`` must also be specified. If omitted, default
            values based on the :attr:`~core.Tensor.dtype` are selected with the below table.
    ...
    Raises:
        ValueError: If no :class:`core.Tensor` can be constructed from an input.
        ...
        AssertionError: If corresponding tensors do not have the same :attr:`~core.Tensor.shape`.
        ...
        AssertionError: If corresponding tensors are quantized, but have different :meth:`~core.Tensor.qscheme`'s.
        AssertionError: If ``check_device`` is ``True``, but corresponding tensors are not on the same
            :attr:`~core.Tensor.device`.
    ...
    | ``dtype``                 | ``rtol``   | ``atol`` |
    +===========================+============+==========+
    | :attr:`~core.float16`    | ``1e-3``   | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.bfloat16`   | ``1.6e-2`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.float32`    | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.float64`    | ``1e-7``   | ``1e-7`` |
    +---------------------------+------------+----------+
    | :attr:`~core.complex32`  | ``1e-3``   | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.complex64`  | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.complex128` | ``1e-7``   | ``1e-7`` |
    +---------------------------+------------+----------+
    | :attr:`~core.quint8`     | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.quint2x4`   | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.quint4x2`   | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.qint8`      | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | :attr:`~core.qint32`     | ``1.3e-6`` | ``1e-5`` |
    +---------------------------+------------+----------+
    | other                     | ``0.0``    | ``0.0``  |
    +---------------------------+------------+----------+
    ...
    .. note::
        ...
        :func:`~core.testing.assert_close` is highly configurable with strict default settings. Users are encouraged
        ...
        >>> import functools
        >>> assert_equal = functools.partial(core.testing.assert_close, rtol=0, atol=0)
    ...
    Examples:
        ...
        >>> expected = core.tensor([1e0, 1e-1, 1e-2])
        >>> actual = core.acos(core.cos(expected))
        >>> core.testing.assert_close(actual, expected)
        ...
        >>> expected = [core.tensor([1.0]), 2.0, np.array(3.0)]
        ...
        >>> expected = core.tensor([1.0, 2.0, 3.0])
        >>> actual = expected.clone()
        >>> # By default, directly related instances can be compared
        >>> core.testing.assert_close(core.nn.Parameter(actual), expected)
        >>> # This check can be made more strict with allow_subclasses=False
        >>> core.testing.assert_close(
        ...     core.nn.Parameter(actual), expected, allow_subclasses=False
        ... )
        Traceback (most recent call last):
        ...
        TypeError: No comparison pair was able to handle inputs of type
        <class 'core.nn.parameter.Parameter'> and <class 'core.Tensor'>.
        >>> # If the inputs are not directly related, they are never considered close
        >>> core.testing.assert_close(actual.numpy(), expected)
        Traceback (most recent call last):
        ...
        TypeError: No comparison pair was able to handle inputs of type <class 'numpy.ndarray'>
        and <class 'core.Tensor'>.
        >>> # Exceptions to these rules are Python scalars. They can be checked regardless of
        >>> # their type if check_dtype=False.
        >>> core.testing.assert_close(1.0, 1, check_dtype=False)

        >>> # NaN != NaN by default.
        >>> expected = core.tensor(float("Nan"))
        >>> actual = expected.clone()
        >>> core.testing.assert_close(actual, expected)
        Traceback (most recent call last):
        ...
        ...
        >>> core.testing.assert_close(actual, expected, equal_nan=True)

        >>> expected = core.tensor([1.0, 2.0, 3.0])
        >>> actual = core.tensor([1.0, 4.0, 5.0])
        >>> # The default error message can be overwritten.
        >>> core.testing.assert_close(actual, expected, msg="Argh, the tensors are not close!")
        Traceback (most recent call last):
        ...
...
@deprecated(
    "`core.testing.assert_allclose()` is deprecated since 1.12 and will be removed in a future release. "
    "Please use `core.testing.assert_close()` instead. "
...
def assert_allclose(
    ...
    .. warning::
       ...
       :func:`core.testing.assert_allclose` is deprecated since ``1.12`` and will be removed in a future release.
       Please use :func:`core.testing.assert_close` instead. You can find detailed upgrade instructions
       `here <https://github.com/pytorch/pytorch/issues/61844>`_.
    """
    if not isinstance(actual, core.Tensor):
        actual = core.tensor(actual)
    if not isinstance(expected, core.Tensor):
        expected = core.tensor(expected, dtype=actual.dtype)
    ...
    if rtol is None and atol is None:
        rtol, atol = default_tolerances(
            ...
            expected,
            dtype_precisions={
                core.float16: (1e-3, 1e-3),
                core.float32: (1e-4, 1e-5),
                core.float64: (1e-5, 1e-8),
            },
        )
