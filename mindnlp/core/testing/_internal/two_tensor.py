...
from mindnlp.core._export.wrappers import mark_subclass_constructor_exportable_experimental
from mindnlp.core.utils._python_dispatch import return_and_correct_aliasing


# A simple tensor subclass that holds two tensors internally, and runs every op on both tensors.
class TwoTensor(core.Tensor):
    @staticmethod
    def __new__(cls, a, b, outer_size=None, outer_stride=None):
        ...
        # I guess it would be more accurate to represent the shape as core.cat(a, b).shape
        shape = outer_size
        kwargs = {}
        ...
        out = core.Tensor._make_wrapper_subclass(cls, shape, **kwargs)
    ...
    def __torch_dispatch__(cls, func, types, args, kwargs):
        ...
        out_flat = [
            cls(o_a, o_b) if isinstance(o_a, core.Tensor) else o_a
            for o_a, o_b in zip(out_a_flat, out_b_flat)
        ]
        out = pytree.tree_unflatten(out_flat, spec)
        from core._higher_order_ops.cond import cond_op
...
class TwoTensorMode(core.utils._python_dispatch.TorchDispatchMode):
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        out = func(*args, **kwargs)
        if core._subclasses.fake_tensor._is_tensor_constructor(func):
