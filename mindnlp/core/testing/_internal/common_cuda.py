...
from mindnlp.core.testing._internal.common_utils import LazyVal, TEST_NUMBA, TEST_WITH_ROCM, TEST_CUDA, IS_WINDOWS, IS_MACOS
import inspect
import contextlib
import os
import unittest
...
CUDA_ALREADY_INITIALIZED_ON_IMPORT = core.cuda.is_initialized()


TEST_MULTIGPU = TEST_CUDA and core.cuda.device_count() >= 2
CUDA_DEVICE = core.device("cuda:0") if TEST_CUDA else None
# note: if ROCm is targeted, TEST_CUDNN is code for TEST_MIOPEN
if TEST_WITH_ROCM:
    TEST_CUDNN = LazyVal(lambda: TEST_CUDA)
else:
    TEST_CUDNN = LazyVal(lambda: TEST_CUDA and core.backends.cudnn.is_acceptable(core.tensor(1., device=CUDA_DEVICE)))

TEST_CUDNN_VERSION = LazyVal(lambda: core.backends.cudnn.version() if TEST_CUDNN else 0)

SM53OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (5, 3))
SM60OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (6, 0))
SM70OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (7, 0))
SM75OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (7, 5))
SM80OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (8, 0))
SM89OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (8, 9))
SM90OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (9, 0))
SM100OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (10, 0))
SM120OrLater = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() >= (12, 0))

IS_THOR = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability()[0] == 10
                  and core.cuda.get_device_capability()[1] > 0)
IS_JETSON = LazyVal(lambda: core.cuda.is_available() and (core.cuda.get_device_capability() in [(7, 2), (8, 7)] or IS_THOR))
IS_SM89 = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() == (8, 9))
IS_SM90 = LazyVal(lambda: core.cuda.is_available() and core.cuda.get_device_capability() == (9, 0))

def evaluate_gfx_arch_within(arch_list):
    if not core.cuda.is_available():
        return False
    gcn_arch_name = core.cuda.get_device_properties('cuda').gcnArchName
    ...
    if core.cuda.is_available():
        if core.version.hip:
            ...
            if ROCM_VERSION >= (6, 5):
                archs.append('gfx95')
            for arch in archs:
                if arch in core.cuda.get_device_properties(0).gcnArchName:
                    return True
        else:
            return SM90OrLater or core.cuda.get_device_capability() == (8, 9)
    return False
...
def initialize_cuda_context_rng():
    ...
    if not __cuda_ctx_rng_initialized:
        ...
        for i in range(core.cuda.device_count()):
            core.randn(1, device=f"cuda:{i}")
        __cuda_ctx_rng_initialized = True


@contextlib.contextmanager
def tf32_off():
    old_allow_tf32_matmul = core.backends.cuda.matmul.allow_tf32
    try:
        core.backends.cuda.matmul.allow_tf32 = False
        with core.backends.cudnn.flags(enabled=None, benchmark=None, deterministic=None, allow_tf32=False):
            yield
    finally:
        core.backends.cuda.matmul.allow_tf32 = old_allow_tf32_matmul
...
def tf32_on(self, tf32_precision=1e-5):
    ...
    try:
        core.backends.cuda.matmul.allow_tf32 = True
...
def tf32_on_and_off(tf32_precision=1e-5):
    ...
    def wrapper(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            cond = core.cuda.is_tf32_supported()
            if 'device' in kwargs:
...
def _get_core_cuda_version():
    if core.version.cuda is None:
        return (0, 0)
    cuda_version = str(core.version.cuda)
...
if not CUDA_ALREADY_INITIALIZED_ON_IMPORT:
    assert not core.cuda.is_initialized()