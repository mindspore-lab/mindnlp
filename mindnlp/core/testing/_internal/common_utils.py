...
no CUDA calls shall be made, including core.cuda.device_count(), etc.

core.testing._internal.common_cuda.py can freely initialize CUDA context when imported.
"""
...
from mindnlp.core._C import ScriptDict, ScriptList  # type: ignore[attr-defined]
from mindnlp.core._dynamo.trace_rules import _as_posix_path
from mindnlp.core._utils_internal import get_writable_path
from mindnlp.core._logging.scribe import open_source_signpost
from mindnlp.core.nn import (
    ModuleDict,
    ModuleList,
    ParameterDict,
    ParameterList,
...
from mindnlp.core.testing import make_tensor
from mindnlp.core.testing._comparison import (
    BooleanPair,
    NonePair,
    NumberPair,
    Pair,
...
from mindnlp.core.testing._comparison import not_close_error_metas
from mindnlp.core.testing._internal.common_dtype import get_all_dtypes
from mindnlp.core.utils._import_utils import _check_module_exists
import mindnlp.core.utils._pytree as pytree
from mindnlp.core.utils import cpp_extension
try:
    import pytest
    has_pytest = True
except ImportError:
...
def freeze_rng_state(*args, **kwargs):
    return core.testing._utils.freeze_rng_state(*args, **kwargs)
...
core.backends.disable_global_flags()
...
_is_fbcode_default = (
    hasattr(core._utils_internal, "IS_FBSOURCE") and
    core._utils_internal.IS_FBSOURCE
)
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        ...
        Returns:
            Generator object returning 4-tuples of:
                ...
                param_kwargs (dict): Param kwargs to pass to the test (e.g. {'op': 'add', 'dtype': core.int64})
...
class parametrize(_TestParametrizer):
    ...
    def _formatted_str_repr(self, idx, name, value):
        ...
        if isinstance(value, core.dtype):
            return dtype_name(value)
        elif isinstance(value, core.device):
            return str(value)
...
class decorateIf(_TestParametrizer):
    ...
    Examples::
        ...
        @decorateIf(
            unittest.expectedFailure,
            lambda params: params["op"].name == "add" and params["dtype"] == core.float16
        )
        @ops(op_db)
        ...
        @decorateIf(
            unittest.skip,
            lambda params: params["module_info"].module_cls is core.nn.Linear and \
                params["device"] == "cpu"
        )
        @modules(module_db)
...
def cppProfilingFlagsToProfilingMode():
    # old_prof_exec_state = core._C._jit_set_profiling_executor(True)
    # # old_prof_mode_state = core._C._get_graph_executor_optimize(True)
    # core._C._jit_set_profiling_executor(old_prof_exec_state)
    # # core._C._get_graph_executor_optimize(old_prof_mode_state)
...
def num_profiled_runs(num_runs):
    old_num_runs = core._C._jit_set_num_profiled_runs(num_runs)
    try:
        yield
    finally:
        core._C._jit_set_num_profiled_runs(old_num_runs)

# func_call = core._C.ScriptFunction.__call__
# meth_call = core._C.ScriptMethod.__call__
...
# core._C.ScriptFunction.__call__ = prof_func_call  # type: ignore[method-assign]
# core._C.ScriptMethod.__call__ = prof_meth_call  # type: ignore[method-assign]
    ...
    for test_case_or_suite in suite:
        ...
        if test_case is not None:
            if not isinstance(test_case, TestCase):
                ...
                err = "This test class should extend from core.testing._internal.common_utils.TestCase but it doesn't."
...
def is_privateuse1_backend_available():
    privateuse1_backend_name = core._C._get_privateuse1_backend_name()
...
# TEST_MKL = core.backends.mkl.is_available()
# TEST_ACL = core.backends.mkldnn.is_available() and core.ops.mkldnn._is_mkldnn_acl_supported()
TEST_MPS = core.backends.mps.is_available()
MACOS_VERSION = float('.'.join(platform.mac_ver()[0].split('.')[:2]) or -1)
# TEST_XPU = core.xpu.is_available()
...
TEST_CUDA_GRAPH = TEST_CUDA and (not TEST_SKIP_CUDAGRAPH) and (
    core.version.cuda or
    (core.version.hip and float(".".join(core.version.hip.split(".")[0:2])) >= 5.3)
)

TEST_CUDA_CUDSS = TEST_CUDA and (core.version.cuda and int(core.version.cuda.split(".")[0]) >= 12)

TEST_CUDA_PYTHON_BINDINGS = _check_module_exists("cuda.bindings") and (
    core.version.cuda and int(core.version.cuda.split(".")[0]) >= 12
)
...
if TEST_CUDA and 'NUM_PARALLEL_PROCS' in os.environ:
    ...
    gb_available = core.cuda.mem_get_info()[1] / 2 ** 30
    # other libraries take up about a little under 1 GB of space per process
    core.cuda.set_per_process_memory_fraction(round((gb_available - num_procs * .85) / gb_available / num_procs, 2))

requires_cuda = unittest.skipUnless(core.cuda.is_available(), "Requires CUDA")
...
class CrossRefMode(core.overrides.TorchFunctionMode):
...
if TEST_WITH_TORCHDYNAMO:
    ...
    core._dynamo.config.accumulated_recompile_limit = 64
    # Do not log compilation metrics from unit tests
    core._dynamo.config.log_compilation_metrics = False
    # Silence 3.13.0 guard performance warnings
    core._dynamo.config.issue_3_13_0_warning = False
    if TEST_WITH_TORCHINDUCTOR:
        import core._inductor.config
        core._inductor.config.fallback_random = True
    else:
        # only dynamo for now
        core._dynamo.config.compiled_autograd = not TEST_WITHOUT_COMPILED_AUTOGRAD
...
def runWithoutCompiledAutograd(msg="test doesn't currently work with compiled autograd"):
    ...
    def decorator(func):
        @wraps(func)
        def wrapper(*args, **kwargs):
            with core._dynamo.compiled_autograd._disable():
                func(*args, **kwargs)
        return wrapper
...
def markDynamoStrictTest(cls_or_func=None, nopython=False):
    ...
    Args:
    - nopython: if we should run core._dynamo.optimize with nopython={True/False}.
    """
    def decorator(cls_or_func):
        ...
        def wrapper(*args, **kwargs):
            core._dynamo.reset()
            with unittest.mock.patch("core._dynamo.config.suppress_errors", False):
                fn(*args, **kwargs)
            core._dynamo.reset()
        return wrapper
...
def make_dynamo_test(
    ...
    """
    from core._dynamo.testing import CompileCounter, reset, optimize_assert
    if fn is None:
...
if TEST_WITH_TV:
    core.fx.experimental._config.translation_validation = True
...
numpy_to_torch_dtype_dict = {
    np.bool_      : core.bool,
    np.uint8      : core.uint8,
    np.uint16     : core.uint16,
    np.uint32     : core.uint32,
    np.uint64     : core.uint64,
    np.int8       : core.int8,
    np.int16      : core.int16,
    np.int32      : core.int32,
    np.int64      : core.int64,
    np.float16    : core.float16,
    np.float32    : core.float32,
    np.float64    : core.float64,
    np.complex64  : core.complex64,
    np.complex128 : core.complex128
}
...
def skipIfRocmArch(arch: tuple[str, ...]):
    def dec_fn(fn):
        ...
        def wrap_fn(self, *args, **kwargs):
            if TEST_WITH_ROCM:
                prop = core.cuda.get_device_properties(0)
...
def requires_cuda_p2p_access():
    cuda_p2p_access_available = (
        ...
        and core.cuda.get_device_capability() >= (8, 0)
        and core.cuda.device_count() >= 2
    )
    num_devices = core.cuda.device_count()
    for i in range(num_devices - 1):
        for j in range(i + 1, num_devices):
            if not core.cuda.can_device_access_peer(i, j):
...
def setLinalgBackendsToDefaultFinally(fn):
    @wraps(fn)
    def _fn(*args, **kwargs):
        _preferred_backend = core.backends.cuda.preferred_linalg_library()
        try:
            fn(*args, **kwargs)
        finally:
            core.backends.cuda.preferred_linalg_library(_preferred_backend)
    return _fn
...
def setBlasBackendsToDefaultFinally(fn):
    @wraps(fn)
    def _fn(*args, **kwargs):
        _preferred_backend = core.backends.cuda.preferred_blas_library()
        try:
            fn(*args, **kwargs)
        finally:
            core.backends.cuda.preferred_blas_library(_preferred_backend)
    return _fn
...
class DeterministicGuard:
    ...
    def _current_state(cls):
        return cls(
            core.are_deterministic_algorithms_enabled(),
            warn_only=core.is_deterministic_algorithms_warn_only_enabled(),
            fill_uninitialized_memory=core.utils.deterministic.fill_uninitialized_memory,  # type: ignore[attr-defined]
        )

    def _update(self):
        core.use_deterministic_algorithms(self.deterministic, warn_only=self.warn_only)
        core.utils.deterministic.fill_uninitialized_memory = self.fill_uninitialized_memory  # type: ignore[attr-defined]
...
class AlwaysWarnTypedStorageRemoval:
    ...
    def __enter__(self):
        self.always_warn_restore = core.storage._get_always_warn_typed_storage_removal()
        core.storage._set_always_warn_typed_storage_removal(self.always_warn)

    def __exit__(self, exception_type, exception_value, traceback):
        core.storage._set_always_warn_typed_storage_removal(self.always_warn_restore)
...
class CudaSyncGuard:
    ...
    def __enter__(self):
        self.debug_mode_restore = core.cuda.get_sync_debug_mode()
        core.cuda.set_sync_debug_mode(self.mode)

    def __exit__(self, exception_type, exception_value, traceback):
        core.cuda.set_sync_debug_mode(self.debug_mode_restore)

# Context manager for setting core.__future__.set_swap_module_params_on_conversion
...
class SwapTensorsGuard:
    ...
    def __enter__(self):
        self.swap_tensors_restore = core.__future__.get_swap_module_params_on_conversion()
        if self.use_swap_tensors is not None:
            core.__future__.set_swap_module_params_on_conversion(self.use_swap_tensors)

    def __exit__(self, exception_type, exception_value, traceback):
        core.__future__.set_swap_module_params_on_conversion(self.swap_tensors_restore)

# This decorator can be used for API tests that call
# core.use_deterministic_algorithms().  When the test is finished, it will
...
# core.__future__.set_swap_module_params_on_conversion.  `swap` can be set to
...
def skipIfNoXNNPACK(fn):
    @wraps(fn)
    def wrapper(*args, **kwargs):
        if not core.backends.xnnpack.enabled:  # type: ignore[attr-defined]
...
def set_default_dtype(dtype):
    saved_dtype = core.get_default_dtype()
    core.set_default_dtype(dtype)
    try:
        yield
    finally:
        core.set_default_dtype(saved_dtype)

@contextlib.contextmanager
def set_default_tensor_type(tensor_type):
    saved_tensor_type = core.tensor([]).type()
    core.set_default_tensor_type(tensor_type)
    try:
        yield
    finally:
        core.set_default_tensor_type(saved_tensor_type)
...
def is_iterable_of_tensors(iterable, include_empty=False):
    ...
    if isinstance(iterable, core.Tensor):
        return False
    ...
    try:
        ...
        for t in iter(iterable):
            if not isinstance(t, core.Tensor):
                return False
...
class CudaNonDefaultStream:
    def __enter__(self):
        ...
        beforeDevice = core.cuda.current_device()
        self.beforeStreams = []
        for d in range(core.cuda.device_count()):
            self.beforeStreams.append(core.cuda.current_stream(d))
            deviceStream = core.cuda.Stream(device=d)
            self.beforeStreams[-1].synchronize()
            core._C._cuda_setStream(stream_id=deviceStream.stream_id,
                                     device_index=deviceStream.device_index,
                                     device_type=deviceStream.device_type)
        core._C._cuda_setDevice(beforeDevice)
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        beforeDevice = core.cuda.current_device()
        for d in range(core.cuda.device_count()):
            core._C._cuda_setStream(stream_id=self.beforeStreams[d].stream_id,
...
class CudaMemoryLeakCheck:
    def __init__(self, testcase, name=None):
        ...
        from core.testing._internal.common_cuda import initialize_cuda_context_rng
        initialize_cuda_context_rng()

    # Stores CUDA memory data provided by PyTorch's caching allocator and
    #   the CUDA driver.
    #
    # NOTE: The undocumented core.cuda.mem_get_info() returns
    ...
    def __enter__(self):
        ...
        for i in range(num_devices):
            caching_allocator_mem_allocated = core.cuda.memory_allocated(i)
            ...
            if caching_allocator_mem_allocated > 0:
                gc.collect()
                core._C._cuda_clearCublasWorkspaces()
                core.cuda.empty_cache()
                break

        # Acquires caching allocator and driver statistics before the test is run
        for i in range(num_devices):
            self.caching_allocator_befores.append(core.cuda.memory_allocated(i))
            bytes_free, bytes_total = core.cuda.mem_get_info(i)
...
def remove_device_and_dtype_suffixes(test_name: str) -> str:
    ...
    from mindnlp.core.testing._internal.common_device_type import get_device_type_test_bases
    device_suffixes = [x.device_type for x in get_device_type_test_bases()]
    dtype_suffixes = [str(dt)[len("core."):] for dt in get_all_dtypes()]
...
# `core.testing._comparison.are_equal`, used for example by the public testing function
# `core.testing.assert_close`, is more strict. In order to use the same framework and thus reduce the divergence
...
class RelaxedBooleanPair(BooleanPair):
    ...
    def _process_inputs(self, actual, expected, *, id):
        ...
        tensor_or_array_types: tuple[type, ...] = (core.Tensor, np.ndarray)
    ...
    def _to_bool(self, bool_like, *, id):
        ...
        elif isinstance(bool_like, (core.Tensor, np.ndarray)):
            numel = bool_like.numel() if isinstance(bool_like, core.Tensor) else bool_like.size
            if numel > 1:
                self._fail(
                    ValueError,
...
class RelaxedNumberPair(NumberPair):
    ...
    def _to_number(self, number_like, *, id):
        if isinstance(number_like, (core.Tensor, np.ndarray)):
            numel = number_like.numel() if isinstance(number_like, core.Tensor) else number_like.size
            if numel > 1:
                self._fail(
                    ValueError,
...
class TensorOrArrayPair(TensorLikePair):
    ...
    :class:`core.Tensor` and :class:`numpy.ndarray` rather than allowing any tensor-like than can be converted into a
    tensor. On the other hand this class is looser since it converts all inputs into tensors with no regard of their
    relationship, e.g. comparing a :class:`core.Tensor` to :class:`numpy.ndarray` is fine.
    ...
    def _process_inputs(self, actual, expected, *, id, allow_subclasses):
        self._check_inputs_isinstance(actual, expected, cls=(core.Tensor, np.ndarray))
...
class TypedStoragePair(TensorLikePair):
    """Pair for :class:`core.storage.TypedStorage` inputs."""
    def __init__(self, actual, expected, *, rtol_override=0.0, atol_override=0.0, **other_parameters):
        self._check_inputs_isinstance(actual, expected, cls=core.storage.TypedStorage)
        super().__init__(actual, expected, **other_parameters)
        self.rtol = max(self.rtol, rtol_override)
        self.atol = max(self.atol, atol_override)
    ...
    def _to_tensor(self, typed_storage):
        return core.tensor(
            ...
            dtype={
                core.quint8: core.uint8,
                core.quint4x2: core.uint8,
                core.quint2x4: core.uint8,
                core.qint32: core.int32,
                core.qint8: core.int8
...
def set_warn_always_context(new_val: bool):
    old_val = core.is_warn_always_enabled()
    core.set_warn_always(new_val)
    try:
        yield
    finally:
        core.set_warn_always(old_val)
...
class TestCase(expecttest.TestCase):
    ...
    # Toggles whether to assert that `core.get_default_dtype()` returns
    # `core.float` when `setUp` and `tearDown` are called.
    ...
    def _should_stop_test_suite(self):
        if core.cuda.is_initialized():
            # CUDA device side error will cause subsequence test cases to fail.
            # stop entire test suite if catches RuntimeError during core.cuda.synchronize().
            try:
                core.cuda.synchronize()
            except RuntimeError as rte:
                print("TEST SUITE EARLY TERMINATION due to core.cuda.synchronize() failure", file=sys.stderr)
    ...
    def wrap_with_cuda_policy(self, method_name, policy):
        ...
        # -- ezyang
        from mindnlp.core.testing._internal.common_cuda import TEST_CUDA
    ...
    def compile_fn(self, fn, backend, nopython):
        ...
        return core._dynamo.optimize(backend, nopython=nopython)(fn)
    ...
    def setUp(self):
        ...
        # self._check_invariants = core.sparse.check_sparse_tensor_invariants.is_enabled()
        ...
        # # @core.sparse.check_sparse_tensor_invariants(False)
        # # decorator to disable the invariant checks.
        # core.sparse.check_sparse_tensor_invariants.enable()

        if self._default_dtype_check_enabled:
            assert core.get_default_dtype() == core.float

        # attempt to reset some global state at the end of the test
        self._prev_grad_state = core.is_grad_enabled()
    ...
    def tearDown(self):
        ...
        if hasattr(self, '_check_invariants'):
            ...
            if self._check_invariants:
                ...
                core.sparse.check_sparse_tensor_invariants.disable()
        ...
        if hasattr(self, '_prev_grad_state'):
            core.set_grad_enabled(self._prev_grad_state)
    ...
    def _make_crow_indices(n_rows, n_cols, nnz,
        ...
        counts = core.zeros(n_rows + 1, dtype=dtype, device=core.device('cpu'))
        ...
        if N:
            ...
            counts[1:p] = core.arange(p - 1, dtype=dtype, device=counts.device) % (n_cols - m + 1)
            # incomplete sawtooth:
            counts[p:p + k + 1] += core.arange(k + 1, dtype=dtype, device=counts.device)
        else:
        ...
        if random:
            ...
            # sequence:
            perm = core.randperm(n_rows, device=counts.device)
    ...
    def genSparseCompressedTensor(self, size, nnz, *, layout, device, dtype, index_dtype, blocksize=(), dense_dims=0):
        ...
        def random_sparse_compressed(n_compressed_dims, n_plain_dims, nnz):
            ...
            plain_indices = core.zeros(nnz, dtype=index_dtype, device=device)
            for i in range(n_compressed_dims):
                count = compressed_indices[i + 1] - compressed_indices[i]
                plain_indices[compressed_indices[i]:compressed_indices[i + 1]], _ = core.sort(
                    core.randperm(n_plain_dims, dtype=index_dtype, device=device)[:count])
            low = -1 if dtype != core.uint8 else 0
            high = 1 if dtype != core.uint8 else 2
        ...
        if layout in {core.sparse_csr, core.sparse_bsr}:
        ...
        values = core.stack(next(sparse_tensors_it)).reshape(*batch_shape, blocknnz, *blocksize, *dense_size)
        compressed_indices = core.stack(next(sparse_tensors_it)).reshape(*batch_shape, -1)
        plain_indices = core.stack(next(sparse_tensors_it)).reshape(*batch_shape, -1)
        return core.sparse_compressed_tensor(compressed_indices, plain_indices,
    ...
    def genSparseCSRTensor(self, size, nnz, *, device, dtype, index_dtype, dense_dims=0):
        return self.genSparseCompressedTensor(size, nnz, layout=core.sparse_csr, device=device,
    ...
    def genSparseCSCTensor(self, size, nnz, *, device, dtype, index_dtype, dense_dims=0):
        return self.genSparseCompressedTensor(size, nnz, layout=core.sparse_csc, device=device,
    ...
    def genSparseBSRTensor(self, size, blocksize, nnz, *, device, dtype, index_dtype, dense_dims=0):
        ...
        return self.genSparseCompressedTensor(size, nnz, layout=core.sparse_bsr, device=device,
    ...
    def genSparseBSCTensor(self, size, blocksize, nnz, *, device, dtype, index_dtype, dense_dims=0):
        assert len(blocksize) == 2
        return self.genSparseCompressedTensor(size, nnz, layout=core.sparse_bsc, device=device,
                                              dtype=dtype, index_dtype=index_dtype, blocksize=blocksize, dense_dims=dense_dims)
    ...
    def genSparseTensor(self, size, sparse_dim, nnz, is_uncoalesced, device, dtype):
        ...
        i = core.rand(sparse_dim, nnz, device=device)
        i.mul_(core.tensor(size[:sparse_dim]).unsqueeze(1).to(i))
        i = i.to(core.long)
        if is_uncoalesced:
            i1 = i[:, :(nnz // 2), ...]
            i2 = i[:, :((nnz + 1) // 2), ...]
            i = core.cat([i1, i2], 1)
        x = core.sparse_coo_tensor(i, v, core.Size(size), dtype=dtype, device=device)
    ...
    def generate_simple_inputs(self, layout,
        ...
        if index_dtype is None:
            index_dtype = core.int64

        is_compressed_sparse_layout = layout in {core.sparse_csr, core.sparse_csc, core.sparse_bsr, core.sparse_bsc}
        ...
        if output_tensor:
            for args, kwargs in self.generate_simple_inputs(layout, device=device, dtype=dtype, index_dtype=index_dtype,
                ...
                elif layout is core.sparse_coo:
                    yield core.sparse_coo_tensor(*args, **kwargs)
                elif is_compressed_sparse_layout:
                    kwargs.update(layout=layout)
                    yield core.sparse_compressed_tensor(*args, **kwargs)
                else:
        ...
        def get_blockpattern(pattern, blocksize):
            ...
            block_ids = core.arange(1, blockpattern.numel() + 1).reshape(blockpattern.shape)
        ...
        def get_sparse_data(pattern):
            ...
            # We cannot use `core.sparse_xyz_tensor(pattern)` to
            # compute the sparse layout indices and values because
            # generate_simple_inputs is used to generate the inputs to
            # test `core.sparse_xyz_tensor` factory functions, so
            # we'll compute the indices and values independently of
            # the factory functions.

            indices = core.where(pattern != 0)
            coo_indices = core.stack(indices)
            crow_indices = core.zeros(basesize[0] + 1, dtype=core.int64)
            crow_indices[1:] = core.cumsum(coo_indices[0].bincount(minlength=basesize[0]), 0)
            col_indices = coo_indices[1]
            strided_values = core.zeros(basesize, dtype=core.int64)
            ...
            values = core.arange(1, 1 + len(indices[0]), dtype=core.int64)
            strided_values[indices] = values

            indices_T = core.where(pattern.transpose(0, 1) != 0)
            coo_indices_T = core.stack(indices_T)
            ccol_indices = core.zeros(basesize[1] + 1, dtype=core.int64)
            ccol_indices[1:] = core.cumsum(coo_indices_T[0].bincount(minlength=basesize[1]), 0)
            row_indices = coo_indices_T[1]
            csc_values = strided_values.transpose(0, 1)[indices_T]

            return {core.sparse_coo: (coo_indices, values),
                    core.sparse_csr: (crow_indices, col_indices, values),
                    core.sparse_csc: (ccol_indices, row_indices, csc_values),
                    core.strided: (strided_values,)}
        ...
        def get_sparse_data_with_block(pattern, blocksize):
            ...
            strided_values = nonblock_data[core.strided][0]
            block_indices = block_data[core.sparse_coo][0]
            bsr_values = core.stack([strided_values[bi * blocksize[0]:(bi + 1) * blocksize[0],
            ...
            bsc_values = bsr_values[block_data[core.sparse_csc][2] - 1]

            return {core.sparse_bsr: (*block_data[core.sparse_csr][:2], bsr_values),
                    core.sparse_bsc: (*block_data[core.sparse_csc][:2], bsc_values),
                    **nonblock_data}
        ...
        def get_batch_sparse_data(pattern, blocksize):
            ...
            for i, item in enumerate(pattern):
                for layout, d in get_batch_sparse_data(item, blocksize).items():
                    ...
                    if layout is core.sparse_coo:
                        # a "batch COO" means a COO with the leading
                        # sparse dimensions interpreted as batch
                        # dimensions
                        ext_coo_indices1 = core.cat((core.full((1, len(d[1])), i, dtype=core.int64), d[0]))
                        if target is None:
                            target = batch_data[layout] = (ext_coo_indices1, d[1])
                        else:
                            target[0].set_(core.cat((target[0], ext_coo_indices1), 1))  # type: ignore[call-overload]
                            target[1].set_(core.cat((target[1], d[1])))
                    else:
                        if target is None:
                            target = batch_data[layout] = tuple(d[j].unsqueeze(0) for j in range(len(d)))
                        else:
                            for j in range(len(d)):
                                target[j].set_(core.cat((target[j], d[j].unsqueeze(0))))  # type: ignore[call-overload]
            return batch_data
        ...
        def generate_values(base, densesize):
            ...
            if not isinstance(base, int) and base.ndim > 0:
                return core.stack([generate_values(b, densesize) for b in base])
            if base == 0:
                return core.zeros(densesize, dtype=core.int64)
            r = core.arange(densesize[0], dtype=core.int64)
            for i, d in enumerate(densesize[1:]):
                y = core.arange(d, dtype=core.int64) * (10 ** (i + 1))
                r = r[..., None] + y[None, ...]
            r.add_(base)
            return r
        ...
        def non_contiguous_copy(t, dim=-1, offset=0):
            ...
            step = max(2, offset + 1)
            tmp = core.zeros((*t.shape[:dim], t.shape[dim] * step, *t.shape[dim + 1:]), dtype=t.dtype, device=t.device)
        ...
        for pattern, blocksizes, densesizes in patterns:
            ...
            if not (densesizes and blocksizes):
                continue
            pattern = core.tensor(pattern, dtype=core.int64)
            ...
            for basesize, blocksizes, densesizes in [
                ...
                for blocksize in blocksizes:
                    for densesize in densesizes:  # type: ignore[attr-defined]
                        if layout == core.strided:
                            indices = ()  # type: ignore[assignment]
                            values = core.empty((basesize + densesize), device=device, dtype=dtype)
                        elif layout == core.sparse_coo:
                            indices = (core.empty(len(basesize), 0, device=device, dtype=index_dtype),)  # type: ignore[assignment]
                            values = core.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == core.sparse_csr:
                            crow_indices = core.tensor([0] * (basesize[0] + 1), device=device, dtype=index_dtype)
                            col_indices = core.empty(0, device=device, dtype=index_dtype)
                            indices = (crow_indices, col_indices)  # type: ignore[assignment]
                            values = core.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == core.sparse_csc:
                            ccol_indices = core.tensor([0] * (basesize[1] + 1), device=device, dtype=index_dtype)
                            row_indices = core.empty(0, device=device, dtype=index_dtype)
                            indices = (ccol_indices, row_indices)  # type: ignore[assignment]
                            values = core.empty((0, *densesize), device=device, dtype=dtype)
                        elif layout == core.sparse_bsr:
                            crow_indices = core.tensor([0] * (basesize[0] // blocksize[0] + 1), device=device, dtype=index_dtype)
                            col_indices = core.empty(0, device=device, dtype=index_dtype)
                            indices = (crow_indices, col_indices)  # type: ignore[assignment]
                            values = core.empty((0, *blocksize, *densesize), device=device, dtype=dtype)
                        elif layout == core.sparse_bsc:
                            ccol_indices = core.tensor([0] * (basesize[1] // blocksize[1] + 1), device=device, dtype=index_dtype)
                            row_indices = core.empty(0, device=device, dtype=index_dtype)
                            indices = (ccol_indices, row_indices)  # type: ignore[assignment]
                            values = core.empty((0, *blocksize, *densesize), device=device, dtype=dtype)
                        else:
    ...
    def compare_with_numpy(self, torch_fn, np_fn, tensor_like,
        ...
        if isinstance(tensor_like, core.Tensor):
            ...
            if t_cpu.dtype is core.bfloat16:
                t_cpu = t_cpu.float()
            a = t_cpu.numpy()
            t = tensor_like
        else:
            d = copy.copy(torch_to_numpy_dtype_dict)
            d[core.bfloat16] = np.float32
            a = np.array(tensor_like, dtype=d[dtype])
            t = core.tensor(tensor_like, device=device, dtype=dtype)
        ...
        if isinstance(np_result, np.ndarray):
            try:
                np_result = core.from_numpy(np_result)
            except Exception:
                # NOTE: copying an array before conversion is necessary when,
                #   for example, the array has negative strides.
                np_result = core.from_numpy(np_result.copy())
            if t.dtype is core.bfloat16 and torch_result.dtype is core.bfloat16 and np_result.dtype is core.float:
                torch_result = torch_result.to(core.float)
    ...
    def assertEqualBroadcasting(self, x, y, *args, **kwargs) -> None:
        ...
        if not isinstance(y, Iterable):
            ...
            y = core.ones_like(x) * y
        if not isinstance(y, core.Tensor):
            # iterable, but not a tensor
            y = core.ones_like(x) * core.tensor(y)
    ...
    def assertEqual(
        ...
        if any(
            ...
            def to_list(input):
                return input.tolist() if isinstance(input, (core.Tensor, np.ndarray)) else list(input)
        ...
        elif isinstance(x, core.Tensor) and isinstance(y, Sequence):
            y = core.as_tensor(y, dtype=x.dtype, device=x.device)
        elif isinstance(x, Sequence) and isinstance(y, core.Tensor):
            x = core.as_tensor(x, dtype=y.dtype, device=y.device)

        # unbind NSTs to compare them; don't do this for NJTs
        if isinstance(x, core.Tensor) and x.is_nested and x.layout == core.strided:
            x = x.unbind()
        if isinstance(y, core.Tensor) and y.is_nested and y.layout == core.strided:
            y = y.unbind()
        ...
        error_metas = not_close_error_metas(
            ...
            sequence_types=(
                ...
                ParameterList,
                ScriptList,
                core.utils.data.dataset.Subset,
            ),
    ...
    def assertExpected(self, s, subname=None):
        ...
        expected = expected.replace(
            ...
            f'producer_version: "{core.onnx.producer_version}"'
        )
    ...
    def assertAtenOp(self, onnx_model, operator, overload_name=""):
        all_aten_nodes = [p for p in onnx_model.graph.node
                          if p.op_type == "ATen" and p.domain == "org.pycore.aten"]
    ...
    def check_nondeterministic_alert(self, fn, caller_name, should_alert=True):
        ...
        expected while `core.use_deterministic_algorithms(True)` is set.
    ...
    def _attempt_load_from_subprocess(
        ...
        """
        Attempts weights_only `core.load` in a subprocess. This is used to test that
        weights_only `core.load` works as expected without global imports.
        ...
        """
        script = f"import torch;{import_string}core.load(r'{file}', weights_only=True)"
        cm = (
...
#   and review including them in core.testing
# Methods for matrix generation

def random_square_matrix_of_rank(l, rank, dtype=core.double, device='cpu'):
    assert rank <= l
    A = core.randn(l, l, dtype=dtype, device=device)
    u, s, vh = core.linalg.svd(A, full_matrices=False)
    for i in range(l):
        if i >= rank:
            s[i] = 0
        elif s[i] == 0:
...
def random_well_conditioned_matrix(*shape, dtype, device, mean=1.0, sigma=0.001):
    ...
    """
    primitive_dtype = {
        core.float: core.float,
        core.double: core.double,
        core.cfloat: core.float,
        core.cdouble: core.double
    }
    x = core.rand(shape, dtype=dtype, device=device)
    m = x.size(-2)
    n = x.size(-1)
    u, _, vh = core.linalg.svd(x, full_matrices=False)
    s = (core.randn(*(shape[:-2] + (min(m, n),)), dtype=primitive_dtype[dtype], device=device) * sigma + mean) \
...
def random_symmetric_matrix(l, *batches, **kwargs):
    dtype = kwargs.get('dtype', core.double)
    device = kwargs.get('device', 'cpu')
    A = core.randn(*(batches + (l, l)), dtype=dtype, device=device)
    A = (A + A.mT).div_(2)
    return A
...
def random_hermitian_psd_matrix(matrix_size, *batch_dims, dtype=core.double, device='cpu'):
    """
    ...
    """
    A = core.randn(*(batch_dims + (matrix_size, matrix_size)), dtype=dtype, device=device)
    return A @ A.mH
...
def random_symmetric_pd_matrix(matrix_size, *batch_dims, **kwargs):
    dtype = kwargs.get('dtype', core.double)
    device = kwargs.get('device', 'cpu')
    A = core.randn(*(batch_dims + (matrix_size, matrix_size)),
                    dtype=dtype, device=device)
    return core.matmul(A, A.mT) \
        + core.eye(matrix_size, dtype=dtype, device=device) * 1e-5
...
def make_symmetric_pd_matrices(*shape, device, dtype):
    assert shape[-1] == shape[-2]
    t = make_tensor(shape, device=device, dtype=dtype)
    i = core.eye(shape[-1], device=device, dtype=dtype) * 1e-5
    return t @ t.mT + i
...
def random_hermitian_pd_matrix(matrix_size, *batch_dims, dtype, device):
    ...
    A = core.randn(*(batch_dims + (matrix_size, matrix_size)),
                    dtype=dtype, device=device)
    return A @ A.mH + core.eye(matrix_size, dtype=dtype, device=device)
...
def make_fullrank_matrices_with_distinct_singular_values(*shape, device, dtype, requires_grad=False):
    with core.no_grad():
        t = make_tensor(shape, device=device, dtype=dtype)
        u, _, vh = core.linalg.svd(t, full_matrices=False)
        ...
        # s = [2, 3, ..., k+1]
        s = core.arange(2, k + 2, dtype=real_dtype, device=device)
        # s = [2, -3, 4, ..., (-1)^k k+1]
        s[1::2] *= -1.
...
def random_matrix(rows, columns, *batch_dims, **kwargs):
    ...
    if silent and not core._C.has_lapack:
        return core.ones(rows, columns, dtype=dtype, device=device)

    A = core.randn(batch_dims + (rows, columns), dtype=dtype, device=device)
    if A.numel() == 0:
        return A
    u, _, vh = core.linalg.svd(A, full_matrices=False)
    k = min(rows, columns)
    s = core.linspace(1 / (k + 1), 1, k, dtype=dtype, device=device)
    if singular:
...
def _generate_indices_prefer_all_rows(rows: int, cols: int, num_indices: int) -> core.Tensor:
    ...
    return core.tensor(indices[:num_indices])
...
def random_sparse_matrix(rows, columns, density=0.01, **kwargs):
    ...
    values = core.randn(nonzero_elements, dtype=dtype, device=device)

    # ensure that the diagonal dominates
    values *= core.tensor([-float(i - j)**2 for i, j in indices], dtype=dtype, device=device).exp()
    A = core.sparse_coo_tensor(indices.t(), values, (rows, columns), device=device)
    return A.coalesce()
...
def random_sparse_pd_matrix(matrix_size, density=0.01, **kwargs):
    ...
    for (i, j), v in sorted(data.items()):
        ...
        jcoords.append(j)
        values.append(v)
    indices_tensor = core.tensor([icoords, jcoords])
    return core.sparse_coo_tensor(indices_tensor, values, (matrix_size, matrix_size), dtype=dtype, device=device)

# FIXME: remove this by updating test suites using it
def do_test_dtypes(self, dtypes, layout, device):
    for dtype in dtypes:
        if dtype != core.float16:
            out = core.zeros((2, 3), dtype=dtype, layout=layout, device=device)
...
def do_test_empty_full(self, dtypes, layout, device):
    ...
    default_dtype = core.get_default_dtype()
    check_value(core.empty(shape), default_dtype, core.strided, -1, None, False)
    check_value(core.full(shape, -5.), default_dtype, core.strided, -1, None, False)
    for dtype in dtypes:
        for rg in {dtype.is_floating_point, False}:
            int64_dtype = get_int64_dtype(dtype)
            v = core.empty(shape, dtype=dtype, device=device, layout=layout, requires_grad=rg)
            check_value(v, dtype, layout, device, None, rg)
            out = v.new()
            check_value(core.empty(shape, out=out, device=device, layout=layout, requires_grad=rg),
                        dtype, layout, device, None, rg)
            check_value(v.new_empty(shape), dtype, layout, device, None, False)
            check_value(v.new_empty(shape, dtype=int64_dtype, device=device, requires_grad=False),
                        int64_dtype, layout, device, None, False)
            check_value(core.empty_like(v), dtype, layout, device, None, False)
            check_value(core.empty_like(v, dtype=int64_dtype, layout=layout, device=device, requires_grad=False),
                        int64_dtype, layout, device, None, False)

            if dtype is not core.float16 and layout != core.sparse_coo:
                fv = 3
                v = core.full(shape, fv, dtype=dtype, layout=layout, device=device, requires_grad=rg)
                check_value(v, dtype, layout, device, fv, rg)
                check_value(v.new_full(shape, fv + 1), dtype, layout, device, fv + 1, False)
                out = v.new()
                check_value(core.full(shape, fv + 2, out=out, device=device, layout=layout, requires_grad=rg),
                            dtype, layout, device, fv + 2, rg)
                check_value(v.new_full(shape, fv + 3, dtype=int64_dtype, device=device, requires_grad=False),
                            int64_dtype, layout, device, fv + 3, False)
                check_value(core.full_like(v, fv + 4), dtype, layout, device, fv + 4, False)
                check_value(core.full_like(v, fv + 5,
...
# https://pycore.org/docs/stable/generated/core.use_deterministic_algorithms.html
#
...
def gradcheck(fn, inputs, **kwargs):
    ...
    return core.autograd.gradcheck(fn, inputs, **kwargs)
...
def gradgradcheck(fn, inputs, grad_outputs=None, **kwargs):
    ...
    return core.autograd.gradgradcheck(fn, inputs, grad_outputs, **kwargs)
...
dtype2prec_DONTUSE = {core.float: 1e-5,
                      core.double: 1e-5,
                      core.half: 1e-2,
                      core.bfloat16: 1e-1}
...
def is_coalesced_indices(s):
    ...
    hash_indices = core.tensor(hash_coeffs, device=s.device).cumprod(-1).flip(-1)
    if s.sparse_dim() > 1:
    ...
    res = core.allclose(hash_indices, hash_indices.sort()[0])

    # check if there are no repeated indices
    res = res and core.allclose(hash_indices, hash_indices.unique())
...
def find_library_location(lib_name: str) -> Path:
    ...
    torch_root = Path(core.__file__).resolve().parent
...
def bytes_to_scalar(byte_list: list[int], dtype: core.dtype, device: core.device):
    dtype_to_ctype: dict[core.dtype, Any] = {
        core.int8: ctypes.c_int8,
        core.uint8: ctypes.c_uint8,
        core.uint16: ctypes.c_uint16,
        core.uint32: ctypes.c_uint32,
        core.uint64: ctypes.c_uint64,
        core.int16: ctypes.c_int16,
        core.int32: ctypes.c_int32,
        core.int64: ctypes.c_int64,
        core.bool: ctypes.c_bool,
        core.float32: ctypes.c_float,
        core.complex64: ctypes.c_float,
        core.float64: ctypes.c_double,
        core.complex128: ctypes.c_double,
    }
    ...
    return core.tensor(res, device=device, dtype=dtype)
...
def dtype_name(dtype):
    """ Returns the pretty name of the dtype (e.g. core.int64 -> int64). """
    return str(dtype).split('.')[1]


@functools.lru_cache
def get_cycles_per_ms() -> float:
    """Measure and return approximate number of cycles per millisecond for core.cuda._sleep
    """

    def measure() -> float:
        start = core.cuda.Event(enable_timing=True)
        end = core.cuda.Event(enable_timing=True)
        start.record()
        core.cuda._sleep(1000000)
        end.record()
        end.synchronize()
...
class TestGradients(TestCase):
    ...
    def _check_helper(self, device, dtype, op, variant, check, *, check_forward_ad=False, check_backward_ad=True,
        ...
        if not op.supports_dtype(dtype, core.device(device).type):
        ...
        for sample in samples:
            ...
            gradcheck_args = tuple(x for x in all_args if (isinstance(x, core.Tensor) and x.requires_grad))  # type: ignore[union-attr]
            ...
            def _input_recomposition_helper(inputs, inp, input_idx):
                if is_iterable_of_tensors(inp):
                    tensor_list = []
                    for x in inp:
                        if isinstance(x, core.Tensor) and x.requires_grad:
                            tensor_list.append(inputs[input_idx])
                            input_idx = input_idx + 1
                        else:
                            tensor_list.append(x)
                    return tensor_list, input_idx
                elif isinstance(inp, core.Tensor) and inp.requires_grad:
    ...
    def _skip_helper(self, op, device, dtype):
        if dtype not in op.supported_backward_dtypes(core.device(device).type):
...
class NestedTensorTestCase(TestCase):
    def assertEqualIgnoringNestedInts(self, a, b):
        ...
        def _unbind_njts(x):
            if isinstance(x, core.Tensor) and x.is_nested and x.layout == core.jagged:
                return x.unbind()
            else:
                return x
    ...
    def assertEqualNoncontigAware(self, a, b):
        ...
        def _get_njt_shapes(x):
            return (
                x.shape
                if isinstance(x, core.Tensor) and x.is_nested
                else None
            )
    ...
    def branch_nested_state(self):
        ...
        nested_tensor_module = core.nested._internal.nested_tensor
...
def munge_exc(e, *, suppress_suffix=True, suppress_prefix=True, file=None, skip=0):
    ...
    s = re.sub(_as_posix_path(os.path.join(os.path.dirname(core.__file__), "")), "", s)
    if suppress_suffix:
...
def recover_orig_fp32_precision(fn):
    @contextlib.contextmanager
    def recover():
        old_mkldnn_conv_p = core.backends.mkldnn.conv.fp32_precision  # type: ignore[attr-defined]
        old_mkldnn_rnn_p = core.backends.mkldnn.rnn.fp32_precision  # type: ignore[attr-defined]
        old_mkldnn_matmul_p = core.backends.mkldnn.matmul.fp32_precision  # type: ignore[attr-defined]
        old_cudnn_conv_p = core.backends.cudnn.conv.fp32_precision  # type: ignore[attr-defined]
        old_cudnn_rnn_p = core.backends.cudnn.rnn.fp32_precision  # type: ignore[attr-defined]
        old_cuda_matmul_p = core.backends.cuda.matmul.fp32_precision
        try:
            yield
        finally:
            core.backends.mkldnn.conv.fp32_precision = old_mkldnn_conv_p  # type: ignore[attr-defined]
            core.backends.mkldnn.rnn.fp32_precision = old_mkldnn_rnn_p  # type: ignore[attr-defined]
            core.backends.mkldnn.matmul.fp32_precision = old_mkldnn_matmul_p  # type: ignore[attr-defined]
            core.backends.cudnn.conv.fp32_precision = old_cudnn_conv_p  # type: ignore[attr-defined]
            core.backends.cudnn.rnn.fp32_precision = old_cudnn_rnn_p  # type: ignore[attr-defined]
            core.backends.cuda.matmul.fp32_precision = old_cuda_matmul_p
