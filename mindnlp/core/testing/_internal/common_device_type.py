...
# from core._inductor.utils import GPU_TYPES
from mindnlp.core.testing._internal.common_cuda import (
...
from mindnlp.core.testing._internal.common_utils import (
    _TestParametrizer,
    clear_tracked_input,
...
# @dtypes(core.float32, core.int64)
...
#     - test_car_cuda_int64
#
# The dtype is passed as a core.dtype object.
#
...
#   for core.add as its "op" argument, the string 'cpu' for its "device" argument,
#   and the dtype core.float32 for is "dtype" argument. The second instantiated
#   test calls the test_car() with the OpInfo for core.sub, a CUDA device string
#   like 'cuda:0' or 'cuda:1' for its "device" argument, and the dtype
#   core.int64 for its "dtype argument."
#
    ...
    def instantiate_test(cls, name, test, *, generic_cls=None):
        ...
        for (
            ...
            cls_device_type = (
                ...
                else core._C._get_privateuse1_backend_name()
            )
...
class CUDATestBase(DeviceTypeTestBase):
    ...
    def setUpClass(cls):
        ...
        t = core.ones(1).cuda()
        cls.no_magma = not core.cuda.has_magma

        # Determines if cuDNN is available and its version
        cls.no_cudnn = not core.backends.cudnn.is_acceptable(t)
        cls.cudnn_version = None if cls.no_cudnn else core.backends.cudnn.version()

        # Acquires the current device as the primary (test) device
        cls.primary_device = f"cuda:{core.cuda.current_device()}"
...
class XPUTestBase(DeviceTypeTestBase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.primary_device = f"xpu:{core.xpu.current_device()}"
...
class PrivateUse1TestBase(DeviceTypeTestBase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.device_type = core._C._get_privateuse1_backend_name()
        ...
        assert (
            cls.device_mod is not None
                                            ...
                                            a module by `core._register_device_module`."""
...
def get_device_type_test_bases():
    ...
    if IS_SANDCASTLE or IS_FBCODE:
        ...
        # ramping up support.
        # elif core.backends.mps.is_available():
...
class ops(_TestParametrizer):
    def __init__(
        ...
        op_list,
        *,
        dtypes: Union[OpDTypes, Sequence[core.dtype]] = OpDTypes.supported,
        allowed_dtypes: Optional[Sequence[core.dtype]] = None,
        skip_if_dynamo=True,
    ):
    ...
    def _parametrize_test(self, test, generic_cls, device_cls):
        ...
        for op in self.op_list:
            ...
            dtypes: Union[set[core.dtype], set[None]]
...
def _has_sufficient_memory(device, size):
    if core.device(device).type == "cuda":
        if not core.cuda.is_available():
            return False
        gc.collect()
        core.cuda.empty_cache()
        # core.cuda.mem_get_info, aka cudaMemGetInfo, returns a tuple of (free memory, total memory) of a GPU
        if device == "cuda":
            device = "cuda:0"
        return (
            core.cuda.memory.mem_get_info(device)[0]
            * core.cuda.memory.get_per_process_memory_fraction(device)
        ) >= size
...
def largeTensorTest(size, device=None, inductor=TEST_WITH_TORCHINDUCTOR):
    ...
    def inner(fn):
        ...
        def dep_fn(self, *args, **kwargs):
            ...
            if inductor and core._inductor.config.cpp_wrapper and _device != "cpu":
                size_bytes *= 2
...
# Ex.
#
# @precisionOverride({core.half : 1e-2, core.float : 1e-4})
# @dtypes(core.half, core.float, core.double)
...
# Ex.
#
# @toleranceOverride({core.float : tol(atol=1e-2, rtol=1e-3},
#                     core.double : tol{atol=1e-4, rtol = 0})
# @dtypes(core.half, core.float, core.double)
# def test_X(self, device, dtype):
#   ...
#
...
#
# The above example sets atol = 1e-2 and rtol = 1e-3 for core.float and
# atol = 1e-4 and rtol = 0 for core.double.
...
# @dtypes((core.long, core.float32), (core.int, core.float64))
class dtypes:
    def __init__(self, *args, device_type="all"):
        if len(args) > 0 and isinstance(args[0], (list, tuple)):
            for arg in args:
                ...
                assert all(isinstance(dtype, core.dtype) for dtype in arg), (
                    f"Unknown dtype in {str(arg)}"
                )
        else:
            assert all(isinstance(arg, core.dtype) for arg in args), (
...
class dtypesIfPRIVATEUSE1(dtypes):
    def __init__(self, *args):
        super().__init__(*args, device_type=core._C._get_privateuse1_backend_name())
...
def onlyCUDAAndPRIVATEUSE1(fn):
    @wraps(fn)
    def only_fn(self, *args, **kwargs):
        if self.device_type not in ("cuda", core._C._get_privateuse1_backend_name()):
...
def disableMkldnn(fn):
    ...
    def disable_mkldnn(self, *args, **kwargs):
        if core.backends.mkldnn.is_available():
            with core.backends.mkldnn.flags(enabled=False):
                return fn(self, *args, **kwargs)
        return fn(self, *args, **kwargs)
...
def skipCPUIfNoLapack(fn):
    return skipCPUIf(not core._C.has_lapack, "PyTorch compiled without Lapack")(fn)


# Skips a test on CPU if FFT is not available.
def skipCPUIfNoFFT(fn):
    return skipCPUIf(not core._C.has_spectral, "PyTorch is built without FFT support")(
        fn
    )
...
def skipCPUIfNoMkldnn(fn):
    return skipCPUIf(
        not core.backends.mkldnn.is_available(),
...
def skipCUDAIfMiopen(fn):
    return skipCUDAIf(core.version.hip is not None, "Marked as skipped for MIOpen")(fn)


def skipCUDAIfNoMiopen(fn):
    return skipCUDAIf(core.version.hip is None, "MIOpen is not available")(
...
flex_attention_supported_platform = unittest.skipUnless(
    ...
    or (
        ...
        and core.utils._triton.has_triton()
        and core.cuda.get_device_capability() >= (8, 0)
    ),
    "Requires CUDA and Triton, or CPU with avx2 and later",
)
# if core.version.hip and "gfx94" in core.cuda.get_device_properties(0).gcnArchName:
#     e4m3_type = core.float8_e4m3fnuz
#     e5m2_type = core.float8_e5m2fnuz
#     E4M3_MAX_POS = core.finfo(core.float8_e4m3fnuz).max
#     E5M2_MAX_POS = core.finfo(core.float8_e5m2fnuz).max
# else:
#     e4m3_type = core.float8_e4m3fn
#     e5m2_type = core.float8_e5m2
#     E4M3_MAX_POS = core.finfo(core.float8_e4m3fn).max
#     E5M2_MAX_POS = core.finfo(core.float8_e5m2).max