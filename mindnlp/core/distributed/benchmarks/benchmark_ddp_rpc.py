...
from mindnlp import core.optim as optim
from core.distributed.optim import DistributedOptimizer
from core.distributed.rpc import RRef, TensorPipeRpcBackendOptions
from core.distributed.rpc.backend_registry import BackendType
...
class HybridModel(core.nn.Module):
    r"""
    ...
    def __init__(self, emb_rref_list, device):
        ...
        fc1 = core.nn.Linear(512, 256)
        fc2 = core.nn.Linear(256, 128)
        relu = core.nn.ReLU()
        fc3 = core.nn.Linear(128, 64)
        fc4 = core.nn.Linear(64, 32)
        fc5 = core.nn.Linear(32, 8)
    ...
    def forward(self, indices, offsets):
        ...
        for emb_rref in self.emb_rref_list:
            ...
            emb_lookups_cat = core.cat(emb_lookups, dim=1)
...
def _run_printable(cmd):
    ...
    buffer = io.BytesIO()
    core.save(proc.stdout.decode("utf-8"), buffer)
    input_tensor = core.ByteTensor(list(buffer.getvalue()))

    output = []
    buffer = io.BytesIO(np.asarray(input_tensor).tobytes())
    output.append(core.load(buffer))
    return output
...
def _run_trainer(emb_rref_list, rank):
    ...
    criterion = core.nn.CrossEntropyLoss()

    def get_next_batch(rank):
        for _ in range(10):
            num_indices = random.randint(20, 50)
            indices = core.LongTensor(num_indices).random_(0, NUM_EMBEDDINGS)
            ...
            offsets_tensor = core.LongTensor(offsets)
            target = core.LongTensor(batch_size).random_(8).cuda(rank)
...
if __name__ == "__main__":
    ...
    print("-------------------------------------------")
    print()
    print(f"* PyTorch version: {core.__version__}")
    print(f"* CUDA version: {core.version.cuda}")
    print()
