...
from mindnlp.core import device, dtype, nn, Tensor
from core.distributed import _remote_device
from core.distributed.nn.jit import instantiator
from core.distributed.rpc.internal import _internal_rpc_pickler
...
class _RemoteModule(nn.Module):
    def __new__(cls, *args, **kwargs):
        ...
        core._C._log_api_usage_once("core.distributed.nn.api.remote_module")
    ...
    def __init__(
        ...
        It can be shared across processors using `RPC framework <https://pycore.org/docs/stable/rpc.html>`__,
        without incurring any overheads of copying the actual module,
        which is equivalent to an :class:`~core.distributed.rpc.RRef`
        ...
        Args:
            ...
            _module_interface_cls (type, optional): The TorchScript interface type for the module
                to be created. The type object should be decorated by @core.jit.interface.
        ...
        Example::
            ...
            >>> from core.distributed.nn.api.remote_module import RemoteModule
            >>>
    ...
    def remote_parameters(self, recurse: bool = True) -> List[rpc.RRef[Parameter]]:
        """
        Return a list of :class:`~core.distributed.rpc.RRef` pointing to the remote module's parameters.

        This can typically be used in conjunction
        with :class:`~core.distributed.optim.DistributedOptimizer`.
        ...
        Returns:
            A list of :class:`~core.distributed.rpc.RRef` (``List[RRef[nn.Parameter]]``)
    ...
    def get_module_rref(self) -> rpc.RRef[nn.Module]:
        """Return an :class:`~core.distributed.rpc.RRef` (``RRef[nn.Module]``) pointing to the remote module."""
    ...
    def _prepare_init(self, remote_device_str: str) -> bool:
        ...
        enable_moving_cpu_tensors_to_cuda = core.device(self.device).type == "cuda"
...
class RemoteModule(_RemoteModule):
    ...
    Args:
        remote_device (str): Device on the destination worker where we'd like to place this module.
            The format should be "<workername>/<device>", where the device field can be parsed as core.device type.
    ...
    Example::
        ...
        `DistributedDataParallel <https://pycore.org/docs/stable/nn.html#core.nn.parallel.DistributedDataParallel>`__ (DDP)
        can be found in this `tutorial <https://pycore.org/tutorials/advanced/rpc_ddp_tutorial.html>`__.
    """
...
def _recursive_script_module_reducer(recursive_script_module):
    ...
    f = io.BytesIO()
    core.jit.save(recursive_script_module, f)
...
_internal_rpc_pickler._register_reducer(
    core.jit.RecursiveScriptModule, _recursive_script_module_reducer
)
