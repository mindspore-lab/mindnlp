...
from core.autograd.graph import save_on_cpu
from core.distributed.utils import _pack_kwargs, _replace_by_prefix, _unpack_kwargs
from core.utils.checkpoint import checkpoint as torch_utils_checkpoint
...
class ActivationWrapper(core.nn.Module, ABC):
    """
    ...
    def named_parameters(
        ...
        *args,
        **kwargs,
    ) -> Iterator[Tuple[str, core.nn.Parameter]]:
        """
...
def offload_wrapper(module: core.nn.Module) -> core.nn.Module:
    """
    ...
    **checkpoint_fn_kwargs,
    ...
    Args:
        ...
        checkpoint_impl (Optional[CheckpointImpl]):
            ...
            be passed into the ``core.utils.checkpoint.checkpoint``
            ...
            from ``core.utils.checkpoint``, keyword arguments will only be
            supported if ``checkpoint_impl`` is passed as ``CheckpointImpl.REENTRANT`.
        checkpoint_fn (Optional[Callable]):
            Functional checkpoint implementation to use. If this is specified,
            it will be used over the default ``core.utils.checkpoint.checkpoint``
...
def apply_activation_checkpointing(
    ...
    from core.distributed.fsdp._wrap_utils import _construct_wrap_fn, _post_order_apply
    from core.distributed.fsdp.wrap import (
        _Policy,
        _recursive_wrap,
