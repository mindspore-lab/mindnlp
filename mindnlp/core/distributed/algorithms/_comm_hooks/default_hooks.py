...
class LowPrecisionState(DefaultState):
    ...
    parameter precision specified by ``parameter_type`` (default: core.float32).
    Builds on top of the :class:`DefaultState`.

    Args:
        parameter_type (core.dtype): The precision of model's parameters.
    ...
    def __init__(
        self,
        process_group,
        parameter_type=core.float32,
    ):
        super().__init__(process_group)
        self.parameter_type = parameter_type


def _decompress(state: LowPrecisionState, grad: core.Tensor):
    """
    ...
    try:
        ...
        raise AttributeError(
            f"Device {grad.device}  does not have a \
                corresponding backend registered as 'core.device_type'."
        ) from e
...
def allreduce_hook(state: DefaultState, grad: core.Tensor):
    r"""
    ...
    Args:
        ...
        grad (core.Tensor): A gradient for the local batch that needs to be communicated across ranks.
    """
...
def reduce_scatter_hook(state: DefaultState, grad: core.Tensor, output: core.Tensor):
    r"""
    ...
    Args:
        state (DefaultState): State information, configures pre- and post-division factors.
        grad (core.Tensor): An unsharded gradient for the local batch that needs to be
        communicated across ranks.
        output (core.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
...
def fp16_compress_hook(
    state: LowPrecisionState, grad: core.Tensor, output: Optional[core.Tensor] = None
):
    r"""
    Implement FSDP communication hook for a simple gradient compression approach.
    Casts ``grad`` to half-precision floating-point format (``core.float16``).
    ...
    Args:
        ...
        grad (core.Tensor): A gradient for the local batch that needs to be communicated across ranks in a lower precision.
        output (core.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
    fp16_hook = functools.partial(_low_precision_hook, core.float16)
...
def bf16_compress_hook(
    ...
    Args:
        ...
        output (core.Tensor): Stores a single shard of the gradient after ``reduce_scatter``.
    """
    bf16_hook = functools.partial(_low_precision_hook, core.bfloat16)
