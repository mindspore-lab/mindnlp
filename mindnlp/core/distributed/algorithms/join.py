...
class Join:
    ...
    Example::
        ...
        >>> # xdoctest: +SKIP
        >>> from mindnlp import core.nn.parallel.DistributedDataParallel as DDP
        >>> from mindnlp import core.distributed.optim.ZeroRedundancyOptimizer as ZeRO
        >>> from core.distributed.algorithms.join import Join
        >>>
        >>> # On each spawned worker
        >>> def worker(rank):
        >>>     dist.init_process_group("nccl", rank=rank, world_size=2)
        >>>     model = DDP(core.nn.Linear(1, 1).to(rank), device_ids=[rank])
        >>>     optim = ZeRO(model.parameters(), core.optim.Adam, lr=0.01)
        >>>     # Rank 1 gets one more input than rank 0
        >>>     inputs = [core.tensor([1.]).to(rank) for _ in range(10 + rank)]
    ...
    def _get_num_nonjoined_procs(self):
        ...
        num_nonjoined_procs = core.zeros(1, device=self._device)
    ...
    def _notify_procs_to_terminate(self):
        ...
        """
        ones = core.ones(1, device=self._device)
    ...
    def notify_join_context(joinable: Joinable):
        ...
        ones = core.ones(1, device=device)
        ...
        if join_config.throw_on_early_termination:
            ...
            zeros = core.zeros(1, device=device)
