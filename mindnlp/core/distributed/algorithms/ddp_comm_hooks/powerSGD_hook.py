...
def _orthogonalize(matrices, epsilon=0):
    ...
    if rank <= 2 or dtype in [core.float16, core.bfloat16]:
...
def _orthogonalize_gram_schmidt(matrices, epsilon=0):
    ...
    If epsilon is 0, this is equivalent to `core.qr(matrices, out=(matrices, _))`,
    """
    ...
    for i in range(num_cols):
        ...
        if epsilon == 0:
            ...
            try:
                col /= core.norm(col, dim=1, keepdim=True)
            ...
            col /= core.norm(col, dim=1, keepdim=True) + epsilon
        # Project it on the rest and remove it.
        if i + 1 < num_cols:
            rest = matrices[:, :, i + 1 :]
            rest -= core.sum(col * rest, dim=1, keepdim=True) * col
...
class PowerSGDState:
    ...
    def __init__(
        ...
        self.error_dict: Dict[int, core.Tensor] = {}
        self.p_memory_dict: Dict[int, core.Tensor] = {}
        self.q_memory_dict: Dict[int, core.Tensor] = {}
...
def powerSGD_hook(
    ...
    if state.use_error_feedback:
        if bucket_index in state.error_dict:
            ...
            state.error_dict[bucket_index] = core.zeros(
        ...
        input_tensor_cp = core.clone(input_tensor).detach()
    ...
    uncompressed_tensors_memory = (
        core.cat([tensor.view(-1) for tensor in uncompressed_tensors])
        if uncompressed_tensors
        else core.tensor([], device=device, dtype=dtype)
    )
    ...
    if not state.warm_start or bucket_index not in state.p_memory_dict:
        ...
        state.p_memory_dict[bucket_index] = core.empty(
            total_Ps_size, device=device, dtype=dtype
        )
        state.q_memory_dict[bucket_index] = core.empty(
    ...
    if not need_randomize_qs:
        ...
        with core.random.fork_rng(devices=[]):
            ...
            core.manual_seed(state.rng.randint(1_000_000_000))
            for q in qs:
                q.copy_(
                    core.randn(
                        *q.shape,
    ...
    def compute_qs(fut):
        ...
        for tensor, p, q in zip(tensors_to_compress, ps, qs):
            core.bmm(tensor.transpose(1, 2), p, out=q)
    ...
    def decompress(fut):
        ...
        for p, q, tensor in zip(ps, qs, tensors_to_compress):
            core.bmm(p, q.transpose(1, 2), out=tensor)
        ...
        def create_low_rank_tensor(fill_random_values, rng):
            ...
            if fill_random_values:
                with core.random.fork_rng(devices=[]):
                    ...
                    core.manual_seed(rng.randint(1_000_000_000))
                    return core.randn(
                        square_side_length,
                        state.matrix_approximation_rank,
                        device="cpu",
