...
def _quantize_per_tensor_backend(x, scale, zero_point):
    ...
    y = core.clamp(y, 0, 255).to(core.uint8)
    return y


def _dequantize_per_tensor_backend(y, scale, zero_point):
    x = scale * (y.to(core.float32) - zero_point)
    return x


def _quantize_per_channel_backend(x, scale, zero_point):
    y = core.zeros(x.size(), device=x.device)
    for i in range(x.size()[0]):
        y[i, :] = core.round(x[i, :] / scale[i]) + zero_point[i]
    y = core.clamp(y, 0, 255).to(core.uint8)
    return y


def _dequantize_per_channel_backend(y, scale, zero_point):
    y = y.to(core.float32).to(y.device)
    x = core.zeros_like(y, device=y.device)
    for i in range(x.size()[0]):
...
def quantization_pertensor_hook(
    ...
    """
    Apply ``core.quantize_per_tensor`` logic to DDP using ``allgather`` protocol.
    ...
    myObserver = core.ao.quantization.MinMaxObserver().to(tensor.device)
    myObserver(tensor)

    s, z = myObserver.calculate_qparams()
    s_and_z = core.FloatTensor([s, z]).to(tensor.device)
    ...
    def dequantize_and_aggregate(fut):
        ...
        aggregated_dequantized_tensor = core.zeros_like(
            all_ranks_quantized_tensor[0], device=tensor.device, dtype=core.float32
        )
...
def quantization_perchannel_hook(
    ...
    """
    Apply``core.quantize_per_channel`` logic to DDP using ``allgather`` protocol.
    ...
    myPerChannelObserver = core.ao.quantization.PerChannelMinMaxObserver().to(
        tensor.device
    )
    myPerChannelObserver(tensor_in_channels)

    s_ch, z_ch = myPerChannelObserver.calculate_qparams()
    s_and_z = core.stack((s_ch, z_ch)).to(tensor.device)
    ...
    def dequantize_and_aggregate(fut):
        ...
        return (
            core.flatten(aggregated_dequantized_tensor).to(tensor.device)[
                : tensor.size()[0]
            ]
            / world_size
        )
