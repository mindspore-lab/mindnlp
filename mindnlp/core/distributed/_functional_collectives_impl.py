...
def _broadcast(input, src, tag, ranks, group_size):
    ...
    return core.ops._c10d_functional.broadcast(
        input,
        src,
        group_name,
    )
...
def _all_reduce(input, reduce_op, tag, ranks, group_size):
    group_name = c10d._resolve_group_name_by_ranks_and_tag(ranks, tag)
    return core.ops._c10d_functional.all_reduce(
        input,
        reduce_op,
        group_name,
    )
...
def _all_reduce_coalesced(inputs, reduce_op, tag, ranks, group_size):
    group_name = c10d._resolve_group_name_by_ranks_and_tag(ranks, tag)
    return core.ops._c10d_functional.all_reduce_coalesced(
        inputs,
        reduce_op,
        group_name,
    )
...
def _all_gather_into_tensor(input, tag, ranks, group_size):
    group_name = c10d._resolve_group_name_by_ranks_and_tag(ranks, tag)
    return core.ops._c10d_functional.all_gather_into_tensor(
        input,
        group_size,
        group_name,
    )
...
def _all_gather_into_tensor_coalesced(input, tag, ranks, group_size):
    group_name = c10d._resolve_group_name_by_ranks_and_tag(ranks, tag)
    return core.ops._c10d_functional.all_gather_into_tensor_coalesced(
        input,
        group_size,
        group_name,
    )
...
def _reduce_scatter_tensor(
    ...
    group_name = c10d._resolve_group_name_by_ranks_and_tag(ranks, tag)
    return core.ops._c10d_functional.reduce_scatter_tensor(
        input,
        reduce_op,
        group_size,
        group_name,
...
def _reduce_scatter_tensor_coalesced(
    ...
    group_name = c10d._resolve_group_name_by_ranks_and_tag(ranks, tag)
    return core.ops._c10d_functional.reduce_scatter_tensor_coalesced(
        inputs,
        reduce_op,
        group_size,
        group_name,
...
def _all_to_all_single(
    ...
    group_name = c10d._resolve_group_name_by_ranks_and_tag(ranks, tag)
    return core.ops._c10d_functional.all_to_all_single(
        input,
        output_split_sizes,
        input_split_sizes,
        group_name,
...
def _wait_tensor(tensor: core.Tensor) -> core.Tensor:
    return core.ops._c10d_functional.wait_tensor(tensor)
