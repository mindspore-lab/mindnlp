...
r"""
Module ``core.distributed.launch``.

``core.distributed.launch`` is a module that spawns up multiple distributed
...
::
    ...
    python -m core.distributed.launch --nproc-per-node=NUM_GPUS_YOU_HAVE
...
::
    ...
    python -m core.distributed.launch --help
...
::
    ...
    >>> core.cuda.set_device(args.local_rank)  # before your code runs

or

::

    >>> with core.cuda.device(args.local_rank):
    >>>    # your code to run
    >>>    ...
...
::
    ...
    >>> core.distributed.init_process_group(backend='YOUR BACKEND',
    >>>                                      init_method='env://')
...
from core.distributed.run import get_args_parser, run
...
@_deprecated(
    "The module core.distributed.launch is deprecated\n"
