...
from core.distributed.device_mesh import DeviceMesh
# from core.fx.experimental.proxy_tensor import get_proxy_mode

from . import _functional_collectives_impl as fun_col_impl


# try:
#     from core.utils._cxx_pytree import tree_map_only
# except ImportError:
#     from core.utils._pytree import tree_map_only  # type: ignore[no-redef]


# if core._running_with_deploy():

#     def is_torchdynamo_compiling():
#         """Can't from mindnlp import coredynamo in torchdeploy builds currently."""
#         return False

# else:
#     try:
#         from core.compiler import is_dynamo_compiling as is_torchdynamo_compiling
#     except Exception:
#         warnings.warn(
#             "Unable to from mindnlp import coredynamo util `is_torchdynamo_compiling`, so won't support torchdynamo correctly"
#         )
...
Issues:
* Where should these ops live? Couldn't `from mindnlp import core` if putting these ops in existing core.distributed files
...
tensor - even return a tensor with a different shape (see ``core.vmap``).
...
Here's how it works under core.compile/dynamo:
all_reduce(...)
...
def wait_tensor(tensor):
    ...
    """
    return core.ops._c10d_functional.wait_tensor(tensor)  # type: ignore[attr-defined]


def broadcast(self: core.Tensor, src: int, group: RANK_TYPES, tag: str = ""):
    """
    ...
    tensor = core.ops._c10d_functional.broadcast(self, src, group_name)
    return _maybe_wrap_tensor(tensor)


def all_reduce(self: core.Tensor, reduceOp: str, group: RANK_TYPES, tag: str = ""):
    """
    ...
    """
    group_name = _resolve_group_name(group, tag)
    tensor = core.ops._c10d_functional.all_reduce(self, reduceOp.lower(), group_name)
    return _maybe_wrap_tensor(tensor)
...
def all_gather_tensor(
    ...
    tensor = core.ops._c10d_functional.all_gather_into_tensor(
        self, group_size, group_name
    )
    res = _maybe_wrap_tensor(tensor)
    # TODO this should be done inside AsyncCollectiveTensor to delay the wait() call
    if gather_dim != 0:
        # core.cat access the data so we already need to wait here, first do wait
        ...
        res = core.cat(core.chunk(res, group_size, dim=0), dim=gather_dim)
    return res
...
def all_gather_tensor_autograd(
    ...
    tensor = core.ops._c10d_functional_autograd.all_gather_into_tensor(
        self, group_size, group_name
    )
...
def reduce_scatter_tensor(
    ...
    if scatter_dim != 0:
        tensor_list = core.chunk(self, group_size, dim=scatter_dim)
        self = core.cat(tensor_list)

    tensor = core.ops._c10d_functional.reduce_scatter_tensor(
        self,
        reduceOp.lower(),
        group_size,
...
def reduce_scatter_tensor_autograd(
    ...
    tensor = core.ops._c10d_functional_autograd.reduce_scatter_tensor(
        self,
        reduceOp.lower(),
        group_size,
        group_name,  # type: ignore[possibly-undefined]
...
def all_reduce_coalesced(
    self: List[core.Tensor], reduceOp: str, group: RANK_TYPES, tag: str = ""
) -> List[core.Tensor]:
    """
    ...
    """
    group_name = _resolve_group_name(group, tag)
    tensor_list = core.ops._c10d_functional.all_reduce_coalesced(  # type: ignore[attr-defined]
        self,
        reduceOp.lower(),
        group_name,
    )
...
def all_gather_into_tensor_coalesced(
    self: List[core.Tensor], group: RANK_TYPES, tag: str = ""
) -> List[core.Tensor]:
    """
    ...
    group_name = _resolve_group_name(group, tag)
    group_size = c10d._get_group_size_by_name(group_name)
    tensor_list = core.ops._c10d_functional.all_gather_into_tensor_coalesced(  # type: ignore[attr-defined]
        self,
        group_size,
        group_name,
    )
...
def reduce_scatter_tensor_coalesced(
    ...
    for idx, (dim, tensor) in enumerate(zip(scatter_dim, inputs)):
        ...
        if dim != 0:
            tensor_list = core.chunk(tensor, group_size, dim=dim)
            inputs[idx] = core.cat(tensor_list)

    tensor_list = core.ops._c10d_functional.reduce_scatter_tensor_coalesced(  # type: ignore[attr-defined]
        inputs,
        reduceOp.lower(),
        group_size,
        group_name,  # type: ignore[possibly-undefined]
...
def _is_view_op(tgt):
    assert isinstance(tgt, core._ops.OpOverload)
    schema = tgt._schema
...
def all_to_all_single(
    ...
    if output_split_sizes is not None:
        assert all(
            isinstance(size, (int, core.SymInt)) for size in output_split_sizes
        ), output_split_sizes
    if input_split_sizes is not None:
        assert all(
            isinstance(size, (int, core.SymInt)) for size in input_split_sizes
        ), input_split_sizes
    group_name = _resolve_group_name(group, tag)
    group_size = c10d._get_group_size_by_name(group_name)
    ...
    tensor = core.ops._c10d_functional.all_to_all_single(  # type: ignore[attr-defined]
        self,
        output_split_sizes,
        input_split_sizes,
        group_name,
...
def all_to_all_single_autograd(
    ...
    if output_split_sizes is None or input_split_sizes is None:
        ...
        output_split_sizes = [self.shape[0] // group_size] * group_size
        input_split_sizes = output_split_sizes
    tensor = core.ops._c10d_functional_autograd.all_to_all_single(  # type: ignore[attr-defined]
        self,
        output_split_sizes,
        input_split_sizes,
        group_name,
...
class AsyncCollectiveTensor(core.Tensor):
    r"""
    ...
    def functional_collective(self, group, tag):
        ...
        tensor = core.ops.c10d_functional.{collective}(self, tag, rankset, group_size)
        return _maybe_wrap_tensor(tensor)
    """
    elem: core.Tensor
    completed: bool
    ...
    @staticmethod
    def __new__(cls, elem: core.Tensor):
        r = core.Tensor._make_wrapper_subclass(  # type: ignore[attr-defined]
            cls,
            elem.size(),
            strides=elem.stride(),
    ...
    def __coerce_same_metadata_as_tangent__(
        ...
        if expected_type is not core.Tensor:
            return None
    ...
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
        if func == core.ops.aten.view.default:
        ...
        if is_view_op:
            out = tree_map_only(core.Tensor, wrap, out)
...
def _expand_group(group: RANK_TYPES, tag: str = "") -> Tuple[str, List[int], int]:
    ...
    # graph_break [('core.* op returned non-Tensor int
    # caused by 'cast_*` functions being treated as 'core.*' ops (iiuc)
    if TYPE_CHECKING:
...
class _FromTorchTensor(core.autograd.Function):
    """
    ...
    @staticmethod
    def backward(ctx, grad_output: core.Tensor) -> core.Tensor:  # type: ignore[override]
        return grad_output
...
def _are_we_tracing() -> bool:
    ...
    if (
        core._C._get_dispatch_mode(core._C._TorchDispatchModeKey.FUNCTIONAL)
        is not None
    ):
        return True
...
def _maybe_wrap_tensor(self) -> core.Tensor:
    if _are_we_tracing():
        return wait_tensor(self)
    res = AsyncCollectiveTensor(self)
    return cast(core.Tensor, res)


@contextlib.contextmanager
def allow_inflight_collective_as_graph_input_ctx(value: bool = True):
    """
    Context manager to temporarily set whether inflight collectives are allowed as core.compile graph inputs.
    ...
    @core.compile(fullgraph=True)
    def all_reduce_wait_compiled(y):
        core.ops.c10d_functional.wait_tensor(y)
        return y * y

    x = core.ones(1280, 1280, device="cuda") + self.rank
    ...
    """
    previous = core._C._distributed_c10d._allow_inflight_collective_as_graph_input()

    try:
        core._C._distributed_c10d._set_allow_inflight_collective_as_graph_input(value)
        yield
    finally:
        core._C._distributed_c10d._set_allow_inflight_collective_as_graph_input(
            previous
        )
...
def _all_reduce_coalesced_meta(self, *args):
    return [core.empty_like(t) for t in self]
...
# if not core._running_with_deploy():
...
#     lib_impl = core.library.Library("_c10d_functional", "IMPL")
...
#     core.fx.node.has_side_effect(core.ops._c10d_functional.wait_tensor.default)
#     core.fx.node.has_side_effect(core.ops._c10d_functional.wait_tensor)

#     # Register legacy ops for backward compatibility
#     # TODO(yifu): remove these in functional collective beta release
#     legacy_lib = core.library.Library("c10d_functional", "DEF")
#     legacy_lib_impl = core.library.Library("c10d_functional", "IMPL")
#     ops_defs = [
...
#         legacy_lib.define(op_def, tags=core.Tag.pt2_compliant_tag)
...
These schemas intentionally match core.distributed.distributed_c10d.* ops that we are trying to remap from
"""
