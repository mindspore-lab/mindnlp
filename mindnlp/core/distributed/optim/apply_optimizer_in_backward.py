...
param_to_optim_hook_handle_map = core.utils.weak.WeakTensorKeyDictionary()
param_to_acc_grad_map = core.utils.weak.WeakTensorKeyDictionary()


@no_type_check
def _apply_optimizer_in_backward(
    optimizer_class: Type[core.optim.Optimizer],
    params: Iterable[core.nn.Parameter],
    ...
    Args:
        optimizer_class: (Type[core.optim.Optimizer]): Optimizer to apply to parameter
    ...
    Example::
        ...
        apply_optimizer_in_backward(core.optim.SGD, [param_1], {"lr": .02})
        apply_optimizer_in_backward(core.optim.Adam, remainder_params, {"lr": .04})

        model(...).sum().backward() # after backward, parameters will already
        # have their registered optimizer(s) applied.

    """
    core._C._log_api_usage_once("core.distributed.optim.apply_optimizer_in_backward")

    @no_type_check
    def _apply_optimizer_in_backward_to_param(param: core.nn.Parameter) -> None:
...
def _get_in_backward_optimizers(module: core.nn.Module) -> List[core.optim.Optimizer]:
    """
    ...
    Args:
        module: (core.nn.Module): model to retrieve in-backward optimizers for

    Returns:
        List[core.optim.Optimizer]: the in-backward optimizers.

    Example::
        _apply_optimizer_in_backward(core.optim.SGD, model.parameters(), {'lr': 0.01})
        optims = _get_optimizers_in_backward(model)
    """
    optims: List[core.optim.Optimizer] = []
