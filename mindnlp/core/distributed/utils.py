...
from core.nn.utils.rnn import PackedSequence
...
def _cast_forward_inputs(
    ...
    **kwargs: Any,
    ...
    def cast_fn(x: core.Tensor) -> core.Tensor:
...
T = TypeVar("T", core.Tensor, PackedSequence)


@overload
def _recursive_to(
    inputs: S, target_device: core.device, use_side_stream_for_tensor_copies: bool
) -> List[S]:
    ...
...
@overload
def _recursive_to(
    inputs: T, target_device: core.device, use_side_stream_for_tensor_copies: bool
) -> Tuple[T]:
    ...
...
def _recursive_to(inputs, target_device, use_side_stream_for_tensor_copies):
    ...
    def to_map(obj):
        if isinstance(obj, (core.Tensor, PackedSequence)):
            ...
            if not use_side_stream_for_tensor_copies:
                ...
                from core.nn.parallel._functions import _get_stream
                ...
                with device_mod.device(target_device.index):
                    ...
                    if isinstance(obj, PackedSequence):
                        ...
                        assert isinstance(output, core.Tensor)
                        output.record_stream(current_stream)  # type: ignore[arg-type]
                return (output,)

        from core.nn.parallel.scatter_gather import _is_namedtuple
...
def _alloc_storage(tensor: core.Tensor, size: core.Size) -> None:
    """
    ...
    """
    with core.no_grad():
        if not core.distributed._functional_collectives.is_torchdynamo_compiling():
...
def _free_storage(tensor: core.Tensor):
    """
...
@overload
def _apply_to_tensors(fn: Callable[[core.Tensor], Q], container: core.Tensor) -> Q:
    ...


@overload
def _apply_to_tensors(fn: Callable[[core.Tensor], Any], container: R) -> R:
    ...
...
def _sync_module_states(
    ...
    """
    module_states: List[core.Tensor] = []
...
def _replace_by_prefix(
    ...
    Usage::
        ...
        state_dict = {"layer.xyz": core.tensor(1)}
        replace_by_prefix_(state_dict, "layer.", "module.layer.")
        assert state_dict == {"module.layer.xyz": core.tensor(1)}
    """
...
def _data_ptr_allocated(tensor: core.Tensor) -> bool:
