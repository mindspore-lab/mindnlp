...
if dist.is_available() or TYPE_CHECKING:
    ...
    # from core.distributed.tensor import distribute_tensor, DTensor, Replicate
...
def _iterate_state_dict(
    ...
    *,
    ...
    Args:
        ...
        device (Optional[core.device]): device passed to tensor functions
    ...
    *,
    ...
    Args:
        ...
        pg (Optional[dist.ProcessGroup]): the process group that is used to
            ...
            DTensor.
        device: (Optional[core.device]): the device that is used to
...
def _create_cpu_state_dict(
    ...
    def tensor_func(
        ...
        if len(obj.size()) == 0:
            return core.tensor(0, dtype=obj.dtype)

        if share_memory:
            t = core.empty(*tuple(obj.size()), dtype=obj.dtype)
            t = t.share_memory_()
            if pin_memory:

                def unpin_memory(t):
                    succ = int(core.cuda.cudart().cudaHostUnregister(t.data_ptr()))
                    assert (
                        succ == 0
                ...
                succ = int(
                    core.cuda.cudart().cudaHostRegister(
                        t.data_ptr(),
            ...
            return core.empty(*tuple(obj.size()), dtype=obj.dtype).pin_memory()
        else:
            return core.empty(*tuple(obj.size()), dtype=obj.dtype)
...
def _distribute_tensors(
    ...
    for key in keys:
        ...
        if _local_state is None or core.is_tensor(_local_state):
            continue
...
# These APIs are from core.distributed.checkpoint.
