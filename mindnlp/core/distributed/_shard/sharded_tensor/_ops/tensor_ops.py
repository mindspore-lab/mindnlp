...
from core.distributed._shard.common_op_utils import _register_default_op
from core.distributed._shard.sharded_tensor import (
    _sharded_op_impl,
    Shard,
    ShardedTensor,
)
...
_register_default_op(core.Tensor.shape.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
_register_default_op(core.Tensor.dtype.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
# _register_default_op(core.Tensor.layout.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
_register_default_op(core.Tensor.size, _sharded_op_impl)
_register_default_op(core.Tensor.dim, _sharded_op_impl)
_register_default_op(core.Tensor.ndim.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
_register_default_op(core.Tensor.is_contiguous, _sharded_op_impl)
_register_default_op(core.Tensor.contiguous, _sharded_op_impl)
_register_default_op(core.Tensor.is_floating_point, _sharded_op_impl)

# __reduce_ex__ to dispatch to get_state/set_state
_register_default_op(core.Tensor.__reduce_ex__, _sharded_op_impl)

# autograd related properties
_register_default_op(core.Tensor.requires_grad.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
# TODO: set grad with a ShardedTensor that consists of all local grads
_register_default_op(core.Tensor.grad.__get__, _sharded_op_impl)  # type: ignore[union-attr]
# _register_default_op(core.Tensor.grad_fn.__get__, _sharded_op_impl)  # type: ignore[union-attr]
# _register_default_op(core.Tensor.is_leaf.__get__, _sharded_op_impl)  # type: ignore[attr-defined]
...
# @_sharded_op_impl(core.Tensor.device.__get__)
def tensor_device(types, args=(), kwargs=None, pg=None):
    ...
    if self_st._local_shards:
        ...
        dev = core.device(core.cuda.current_device())
    return dev


# @_sharded_op_impl(core.Tensor.is_meta.__get__)  # type: ignore[attr-defined]
...
def sharded_type_as_check(*args, **kwargs):
    ...
    Args: same as ``core.Tensor.type_as``.

    Return: None
    """
    if len(args) < 2:
        raise ValueError("Needs to give a tensor to cast type as!")
    if not isinstance(args[1], core.Tensor) and not isinstance(args[1], ShardedTensor):
...
def sharded_type_as(args, kwargs, pg):
    """
    Handles ``__torch_function__`` dispatch for the ``core.Tensor.type_as`` op.
...
_register_sharded_op_on_local_shards(
    core.Tensor.__deepcopy__,
    customized_func=sharded_deepcopy,
)


@_sharded_op_impl(core.Tensor.copy_)
...
def sharded_clone(args, kwargs, pg):
    ...
    if desire_memory_format and desire_memory_format != core.preserve_format:
        raise RuntimeError("Only support core.preserve_format for ShardedTensor!")
    cloned_local_shards = [
        Shard(
...
# @_sharded_op_impl(core.Tensor.requires_grad_)
def tensor_requires_grad_set(types, args=(), kwargs=None, pg=None):
    ...
    with core._C.DisableTorchFunctionSubclass():
