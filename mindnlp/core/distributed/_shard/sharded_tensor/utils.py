...
from core.distributed import distributed_c10d as c10d
...
def _flatten_tensor_size(size) -> core.Size:
    """
    Checks if tensor size is valid, then flatten/return a core.Size object.
    """
...
def build_metadata_from_local_shards(
    ...
    for local_shard in local_shards:
        ...
        if (
            local_shard_tensor.layout != core.strided
            or local_shard_tensor.layout != first_shard_layout
        ):
            raise ValueError(
                f"Only core.strided layout is currently supported, but found "
