...
from core.distributed.nn.functional import all_to_all, all_to_all_single
...
def build_reshard_metadata(
    ...
    Args:
        st_size (core.Size): The size of the sharded tensor.
        sharding_spec (:class:`core.distributed._shard.sharding_spec.ShardingSpec`): The
...
def reshard_local_shard(
    ...
    if rearrange_input:
        ...
        local_tensor = local_tensor.index_select(
            reshard_dim, core.tensor(indices, device=local_tensor.device)
        )

    # Because reshard_dim != original shard_dim. We need to compute the
    # size of tensor from each rank.
    output_tensor_list = [core.tensor(1)] * world_size
    ...
    for idx, placement in enumerate(sharding_spec.placements):  # type: ignore[attr-defined]
        ...
        output_tensor_list[
            placement.rank()
        ] = core.empty(  # type: ignore[union-attr, index]
    ...
    input_tensor_tuple = core.split(local_tensor, input_split_sizes, dim=reshard_dim)
    ...
    local_tensor = core.cat(output_tensor_list, dim=current_sharding_dim)
    local_shards = [Shard(local_tensor, shards_metadata[current_rank])]
    return local_shards, shards_metadata
