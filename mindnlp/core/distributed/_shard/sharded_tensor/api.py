...
from mindnlp import core.distributed._shard.sharding_spec as shard_spec
from core.distributed import distributed_c10d, rpc
from core.distributed._shard._utils import DEPRECATE_MSG
from core.distributed._shard.sharding_spec._internals import (
    check_tensor,
    validate_non_overlapping_shards_metadata,
)
from core.distributed._shard.sharding_spec.api import (
    _dispatch_custom_op,
    _has_custom_op,
)
from core.distributed.remote_device import _remote_device
from core.utils import _pytree as pytree
...
class ShardedTensorBase(core.Tensor):
    ...
    def _init_from_local_shards_and_global_metadata(
        ...
        # if tensor_properties.layout != core.strided:
        #     raise ValueError("Only core.strided layout is currently supported")
...
class ShardedTensor(ShardedTensorBase):
    """
    ShardedTensor is an core.Tensor subclass to represent Tensors that are sharded
    ...
    create_op specified by tensor_init_params.create_op, e.g., core.ones, or
    core.empty

    Args:
        sharding_spec (:class:`core.distributed._shard.sharding_spec.ShardingSpec`): The specification
    ...
    Keyword args:
        ...
        layout (:class:`core.layout`, optional): the desired layout of returned Tensor.
            Default: ``core.strided``.
        requires_grad (bool, optional): If autograd should record operations on the
            returned tensor. Default: ``False``.
        pin_memory (bool, optional): If set, returned tensor would be allocated in
            the pinned memory. Works only for CPU tensors. Default: ``False``.
        memory_format (:class:`core.memory_format`, optional): the desired memory format of
            returned Tensor. Default: ``core.contiguous_format``.
        init_rrefs (bool, optional): Whether or not to initialize
            :class:`core.distributed.rpc.RRef`s pointing to remote shards.
    ...
    .. note:: ShardedTensor uses collectives to do various operations, i.e. it
        ...
        used is given by ``core.cuda.current_device()`` and it is the user's
        responsibility to ensure that this is set so that each rank has an
        individual GPU, via ``core.cuda.set_device()``
    ...
    def __init__(
        ...
        *size,
        ...
        if memory_format != core.contiguous_format:
            raise ValueError(
                "Only core.contiguous_format memory_format is currently supported"
            )
        ...
        if self._init_rrefs:
            ...
            if not rpc._is_current_rpc_agent_set():
                raise RuntimeError(
                    ...
                    " core.distributed.rpc.init_rpc if init_rrefs is set to True"
                )
            self._init_rpc()
    ...
    def _init_rpc(self):
        ...
        core.futures.wait_all(futs)

        # Barrier for all RPCs to finish on all ranks.
        rpc.api._all_gather(None)

    def _get_preferred_device(self) -> core.device:
        """
        ...
        if dist.get_backend(self._process_group) == dist.Backend.NCCL:
            return core.device(core.cuda.current_device())
        return core.device("cpu")

    def gather(  # type: ignore[override]
        self,
        dst: int = 0,
        out: Optional[core.Tensor] = None,
        enforce_dtype: bool = False,
        dtype: Optional[core.dtype] = None,
    ) -> None:
        """
        ...
        Args:
            dst(int): The rank where full tensor is constructed.
                Default: 0
            out (:class `core.Tensor`, optional): The output full tensor.
            ...
            dtype (core.dtype): Force the gathered tensors to be this dtype.
                Default: ``None``
        """
        ...
        if rank == dst:
            ...
            gather_list = [
                core.empty((max_rank_size,), device=out.device, dtype=dtype)
    ...
    def cpu(
        self, memory_format=core.preserve_format, process_group=None
    ) -> ShardedTensor:
        """
        ...
        # core.Tensor subclass, see https://github.com/pytorch/pytorch/issues/75402
        if (
            memory_format != core.preserve_format
            and memory_format != core.contiguous_format
        ):
            raise RuntimeError(
                "Only `core.contiguous_format` or "
                "`core.preserve_format` is supported!"
            )
        all_on_cpu = True
        ...
        for shard in self._local_shards:
            ...
            metadata.placement._device = core.device("cpu")  # type: ignore[union-attr]
        ...
        for meta in st_meta.shards_metadata:
            if meta.placement.device().type != "cpu":  # type: ignore[union-attr]
                meta.placement._device = core.device("cpu")  # type: ignore[union-attr]
    ...
    def cuda(
        ...
        if device is not None:
            device = core.device(device) if isinstance(device, str) else device
            assert (
                isinstance(device, core.device)
                and device.index == core.cuda.current_device()
            ), """Only device without device id (e.g. "cpu" or "cuda") is expected for ShardedTensor!"""

        current_device = core.device(core.cuda.current_device())
    ...
    def to(self, *args, **kwargs) -> ShardedTensor:
        ...
        if self._local_shards:
            ...
            current_device = core.device("cpu")
        else:
            current_device = core.device(core.cuda.current_device())
        ...
        if len(args) == 1:
            if isinstance(args[0], core.dtype):
                dtype_to = args[0]
            elif isinstance(args[0], core.device):
                device_to = args[0]
            elif isinstance(args[0], (str, int)):
                device_to = core.device(args[0])
            elif isinstance(args[0], core.Tensor):
        ...
        if device_to.type == "cuda":
            ...
            device_to = core.device(current_idx)

        copy_tensor = kwargs.get("copy", False)
        non_blocking = kwargs.get("non_blocking", False)
        memory_format = kwargs.get("memory_format", core.preserve_format)
    ...
    def _init_from_local_tensor(
        ...
        *global_size: Sequence[int],
        ...
        Args:
            ...
            sharding_spec (:class:`core.distributed._shard.sharding_spec.ShardingSpec`):
        ...
        Examples:
            ...
            >>> tensor = core.arange(2, dtype=core.int64) + 1 + 2 * rank
            >>> local_tensor = core.unsqueeze(core.cat([tensor, tensor + 2]))
            >>> local_tensor
    ...
    def reshard(self, resharding_spec: shard_spec.ShardingSpec) -> ShardedTensor:
        ...
        Args:
            resharding_spec (:class:`core.distributed._shard.sharding_spec.ShardingSpec`): The
        ...
        Examples:
            >>> # xdoctest: +SKIP
            >>> # We have 2 process groups, 2 ranks.
            >>> tensor = core.arange(4, dtype=core.int64) + 1 + 2 * rank
            >>> tensor = core.stack([tensor, tensor])
            >>> tensor
    ...
    def __setstate__(self, state):
        ...
        # Setup process group
        from core.distributed._shard.api import _get_current_process_group
