...
from core.distributed import distributed_c10d
...
def _shard_tensor(
    tensor: core.Tensor, sharding_spec: ShardingSpec, src_rank=0, process_group=None
) -> ShardedTensor:
    """
    Given a :class:`core.Tensor`, it shards that tensor according to the provided
    ``sharding_spec``. ``src_rank`` denotes the source rank which would be
    used as the ground truth of the data which would be scattered as shards
    across the rest of the ranks.

    Args:
        tensor (:class:`core.Tensor`): Tensor needs to be sharded.
    ...
    .. warning::
        Only :class:`core.distributed._shard.sharding_spec.ChunkShardingSpec` is
...
def shard_parameter(
    ...
    """
    Given a :class:`core.nn.Module`, a ``param_name`` for a parameter in that
    ...
    :class:`core.distributed._sharded_tensor.ShardedTensor`

    Args:
        module (:class:`core.nn.Module`): Module whose parameter needs to be sharded.
...
def _reshard_output(
    module: core.nn.Module, resharding_spec: ShardingSpec
) -> core.nn.Module:
    """
    ...
    Args:
        module (:class:`core.nn.Module`): Module whose output needs to be resharded.
        resharding_spec (:class:`core.distributed._shard.sharding_spec.ShardingSpec`):
            The specification describing how the output of the module will be resharded.

    Returns:
        A :class:`core.nn.Module` object with reshard API hooked.
    """
...
def _collect_local_shard(module: core.nn.Module) -> core.nn.Module:
    """
    ...
    Args:
        module (:class:`core.nn.Module`): Module whose output is ShardedTensor and the
            local tensor value needs to be returned.

    Returns:
        A :class:`core.nn.Module` object with collection API hooked.
    """
...
def shard_module(module: nn.Module, plan: ShardingPlan, src_rank=0, process_group=None):
    ...
    Args:
        module (:class:`core.nn.Module`): The module to apply sharding to
        plan (:class:`core.distributed._shard.sharding_plan.ShardingPlan`):
