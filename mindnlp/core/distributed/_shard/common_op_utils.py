...
def _basic_validation(op, args=(), kwargs=None):
    ...
    cur_pg: Optional[core.distributed.ProcessGroup] = None
...
def _register_default_op(op, decorator):
    ...
    def tensor_default_op(types, args=(), kwargs=None, pg=None):
        ...
        behave the same as ``core.Tensor`` such as ``core.Tensor.shape`` or
        ``core.Tensor.dtype``. We simply lower to the real op call with
        DisableTorchFunctionSubclass context like ``core.Tensor.__torch_function__``
        to avoid recursions.
        """
        if kwargs is None:
            kwargs = {}
