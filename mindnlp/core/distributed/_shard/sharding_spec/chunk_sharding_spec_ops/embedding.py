...
@custom_sharding_spec_op(ChunkShardingSpec, core.nn.functional.embedding)
def sharded_embedding(types, args, kwargs, pg):
    """
    Handles ``__torch_function__`` dispatch for ``core.nn.functional.embedding``.
...
def _handle_row_wise_sharding(
    ...
    local_input_embeddings = core.nn.functional.embedding(
        lookup_input,
