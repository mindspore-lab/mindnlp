...
from core.distributed._shard.sharding_spec import ChunkShardingSpec
from core.distributed._shard.sharding_spec.api import custom_sharding_spec_op
from core.distributed.nn.functional import all_gather, reduce_scatter
...
# @custom_sharding_spec_op(ChunkShardingSpec, core.nn.functional.embedding_bag)
def sharded_embedding_bag(types, args, kwargs, pg):
    """
    Handles ``__torch_function__`` dispatch for ``core.nn.functional.embedding_bag``.
...
def _validate_embedding_bag_param(args, kwargs):
    ...
    if per_sample_weights is not None and not isinstance(
        per_sample_weights, core.Tensor
    ):
    ...
    if int(core.min(input).item()) < 0:
        raise ValueError(
            "Index out of range in Input %d %d",
            int(core.min(input).item()),
            weight_size[1],
        )
    if int(core.max(input).item()) >= weight_size[0]:
        raise ValueError(
            "Index out of range in Input %d %d",
            int(core.max(input).item()),
            weight_size[1],
        )
...
def _handle_col_wise_sharding(
    ...
    output = _handle_col_wise_sharding_base(
        core.nn.functional.embedding_bag,
        1,
        input,
        world_size,
        weight,
...
def _handle_row_wise_sharding(
    ...
    if input.dim() > 1 and per_sample_weights is None:
        ...
        gather_inp = core.cat(gathered_inputs, dim=cat_dim)
        if per_sample_weights is not None:
            per_sample_weights = core.cat(gathered_per_sample_weights, dim=cat_dim)
        offset_add = 0 if input.dim() > 1 else input.size(0)
        if offsets is not None:
            offsets_list = core.cat(
    ...
    if max_norm is not None:
        core.nn.functional.embedding_bag(
            core.unique(lookup_input)[:-1],
            local_shard,
            offsets=core.tensor([0], device=local_shard.device, dtype=core.long),
            mode=mode,
        ...
        max_norm = None
    result = core.nn.functional.embedding_bag(
        lookup_input,
        core.cat([local_shard, padding_row]),
    ...
    result = reduce_scatter(
        core.empty_like(local_shards[0]),
        list(local_shards),
        op=op,
        group=pg,
    )
    ...
    if mode == "mean":
        if input.dim() > 1:
            ...
            split_sizes = core.sum(
                core.ne(input, padding_idx), dim=-1, dtype=local_shard.dtype
            )
        else:
            split_sizes = core.cat(
                (
        ...
        return core.div(result, split_sizes.unsqueeze(1))
...
def _all_gather_embedding_bag_input(input, per_sample_weights, offsets, pg):
    ...
    gathered_inputs = all_gather(core.stack(input_to_gather), group=pg)
