...
from mindnlp import core.distributed._shard.sharded_tensor.metadata as sharded_tensor_meta
from mindnlp import core.distributed.distributed_c10d as distributed_c10d
from core.distributed._shard._utils import narrow_tensor
from core.distributed._shard.metadata import ShardMetadata
from core.distributed._shard.sharded_tensor.shard import Shard
from core.distributed._shard.sharded_tensor.utils import (
...
class ChunkShardingSpec(ShardingSpec):
    ...
    along a single dimension into equal chunks (similar to :meth:`core.chunk`).

    The semantics of how a tensor is partitioned is inline with
    :meth:`core.chunk`, where ``dim`` in core.chunk corresponds to the
    specified ``dim`` and ``chunks`` in core.chunk is the number of elements
    ...
    Args:
        ...
        placement(List[Union[_remote_device, str]]):
            ...
            be a list of
            :class:`core.distributed._remote_device`'s. This list
            could also contain a string which represents remote
            device as accepted by
            :class:`core.distributed._remote_device`
    """

    ShardingDim = Union[int, str]

    dim: ShardingDim
    placements: List[Union[core.distributed._remote_device, str]]

    def __post_init__(self):
        self._verify_dim(self.dim)
        for i, remote_device in enumerate(self.placements):
            if not isinstance(remote_device, core.distributed._remote_device):
                self.placements[i] = core.distributed._remote_device(remote_device)
    ...
    def shard(
        self, tensor: core.Tensor, src_rank: int = 0, process_group=None
    ) -> "ShardedTensor":
        """
        Args:
        ...
        tensors_to_scatter_: Optional[List[core.Tensor]] = None
        if current_rank == src_rank:
            tensors_to_scatter_ = []
