...
from mindnlp import core.distributed._functional_collectives as funcol
from mindnlp import core.distributed.distributed_c10d as c10d
from core._C._distributed_c10d import _SymmetricMemory, Work as _Work
...
def enable_symm_mem_for_group(group_name: str) -> None:
    ...
    store = core._C._distributed_c10d.FileStore(f"/tmp/{uid}", group.size())
...
_group_name_to_workspace_tensor: Dict[str, Optional[core.Tensor]] = {}
...
def get_symm_mem_workspace(group_name: str, min_size: int) -> _SymmetricMemory:
    ...
    if tensor is None or size < min_size:
        if core.cuda.is_current_stream_capturing():
        ...
        tensor = _SymmetricMemory.empty_strided_p2p(
            ...
            [1],
            core.uint8,
            core.device(f"cuda:{core.cuda.current_device()}"),
            group_name,
        )
...
_backend_streams: Dict[int, core.cuda.Stream] = {}


def _get_backend_stream(priority: int = 0) -> core.cuda.Stream:
    if priority not in _backend_streams:
        _backend_streams[priority] = core.cuda.Stream(priority=priority)
    return _backend_streams[priority]


def _pipelined_multi_all_gather_and_consume(
    shard: List[core.Tensor],
    shard_consumer: Callable[[List[core.Tensor], int], None],
    ag_out: List[core.Tensor],
    group_name: str,
) -> None:
    """
    ...
    backend_stream.wait_stream(core.cuda.current_stream())
    ...
    def copy_shard(dst: List[core.Tensor], src: List[core.Tensor]) -> None:
        for d, s in zip(dst, src):
            d.copy_(s)

    def get_p2p_bufs(remote_rank: int) -> List[core.Tensor]:
        offset_bytes = 0
        bufs = []
        for x in shard:
    ...
    shards: List[List[core.Tensor]] = [[] for _ in range(group_size)]
    for x in ag_out:
        for i, y in enumerate(x.chunk(group_size)):
            shards[i].append(y)
    ...
    core.cuda.current_stream().wait_stream(backend_stream)
    symm_mem.barrier(channel=0)


def _pipelined_all_gather_and_consume(
    shard: core.Tensor,
    shard_consumer: Callable[[core.Tensor, int], None],
    ag_out: core.Tensor,
    group_name: str,
) -> None:
    """
    ...
    def adapter(shard: List[core.Tensor], rank: int) -> None:
...
def _pipelined_produce_and_all2all(
    chunk_producer: Callable[[int, core.Tensor], None],
    output: core.Tensor,
    group_name: str,
) -> None:
    """
    ...
    communication:
        ...
        dist.all_to_all_single(output=output, input=core.cat(chunks))
    """
...
lib = core.library.Library("symm_mem", "DEF")  # noqa: TOR901
lib.define(
...
def _check_and_verify_fp8_all_gather_scale_mode(
    shard: core.Tensor, scale: Optional[core.Tensor], gather_dim: int, group_size: int
) -> _ScaleMode:
...
def _fused_all_gather_matmul_impl(
    mm_out_op: core._ops.OpOverload,
    A_shard: core.Tensor,
    Bs: List[core.Tensor],
    A_scale: Optional[core.Tensor],
    kwargs_list: List[Dict[str, Any]],
    out_dtypes: List[Optional[core.dtype]],
    gather_dim: int,
    group_name: str,
) -> Tuple[core.Tensor, List[core.Tensor]]:
    if A_shard.dim() < 2:
    ...
    def unflatten(t: core.Tensor) -> core.Tensor:
    ...
    if scale_mode == _ScaleMode.ROW_WISE_SHARDED:
        ...
        def row_wise_sharded_consumer(shard: List[core.Tensor], rank: int) -> None:
        ...
        def row_wise_replicated_consumer(shard: core.Tensor, rank: int) -> None:
            for idx, (B, kwargs) in enumerate(zip(Bs, kwargs_list)):
                mm_out_op(
                    shard,
                    B,
        ...
        def default_consumer(shard: core.Tensor, rank: int) -> None:
            for idx, (B, kwargs) in enumerate(zip(Bs, kwargs_list)):
...
@core.library.impl(lib, "fused_all_gather_matmul", "Meta")
def _fused_all_gather_matmul_fallback(
    ...
    A = core.ops._c10d_functional.all_gather_into_tensor(
        A_shard.contiguous(), group_size, group_name
    )
    A = core.ops._c10d_functional.wait_tensor(A)
    A = A.view(group_size, *A_shard.shape).movedim(gather_dim + 1, 1).flatten(0, 1)
    return A.movedim(0, gather_dim), [
        core.matmul(A, B).movedim(0, gather_dim) for B in Bs
    ]


@core.library.impl(lib, "fused_all_gather_matmul", "CUDA")
def _fused_all_gather_matmul(
    ...
    with core.profiler.record_function("fused_all_gather_matmul"):
        return _fused_all_gather_matmul_impl(
            core.ops.aten.mm.out,
            A_shard,
            Bs,
            None,
            [{} for B in Bs],
...
def _fused_all_gather_matmul_native(
    ...
    current_stream = core.cuda.current_stream()
...
@core.library.impl(lib, "fused_all_gather_scaled_matmul", "Meta")
def _fused_all_gather_scaled_matmul_fallback(
    A_shard: core.Tensor,
    Bs: List[core.Tensor],
    A_scale: core.Tensor,
    B_scales: List[core.Tensor],
    gather_dim: int,
    group_name: str,
    biases: List[Optional[core.Tensor]],
    ...
    if scale_mode == _ScaleMode.ROW_WISE_SHARDED:
        ...
        A_scale = core.ops._c10d_functional.all_gather_into_tensor(
            A_scale.contiguous(), group_size, group_name
        )
        A_scale = core.ops._c10d_functional.wait_tensor(A_scale)
        A_scale = (
    ...
    def scaled_matmul(
        ...
        out_dtype: Optional[core.dtype],
        use_fast_accum: bool,
    ) -> core.Tensor:
        leading_dims = A.shape[:-1]
        res = core.ops.aten._scaled_mm(
            A.flatten(0, -2),
            B,
            A_scale,
            B_scale,
...
@core.library.impl(lib, "fused_all_gather_scaled_matmul", "CUDA")
def _fused_all_gather_scaled_matmul(
    ...
    communication:
        ...
        leading_dims = A.shape[:-1]
        res = core.ops.aten._scaled_mm(A.flatten(0, -2), B, A_scale, B_scale)
    ...
    with core.profiler.record_function("fused_all_gather_scaled_matmul"):
        return _fused_all_gather_matmul_impl(
            core.ops.aten._scaled_mm.out,
            A_shard,
            Bs,
            A_scale,
            [
...
def _fused_matmul_reduce_scatter_impl(
    ...
    if B.dim() != 2:
        raise ValueError("B must be a matrix")
    if reduce_op == "sum":
        reduce_fn = partial(core.sum, dim=0)
    elif reduce_op == "avg":
        reduce_fn = partial(core.mean, dim=0)
    else:
    ...
    # Computing block-wise matmul along the first dim of A
    def chunk_producer(rank: int, out: core.Tensor) -> None:
...
@core.library.impl(lib, "fused_matmul_reduce_scatter", "Meta")
...
@core.library.impl(lib, "fused_matmul_reduce_scatter", "CUDA")
def _fused_matmul_reduce_scatter(
    ...
    with core.profiler.record_function("fused_matmul_reduce_scatter"):
        return _fused_matmul_reduce_scatter_impl(
            mm_out_op=core.ops.aten.mm.out,
            A=A,
            B=B,
            A_scale=None,
            kwargs={},
...
@core.library.impl(lib, "fused_scaled_matmul_reduce_scatter", "Meta")
def _fused_scaled_matmul_reduce_scatter_fallback(
    ...
    scatter_dim: int,
    group_name: str,
    bias: Optional[core.Tensor] = None,
    result_scale: Optional[core.Tensor] = None,
    out_dtype: Optional[core.dtype] = None,
...
@core.library.impl(lib, "fused_scaled_matmul_reduce_scatter", "CUDA")
def _fused_scaled_matmul_reduce_scatter(
    ...
    with core.profiler.record_function("fused_matmul_reduce_scatter"):
        return _fused_matmul_reduce_scatter_impl(
            mm_out_op=core.ops.aten._scaled_mm.out,
            A=A,
            B=B,
            A_scale=A_scale,
            kwargs={
...
def _maybe_convert_scalar_types_to_dtypes(
...
) -> List[Optional[core.dtype]]:
    """
    When a list of `core.dtype`s is passed through the dispatcher as
    `ScalarType[]`, it is converted to a list of scalar type enum values. This
    function converts it back to a list of `core.dtype`s.
    """
    ...
    _SCALAR_TYPE_TO_DTYPE = {
        ...
        15: core.bfloat16,
        16: core.float8_e5m2,
        17: core.float8_e4m3fn,
        18: core.float8_e5m2fnuz,
        19: core.float8_e4m3fnuz,
    }
    if any(not isinstance(x, (type(None), int)) for x in scalar_types):
        return scalar_types

    dtypes: List[Optional[core.dtype]] = []
...
@core.library.impl(lib, "_low_contention_all_gather", "Meta")
...
@core.library.impl(lib, "_low_contention_all_gather", "CUDA")
def _low_contention_all_gather(
    ...
    _get_backend_stream().wait_stream(core.cuda.current_stream())
    with core.cuda.stream(_get_backend_stream()):
        ...
        symm_mem.barrier()
        core._C._distributed_c10d._register_work(output, Work())
        return output


@core.library.impl(lib, "_low_contention_reduce_scatter", "Meta")
...
def _low_contention_reduce_scatter_with_symm_mem_input(
    ...
    a2a_res = core.empty_like(tensor)
    ...
    with core.cuda.stream(_get_backend_stream()):
        ...
        core._C._distributed_c10d._register_work(ret, Work())
        return ret
...
@core.library.impl(lib, "_low_contention_reduce_scatter", "CUDA")
...
from core.types import _device, _dtype, _int
...
def empty(  # type: ignore[misc]
    *size: Any,
    ...
    Similar to :func:`core.empty()`. The returned tensor can be used by
    :func:`core._distributed._symmetric_memory.rendezvous()` to establish a
    ...
    Keyword args:
        dtype (:class:`core.dtype`, optional): the desired data type of returned tensor.
            Default: if ``None``, uses a global default (see :func:`core.set_default_dtype`).
        device (:class:`core.device`, optional): the desired device of returned tensor.
            Default: if ``None``, uses the current device for the default tensor type
            (see :func:`core.set_default_device`). :attr:`device` will be the CPU
    ...
    if device is None:
        device = core.get_default_device()

    return _SymmetricMemory.empty_strided_p2p(
        size=size,
        stride=core._prims_common.make_contiguous_strides_for(size),
        dtype=dtype,
...
def rendezvous(
    tensor: core.Tensor, group: Union[str, "ProcessGroup"]
) -> _SymmetricMemory:
    r"""
    ...
    Args:
        tensor (:class:`core.Tensor`): the local tensor used to establish the symmetric memory tensor.
            It must be allocated via :func:`core._distributed._symmetric_memory.empty()`. The shape,
            dtype, and device type must be identical across all participating processes.
        group (Union[str, :class:`core.distributed.ProcessGroup`]): The group identifying the
