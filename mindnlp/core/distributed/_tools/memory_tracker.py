...
class MemoryProfileDispatchMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=..., kwargs=None):
        rs = func(*args, **kwargs)
        if func == core.ops.aten.detach.default:
            return rs
        func_name: str = (
...
class MemoryTracker:
    ...
    def __init__(self) -> None:
        core._C._log_api_usage_once("core.distributed.memory_tracker")
    ...
    def stop(self) -> None:
        ...
        """
        self._num_cuda_retries = core.cuda.memory_stats().get("num_alloc_retries", 0)
    ...
    def _create_backward_hook(self, name: str) -> Callable:
        ...
        def _backward_hook(
            module: nn.Module, grad_input: core.Tensor, grad_output: core.Tensor
        ) -> None:
    ...
    def _record_memory_stats(self, fn_name: str) -> None:
        ...
        """
        memory_allocated: float = core.cuda.memory_allocated() / BYTES_PER_MB
        memory_reserved: float = core.cuda.memory_reserved() / BYTES_PER_MB
        memory_active: float = (
            core.cuda.memory_stats().get("active_bytes.all.current", 0) / BYTES_PER_MB
        )
