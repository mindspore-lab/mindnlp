...
from mindnlp.core import nan, nn, UntypedStorage
from core._guards import active_fake_mode
from core._subclasses.fake_tensor import FakeTensorMode
from core.distributed._tools.mod_tracker import ModTracker
from core.distributed._tools.runtime_estimator import RuntimeEstimator
from core.testing._internal.composite_compliance import (
    is_inplace,
    is_inplace_view_fn,
    is_view_fn,
)
...
from core.utils._pytree import tree_flatten
from core.utils.checkpoint import SAC_IGNORED_OPS
...
_ADDITIONAL_IGNORED_OPS = {
    ...
    core.ops.profiler._record_function_exit._RecordFunction,  # type: ignore[attr-defined]
    aten.clone.default,  # type: ignore[attr-defined] # seems needed for core.compile
}
...
def _get_untyped_storages(t: core.Tensor) -> Set[core.UntypedStorage]:
    """
    Retrieves untyped storages from a `core.Tensor` or one of its traceable wrapper-subclass.

    Args:
       t (core.Tensor): Input `core.Tensor` or traceable wrapper-subclass of `core.Tensor`.

    Returns:
        Set[core.UntypedStorage]: Set of untyped storages.
...
class SACEstimator(TorchDispatchMode):
    ...
    runtime trade-offs of functions or ``core.nn.Module``s for Selective Activation Checkpointing (SAC). It provides
    ...
    def _pack_hook(self, x: core.Tensor) -> core.Tensor:
        ...
        # Used in conjunction with core.autograd.graph.saved_tensors_hooks
    ...
    def _get_force_store_random(self, inputs: Any) -> bool:
        ...
        return all(not isinstance(x, core.Tensor) for x in flat_inputs)
    ...
    def __torch_dispatch__(  # type: ignore[no-untyped-def]
        ...
        cuda_devices: Set[core.device] = set()
        for o in flat_outs:
            if isinstance(o, core.Tensor):
        ...
        is_rand_op = core.Tag.nondeterministic_seeded in func.tags
        if is_view_like:
            nbytes = 0
