...
from mindnlp import core.utils._pytree as pytree
from core._guards import active_fake_mode
from core._inductor.utils import get_device_tflops, get_gpu_dram_gbps
from core._subclasses.fake_tensor import FakeTensorMode
from core.distributed._tools.mod_tracker import ModTracker
from core.utils._mode_utils import no_dispatch
from core.utils._python_dispatch import TorchDispatchMode
from core.utils.flop_counter import flop_registry
...
class RuntimeEstimator(TorchDispatchMode):
    ...
    _float_types: Set[core.dtype] = {
        core.float16,
        core.bfloat16,
        core.float32,
        core.float64,
    }
    _no_fallback_kernel: Set[core._ops._OpNamespace] = set()
    ...
    def _maybe_run_and_benchmark_fallback_kernel(  # type: ignore[no-untyped-def]
        ...
        if core.Tag.inplace_view in func.tags:  # type: ignore[attr-defined]
        ...
        with no_dispatch():
            ...
            def to_real_tensor(e):  # type: ignore[no-untyped-def]
                if cls.fake_mode.is_our_fake(e):
                    if e.dtype in cls._float_types:
                        out = core.rand_like(e, device=e.fake_device)
                    else:
                        out = core.ones_like(e, device=e.fake_device)
                    if e.is_sparse:
            ...
            for _ in range(warmup_iters):
                func(*args, **kwargs)
            start_event = core.cuda.Event(enable_timing=True)
            end_event = core.cuda.Event(enable_timing=True)
            start_event.record(core.cuda.current_stream())
            for _ in range(actual_iters):
                func(*args, **kwargs)
            end_event.record(core.cuda.current_stream())
    ...
    def _roofline_estimate(cls, func, args, kwargs) -> Tuple[Any, float]:  # type: ignore[no-untyped-def]
        ...
        def get_num_bytes(t: core.Tensor) -> int:
            """
            Calculates the memory consumption of a tensor.

            Args:
                t (core.Tensor): The input tensor.
        ...
        def get_transfer_time(flat_args_kwargs, flat_outs) -> float:  # type: ignore[no-untyped-def]
            ...
            Args:
                flat_args_kwargs (List[core.Tensor]): The flat list of arguments and keyword arguments.
                flat_outs (List[core.Tensor]): The flat list of outputs.
            ...
            write_bytes = sum(
                get_num_bytes(t) for t in flat_outs if isinstance(t, core.Tensor)
            )
        ...
        if func_packet not in _IGNORE_OPS:
            ...
            out_dtypes = {
                t.dtype
                for t in flat_outs
                if isinstance(t, core.Tensor) and t.dtype in cls._float_types
            }
