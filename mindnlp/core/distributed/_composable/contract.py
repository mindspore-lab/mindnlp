...
def contract(state_cls: Type[_State] = _State):
    ...
    Example::
        ...
        >>> my_feature(model.l2)
        >>> model(core.randn(2, 10)).sum().backward()
    """

    # wraps will make functions decorated with contract() pickleable - needed for integration with core.package
    @wraps(state_cls)
    def inner(func):
        @wraps(func)
        def wrapper(
            ...
            all_orig_named_buffers: List[Dict[str, core.Tensor]] = []
            ...
            all_new_named_buffers: List[Dict[str, core.Tensor]] = []
