...
from core.distributed.fsdp._common_utils import _named_parameters_with_duplicates
from core.distributed.tensor import Shard
from core.profiler import record_function
from core.utils._pytree import tree_flatten, tree_unflatten
...
logger = logging.getLogger("core.distributed._composable.fsdp")
...
class FSDPCommContext:
    ...
    def lazy_init(self, device: core.device):
...
class AllGatherState(NamedTuple):
    ...
    event: core.Event  # all-gather copy-out


class ReduceScatterState(NamedTuple):
    reduce_scatter_input: core.Tensor
    event: core.Event  # reduce-scatter event


class AllReduceState(NamedTuple):
    all_reduce_input: core.Tensor
    event: core.Event  # all-reduce event


class FSDPParamGroup:
    """This class represents a parameter group to communicate together."""

    _orig_dtype: core.dtype
    _reduce_dtype: Optional[core.dtype]
    ...
    def __init__(
        ...
        self._post_reduce_event: Optional[core.Event] = None
        # Holds the reshard-after-forward CUDA event when resharding to a
        # different world size, which should be waited on in the next unshard
        self._reshard_after_forward_event: Optional[core.Event] = None

        # Only for HSDP, if accumulating gradients without all-reduce, save the
        # partial reduce output (only reduce-scattered but not all-reduced)
        self._partial_reduce_output: Optional[core.Tensor] = None
    ...
    def _wait_all_gather_streams_on_event(self, event: core.Event):
    ...
    def finalize_backward(self):
        ...
        if self._all_gather_result is not None:
            ...
            if (event := self._all_gather_result.all_gather_event) is not None:
                core.cuda.current_stream().wait_event(event)
    ...
    def _register_post_backward_hook(
        ...
        if (not core._dynamo.config.skip_fsdp_hooks) or compiled_autograd_enabled():
            return args, kwargs
        ...
        inp_tensors: List[core.Tensor] = []
        for i, obj in enumerate(args_kwargs_list):
            if core.is_tensor(obj) and obj.requires_grad:
...
class RegisterPostBackwardFunction(core.autograd.Function):
    @staticmethod
...
compile the forward part if you want to use Traceable FSDP2."""
            core._dynamo.comptime.comptime.print(msg)
            raise RuntimeError(msg)

    @staticmethod
    def forward(ctx, param_group: FSDPParamGroup, *inputs: core.Tensor):
    ...
    @staticmethod
    def backward(ctx, *grads: core.Tensor):
        RegisterPostBackwardFunction._assert_not_tracing_fsdp()
