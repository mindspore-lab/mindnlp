...
from core._prims_common import make_contiguous_strides_for
from core.distributed._functional_collectives import AsyncCollectiveTensor
from core.distributed.tensor import DTensor, Replicate, Shard
from core.distributed.tensor._dtensor_spec import DTensorSpec, TensorMeta
from core.distributed.tensor.device_mesh import _mesh_resources
from core.distributed.tensor.placement_types import _StridedShard, Placement
...
- All-gather inputs: the ``core.Tensor`` or ``Tensor`` s passed to all-gather,
  derived from the sharded parameter
- All-gather output: the ``core.Tensor`` or ``Tensor`` s resulting from
...
For the default ``core.Tensor`` case, there is only one all-gather input, and
...
all-gather output and unsharded parameter. For non-``core.Tensor`` extensions,
...
it in-place thereafter. For the default ``core.Tensor` original parameter
case, the all-gather output and unsharded parameter share the same
data, so we use storage resizing on the all-gather output.
"""

lib = core.library.Library("fsdp", "FRAGMENT")  # noqa: TOR901

lib.define("copy_(Tensor(a!) tensor, Tensor data) -> ()")


@core.library.impl(lib, "copy_", "Meta")
@core.library.impl(lib, "copy_", "CUDA")
@core.library.impl(lib, "copy_", "CPU")
...
@core.library.impl(lib, "copy_", "Functionalize")
def copy__functionalize(tensor, data):
    core._sync(tensor)
    core._sync(data)
    tensor_inner = core._from_functional_tensor(tensor)
    data_inner = core._from_functional_tensor(data)
    with core._C._ExcludeDispatchKeyGuard(
        core._C.DispatchKeySet(core._C.DispatchKey.Functionalize)
    ):
        core.ops.fsdp.copy_.default(tensor_inner, data_inner)


core.fx.node.has_side_effect(core.ops.fsdp.copy_.default)
...
class ExtensionsData:
    ...
    all_gather_input_sizes: Sequence[core.Size] = ()  # ND
...
class FSDPParam:
    ...
    param_dtype: Optional[core.dtype]
    reduce_dtype: Optional[core.dtype]
    _orig_size: core.Size  # ND
    sharded_size: core.Size  # ND
    contiguous_sharded_stride: Tuple[int, ...]
    padded_sharded_param_size: core.Size  # ND
    sharded_post_forward_size: core.Size  # ND
    contiguous_sharded_post_forward_stride: Tuple[int, ...]
    _sharded_param_data: core.Tensor  # 1D
    sharded_param: nn.Parameter  # ND
    _sharded_post_forward_param_data: Optional[core.Tensor]  # 1D
    _sharded_post_forward_param: Optional[nn.Parameter]  # ND
    _unsharded_param: nn.Parameter  # ND
    unsharded_accumulated_grad: Optional[core.Tensor]  # ND
    _sharding_spec: DTensorSpec
    # DTensor attributes (only defined for DTensor `param`):
    _tp_spec: DTensorSpec
    all_gather_outputs: List[core.Tensor]  # 1D
    # All-gather extension attributes
    _extensions_data: ExtensionsData
    _unsharded_inner_tensors: List[core.Tensor]
    ...
    def __init__(
        ...
        self.grad_offload_event: Optional[core.Event] = None
        self._init_sharded_param(param, device, shard_placement_fn)
        if self.post_forward_mesh_info:
            self._init_sharded_post_forward_param_metadata(param)
        self._init_extensions()
        self.all_gather_outputs: List[core.Tensor] = []
    ...
    def _init_sharded_post_forward_param_metadata(self, param: core.Tensor) -> None:
    ...
    def _init_extensions(self) -> None:
        ...
        self._unsharded_inner_tensors: List[core.Tensor] = []

    def init_all_gather_outputs(
        self,
        all_gather_input_numels: List[int],
        all_gather_input_dtypes: List[core.dtype],
        world_size: int,
        device: core.device,
        ...
        self.all_gather_outputs = [
            core.empty(core.Size([numel * world_size]), dtype=dtype, device=device)
            for numel, dtype in zip(all_gather_input_numels, all_gather_input_dtypes)
        ]

    def init_unsharded_param(self):
        """
        [Note: Invariants for core.compile Traceable FSDP2]
        ...
        unsharded_param = core.as_strided(
            unsharded_tensor,
            self._orig_size,
        ...
        if hasattr(self, "_unsharded_param"):
            ...
            with core.no_grad(), core.autograd._unsafe_preserve_version_counter(
                self._unsharded_param
            ):
                ...
                core.ops.fsdp.copy_(self._unsharded_param, unsharded_param)
        else:
            self._unsharded_param = nn.Parameter(
                unsharded_param, requires_grad=self.sharded_param.requires_grad
            )

    def _unflatten_all_gather_outputs(self) -> Tuple[core.Tensor, ...]:
        return tuple(
            t.view(-1, *s[1:])
            for t, s in zip(
    ...
    def to_sharded_post_forward(self) -> None:
        ...
        sharded_post_forward_tensor = core.as_strided(
            self._sharded_post_forward_param_data,
    ...
    def to_sharded_dtensor(self, tensor: core.Tensor) -> DTensor:
        """
    ...
    def to_sharded_post_forward_dtensor(self, tensor: core.Tensor) -> DTensor:
    ...
    @property
    def all_gather_inputs(self) -> List[core.Tensor]:  # 1D
        ...
        if self.sharded_state == ShardedState.SHARDED:
            ...
            all_gather_input = _to_dtype_if_needed(
                cast(core.Tensor, self._sharded_post_forward_param_data),
                self.param_dtype,
            )
            return [all_gather_input]
        return [core.empty(0)]  # mypy
    ...
    @property
    def _sharded_local_tensor(self) -> core.Tensor:
    ...
    def reset_sharded_param(self):
        ...
        if new_param is not self.sharded_param:
            if core.__future__.get_swap_module_params_on_conversion():
                raise AssertionError(
...
def alloc_storage(tensor: core.Tensor) -> None:
    size = tensor.numel() * tensor.itemsize
    if (storage := tensor.untyped_storage()).size() != size:
        storage.resize_(size)


def free_storage(tensor: core.Tensor) -> None:
...
def set_requires_grad_if_needed(
    src_tensor: core.Tensor, dst_tensor: core.Tensor
) -> None:
