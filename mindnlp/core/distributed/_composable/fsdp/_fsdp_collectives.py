...
from core.distributed.device_mesh import _get_device_handle
from core.distributed.distributed_c10d import ReduceOp
from core.distributed.tensor import DTensor
...
class AllGatherResult(NamedTuple):
    all_gather_output: core.Tensor
    all_gather_event: Optional[core.Event]
    all_gather_work: Optional[dist.distributed_c10d.Work]
    # For each parameter, the all-gather input dtype for each input
    param_all_gather_input_dtypes: List[List[core.dtype]]
...
@core.library.impl(lib, "all_gather_copy_in", "Meta")
def all_gather_copy_in_meta(
    all_gather_inputs: List[core.Tensor],
...
@core.library.impl(lib, "all_gather_copy_in", "CUDA")
@core.library.impl(lib, "all_gather_copy_in", "CPU")
def all_gather_copy_in_cuda(
    ...
    foreach_copy_dsts = core.split(all_gather_input, inp_split_sizes)
    with core.no_grad():
        core._foreach_copy_(foreach_copy_dsts, all_gather_inputs)
    return all_gather_input, all_gather_output
...
@core.library.impl(lib, "split_with_sizes_copy", "Meta")
@core.library.impl(lib, "split_with_sizes_copy", "CUDA")
@core.library.impl(lib, "split_with_sizes_copy", "CPU")
def split_with_sizes_copy(
    all_gather_output: core.Tensor,
    all_gather_input_split_sizes: List[int],
    dim: int,
    out: List[core.Tensor],
) -> None:
    core.split_with_sizes_copy(
...
@core.library.impl(lib, "chunk_cat", "Meta")
@core.library.impl(lib, "chunk_cat", "CUDA")
@core.library.impl(lib, "chunk_cat", "CPU")
def chunk_cat(
    ...
    core._chunk_cat(tensors, dim, num_chunks, out=out)
...
def foreach_all_gather(
    ...
    async_op: bool,
    all_gather_copy_in_stream: core.Stream,
    all_gather_stream: core.Stream,
    device: core.device,
    ...
    with device_handle.stream(all_gather_copy_in_stream):
        ...
        if dtype == core.uint8:
            all_gather_inputs = [
                t.view(core.uint8) for ts in param_all_gather_inputs for t in ts
            ]
        else:
            all_gather_inputs = [t for ts in param_all_gather_inputs for t in ts]
        inp_split_sizes = [t.numel() for t in all_gather_inputs]
        all_gather_input_numel = sum(inp_split_sizes)
        all_gather_input, all_gather_output = core.ops.fsdp.all_gather_copy_in(
            all_gather_inputs,
            inp_split_sizes,
...
def _get_param_all_gather_inputs(
    ...
    param_all_gather_inputs: List[List[core.Tensor]] = [[] for _ in fsdp_params]
    foreach_copy_indices: List[int] = []
    foreach_copy_inputs: List[core.Tensor] = []
    ...
    for i, fsdp_param in enumerate(fsdp_params):
        if use_foreach_copy(fsdp_param):
            ...
            all_gather_input = (
                ...
                else cast(core.Tensor, fsdp_param._sharded_post_forward_param_data)
            )
    ...
    if foreach_copy_inputs:
        ...
        flat_foreach_copy_input = core.empty(
            (sum(foreach_copy_input_numels),), device=device, dtype=param_dtype
        )
        splits = core.split(flat_foreach_copy_input, foreach_copy_input_numels)
        core._foreach_copy_(splits, foreach_copy_inputs)
...
def foreach_all_gather_copy_out(
    ...
    split_with_sizes_out: List[core.Tensor] = []
    ...
    for all_gather_input_numels, all_gather_input_dtypes, fsdp_param in zip(
        ...
        # per AllGather. See [Note: Invariants for core.compile Traceable FSDP2].
    ...
    if all_gather_output.dtype == core.uint8:
        out = [t.view(world_size, -1).view(core.uint8) for t in split_with_sizes_out]
    else:
        out = [t.view(world_size, -1) for t in split_with_sizes_out]
    core.ops.fsdp.split_with_sizes_copy(
        ...
        for param_all_gather_output, target_all_gather_output in zip(
            ...
            padded_sharded_size = (
                ...
                if fsdp_param.sharded_state == ShardedState.SHARDED
                    ...
                    core.Tensor, fsdp_param._sharded_post_forward_param_data
                ).size()
            )
            ...
            core.cat(chunks, dim=shard_dim, out=cat_out)
            core._C._autograd._unsafe_set_version_counter(
...
def foreach_reduce(
    fsdp_params: List[FSDPParam],
    unsharded_grads: List[core.Tensor],
    reduce_scatter_group: dist.ProcessGroup,
    reduce_scatter_stream: core.Stream,
    ...
    all_reduce_stream: core.Stream,
    all_reduce_grads: bool,
    partial_reduce_output: Optional[core.Tensor],  # only used for HSDP
) -> Tuple[
    core.Tensor,
    core.Event,
    core.Event,
    ...
    for i, (fsdp_param, unsharded_grad) in enumerate(zip(fsdp_params, unsharded_grads)):
        ...
        chunks = core.chunk(unsharded_grad, world_size, dim=shard_dim)
        unsharded_grads[i] = core.cat(chunks, dim=0)
    padded_unsharded_sizes = tuple(
        _get_dim0_padded_size(grad.size(), world_size) for grad in unsharded_grads
    )
    reduce_scatter_input_numel = sum(s.numel() for s in padded_unsharded_sizes)
    reduce_scatter_output_numel = reduce_scatter_input_numel // world_size
    reduce_scatter_input = core.empty(
    ...
    with device_handle.stream(post_reduce_stream):
        ...
        for padded_unsharded_size, fsdp_param in zip(
            ...
            new_sharded_grad = core.as_strided(
                reduce_output,
            ...
            if fsdp_param.offload_to_cpu:
                ...
                new_sharded_grad = new_sharded_grad.to(
                    core.device("cpu"), non_blocking=non_blocking
                )
                if non_blocking:
...
def _get_gradient_divide_factors(
    ...
    reduce_dtype: core.dtype,
) -> Union[Tuple[None, None], Tuple[float, float]]:
    # For fp32/bf16, we do not need to worry about overflow/underflow, so we
    # use NCCL's built-in division to avoid separate div kernels
    if reduce_dtype in (core.float32, core.bfloat16):
        return None, None
...
def _div_if_needed(tensor: core.Tensor, div_factor: Optional[float]) -> None:
