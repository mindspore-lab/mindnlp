...
from core.distributed._composable_state import _State
...
class _ReplicateState(_State):
    ...
    def lazy_init(self) -> None:
        @core._disable_dynamo(recursive=True)
        def _lazy_init():
    ...
    def init(
        ...
        **kwargs,
        ...
        from core.distributed.tensor.parallel.ddp import _localize_dtensor
        ...
        if "device_id" in kwargs:
            ...
            # user can pass in device_id as a Union[int, core.device] even for
            ...
            if kwargs["device_id"] is not None:
                ...
                if isinstance(device_id, core.device) and device_id.type == "cpu":
...
def replicate(
    ...
    **kwargs,
    ...
    Args:
        module (core.nn.Module): module to replicate

    Example::
        >>> # xdoctest: +REQUIRES(module:core._C._distributed_c10d)
        >>> module = nn.Linear(3, 3)
        >>> replicate(module)
    """
    core._C._log_api_usage_once("core.distributed.replicate")

    # TODO(fegin): using kwargs is not a good idea if we would like to make
    # replicate a formal API to replace DDP.
    if "device_id" in kwargs:
        if not isinstance(kwargs["device_id"], (int, core.device)):
            raise RuntimeError(
                "Expected device_id to be int or core.device, "
    ...
    if device_mesh is not None:
        from core.distributed.device_mesh import _mesh_resources
        ...
        if root_mesh != device_mesh:
            ...
            from core.distributed.tensor.parallel.ddp import (
                _localize_dtensor,
