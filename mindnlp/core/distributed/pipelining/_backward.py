...
from core.autograd.graph import GradientEdge, Node
...
def _get_grad_fn_or_grad_acc(t: core.Tensor) -> Union[Node, None]:
    """
...
def stage_backward_input(
...
) -> Tuple[Tuple[Optional[core.Tensor], ...], List[Dict[str, Any]]]:
    """
    ...
    if output_grads is None:
        ...
        output_grads = [
            core.ones_like(stage_output) for stage_output in stage_outputs_or_loss
        ]

    dinputs = core.autograd.grad(
...
) -> Tuple[Optional[core.Tensor], ...]:
    # map weights to param_group_weights
    grad_acc_to_weight = {}
    weight_grads: List[Optional[core.Tensor]] = []
...
def stage_backward(
    ...
    try:
        ...
        # tensor values here
        stage_output_tensors: List[core.Tensor] = []
        ...
        def extract_tensors_with_grads(
            ...
            if isinstance(output_val, core.Tensor):
                if not output_val.requires_grad and output_val.grad_fn is None:
                    return
                assert isinstance(
                    grad_val, (core.Tensor, type(None))
        ...
        # Alternative impl: `core.autograd.grad`.
        # Note that `core.autograd.grad` will not accumulate gradients into the
        # model's parameters.
        """
        inputs_with_grad = []
        for val in input_values:
            if isinstance(val, core.Tensor) and val.requires_grad:
                inputs_with_grad.append(val)

        grad_inputs = core.autograd.grad(
