...
sum_reducer = _LossReducer(core.tensor(0.0), lambda a, b: a + b)
...
def _shard_dict_of_args(
    ...
    for arg_key, arg in args_dict.items():
        ...
        for v, chunk_v in zip(flat, chunk_spec_flat):
            if chunk_v is _Replicate or not isinstance(v, core.Tensor):
                ...
                # Throw an error
                assert isinstance(v, core.Tensor), f"{v} is not a tensor"
                ...
                chunk_tensors = core.tensor_split(
    ...
    for arg_idx, arg in enumerate(spec_flattened):
        if isinstance(arg, TensorChunkSpec):
            ...
            if _debug_mask_minibatches:
                ...
                meta_chunks = core.tensor_split(
                    core.empty(*overall_shape, device="meta"),
            ...
            args_flattened.append(core.cat(values_to_cat, dim=arg.split_dim))
