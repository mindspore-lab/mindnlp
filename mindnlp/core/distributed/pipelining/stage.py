...
from mindnlp import core.fx as fx
from mindnlp import core.nn as nn
from core._subclasses.fake_tensor import FakeTensor
from core.distributed._composable.fsdp.fully_shard import FSDPModule, fully_shard
from core.fx.node import map_aggregate
from core.nn.parallel import DistributedDataParallel
...
def _normalize_model_output_as_tuple(output: Any) -> Tuple[Any]:
    ...
    core.export tracing, compiled models may also return a list instead of a Tuple, which we will normalize back to a
...
def _make_tensor_from_meta(
    example: Union[core.Tensor, FakeTensor],
    device: core.device,
) -> core.Tensor:
    """
...
class _PipelineStageBase(ABC):
    ...
    def __init__(
        ...
        """
        Args:
            submodule (core.nn.Module): The module to be executed in this stage.
            stage_index (int): The index of this stage.
            num_stages (int): The total number of stages in this pipeline.
            device (core.device): The device to run this stage on.
        ...
        # Run time states
        self._outputs_meta: Optional[Tuple[core.Tensor, ...]] = None
        # map microbatch ID to list of forward tensor args
        self.fwd_cache: Dict[int, Tuple[Any, List[core.Tensor]]] = {}
        # map microbatch ID to list of backward grad tensor args
        self.bwd_cache: Dict[int, Tuple[Optional[core.Tensor], ...]] = {}
    ...
    def _configure_outputs_meta(self, outputs_meta: Tuple[core.Tensor, ...]):
        """
    ...
    def get_outputs_meta(self) -> Tuple[core.Tensor, ...]:
    ...
    def set_local_bwd_input(
        self, next_stage_bwd_outputs: Tuple[Optional[core.Tensor], ...], mb_index: int
    ) -> None:
        """
    ...
    def get_bwd_send_ops(self, bwd_chunk_id: int) -> List[dist.P2POp]:
        ...
        for grad, grad_recv_stage in zip(grads_input, self.grad_send_info):
            if isinstance(grad, core.Tensor) and grad_recv_stage is not None:
                logger.debug(
    ...
    def clear_runtime_states(self) -> None:
        ...
        # `core.autograd.backward()` will accumulate gradients into leaf
    ...
    ) -> Tuple[Tuple[Optional[core.Tensor], ...], Optional[List[Dict[str, Any]]]]:
        """
        ...
        if isinstance(self.submod, DistributedDataParallel):
            if last_backward:
                ...
                self.submod.reducer.prepare_for_backward(  # type: ignore[union-attr, operator]
                    list(
                        core.nn.parallel.distributed._find_tensors(  # type: ignore[attr-defined]
                            bwd_kwargs["stage_output"]
                        )
                    )
                )
    ...
    def backward_one_chunk(
        ...
        if self.is_last:
            ...
            # `core.autograd.backward` will accumulate the gradient into the
        ...
        grads_input: Tuple[Optional[core.Tensor], ...] = ()
    ...
    def _validate_fwd_outputs(self, outputs: Tuple[core.Tensor, ...]):
...
class _PipelineStage(_PipelineStageBase):
    def __init__(
        ...
        Args:
            stage_module (core.nn.Module): the module to be wrapped by this stage
            ...
            device (core.device): the device to be used by this stage
    ...
    def _create_act_send_info(self):
        ...
        output_vals: Tuple[core.Tensor] = tuple(
...
class PipelineStage(_PipelineStageBase):
    ...
    Args:
        ...
        device (core.device): The device where this stage is located.
        input_args (Union[core.Tensor, Tuple[core.tensor]], optional): The input arguments for the submodule.
        output_args (Union[core.Tensor, Tuple[core.tensor]], optional): The output arguments for the submodule.
    ...
    def __init__(
        ...
        num_stages: int,
        device: core.device,
        input_args: Optional[Union[core.Tensor, Tuple[core.Tensor, ...]]] = None,
        output_args: Optional[Union[core.Tensor, Tuple[core.Tensor, ...]]] = None,
        group: Optional[dist.ProcessGroup] = None,
        dw_builder: Optional[Callable[[], Callable[..., None]]] = None,
    ):
        super().__init__(submodule, stage_index, num_stages, device, group, dw_builder)
        self.inputs: Optional[List[core.Tensor]] = None
        self.inputs_meta: Optional[Tuple[core.Tensor, ...]] = None
        ...
        if input_args is None:
            ...
            self.inputs_meta = (
                (input_args,) if isinstance(input_args, core.Tensor) else input_args
            )
            if output_args is None:
                ...
                try:
                    ...
                    output_args = tree_map_only(
                        core.Tensor, lambda x: x.to("meta"), output_args
                    )
                except Exception as e:
                    raise RuntimeError(
            ...
            self._configure_outputs_meta(
                (output_args,) if isinstance(output_args, core.Tensor) else output_args
            )

        # these are the buffers used in backwards send/recv, they are allocated later
        self.outputs_grad: List[core.Tensor] = []
    ...
    def _shape_inference(
        ...
        if (
            ...
            args = tree_map_only(core.Tensor, lambda x: x.to("meta"), args)
        else:
            assert (
                len(args) == 0
        ...
        args = tree_map_only(
            core.Tensor, lambda x: core.zeros_like(x, device=self.device), args
        )
        ...
        outputs_meta = tuple(
            tree_map_only(core.Tensor, lambda x: x.to("meta"), outputs)
        )
    ...
    def _init_p2p_neighbors(self):
        ...
        """
        ops = []
        recv_tensor = core.zeros(1, device="cuda")
        send_tensor = core.ones(1, device="cuda")
        # forward
        if not self.is_first:
