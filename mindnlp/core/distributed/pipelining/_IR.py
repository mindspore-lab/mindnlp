...
from core.distributed import ProcessGroup
from core.export import ExportedProgram
from core.export.unflatten import (
    _assign_attr,
    _AttrKind,
    _sink_params,
    InterpreterModule,
...
from core.fx.passes.split_module import split_module
...
def _find_loss_output(mod: core.nn.Module, g: fx.Graph, output_loss_value_spec):
...
class PipeSequential(core.nn.Sequential):
    @staticmethod
    def from_sequential(sequential_instance: core.nn.Sequential):
...
class LossWrapper(core.nn.Module):
    """
    ...
    be backpropagated by PiPPy. The above class would then be instantiated like::
        ...
        loss_fn = core.nn.MSELoss() # for the sake of demonstration
...
core.library.define("pippy::_pipe_split", "() -> ()")


@core.library.impl("pippy::_pipe_split", "BackendSelect")
def _pipe_split():
    return None


@core.library.register_fake("pippy::_pipe_split")  # type: ignore[no-redef]
def _pipe_split():  # noqa: F811
    return None


# Add an alias for convenience
aten_pipe_split_alias = core.ops.pippy._pipe_split.default
...
def pipe_split():
    ...
    """
    return core.ops.pippy._pipe_split()
...
class DetachExecutor(fx.Interpreter):
    ...
    def call_module(self, target, args, kwargs):
        def detach_tensors(a):
            if isinstance(a, core.Tensor) and a.requires_grad:
...
def _direct_serialization_deserialize(body, nodes):
    ...
    class DummyModule(core.nn.Module):
...
def _modify_graph_op_device(
    gm: core.fx.GraphModule,
    new_device: core.device,
):
    """
    ...
    for node in gm.graph.nodes:
        if node.op == "call_function":
            ...
            if isinstance(submod, core.fx.GraphModule):
...
class Pipe(core.nn.Module):
    def __init__(
        ...
        params_to_users: Dict[core.nn.Parameter, Dict[str, str]] = {}
    ...
    def get_stage_module(self, stage_idx: int) -> core.nn.Module:
        """
    ...
    def _from_traced(
        ...
        split_policy: Optional[
            Callable[[core.fx.GraphModule], core.fx.GraphModule]
        ] = None,
    ):
        """
        ...
        def move_param_to_callee(
            ...
            assert isinstance(param_val, core.Tensor), (
                f"Expected '{param_fqn}' to be {core.Tensor} but got {type(param_val)}."
                + (
                    f" It might happen if module '{param_fqn}' was passed to some 'leaf function'"
                    f"(see https://pycore.org/docs/stable/fx.html#fx.wrap). Please inspect "
                    f"usages of '{param_fqn}' in the traced graph."
                    if isinstance(param_val, core.nn.Module)
                    else ""
                )
            )
    ...
    def _trace_with_export(
        ...
        try:
            ep = core.export.export_for_training(
                mod,
                example_args,
                example_kwargs,
            )
            ...
            raise RuntimeError(
                ...
                "graph breaks, see https://pycore.org/docs/stable/export.html"
            ) from e
    ...
    def build_stage(
        ...
        # `core.ones`, `core.zeros`, `core.rand`, etc.
        if isinstance(stage_module, core.fx.GraphModule):
            _modify_graph_op_device(stage_module, device)
        else:
            logger.warning(
                f"Expected a `core.fx.GraphModule` but got {type(stage_module)}"  # noqa: G004
            )
...
def annotate_split_points(mod: core.nn.Module, spec: Dict[str, SplitPoint]):
