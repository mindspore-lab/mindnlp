...
from mindnlp.core.distributed.logging_handlers import _log_handlers
...
def _get_msg_dict(func_name, *args, **kwargs) -> Dict[str, Any]:
    if dist.is_initialized():
        ...
        if msg_dict["backend"] == "nccl":
            nccl_version = core.cuda.nccl.version()
...
def _time_logger(func: Callable[_P, _T]) -> Callable[_P, _T]:
    @functools.wraps(func)
    def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _T:
        # with _WaitCounter(f"pycore.wait_counter.c10d.{func.__name__}").guard():
