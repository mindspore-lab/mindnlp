...
core.save() and core.load() is not recommended when checkpointing sharded models.
"""
...
from core.distributed.checkpoint.optimizer import load_sharded_optimizer_state_dict
from core.distributed.fsdp import FullyShardedDataParallel as FSDP
from core.distributed.fsdp.fully_sharded_data_parallel import StateDictType
...
def init_model():
    model = FSDP(core.nn.Linear(4, 4).cuda(dist.get_rank()))
    optim = core.optim.Adam(model.parameters(), lr=0.1)
    model(core.rand(4, 4)).sum().backward()
    optim.step()
