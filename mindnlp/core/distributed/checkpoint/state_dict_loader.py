...
from core.distributed.checkpoint.default_planner import _EmptyStateDictLoadPlanner
...
def load(
    ...
    *,
    ...
    .. warning::
        ...
        All non-tensor data is loaded using `core.load()` and modified in place
        on state_dict.
    ...
    Examples
        ...
        >>> fs_storage_reader = core.distributed.checkpoint.FileSystemReader("/checkpoint/1")

        >>> core.distributed.checkpoint.load_state_dict(
    ...
    .. note::
        ...
        rank has an individual GPU, via ``core.cuda.set_device()``.
    """

    no_dist = not (dist.is_available() and dist.is_initialized())
    if no_dist:
        warnings.warn(
            "core.distributed is unavailable or uninitialized, assuming the intent is to load in a single process."
        )
...
def _load_state_dict(
    ...
    # core._C._log_api_usage_once("core.distributed.checkpoint.load_state_dict")
...
def _load_state_dict_from_keys(
    ...
    *,
    ...
    .. warning::
        ...
        All non-tensor data is loaded using `core.load()`
    ...
    """
    core._C._log_api_usage_once(
        "core.distributed.checkpoint._load_state_dict_from_keys"
    )
