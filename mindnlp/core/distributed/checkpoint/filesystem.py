...
# from core._utils import _get_available_device_type, _get_device_module
...
from core.distributed.checkpoint.staging import BlockingAsyncStager
from core.distributed.checkpoint.storage import (
    StorageReader,
    StorageWriter,
    WriteResult,
)
from core.distributed.checkpoint.utils import _create_file_view
...
class _TensorLoader(ABC):
    ...
    @abstractmethod
    def values(self) -> Iterator[Tuple[core.Tensor, object]]:
        pass
...
class _OverlappingCpuLoader(_TensorLoader):
    def __init__(
        ...
        stream: Optional[core.Stream] = None,
        ...
        self.stream = cast(
            core.cuda.Stream, stream or self.device_module.current_stream()
        )
    ...
    def _drain(self) -> List[Tuple[core.Tensor, object]]:
        drained = []
    ...
    def _refill(self) -> None:
        with self.device_module.stream(self.stream):
            while not self._done and self.in_flight_data < self.inflight_threshhold:
                ...
                elif tensor.device == core.device("cpu"):
                    if (
    ...
    def _finish(self) -> Iterable[Tuple[core.Tensor, object]]:
        assert self._done
...
def _item_size(item: WriteItem) -> int:
    ...
    return size * core._utils._element_size(dtype)
...
def _write_item(
    stream: io.IOBase,
    data: Union[io.BytesIO, core.Tensor],
    ...
    if write_item.type == WriteItemType.BYTE_IO:
        ...
        assert isinstance(data, core.Tensor)
        assert data.device == core.device("cpu")
        core.save(data, cast(IO[bytes], stream))
...
def _write_files_from_queue(
    ...
    try:
        while True:
            ...
            custom_backend_name = core._C._get_privateuse1_backend_name()
