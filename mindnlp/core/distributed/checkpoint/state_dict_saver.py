...
from core.distributed._state_dict_utils import _offload_state_dict_to_cpu
from core.distributed.checkpoint._storage_utils import _storage_setup
from core.distributed.checkpoint.default_planner import DefaultSavePlanner
from core.distributed.checkpoint.logger import _dcp_method_logger
from core.distributed.checkpoint.metadata import Metadata, STATE_DICT_TYPE
from core.distributed.checkpoint.planner import SavePlan, SavePlanner
from core.distributed.checkpoint.staging import AsyncStager
from core.distributed.checkpoint.stateful import Stateful
from core.distributed.checkpoint.storage import StorageWriter
...
def save(
    ...
    *,
    ...
    This function is different from ``core.save()`` as it handles
    ...
    Example:
        ...
        >>> fs_storage_writer = core.distributed.checkpoint.FileSystemWriter("/checkpoint/1")
        >>> core.distributed.checkpoint.save(
    ...
    .. note::
        ...
        In this case, the device used is given by ``core.cuda.current_device()``
        and it is the user's responsibility to ensure that this is set so that
        each rank has an individual GPU, via ``core.cuda.set_device()``.
    """
    core._C._log_api_usage_once("core.distributed.checkpoint.save")

    no_dist = not (dist.is_available() and dist.is_initialized())
    if no_dist:
        warnings.warn(
            "core.distributed is unavailable or uninitialized, assuming the intent is to save in a single process."
        )
...
def async_save(
    ...
    *,
    ...
    Example:
        ...
        >>> fs_storage_writer = core.distributed.checkpoint.FileSystemWriter("/checkpoint/1")
        >>> checkpoint_future = core.distributed.checkpoint.async_save(
        >>>     state_dict=state_dict,
        >>>     storage_writer=fs_storage_writer,
        >>> )
        >>>
    ...
    """
    core._C._log_api_usage_once("core.distributed.checkpoint.async_save")

    if dist.is_available() and dist.is_initialized():
        pg = process_group or _get_default_group()
        assert (
            core.device("cpu") in pg._device_types  # type: ignore[attr-defined]
...
def _save_state_dict(
    ...
    core._C._log_api_usage_once("core.distributed.checkpoint.save_state_dict")
