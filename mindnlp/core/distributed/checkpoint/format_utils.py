...
from core.distributed.checkpoint import FileSystemReader, FileSystemWriter
from core.distributed.checkpoint._nested_dict import flatten_state_dict
from core.distributed.checkpoint.default_planner import (
...
from core.distributed.checkpoint.planner import LoadItemType, LoadPlan, LoadPlanner
from core.distributed.checkpoint.planner_helpers import _create_chunk_list
from core.distributed.checkpoint.state_dict_loader import _load_state_dict
from core.distributed.checkpoint.state_dict_saver import _save_state_dict
    ...
    def read_data(self, plan: LoadPlan, planner: LoadPlanner) -> Future[None]:
        ...
        for req in plan.items:
            ...
            if self.is_coordinator:
                ...
                tensor = core.empty_like(planner.state_dict[req.storage_index.fqn])
...
def torch_save_to_dcp(
    ...
    state_dict = core.load(torch_save_path, weights_only=False)
    # we don't need stateful behavior here because the expectation is anything loaded by
    # core.load would not contain stateful objects.
    _save_state_dict(
