...
from core._utils import _get_device_module
from core.distributed._shard.sharded_tensor.api import ShardedTensor
from core.distributed._shard.sharded_tensor.metadata import (
    TensorProperties as ShardTensorProperties,
)
from core.distributed._shard.sharded_tensor.shard import Shard
from core.distributed._shard.sharding_spec.chunk_sharding_spec import ChunkShardingSpec
from core.distributed.checkpoint._nested_dict import unflatten_state_dict
from core.distributed.checkpoint.default_planner import DefaultLoadPlanner
from core.distributed.checkpoint.metadata import (
...
from core.distributed.checkpoint.planner import LoadPlan, LoadPlanner
from core.distributed.checkpoint.planner_helpers import (
    _create_read_items,
    create_read_items_for_chunk_list,
)
from core.distributed.checkpoint.state_dict_loader import load_state_dict
from core.distributed.checkpoint.storage import StorageReader
from core.distributed.checkpoint.utils import (
    _element_wise_add,
    _element_wise_sub,
    _normalize_device_info,
)
from core.distributed.distributed_c10d import _get_default_group
from core.distributed.fsdp._shard_utils import _create_chunk_sharded_tensor
...
def _is_nested_tensor(val: core.Tensor) -> bool:
...
def _alloc_tensor(
    ...
    if device_type == "cpu":
        device = cast(core.device, _get_device_module(device_type).current_device())
    else:
        device = core.device(
...
class _ReaderWithOffset(DefaultLoadPlanner):
        ...
        for fqn, obj in self.state_dict.items():
            ...
            local_chunks = [
                ChunkStorageMetadata(
                    ...
                    sizes=core.Size(original_shard.metadata.shard_sizes),
                )
            ]
            ...
            for ri in reqs:
                ...
                original_index = dataclasses.replace(
                    ri.dest_index, offset=core.Size(original_offset)
                )
    ...
    def lookup_tensor(self, index: MetadataIndex) -> core.Tensor:
...
def load_sharded_optimizer_state_dict(
    ...
    >>> # xdoctest: +SKIP
    >>> from mindnlp import core.distributed.checkpoint as dist_cp
    >>> # Save
    >>> model: core.nn.Model
    >>> optim_params = model.parameters()
    >>> optim = core.optim.SGD(optim_params, lr=0.01)
    >>> # Save
    ...
    for key, value in metadata.state_dict_metadata.items():
        ...
        if value.size.numel() == 1:
            ...
            st_md = sharding_spec.build_metadata(core.Size(alloc_size), properties)
            local_shards = []
