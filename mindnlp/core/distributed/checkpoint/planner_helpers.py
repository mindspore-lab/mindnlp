...
# from core.distributed.tensor._utils import compute_local_shape_and_global_offset
...
def _create_chunk_from_tensor(tensor: core.Tensor) -> ChunkStorageMetadata:
    return ChunkStorageMetadata(
        offsets=core.Size([0] * len(tensor.size())), sizes=tensor.size()
    )


def _chunk_for_shard(shard_md: ShardMetadata) -> ChunkStorageMetadata:
    return ChunkStorageMetadata(
        offsets=core.Size(shard_md.shard_offsets),
        sizes=core.Size(shard_md.shard_sizes),
    )
...
#     )
#     sizes, offsets = core.Size(sizes), core.Size(offsets)
...
def _create_write_item_for_tensor(fqn: str, tensor: core.Tensor) -> WriteItem:
    offsets = core.Size([0] * len(tensor.size()))
    return WriteItem(
        index=MetadataIndex(fqn, offsets),
...
def _create_read_item_for_byteio(
    ...
    return ReadItem(
        ...
        dest_offsets=core.Size((dest_offset,)),
        storage_index=storage_index,
        storage_offsets=core.Size((storage_offset,)),
        lengths=core.Size((length,)),
    )
...
def _create_read_item_for_tensor(
    ...
    return ReadItem(
        ...
        dest_index=dest_index,
        dest_offsets=core.Size(dest_offsets),
        storage_index=storage_index,
        storage_offsets=core.Size(storage_offsets),
        lengths=core.Size(lengths),
    )
...
def _create_write_items(fqn: str, object: Any) -> List[WriteItem]:
    ...
    elif isinstance(object, core.Tensor):
...
def _create_chunk_list(tensor: core.Tensor) -> List[ChunkStorageMetadata]:
    ...
    elif isinstance(tensor, core.Tensor):
...
def _init_state_dict(state_dict: Dict[str, Any]) -> Any:
    """
    Initializes meta tensor if the meta tensor is DTensor or core.Tensor.
    """
    ...
    #         device = cast(
    #             core.device, _get_device_module(device_type).current_device()
    #         )
    #         new_local_tensor = core.empty_like(value.to_local(), device=device)
    ...
    def tensor_func(value: core.Tensor):
        device = getattr(value, "device", None)
        if device == core.device("meta"):
            device_type = dist.distributed_c10d._get_pg_default_device().type
            device = cast(
            ...
            tensor = core.empty_like(value, device=device)
            return tensor
        else:
            return value
...
def _iterate_state_dict(
    ...
    elif isinstance(iter_object, core.Tensor):
