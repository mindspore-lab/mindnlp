...
from core._C._distributed_rpc import _TensorPipeRpcBackendOptionsBase

from . import constants as rpc_contants


DeviceType = Union[int, str, core.device]
...
) -> Dict[core.device, core.device]:
    full_device_map: Dict[core.device, core.device] = {}
    reverse_map: Dict[core.device, core.device] = {}
    for k, v in device_map.items():
        k, v = core.device(k), core.device(v)
        if v in reverse_map:
            raise ValueError(
...
class TensorPipeRpcBackendOptions(_TensorPipeRpcBackendOptionsBase):
    ...
    :class:`~core.distributed.rpc.TensorPipeAgent`, derived from
    :class:`~core.distributed.rpc.RpcBackendOptions`.

    Args:
        num_worker_threads (int, optional): The number of threads in the
            thread-pool used by
            :class:`~core.distributed.rpc.TensorPipeAgent` to execute
            requests (default: 16).
        rpc_timeout (float, optional): The default timeout, in seconds,
            ...
            RPCs in :meth:`~core.distributed.rpc.rpc_sync` and
            :meth:`~core.distributed.rpc.rpc_async` if necessary.
        init_method (str, optional): The URL to initialize the distributed
            ...
            same argument of :meth:`~core.distributed.init_process_group`
            (default: ``env://``).
        device_maps (Dict[str, Dict], optional): Device placement mappings from
            ...
            the dictionary (``Dict`` of ``int``, ``str``, or ``core.device``)
            that maps this worker's devices to the callee worker's devices.
            (default: ``None``)
        devices (List[int, str, or ``core.device``], optional): all local
    ...
    def set_device_map(self, to: str, device_map: Dict[DeviceType, DeviceType]):
        ...
        Args:
            to (str): Callee name.
            device_map (Dict of int, str, or core.device): Device placement
    ...
    def set_devices(self, devices: List[DeviceType]):
        ...
        Args:
            devices (List of int, str, or core.device): local devices used by
