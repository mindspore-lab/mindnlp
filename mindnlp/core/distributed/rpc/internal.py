...
from core._C._distributed_rpc import _get_current_rpc_agent


__all__ = ["RPCExecMode", "serialize", "deserialize", "PythonUDF", "RemoteException"]

# Thread local tensor tables to store tensors while pickling core.Tensor
# objects
...
class _InternalRPCPickler:
    ...
    def __init__(self):
        ...
        self._dispatch_table[core.Tensor] = self._tensor_reducer
    ...
    def _script_module_receiver(cls, script_module_serialized):
        """
        Given a serialized representation of a ScriptModule created with core.jit.save,
    ...
    def serialize(self, obj):
        ...
        if isinstance(obj, core.jit.ScriptModule):
            # Ignore type error because dispatch_table is defined in third-party package
...
def _start_record_function(exec_type, func_name, current_worker_name, dest_worker_name):
    ...
    Returns:
        An instance of `core.autograd._RecordFunction`.
    """
    assert core.autograd._profiler_enabled(), "Autograd profiler should be enabled."
    profile_key = f"rpc_{exec_type.value}#{str(func_name)}({current_worker_name} -> {dest_worker_name})"
    rf = core.autograd._RecordFunction()  # type: ignore[attr-defined]
    core.autograd._run_before_callbacks(rf, profile_key)  # type: ignore[attr-defined]
    return rf
