...
def async_execution(fn):
    ...
    is guaranteed to be a :class:`~core.futures.Future` object and this
    function can run asynchronously on the RPC callee. More specifically, the
    callee extracts the :class:`~core.futures.Future` returned by the wrapped
    function and installs subsequent processing steps as a callback to that
    :class:`~core.futures.Future`. The installed callback will read the value
    from the :class:`~core.futures.Future` when completed and send the
    value back as the RPC response. That also means the returned
    :class:`~core.futures.Future` only exists on the callee side and is never
    ...
    :meth:`~core.distributed.rpc.rpc_async` or waiting for other signals.
    ...
    Example::
        The returned :class:`~core.futures.Future` object can come from
        :meth:`~core.distributed.rpc.rpc_async`,
        :meth:`~core.futures.Future.then`, or :class:`~core.futures.Future`
        constructor. The example below shows directly using the
        :class:`~core.futures.Future` returned by
        :meth:`~core.futures.Future.then`.

        >>> from core.distributed import rpc
        >>>
        ...
        >>>     return rpc.rpc_async(to, core.add, args=(x, y)).then(
        ...
        >>>         ret_fut = core.futures.Future()
        >>>         rpc.rpc_async(to, core.add, args=(x, y)).then(
        ...
        :meth:`core.distributed.rpc.RRef.rpc_sync`,
        :meth:`core.distributed.rpc.RRef.rpc_async`, and
        :meth:`core.distributed.rpc.RRef.remote`.

        >>> from core.distributed import rpc
        >>>
        >>> # reuse the AsyncExecutionClass class above
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.rpc_sync().static_async_add("worker2", core.ones(2), 1, 2)
        >>> print(ret)  # prints tensor([4., 4.])
        >>>
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        ...
        >>>
        >>> rref = rpc.remote("worker1", AsyncExecutionClass)
        >>> ret = rref.remote().static_async_add("worker2", core.ones(2), 1, 2).to_here()
        >>> print(ret)  # prints tensor([4., 4.])
    """
