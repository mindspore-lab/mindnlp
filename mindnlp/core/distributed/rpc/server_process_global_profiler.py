...
from core.autograd.profiler_legacy import profile
...
class _server_process_global_profile(profile):
    """
    It has the same API as ``core.autograd.profiler.profile`` class,
    ...
    def __enter__(self):
        ...
        profiler_kind = (
            core.autograd.ProfilerState.CUDA
            if self.use_cuda
            else core.autograd.ProfilerState.CPU
        )
        profiler_config = core.autograd.ProfilerConfig(
            profiler_kind,
            self.record_shapes,
            self.profile_memory,
            False,
            False,
            False,
            core.profiler._ExperimentalConfig(),
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        Attributes:
            function_events (core.autograd.profiler.EventList).  It's a list that has helper
            methods, like 1) show record items in a pretty-print table.
            2) do averaging by grouping on keys. 3) and more.

            process_global_function_events (List[core.autograd.profiler.FunctionEvent]).
        ...
        for thread_local_events in process_global_events:
            ...
            thread_local_function_events = (
                core.autograd.profiler_legacy._parse_legacy_records(
                    thread_local_events
                )
            )
        ...
        self.function_events = core.autograd.profiler_util.EventList(
