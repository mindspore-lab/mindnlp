...
def get_approximate_basis(
    ...
    R = core.randn(A.shape[-1], q, dtype=dtype, device=A.device)

    # The following code could be made faster using core.geqrf + core.ormqr
    # but geqrf is not differentiable

    X = matmul(A, R)
    if M is not None:
        X = X - matmul(M, R)
    Q = core.linalg.qr(X).Q
...
def svd_lowrank(
    ...
    .. note:: In general, use the full-rank SVD implementation
              :func:`core.linalg.svd` for dense matrices due to its 10x
              higher performance characteristics. The low-rank SVD
              will be useful for huge sparse matrices that
              :func:`core.linalg.svd` cannot handle.
...
def _svd_lowrank(
    ...
    if M is not None:
        B = B - matmul(Q.mH, M)
    U, S, Vh = core.linalg.svd(B, full_matrices=False)
    V = Vh.mH
    U = Q.matmul(U)
...
def pca_lowrank(
    ...
    if not core.jit.is_scripting():
        if type(A) is not core.Tensor and has_torch_function((A,)):
            return handle_torch_function(
    ...
    if _utils.is_sparse(A):
        ...
        C_t = core.sparse_coo_tensor(
            indices, c.values(), (n, 1), dtype=dtype, device=A.device
        )

        ones_m1_t = core.ones(A.shape[:-2] + (1, m), dtype=dtype, device=A.device)
        M = core.sparse.mm(C_t, ones_m1_t).mT
