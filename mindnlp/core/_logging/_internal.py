...
from mindnlp.core._guards import CompileId
from mindnlp.core._utils_internal import log_trace_structured_event
from mindnlp.core.utils._traceback import CapturedTraceback
...
trace_log = logging.getLogger("core.__trace")
...
# sample usage: core._logging.set_logs(**core._logging.DEFAULT_LOGGING)
DEFAULT_LOGGING = {
    "dynamo": logging.INFO,
    "aot": logging.INFO,
...
def set_logs(
    *,
    ...
    Example::
        ...
        >>> core._logging.set_logs(dynamo=logging.DEBUG, graph_code=True)

        # The following enables the logs for a different module

        >>> core._logging.set_logs(modules={"unregistered.module.name": logging.DEBUG})
    """
...
def getArtifactLogger(module_qname, artifact_name) -> logging.Logger:
    if artifact_name not in log_registry.artifact_names:
        raise ValueError(
            ...
            f"please call register_artifact({repr(artifact_name)}) in core._logging.registrations."
        )
...
def process_env_var_string_for_windows(env_var_str: str) -> str:
    """
    When we setup logging config as guide: https://docs.pycore.org/docs/stable/logging.html
    Such as:
...
class TorchLogsFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if (
            not self._is_trace
            and (trace_id := core._guards.CompileContext.current_trace_id())
            is not None
        ):
    ...
    def emit(self, record) -> None:
        if self.stream is None:
            if self.root_dir is None:
                ...
                import core.version as torch_version
                ...
                elif not core._utils_internal.justknobs_check("pytorch/trace:enable"):
                    log.info(
...
def add_structured_logging_overhead(time_spent: float) -> None:
    ...
    key = None
    if (trace_id := core._guards.CompileContext.current_trace_id()) is not None:
    ...
    *,
    ...
    if trace_log.handlers:
        ...
        if not suppress_context:
            ...
            trace_id = core._guards.CompileContext.current_trace_id()
            if expect_trace_id and trace_id is None and compile_id is None:
                ...
                record["stack"] = core._logging.structured.from_traceback(
...
import mindnlp.core._guards
import mindnlp.core._utils_internal
import mindnlp.core.distributed as dist