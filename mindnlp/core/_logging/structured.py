...
import mindnlp.core._logging._internal
...
def intern_string(s: Optional[str]) -> int:
    ...
    if r is None:
        r = len(INTERN_TABLE)
        INTERN_TABLE[s] = r
        core._logging._internal.trace_structured(
...
def dump_file(filename: str) -> None:
    ...
    from core.fx.graph_module import _loader
...
def get_user_stack(num_frames: int) -> list[dict[str, Any]]:
    from core._guards import TracingContext
    from core.utils._traceback import CapturedTraceback
...
def get_framework_stack(
    ...
    """
    from core.fx.experimental.symbolic_shapes import uninteresting_files
    from core.utils._traceback import CapturedTraceback
