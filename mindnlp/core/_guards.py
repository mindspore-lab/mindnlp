...
import torch
from mindnlp.core.utils import _pytree as pytree
from mindnlp.core.utils._backport_slots import dataclass_slots
from mindnlp.core.utils._traceback import CapturedTraceback, format_frame
from mindnlp.core.utils.weak import WeakTensorKeyDictionary
...
if TYPE_CHECKING:
    ...
    from mindnlp.core._dynamo.codegen import PyCodegen
    from mindnlp.core._funccore._aot_autograd.schemas import ViewAndMutationMeta
    from mindnlp.core._subclasses.fake_tensor import FakeTensorMode


"""
core._guards is the definitional source of truth for general purpose guard structures.
...
class CompileId:
    ...
    # core.compiling a compiled autograd graph
...
class GuardSource(enum.Enum):
    ...
    def is_specialized_nn_module(self) -> bool:
        import core._dynamo.config as config
...
class Guard:
    ...
    def sort_key(self) -> tuple[bool, int, int, str, int]:
        ...
        is_duplicate_input = (
            ...
            and self.create_fn.func is core._dynamo.guards.GuardBuilder.DUPLICATE_INPUT
        )
        return (
            is_duplicate_input,
...
core._dynamo.guards._parse_guard_env_guards to avoid a RuntimeError.
"""
...
class ModuleContextCheckpointState:
    nn_modules: dict[str, core.nn.Module] = {}

    def __init__(self, nn_modules: dict[str, core.nn.Module]) -> None:
...
class GlobalContext(Checkpointable[GlobalContextCheckpointState]):
    ...
    For example, core.is_grad_enabled.
    """
...
class HopSubgraphCache:
    ...
    def add_lazy_bwd_entry(
        ...
        gmod: core.fx.GraphModule,
    ) -> int: ...

    @abstractmethod
    def get_lazy_bwd_entry(
        self, identifier: str, tangent_metadata: tuple[object]
    ) -> tuple[Optional[core.fx.GraphModule], Optional[int]]: ...
...
class InvokeSubgraphCache(HopSubgraphCache):
    def __init__(self) -> None:
        ...
        self.lazy_bwd_cache: dict[
            str, dict[tuple[object], tuple[core.fx.GraphModule, int]]
        ] = defaultdict(dict)
...
class HopDispatchSetCache:
    def __init__(self) -> None:
        ...
        from core._higher_order_ops.invoke_subgraph import invoke_subgraph
...
def detect_fake_mode(inputs: Any = None) -> Optional[FakeTensorMode]:
    ...
    """
    from core._subclasses.fake_tensor import FakeTensor, FakeTensorMode
    ...
    from core.utils._python_dispatch import _get_current_dispatch_mode_stack
