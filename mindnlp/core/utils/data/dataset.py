...
from ... import default_generator, Generator, Tensor
from ...ops import randperm
...
class Dataset(Generic[_T_co]):
    ...
    :class:`~core.utils.data.Sampler` implementations and the default options
    of :class:`~core.utils.data.DataLoader`. Subclasses could also
    optionally implement :meth:`__getitems__`, for speedup batched samples
    loading. This method accepts list of indices of samples of batch and returns
    list of samples.

    .. note::
      :class:`~core.utils.data.DataLoader` by default constructs an index
    ...
    # core.utils.data._utils.fetch._MapDatasetFetcher
...
class IterableDataset(Dataset[_T_co], Iterable[_T_co]):
    ...
    When a subclass is used with :class:`~core.utils.data.DataLoader`, each
    item in the dataset will be yielded from the :class:`~core.utils.data.DataLoader`
    iterator. When :attr:`num_workers > 0`, each worker process will have a
    different copy of the dataset object, so it is often desired to configure
    each copy independently to avoid having duplicate data returned from the
    workers. :func:`~core.utils.data.get_worker_info`, when called in a worker
    process, returns information about the worker. It can be used in either the
    dataset's :meth:`__iter__` method or the :class:`~core.utils.data.DataLoader` 's
    ...
    Example 1: splitting workload across all workers in :meth:`__iter__`::
        ...
        >>> class MyIterableDataset(core.utils.data.IterableDataset):
        ...
        ...
        ...     def __iter__(self):
        ...         worker_info = core.utils.data.get_worker_info()
        ...
        >>> print(list(core.utils.data.DataLoader(ds, num_workers=0)))
        ...
        >>> print(list(core.utils.data.DataLoader(ds, num_workers=2)))
        [tensor([3]), tensor([5]), tensor([4]), tensor([6])]

        >>> # With even more workers
        >>> # xdoctest: +IGNORE_WANT("non deterministic")
        >>> print(list(core.utils.data.DataLoader(ds, num_workers=12)))
        [tensor([3]), tensor([5]), tensor([4]), tensor([6])]
    ...
    Example 2: splitting workload across all workers using :attr:`worker_init_fn`::
        ...
        >>> # Worker 0 fetched [3, 4].  Worker 1 fetched [5, 6].
        >>> print(list(core.utils.data.DataLoader(ds, num_workers=2, worker_init_fn=worker_init_fn)))
        [3, 5, 4, 6]

        >>> # With even more workers
        >>> print(list(core.utils.data.DataLoader(ds, num_workers=12, worker_init_fn=worker_init_fn)))
        [3, 4, 5, 6]
    """
...
class Subset(Dataset[_T_co]):
    ...
    def __getitems__(self, indices: List[int]) -> List[_T_co]:
        ...
        # see core.utils.data._utils.fetch._MapDatasetFetcher
...
def random_split(
    ...
    Example:
        >>> # xdoctest: +SKIP
        >>> generator1 = core.Generator().manual_seed(42)
        >>> generator2 = core.Generator().manual_seed(42)
