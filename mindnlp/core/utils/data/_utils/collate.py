...
def default_convert(data):
    r"""
    Convert each NumPy array element into a :class:`core.Tensor`.

    If the input is a `Sequence`, `Collection`, or `Mapping`, it tries to convert each element inside to a :class:`core.Tensor`.
    If the input is not an NumPy array, it is left unchanged.
    This is used as the default function for collation when both `batch_sampler` and `batch_size`
    are NOT defined in :class:`~core.utils.data.DataLoader`.

    The general input type to output type mapping is similar to that
    of :func:`~core.utils.data.default_collate`. See the description there for more details.
...
def collate(
    ...
    *,
    ...
    Examples:
        ...
        ...     return core.stack(batch, 0)
        >>> def custom_collate(batch):
        ...     collate_map = {core.Tensor: collate_tensor_fn}
        ...     return collate(batch, collate_fn_map=collate_map)
        >>> # Extend `default_collate` by in-place modifying `default_collate_fn_map`
        >>> default_collate_fn_map.update({core.Tensor: collate_tensor_fn})
...
def collate_tensor_fn(
    ...
    *,
    ...
    #         "please provide a custom collate_fn to handle them appropriately."
    #     )
    # if core.utils.data.get_worker_info() is not None:
...
def collate_numpy_array_fn(
    ...
    *,
    ...
    return collate([core.as_tensor(b) for b in batch], collate_fn_map=collate_fn_map)
...
def collate_float_fn(
    ...
    *,
    ...
    return core.tensor(batch, dtype=core.float64)
...
def default_collate(batch):
    ...
    The exact output type can be a :class:`core.Tensor`, a `Sequence` of :class:`core.Tensor`, a
    Collection of :class:`core.Tensor`, or left unchanged, depending on the input type.
    This is used as the default function for collation when
    `batch_size` or `batch_sampler` is defined in :class:`~core.utils.data.DataLoader`.

    Here is the general input type (based on the type of the element within the batch) to output type mapping:

        * :class:`core.Tensor` -> :class:`core.Tensor` (with an added outer dimension batch size)
        * NumPy Arrays -> :class:`core.Tensor`
        * `float` -> :class:`core.Tensor`
        * `int` -> :class:`core.Tensor`
        * `str` -> `str` (unchanged)
