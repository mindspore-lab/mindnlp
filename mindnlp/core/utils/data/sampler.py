...
class Sampler(Generic[_T_co]):
    ...
    Example:
        ...
        >>>         sizes = core.tensor([len(x) for x in self.data])
        >>>         yield from core.argsort(sizes).tolist()
        >>>
        ...
        >>>         sizes = core.tensor([len(x) for x in self.data])
        >>>         for batch in core.chunk(core.argsort(sizes), len(self)):
        >>>             yield batch.tolist()

    .. note:: The :meth:`__len__` method isn't strictly required by
              :class:`~core.utils.data.DataLoader`, but is expected in any
              calculation involving the length of a :class:`~core.utils.data.DataLoader`.
    """
    ...
    # data, e.g., `core.utils.data.Sampler`, with its subclasses optionally
...
class RandomSampler(Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        if self.generator is None:
            seed = int(core.empty((), dtype=core.int64).random_().item())
        ...
        if self.replacement:
            for _ in range(self.num_samples // 32):
                yield from core.randint(
                    high=n, size=(32,), dtype=core.int64, generator=generator
                ).tolist()
            yield from core.randint(
                high=n,
            ...
            for _ in range(self.num_samples // n):
                yield from core.randperm(n, generator=generator).tolist()
            yield from core.randperm(n, generator=generator).tolist()[
                : self.num_samples % n
            ]
...
class SubsetRandomSampler(Sampler[int]):
    ...
    def __iter__(self) -> Iterator[int]:
        for i in core.randperm(len(self.indices), generator=self.generator):
...
class WeightedRandomSampler(Sampler[int]):
    ...
    def __init__(
        ...
        weights_tensor = core.as_tensor(weights, dtype=core.double)
