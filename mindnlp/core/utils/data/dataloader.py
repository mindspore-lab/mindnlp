...
# from mindnlp import core.utils.data.graph_settings
from mindnlp.core._utils import ExceptionWrapper
from mindnlp.core.utils.data import _utils
# from core.utils.data.datapipes.datapipe import (
...
# (one has to explicitly directly `from mindnlp import core.utils.data.dataloader`), there
...
class _InfiniteConstantSampler(Sampler):
    ...
    Used as sampler for :class:`~core.utils.data.IterableDataset`.
    """
...
def _sharding_worker_init_fn(worker_init_fn, world_size, rank_id, worker_id):
    ...
    info = core.utils.data.get_worker_info()
    ...
    core.utils.data.graph_settings.apply_sharding(
...
def _share_dist_seed(generator, pg):
    _shared_seed = core.empty((), dtype=core.int64).random_(generator=generator)
...
class DataLoader(Generic[_T_co]):
    ...
    The :class:`~core.utils.data.DataLoader` supports both map-style and
    iterable-style datasets with single- or multi-process loading, customizing
    loading order and optional automatic batching (collation) and memory pinning.

    See :py:mod:`core.utils.data` documentation page for more details.
    ...
    Args:
        ...
        generator (core.Generator, optional): If not ``None``, this RNG will be used
    ...
    .. warning:: ``len(dataloader)`` heuristic is based on the length of the sampler used.
                 When :attr:`dataset` is an :class:`~core.utils.data.IterableDataset`,
                 ...
                 :class:`~core.utils.data.IterableDataset` interacts with
    ...
    def __init__(
        ...
        *,
        ...
        # core._C._log_api_usage_once("python.data_loader")
        ...
        if isinstance(dataset, IterableDataset):
            ...
            #         dataset = core.utils.data.graph_settings.apply_shuffle_settings(
        ...
        # core.set_vital("Dataloader", "enabled", "True")  # type: ignore[attr-defined]
        ...
        if multiprocessing_context is not None:
            if self.num_workers > 0:
                if isinstance(multiprocessing_context, str):
                    valid_start_methods = core.multiprocessing.get_all_start_methods()
                    ...
                    multiprocessing_context = core.multiprocessing.get_context(
    ...
    def check_worker_number_rationality(self):
        ...
        #        other than `core.set_num_threads` to 1 in the worker process, if the passing
...
class _BaseDataLoaderIter:
    def __init__(self, loader: DataLoader) -> None:
        ...
        #     shared_rng = core.Generator()
        #     shared_rng.manual_seed(self._shared_seed)
        #     self._dataset = core.utils.data.graph_settings.apply_random_seed(
        ...
        if len(loader.pin_memory_device) == 0:
            self._pin_memory = loader.pin_memory and core.cuda.is_available()
    ...
    def __next__(self) -> Any:
        # with core.autograd.profiler.record_function(self._profile_name):
        ...
        if (
            ...
            if self._num_workers > 0:
                warn_msg += (
                    ...
                    "https://pycore.org/docs/stable/data.html#core.utils.data.IterableDataset for examples."
                )
            warnings.warn(warn_msg)
        return data
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        if loader.multiprocessing_context is None:
            multiprocessing_context = core.multiprocessing
        else:
        ...
        if self._pin_memory:
            ...
            if self._pin_memory_device == "xpu":
                current_device = core.xpu.current_device()  # type: ignore[attr-defined]
            elif self._pin_memory_device == core._C._get_privateuse1_backend_name():
                custom_device_mod = getattr(
                    torch, core._C._get_privateuse1_backend_name()
                )
                current_device = custom_device_mod.current_device()
            else:
                current_device = core.cuda.current_device()  # choose cuda for default
            ...
            try:
                ...
                if e.errno == errno.EMFILE:
                    raise RuntimeError(
                        ...
                        " `core.multiprocessing.set_sharing_strategy('file_system')`"
    ...
    # `core.multiprocessing`) raises a `RuntimeError: received 0 items of ancdata`
    #
