        ...
        for k in tensor._backward_hooks:
            ...
            if not hasattr(hook, "__torch_unserializable__"):
                warnings.warn(f"backward hook {repr(hook)} on tensor will not be "
                              ...
                              "decorate the function with @core.utils.hooks.unserializable_hook "
...
class BackwardHook:
    ...
    def _apply_on_tensors(self, fn, args):
        ...
        # if not (requires_grad and core.is_grad_enabled()):
        #     return args, None

        # new_tensors = core.nn.modules._functions.BackwardHookFunction.apply(*tensors)
        new_tensors = tensors
