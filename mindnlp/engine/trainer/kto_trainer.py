...
from ..train_args import KTOConfig
from .utils import (
...
class KTOTrainer(Trainer):
    ...
    # def get_train_dataloader(self) -> DataLoader:
        ...
        #         reference_completion_logps, reference_KL_logp = self.compute_reference_log_probs(padded_batch)
    ...
    def get_batch_logps(
        ...
        per_token_logps = ops.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
    ...
    def get_batch_loss_metrics(
        ...
        num_chosen = ms.Tensor([len(chosen_rewards)])
        num_rejected = ms.Tensor([len(rejected_rewards)])
    ...
    def prediction_step(
        ...
        with no_grad(), prediction_context_manager:
            loss, metrics = self.get_batch_loss_metrics(model, inputs)
