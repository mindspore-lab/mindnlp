...
from mindnlp.engine import Trainer, TrainingArguments, has_length
    ...
    if not kwargs["is_encoder_decoder"]:
        ...
        if model is not None and hasattr(model, "prepare_decoder_input_ids_from_labels"):
            batch[f"{kwargs['prefix']}completion_decoder_input_ids"] = model.prepare_decoder_input_ids_from_labels(
                labels=ms.tensor(batch["completion_labels"])
            )
...
class KTOTrainer(Trainer):
    ...
    def __init__(
        ...
        compute_metrics: Optional[Callable[[], Dict]] = None,
        ...
        # Get KL datasets
        # 1 is the total number of device
        total_batch_size = (
...
###############################prepare for trainset and evalset
...
###############################
        ...
        if self.loss_type == "bco":
            ...
            embeddings = ops.cat((chosen_embeddings, rejected_embeddings), axis=0)
            labels = ops.cat(
                (ops.ones_like(chosen_embeddings[:, 0]), ops.zeros_like(rejected_embeddings[:, 0])), axis=0
            )
    ...
    # def _get_chosen_prob(self, prompt_embeddings:ms.Tensor) -> ms.Tensor:
    #     """
    ...
    def _vectorize_prompt(self, input_ids: ms.Tensor, attention_mask: ms.Tensor) -> ms.Tensor:
        """
    ...
    def _get_prompt_embeddings(
        self, batch: Dict[str, Union[List, ms.Tensor]]
    ...
    def _get_sample_prompt_embeddings(self, dataset: Dataset, sample_size: int = 512) -> ms.Tensor:
        """
        ...
        input_ids = pad_sequence(
            [ms.Tensor(ids) for ids in batch["embedding_input_ids"]],
            batch_first=True,
            padding_value=self.embedding_tokenizer.pad_token_id,
        )
        attention_mask = pad_sequence(
            [ms.Tensor(ids) for ids in batch["embedding_attention_mask"]],
            batch_first=True,
            padding_value=0,
        )
    ...
    #             name="reference_logps", column=ops.cat(reference_completion_logps).float().numpy()
    #         )
    #         self.train_dataset = self.train_dataset.add_column(
    #             name="reference_KL_logps", column=ops.cat(reference_KL_logps).float().numpy()
    #         )
    ...
    def get_batch_logps(
        ...
        per_token_logps = ops.gather_elements(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)
    ...
    def kto_loss(
        ...
        if policy_chosen_logps.shape[0] != 0 or reference_chosen_logps.shape[0] != 0:
            ...
            chosen_losses = 1 - ops.sigmoid(self.beta * (chosen_logratios - kl))
            chosen_rewards = self.beta * chosen_logratios
        else:
            # lists can't be empty -- if they are, then accelerate.gather will hang
            chosen_losses = ms.Tensor([])
            chosen_rewards = ms.Tensor([])

        if policy_rejected_logps.shape[0] != 0 or reference_rejected_logps.shape[0] != 0:
            rejected_logratios = policy_rejected_logps - reference_rejected_logps
            rejected_losses = 1 - ops.sigmoid(self.beta * (kl - rejected_logratios))
            rejected_rewards = self.beta * rejected_logratios
        else:
            # lists can't be empty -- if they are, then accelerate.gather will hang
            rejected_losses = ms.Tensor([])
            rejected_rewards = ms.Tensor([])
    ...
    def _get_udm_weight(self, rejected_embeddings: ms.Tensor) -> ms.Tensor:
    ...
    def bco_loss(
        ...
        reference_chosen_logps: ms.Tensor,
        reference_rejected_logps: ms.Tensor,
        chosen_embeddings: Optional[ms.Tensor],
        rejected_embeddings: Optional[ms.Tensor],
    ) -> Tuple[ms.Tensor, ms.Tensor, ms.Tensor, ms.Tensor]:
        ...
        rewards = ops.cat((chosen_rewards, rejected_rewards), 0).mean()
        self.running.update(rewards)
        rewards_mean = self.running.mean

        chosen_losses = -ops.logsigmoid(chosen_rewards - rewards_mean)
        rejected_losses = -ops.logsigmoid(-(rejected_rewards - rewards_mean))

        if self.match_underlying_distribution:
            chosen_weight = ops.ones_like(chosen_losses)
            rejected_weight = self._get_udm_weight(rejected_embeddings)

            losses = ops.cat((chosen_weight * chosen_losses, rejected_weight * rejected_losses), axis=0)
        else:
            losses = ops.cat((chosen_losses, rejected_losses), axism=0)

        return losses, chosen_rewards, rejected_rewards, ms.Tensor(rewards_mean)

    def get_batch_loss_metrics(
        self,
        model,
        batch: Dict[str, Union[List, ms.Tensor]],
    ):
        """Compute the KTO loss and other metrics for the given batch of inputs for train or test."""
        metrics = {}
        batch = {k: (v if isinstance(v, ms.Tensor) else v) for k, v in batch.items()}
        ...
        num_chosen = ops.Tensor([len(chosen_rewards)])
        num_rejected = ops.Tensor([len(rejected_rewards)])
    ...
    ) -> Union[ms.Tensor, Tuple[ms.Tensor, Dict[str, ms.Tensor]]]:
    ...
    def get_batch_samples(self, model, batch: Dict[str, ops.LongTensor]) -> Tuple[str, str]:
        """Generate samples from the model and reference model for the given batch of inputs."""

        # If one uses `generate_during_eval` with peft + bf16, we need to explicitly call generate with
        # the torch cuda amp context manager as some hidden states are silently casted to full precision.
        generate_context_manager = nullcontext
    ...
    def prediction_step(
        ...
        prediction_context_manager = nullcontext
        ...
        logits = ops.stack(logits).mean(axis=1)
        labels = ops.zeros(logits.shape[0])
    ...
    def log(self, logs: Dict[str, float]) -> None:
        ...
        for split in ["chosen", "rejected"]:
            if f"count/{split}" in self._stored_metrics[train_eval]:
                count_sum = ms.Tensor(self._stored_metrics[train_eval][f"count/{split}"]).sum().item()
                logs[f"{prefix}rewards/{split}"] = (
                    ms.Tensor(self._stored_metrics[train_eval][f"rewards/{split}_sum"]).sum().item() / count_sum
                )
                logs[f"{prefix}logps/{split}"] = (
                    ms.Tensor(self._stored_metrics[train_eval][f"logps/{split}_sum"]).sum().item() / count_sum
                )
        ...
        for key, metrics in self._stored_metrics[train_eval].items():
            logs[f"{prefix}{key}"] = ms.Tensor(metrics).mean().item()
