...
class CPOTrainer(Trainer):
        ...
        if not self.is_encoder_decoder:
            ...
            if model is not None and hasattr(model, "prepare_decoder_input_ids_from_labels"):
                batch["rejected_decoder_input_ids"] = model.prepare_decoder_input_ids_from_labels(
                    labels=ms.tensor(batch["rejected_labels"])
                )
                batch["chosen_decoder_input_ids"] = model.prepare_decoder_input_ids_from_labels(
                    labels=ms.tensor(batch["chosen_labels"])
                )
    ...
    def concatenated_inputs(
        ...
        for k in batch:
            if k.startswith("chosen") and isinstance(batch[k], ms.Tensor):
        ...
        for k in batch:
            if k.startswith("rejected") and isinstance(batch[k], ms.Tensor):
                if "labels" in k or is_encoder_decoder:
                    pad_value = label_pad_token_id
                elif k.endswith("_input_ids"):
                    pad_value = padding_value
                ...
                concatenated_batch[concatenated_key] = ops.cat(
                    (
    ...
    def log(self, logs: Dict[str, float]) -> None:
        ...
        for key, metrics in self._stored_metrics[train_eval].items():
            logs[key] = ms.tensor(metrics).mean().item()
