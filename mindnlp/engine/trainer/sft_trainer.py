...
def unwrap_model(model: nn.Module, recursive: bool = False) -> nn.Module:
    ...
    Args:
        model (`nn.Cell`): The model to unwrap.
...
class SFTTrainer(Trainer):
    ...
    def __init__(
        ...
        #因为没有prepare_model_for_kbit这个函数，所以整段注释
    ...
    def _prepare_dataset(
        ...
        column_names = (
            dataset.get_col_names() if isinstance(dataset, (Dataset, IterableDataset)) else None
        )
    ...
    def _prepare_non_packed_dataloader(
        ...
        def tokenize(text):
            ...
            outputs = tokenizer(text, truncation=True, max_length=max_seq_length,  padding='max_length')

            if use_formatting_func and not self._dataset_sanity_checked:
                if not isinstance(formatting_func(text), list):
                    raise ValueError(
        ...
        extra_columns = list(set(dataset.get_col_names()) - set(signature_columns))
        ...
        #需要重新写这个函数
        #https://github.com/mindspore-lab/mindnlp/blob/master/examples/classification/gpt_imdb_finetune.ipynb
        #mindspore数据处理
        ...
        # tokenized_dataset = dataset.map(operations=[tokenize], input_columns=['text'], output_columns=['input_ids', 'attention_mask'])
        # tokenized_dataset = tokenized_dataset.map(operations=transforms.TypeCast(mindspore.int32), input_columns="label", output_columns="labels")
