...
def recursive_print(name, val, spaces=0):
    ...
    elif isinstance(val, mindspore.Tensor):
        print(msg, ":", val.size())
    else:
        print(msg, ":", val)
...
def main():
    ...
    print(f'Extracting MindSpore state dictionary from "{args.path_to_checkpoint}"')
    if args.path_to_checkpoint.endswith(".zip"):
        with zipfile.ZipFile(args.path_to_checkpoint, "r") as checkpoint:
            with checkpoint.open("release/mp_rank_00/model_optim_rng.pt") as pytorch_dict:
                input_state_dict = mindspore.load_checkpoint(pytorch_dict, map_location="cpu")
    else:
        input_state_dict = mindspore.load_checkpoint(args.path_to_checkpoint, map_location="cpu")
    ...
    output_checkpoint_file = os.path.join(basename, "mindSpore_model.bin")
    print(f'Saving checkpoint to "{output_checkpoint_file}"')
    mindspore.save_checkpoint(output_state_dict, output_checkpoint_file)
