...
class MegatronBertEmbeddings(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.register_buffer("position_ids", mindspore.numpy.arange(config.max_position_embeddings).expand((1, -1)))
    ...
    def forward(
        ...
        if self.position_embedding_type == "relative_key" or self.position_embedding_type == "relative_key_query":
            ...
            if use_cache:
                ...
                position_ids_l = ops.arange(query_length, dtype=Tensor.long).view(-1, 1)
            position_ids_r = ops.arange(key_length, dtype=Tensor.long).view(1, -1)
...
class MegatronBertForPreTrainingOutput(ModelOutput):
    ...
    Args:
        loss (*optional*, returned when `labels` is provided, `Tensor(dtype=mindspore.float32)` of shape `(1,)`):
            Total loss as the sum of the masked language modeling loss and the next sequence prediction
            (classification) loss.
        prediction_logits (`Tensor(dtype=mindspore.float32)` of shape `(batch_size, sequence_length, config.vocab_size)`):
            Prediction scores of the language modeling head (scores for each vocabulary token before SoftMax).
        seq_relationship_logits (`Tensor(dtype=mindspore.float32)` of shape `(batch_size, 2)`):
            Prediction scores of the next sequence prediction (classification) head (scores of True/False continuation
            before SoftMax).
        hidden_states (`tuple(Tensor(dtype=mindspore.float32))`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `Tensor(dtype=mindspore.float32)` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`.

            Hidden-states of the model at the output of each layer plus the initial embedding outputs.
        attentions (`tuple(Tensor(dtype=mindspore.float32))`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `Tensor(dtype=mindspore.float32)` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`.
    ...
    loss: Optional[Tensor(dtype=mindspore.float32)] = None
    prediction_logits: Tensor(dtype=mindspore.float32) = None
    seq_relationship_logits: Tensor(dtype=mindspore.float32) = None
    hidden_states: Optional[Tuple[Tensor(dtype=mindspore.float32)]] = None
    attentions: Optional[Tuple[Tensor(dtype=mindspore.float32)]] = None
...
MEGATRON_BERT_INPUTS_DOCSTRING = r"""
    Args:
        ...
        head_mask (`Tensor(dtype=mindspore.float32)` of shape `(num_heads,)` or `(num_layers, num_heads)`, *optional*):
            Mask to nullify selected heads of the self-attention modules. Mask values selected in `[0, 1]`:

            - 1 indicates the head is **not masked**,
            - 0 indicates the head is **masked**.

        inputs_embeds (`Tensor(dtype=mindspore.float32)` of shape `({0}, hidden_size)`, *optional*):
...
class MegatronBertModel(MegatronBertPreTrainedModel):
    ...
    def forward(
        ...
        encoder_attention_mask (`Tensor(dtype=mindspore.float32)` of shape `(batch_size, sequence_length)`, *optional*):
            Mask to avoid performing attention on the padding token indices of the encoder input. This mask is used in
            the cross-attention if the model is configured as a decoder. Mask values selected in `[0, 1]`:

            - 1 for tokens that are **not masked**,
            - 0 for tokens that are **masked**.
        past_key_values (`tuple(tuple(Tensor(dtype=mindspore.float32)))` of length `config.n_layers` with each tuple having 4 tensors of shape `(batch_size, num_heads, sequence_length - 1, embed_size_per_head)`):
        ...
        if token_type_ids is None:
            token_type_ids = ops.zeros(input_shape, dtype=Tensor.long)
...
class MegatronBertForPreTraining(MegatronBertPreTrainedModel):
    ...
    def forward(
        ...
        next_sentence_label (`Tensor(dtype=mindspore.int64)` of shape `(batch_size,)`, *optional*):
...
class MegatronBertForNextSentencePrediction(MegatronBertPreTrainedModel):
    ...
    def forward(
        ...
        >>> outputs = model(**encoding, labels=mindspore.Tensor(dtype=mindspore.int64)([1]))
    ...
    def forward(
        ...
        if labels is not None:
            if self.config.problem_type is None:
                ...
                elif self.num_labels > 1 and (labels.dtype == Tensor.long or labels.dtype == Tensor.int):
...
class MegatronBertForQuestionAnswering(MegatronBertPreTrainedModel):
    ...
    def forward(
        ...
        end_positions (`Tensor(dtype=mindspore.int64)` of shape `(batch_size,)`, *optional*):
        ...
        if start_positions is not None and end_positions is not None:
            ...
            loss_fct = mindspore.nn.CrossEntropyLoss(ignore_index=ignored_index)
