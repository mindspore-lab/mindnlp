...
from .codellama_config import CodellamaConfig
...
class CodellamaRMSNorm(nn.Cell):
    '''
    CodellamaRMSNorm
    '''
    def __init__(self, hidden_size, eps=1e-6):
        """
        CodellamaRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
class CodellamaRotaryEmbedding(nn.Cell):
    '''
...
class CodellamaAttention(nn.Cell):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: CodellamaConfig):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = CodellamaRotaryEmbedding(self.head_dim, max_position_embeddings=self.max_position_embeddings)
...
class CodellamaDecoderLayer(nn.Cell):
    '''
    CodellamaDecoder
    '''
    def __init__(self, config: CodellamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = CodellamaAttention(config=config)
        self.mlp = CodellamaMLP(
            hidden_size=self.hidden_size,
            intermediate_size=config.intermediate_size,
            hidden_act=config.hidden_act,
        )
        self.input_layernorm = CodellamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = CodellamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class CodellamaPreTrainedModel(PreTrainedModel):
    '''
    CodellamaPreTrainedModel
    '''
    config_class = CodellamaConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["CodellamaDecoderLayer"]
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, CodellamaModel):
...
class CodellamaModel(CodellamaPreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`CodellamaDecoderLayer`]
    Args:
    ...
    def __init__(self, config: CodellamaConfig):
        ...
        self.layers = nn.CellList([CodellamaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = CodellamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class CodellamaForCausalLM(CodellamaPreTrainedModel):
    '''
    CodellamaForCausalLM
    '''
    def __init__(self, config):
        super().__init__(config)
        self.model = CodellamaModel(config)
    ...
    def construct(
        ...
        Example:
        ```python
        >>> from transformers import AutoTokenizer, CodellamaForCausalLM
        >>> model = CodellamaForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
...
class CodellamaForSequenceClassification(CodellamaPreTrainedModel):
    '''
    CodellamaForSequenceClassification
    '''
