...
from mindspore.nn import Embedding, Cell
...
'''def unwrap_model(model, *args, **kwargs):
    raw_model = model._layers if isinstance(model, mindspore.DataParallel) else model
    return raw_model'''


def get_parameter_dtype(parameter: Cell) -> mindspore.dtype:
...
class PretrainedModel(Cell, GenerationMixin, ConversionMixin):
    """
