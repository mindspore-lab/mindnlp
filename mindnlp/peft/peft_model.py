...
class PeftModel(nn.Cell):
    ...
    def load_adapter(
        self, model_id: str, adapter_name: str, is_trainable: bool = False, **kwargs
    ):
        """load adapter to peft model, called by `model.from_pretrained`."""
        # NOTE: remove download logic.
        if adapter_name not in self.peft_config:
            raise ValueError(
                f"{adapter_name} is not a valid adapter name. Valid names: {self.peft_config.keys()}"
            )
    ...
    def add_adapter(self, adapter_name: str, peft_config: PeftConfig):
        ...
        try:
            ...
            ):  # add_adapter methods for prompt learning setup
        ...
        if isinstance(peft_config, PromptLearningConfig):
            ...
            if (
                ...
                if model_kwargs["past_key_values"] is None:
                    ...
                    prompts = prompts.to(inputs_embeds.dtype)
                    model_kwargs["inputs_embeds"] = ops.cat(
                        (prompts, inputs_embeds), axis=1
                    )
        ...
        **kwargs,
        ...
        if decoder_attention_mask is not None:
            ...
            decoder_attention_mask = ops.cat(
                (prefix_attention_mask, decoder_attention_mask), axis=1
            )
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if attention_mask is not None:
                ...
                kwargs["attention_mask"] = ops.cat(
                    (prefix_attention_mask, attention_mask), axis=1
                )

            prompts = self.get_prompt(batch_size=batch_size)
            prompts = prompts.to(inputs_embeds.dtype)
            inputs_embeds = ops.cat(
                (prompts[:, : peft_config.num_virtual_tokens], inputs_embeds), axis=1
            )
            ...
            if not isinstance(peft_config, PromptLearningConfig):
                ...
                if peft_config.peft_type == PeftType.PREFIX_TUNING:
                    ...
                    if "attention_mask" in kwargs:
                        ...
                        kwargs["attention_mask"] = ops.cat(
                            (prefix_attention_mask, kwargs["attention_mask"]), axis=1
                        )
        ...
        **kwargs,
        ...
        if peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if kwargs.get("token_type_ids", None) is not None:
                kwargs["token_type_ids"] = ops.cat(
                    (
                        ops.zeros(batch_size, peft_config.num_virtual_tokens).to(
    ...
    def _prefix_tuning_forward(
        ...
        **kwargs,
        ...
        if "past_key_values" in fwd_params:
            ...
            if "dropout" in [
                name for name, _ in list(self.base_model.cells_and_names())
            ]:
