...
将 PyTorch 保存的特征文件转换为 NumPy 格式，避免 torch_npu 序列化兼容性问题
"""
import os
import sys
import json
...
# 必须在导入 mindtorch 之前设置环境，使用纯 PyTorch
...
def convert_pt_to_npy(input_dir: Path, output_dir: Path):
    ...
    for pt_file in tqdm(pt_files, desc="Converting features"):
        ...
        try:
            # 使用纯 PyTorch 加载（在导入 mindtorch 之前）
            ...
            # 转换为 numpy（确保完全转换，不保留 torch 对象）
            if isinstance(features, torch.Tensor):
                features_np = features.cpu().numpy()
            elif isinstance(features, dict):
                # 如果是嵌套字典，递归提取所有 torch.Tensor
            ...
            # 保存 numpy 数组（纯数值数组，不包含 torch 对象）
